//! WHY: swarmctl's `red-swarm plan` verb (OPFOR-04, SC 4).
//!
...
/// Dispatch `red-swarm <subcommand>`. The only subcommand today is `plan`.
...
mod tests {
    ...
    /// SC 4, the CLI half: the red-swarm output path reads no OS entropy and no
    /// wall clock. This extends Task 1's `no_entropy_path_exists_in_the_red_lane`
    /// to the CLI module. The file is located from the repo root, not from
