...
/// An opaque hold id that has passed [`is_opaque_hold_id`].
///
/// The shape is `common.schema.json#/$defs/HoldId`,
/// `^[A-Za-z0-9][A-Za-z0-9_-]{7,63}$`: URL-safe because it is a path parameter
/// on `POST /v1/response/holds/{hold_id}/decide`, and COLON-FREE so the
/// forbidden `hold:{hunt_id}:{held_at_ms}` derived form — `hunt_id` is the
/// telemetry event id, a join key into detection data — is unrepresentable
/// rather than merely warned about. Six formats were in circulation across the
/// wave-2 artifact set and two used the colon prefix; this type is where the
/// shape is decided, and it can only be built through [`HoldId::parse`].
#[derive(Debug, Clone, PartialEq, Eq, Hash, PartialOrd, Ord, Serialize)]
#[serde(transparent)]
pub struct HoldId(String);

impl HoldId {
    /// Admit a hold id, or refuse it with [`TagError::MalformedHoldId`].
    ///
    /// # Errors
    ///
    /// [`TagError::MalformedHoldId`] when `value` does not match the opaque
    /// pattern — too short, too long, a leading `_`/`-`, a colon, a slash, or
    /// anything outside `[A-Za-z0-9_-]`.
    pub fn parse(value: &str) -> Result<Self, TagError> {
        if is_opaque_hold_id(value) {
            Ok(Self(value.to_string()))
        } else {
            Err(TagError::MalformedHoldId(value.to_string()))
        }
    }
...
impl<'de> Deserialize<'de> for HoldId {
...
impl TagSet {
    ...
    pub fn assert_publishable(&self, kind: u16) -> Result<(), TagError> {
        ...
        match kind {
            ...
            crate::KIND_CARD if !self.p.is_empty() => {
...
mod tests {
    ...
    fn an_uppercase_pubkey_is_refused_before_signing() {
        let tags = TagSet::hold_notice("uuid", vec![PK.to_uppercase()], "h_a07aeacf", None);
        assert!(matches!(
    ...
    fn a_truncated_pubkey_is_refused_before_signing() {
        let tags = TagSet::hold_notice("uuid", vec![PK[..63].to_string()], "h_a07aeacf", None);
    ...
    #[test]
    fn a_hold_id_can_only_be_built_through_parse() {
        let id = HoldId::parse("27799e23-ab25-4659-b381-3de47ea7ca4d").expect("a UUID is admitted");
        assert_eq!(id.as_str(), "27799e23-ab25-4659-b381-3de47ea7ca4d");
        assert_eq!(id.to_string(), "27799e23-ab25-4659-b381-3de47ea7ca4d");
        assert!(matches!(
            HoldId::parse("hold:hunt-evt-1:1773738882600"),
            Err(TagError::MalformedHoldId(_))
        ));
        assert!(HoldId::parse("").is_err());
        assert!(HoldId::parse(&"a".repeat(65)).is_err());
        assert!(HoldId::parse(&"a".repeat(64)).is_ok());
        // Serde goes through the same gate: a colon form does not decode.
        assert!(serde_json::from_str::<HoldId>("\"hold:x:1\"").is_err());
        let decoded: HoldId = serde_json::from_str("\"h_a07aeacf\"").expect("decodes");
        assert_eq!(serde_json::to_string(&decoded).unwrap(), "\"h_a07aeacf\"");
    }

    #[test]
    fn card_tags_render_in_registry_order_with_no_p() {
        let tags = TagSet::card(
            CardKind::Finding,
            "3f1e77c2-0b41-4c6a-9d55-8ac1e2f40911",
            Some("data_exfiltration".into()),
            Some("HIGH".into()),
        );
        assert_eq!(tags.assert_publishable(crate::KIND_CARD), Ok(()));
        assert_eq!(
            tags.to_tags(),
            vec![
                vec![
                    "h".to_string(),
                    "3f1e77c2-0b41-4c6a-9d55-8ac1e2f40911".to_string()
                ],
