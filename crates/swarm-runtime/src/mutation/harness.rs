...
impl DefaultEvolutionMutationHarness {
    ...
    pub fn create_autonomous_mutation_spec(
        ...
        let population = population_store.load_trusted(&self.signer_agent_id)?;
    ...
    pub fn append_variant(
        ...
        let base_manifest =
            load_detector_experiment_manifest(Path::new(&lookup.report.base_experiment_path))?;
        let base_genome = EvolutionDetectorGenome::from_candidate(&base_manifest.candidate)?;
        let mutation_dimensions = if let Some(target_genome) = request.target_genome.as_ref() {
            if !request.overrides.is_empty() {
                return Err(EvolutionMutationError::InvalidMutationSpecRequest {
                    reason: format!(
                        "variant `{}` mixes a typed target genome with legacy process-tree overrides",
                        request.strategy_id
                    ),
                });
            }
            mutation_dimensions_for_target_genome(&base_genome, target_genome)?
        } else {
            if base_genome.strategy() != "suspicious_process_tree" {
                return Err(EvolutionMutationError::InvalidMutationSpecRequest {
                    reason: format!(
                        "variant `{}` requires a typed target genome for detector `{}`",
                        request.strategy_id,
                        base_genome.strategy()
                    ),
                });
            }
        ...
        let variant = EvolutionMutationVariantSpec {
            ...
            target_genome: request.target_genome,
    ...
    pub fn materialize_batch(
        ...
        for (index, variant) in spec.report.variants.iter().enumerate() {
            let report =
                materialize_variant_report(&spec.report, variant, created_at_ms + index as i64)?;
    ...
    pub fn load_population(
        ...
        Ok(store.load_trusted(&self.signer_agent_id)?)
    }
    ...
    pub fn refresh_population(
        ...
        self.refresh_population_with_policy(
            population_results_dir,
            drafting,
    ...
    #[allow(clippy::too_many_arguments)]
    pub fn refresh_benchmark_population(
        &self,
        population_results_dir: impl AsRef<Path>,
        drafting: &DefaultEvolutionDraftingHarness,
        experiment_results_dir: impl AsRef<Path>,
    ...
    #[allow(clippy::too_many_arguments)]
    fn refresh_population_with_policy(
        &self,
        population_results_dir: impl AsRef<Path>,
        drafting: &DefaultEvolutionDraftingHarness,
        experiment_results_dir: impl AsRef<Path>,
        ...
        for candidate in ranking
            .ranked_candidates
            .iter()
            .filter(|candidate| include_blocked_candidates || candidate.ready_for_review)
        {
            let validation = drafting
    ...
    pub fn evaluate_adversarial_pressure(
        ...
        let episode_store = FileEvolutionEpisodeStore::open_signed(
            population_results_dir.as_ref().join("episodes"),
            self.signer_agent_id.clone(),
            self.signing_key.clone(),
        )?;
    ...
    pub fn select_population_candidate(
        ...
        let Some(mut state) = store.load_trusted(&self.signer_agent_id)? else {
            return Ok(None);
        };
