...
/// Typed detector genome carried through mutation planning and materialization.
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
#[serde(tag = "strategy", rename_all = "snake_case")]
pub enum EvolutionDetectorGenome {
...
impl EvolutionDetectorGenome {
    pub(crate) fn strategy(&self) -> &'static str {
        match self {
            Self::SuspiciousProcessTree { .. } => "suspicious_process_tree",
            Self::BehavioralAnomaly { .. } => "behavioral_anomaly",
            Self::FilelessExecution { .. } => "fileless_execution",
            Self::DnsExfiltration { .. } => "dns_exfiltration",
        }
    }
    ...
    pub(crate) fn from_candidate(
        ...
        match candidate {
            ...
            other => Err(ReplayHarnessError::UnsupportedDetector {
                strategy: format!(
                    "mutation genomes not yet supported for detector `{}`",
                    other.strategy_id()
                ),
            }
            .into()),
    ...
    pub(crate) fn to_candidate_manifest(
        &self,
        strategy_id: String,
        description: String,
    ) -> DetectorCandidateManifest {
        match self {
            Self::SuspiciousProcessTree { profile } => {
            ...
            Self::BehavioralAnomaly { profile } => DetectorCandidateManifest::BehavioralAnomaly {
                strategy_id,
                description,
                profile: profile.clone(),
            },
            Self::FilelessExecution { profile } => DetectorCandidateManifest::FilelessExecution {
                strategy_id,
                description,
                profile: profile.clone(),
            },
            Self::DnsExfiltration { profile } => DetectorCandidateManifest::DnsExfiltration {
                strategy_id,
                description,
                profile: profile.clone(),
            },
    ...
    pub(crate) fn profile_json(&self) -> Result<serde_json::Value, serde_json::Error> {
        match self {
            Self::SuspiciousProcessTree { profile } => serde_json::to_value(profile),
            Self::BehavioralAnomaly { profile } => serde_json::to_value(profile),
            Self::FilelessExecution { profile } => serde_json::to_value(profile),
            Self::DnsExfiltration { profile } => serde_json::to_value(profile),
        }
    }
    ...
    pub(crate) fn thresholds(&self) -> (f64, f64) {
        match self {
            ...
            Self::BehavioralAnomaly { profile } => (
                profile.high_confidence_threshold,
                profile.medium_confidence_threshold,
            ),
            Self::FilelessExecution { profile } => (
                profile.high_confidence_threshold,
                profile.medium_confidence_threshold,
            ),
            Self::DnsExfiltration { profile } => (
                profile.high_confidence_threshold,
                profile.medium_confidence_threshold,
            ),
        }
...
impl EvolutionMutationProfileOverrides {
    pub(crate) fn is_empty(&self) -> bool {
        self.add_suspicious_parents.is_empty()
            && self.remove_suspicious_parents.is_empty()
            && self.add_suspicious_children.is_empty()
            && self.remove_suspicious_children.is_empty()
            && self.high_confidence_threshold.is_none()
            && self.medium_confidence_threshold.is_none()
    }
...
pub struct EvolutionMutationVariantCreateRequest {
    ...
    pub target_genome: Option<EvolutionDetectorGenome>,
}
