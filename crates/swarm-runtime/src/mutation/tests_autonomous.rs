...
fn sample_population_state(now_ms: i64) -> EvolutionPopulationState {
    EvolutionPopulationState {
...
#[test]
fn population_store_rejects_tampered_signed_state_after_reopen() {
    let root = unique_temp_dir("population-tampered");
    let population_dir = root.join("population");
    ...
    let state_path = population_dir.join("state.json");
    let mut envelope: swarm_core::signed_state::SignedStateEnvelope<EvolutionPopulationState> =
        serde_json::from_str(&fs::read_to_string(&state_path).unwrap()).unwrap();
    envelope.statement.payload_json = envelope
        .statement
        .payload_json
        .replace("candidate-a", "candidate-a-tampered");
    fs::write(
        &state_path,
    ...
    let reopened = FileEvolutionPopulationStore::open_signed(
        &population_dir,
        signer_agent_id.clone(),
        test_signing_key(),
    )
    .unwrap();
    let error = reopened.load_trusted(&signer_agent_id).unwrap_err();
    assert!(matches!(
        error,
...
#[test]
fn population_store_rejects_replayed_state_after_newer_sequence_is_accepted() {
    let root = unique_temp_dir("population-replay");
    let population_dir = root.join("population");
    let signer_agent_id = test_signer_agent_id();
    let store = FileEvolutionPopulationStore::open_signed(
        &population_dir,
    ...
    .unwrap();
    let first = sample_population_state(1_800_400_000_000);
    store.persist(&first).unwrap();
    let original_state_raw = fs::read_to_string(population_dir.join("state.json")).unwrap();

    let mut newer = sample_population_state(1_800_400_010_000);
    newer.updated_at_ms += 5_000;
    newer.ranking_id = "ranking:test:next".to_string();
    newer.members[0].fitness = 0.95;
    newer.members[0].summary = "candidate-a-next".to_string();
    store.persist(&newer).unwrap();

    fs::write(population_dir.join("state.json"), original_state_raw).unwrap();

    let error = store.load_trusted(&signer_agent_id).unwrap_err();
    assert!(matches!(
        error,
        super::EvolutionPopulationStoreError::InvalidSignature {
...
#[test]
fn episode_store_rejects_tampered_signed_report_after_reopen() {
    let root = unique_temp_dir("episode-tampered");
    let population_dir = root.join("population");
    let signer_agent_id = test_signer_agent_id();
    let mutation = DefaultEvolutionMutationHarness::from_path(
        root.join("mutations"),
    ...
    let record = FileEvolutionEpisodeStore::open(population_dir.join("episodes"))
        .unwrap()
        .latest(1)
        .unwrap()
        .into_iter()
        .next()
        .unwrap();
    let report_path = PathBuf::from(record.bundle_path);
    let mut envelope: swarm_core::signed_state::SignedStateEnvelope<super::EvolutionEpisodeReport> =
        serde_json::from_str(&fs::read_to_string(&report_path).unwrap()).unwrap();
    envelope.statement.payload_json = envelope.statement.payload_json.replace(
        "office_baseline_control",
        "office_baseline_control_tampered",
    );
    fs::write(
        &report_path,
        serde_json::to_string_pretty(&envelope).unwrap(),
    ...
    let reopened = FileEvolutionEpisodeStore::open_signed(
        population_dir.join("episodes"),
        signer_agent_id.clone(),
        test_signing_key(),
    )
    .unwrap();
    let error = reopened
        .load_trusted(&result.episode.episode_id, &signer_agent_id)
        .unwrap_err();
    assert!(matches!(
        error,
        super::EvolutionEpisodeStoreError::InvalidSignature { .. }
    ));
}

#[test]
fn episode_store_rejects_replayed_report_after_newer_sequence_is_accepted() {
    let root = unique_temp_dir("episode-replay");
    let population_dir = root.join("population");
    let signer_agent_id = test_signer_agent_id();
    let mutation = DefaultEvolutionMutationHarness::from_path(
        root.join("mutations"),
    ...
    let record = FileEvolutionEpisodeStore::open(&episode_dir)
        .unwrap()
        .latest(1)
        .unwrap()
        .into_iter()
        ...
        .unwrap();
    let report_path = PathBuf::from(record.bundle_path);
    let original_report_raw = fs::read_to_string(&report_path).unwrap();

    let store = FileEvolutionEpisodeStore::open_signed(
        &episode_dir,
        signer_agent_id.clone(),
        test_signing_key(),
    )
    .unwrap();
    let mut updated = result.episode.clone();
    updated.created_at_ms += 1;
    updated.blue_fitness.final_fitness += 0.01;
    store.persist(&updated).unwrap();

    fs::write(&report_path, original_report_raw).unwrap();
...
fn population_selection_respects_hourly_proposal_limit() {
    ...
    store.persist(&sample_population_state(now_ms)).unwrap();
