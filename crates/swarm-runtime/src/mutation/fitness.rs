...
pub(crate) fn materialize_variant_report(
    ...
    let base_experiment_path = Path::new(&spec.base_experiment_path);
    ...
    let (candidate, legacy_profile, genome, applied_changes) = if let Some(target_genome) =
        &variant.target_genome
    {
        let applied_changes = variant
            .mutation_dimensions
            .iter()
            .map(|dimension| format!("set detector genome field `{dimension}`"))
            .collect::<Vec<_>>();
        (
            target_genome.to_candidate_manifest(
                variant.strategy_id.clone(),
                variant.strategy_description.clone(),
            ),
            match target_genome {
                EvolutionDetectorGenome::SuspiciousProcessTree { profile } => Some(profile.clone()),
            ...
            Some(target_genome.clone()),
            applied_changes,
        )
    } else {
        let request = variant.overrides.to_materialization_request(
            spec.draft_id.clone(),
            base_experiment_path.to_path_buf(),
        )?;
        ...
        (
            ...
            Some(profile.clone()),
            Some(EvolutionDetectorGenome::SuspiciousProcessTree { profile }),
            applied_changes,
        )
    };
