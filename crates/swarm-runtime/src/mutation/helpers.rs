...
pub(crate) fn mutation_dimensions_for_target_genome(
    base: &EvolutionDetectorGenome,
    target: &EvolutionDetectorGenome,
) -> Result<Vec<String>, EvolutionMutationError> {
    if base.strategy() != target.strategy() {
        return Err(EvolutionMutationError::InvalidMutationSpecRequest {
            reason: format!(
                "target genome `{}` does not match base experiment detector `{}`",
                target.strategy(),
                base.strategy()
            ),
        });
    ...
    let base_profile = base.profile_json()?;
    let target_profile = target.profile_json()?;
    let mut dimensions = Vec::new();
    let Some(base_fields) = base_profile.as_object() else {
        return Err(EvolutionMutationError::InvalidMutationSpecRequest {
            reason: format!(
                "base genome `{}` did not encode as an object",
                base.strategy()
            ),
        });
    };
    let Some(target_fields) = target_profile.as_object() else {
        return Err(EvolutionMutationError::InvalidMutationSpecRequest {
            reason: format!(
                "target genome `{}` did not encode as an object",
                target.strategy()
            ),
        });
    };

    let field_names = base_fields
        .keys()
        .chain(target_fields.keys())
        .cloned()
        .collect::<BTreeSet<_>>();
    for field in field_names {
        if base_fields.get(&field) != target_fields.get(&field) {
