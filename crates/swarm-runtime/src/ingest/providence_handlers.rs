...
use crate::providence::{
    ...
    build_scoped_providence_links, resolve_feedback_target,
};
...
use swarm_core::types::{
    AgentId, ProvidenceCreateIncidentBody, ProvidenceFeedbackAction, ProvidenceIncidentStatus,
    SWARM_PROVIDENCE_FEEDBACK_SCHEMA, SWARM_PROVIDENCE_FEEDBACK_SCHEMA_VERSION,
    SWARM_PROVIDENCE_WEBHOOK_SCHEMA, SWARM_PROVIDENCE_WEBHOOK_SCHEMA_VERSION, Severity,
    SwarmAction, SwarmFeedbackSignal, SwarmProvidenceAggregateContext,
...
pub(crate) async fn providence_feedback_handler(
    ...
    let outcome = apply_providence_feedback(&state, &request, &target).await?;
    ...
    stack
        ...
        .ok_or_else(|| {
            ProvidenceFeedbackError::service_unavailable(
                "Providence feedback is unavailable because providence_webhook is not configured",
            )
        })
}
...
) -> Result<Value, ProvidenceFeedbackError> {
    match request.action {
        ProvidenceFeedbackAction::Confirm => {
            let deposit =
                signed_providence_feedback_deposit(state, request, target, 1.0, Some("confirm"))
                    .await?;
            state
                .current_substrate()
                .deposit(deposit)
        ...
        ProvidenceFeedbackAction::Dismiss => {
            let deposit =
                signed_providence_feedback_deposit(state, request, target, 0.0, Some("dismiss"))
                    .await?;
            state
                .current_substrate()
                .deposit(deposit)
...
pub(super) fn build_providence_notification_payload(
    ...
    agent_health: Option<&std::sync::Arc<arc_swap::ArcSwap<Vec<swarm_core::agent::AgentHealthEntry>>>>,
    ...
    let agent_health = agent_health
        ...
        .unwrap_or_default();
    let bridge_health = bridge_health
