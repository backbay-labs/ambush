    use super::{
        ...
        DemoProofPackage, DemoReplayRequest, DemoReplayResponse,
        IngestRequest, IngestResponse, IngestState,
        StrategyProposalRoute, detect_http_router, ingest_router,
        validate_and_parse,
    };
    use super::health::{HeapPressureSnapshot, response_adapter_kind};
    use super::platform_api::{
        PlatformApiEnvelope, PlatformFindingSummary, PlatformIncidentSummary,
        PlatformRuntimeStatus, PlatformAssetPosture,
    };
    ...
    use swarm_core::config::{
        ...
        PolicyRuleConfig, PolicyRuleDecision, PromotionConfig, ResponseAdapterConfig, RetryConfig,
        RoutingRule, RuntimeMode, RuntimeSettings, SwarmConfig, TelemetrySourceConfig,
        WebhookConfig,
    };
    ...
    fn seed_platform_replay_bundle(
        ...
        let event =
            validate_and_parse(process_event_json(event_id, host_id, created_at_ms)).unwrap();
    ...
    fn configure_evolution_paths(config: &mut SwarmConfig, root: &Path) {
        ...
        config.evolution.paths.experiment_results_dir =
            root.join("experiments").display().to_string();
    ...
    fn degraded_ingest_state() -> IngestState {
        ...
        state
            .detector_status
            .store(Arc::new(super::health::DetectorRuntimeStatus::reload_failed(
    ...
    async fn platform_api_routes_require_bearer_and_api_key_but_health_and_ingest_do_not() {
        ...
        let app = detect_http_router(
            IngestState::from_config(temp_path("platform-auth"), config).unwrap(),
        );
    ...
    async fn platform_findings_endpoint_returns_filtered_cursor_paginated_envelope() {
        ...
        let second_page: PlatformApiEnvelope<PlatformFindingSummary> =
            parse_json(second_page).await;
    ...
    async fn platform_incidents_endpoint_returns_filtered_cursor_paginated_envelope() {
        ...
        let second_page = app
            ...
            .unwrap();
        assert_eq!(second_page.status(), StatusCode::OK);
        let second_page: PlatformApiEnvelope<PlatformIncidentSummary> =
            parse_json(second_page).await;
        assert_eq!(second_page.data.len(), 1);
    ...
    async fn demo_widget_endpoint_sets_embed_headers_and_renders_scoped_context() {
        ...
        config.operator.auth.token_env = "SWARM_OPERATOR_WIDGET_TEST_TOKEN".to_string();
        unsafe {
            std::env::set_var(
    ...
    async fn providence_webhook_payload_includes_runtime_context_and_links() {
        ...
        config.operator.auth.token_env = "SWARM_PROVIDENCE_LINK_TEST_TOKEN".to_string();
        unsafe {
            std::env::set_var(
        ...
        let response = app
            .oneshot(
                Request::builder()
                    .method("GET")
                    .uri(
                        "/v1/events/stream?types=agent_action,response_execution&hunt_id=evt-widget-1",
                    )
                    .body(Body::empty())
                    .unwrap(),
            )
    ...
    mod providence_feedback {
        ...
        use swarm_core::types::{
            AgentId, ProvidenceFeedbackAction, SwarmProvidenceFeedbackRequest,
        };
        ...
        async fn signed_feedback_endpoint_persists_audit_entry() {
            ...
            let state =
                IngestState::from_config(super::temp_path("feedback-audit"), config).unwrap();
        ...
        async fn feedback_actions_translate_into_runtime_side_effects() {
            ...
            config.investigation.enabled = true;
            let state =
                IngestState::from_config(super::temp_path("feedback-actions"), config).unwrap();
        ...
        async fn dismiss_feedback_reaches_kitten_or_pending_fallback() {
            ...
            let pending_state =
                IngestState::from_config(super::temp_path("feedback-pending"), pending_config)
                    .unwrap();
            super::seed_platform_replay_bundle(
                &pending_state,
    ...
    async fn platform_findings_stream_endpoint_emits_canonical_finding_events() {
        ...
        let response = app
            .oneshot(
                authorized_platform_api_request(
                    "GET",
                    "/v2/api/stream/findings?host_id=host-stream",
                )
                .body(Body::empty())
                .unwrap(),
            )
    ...
    async fn healthz_includes_async_lane_component_when_enabled() {
        ...
        config.correlation.enabled = true;
        let app = detect_http_router(
            IngestState::from_config(temp_path("healthz-async"), config).unwrap(),
        );
        let response = app
            .oneshot(
                Request::builder()
