...
use crate::mutation::EvolutionDetectorGenome;
use crate::replay::{
...
pub struct EvolutionMaterializationReport {
    ...
    pub profile: Option<SuspiciousProcessTreeProfile>,
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub genome: Option<EvolutionDetectorGenome>,
    pub manifest_sha256: String,
    pub lineage_sha256: String,
    pub applied_changes: Vec<String>,
}

impl EvolutionMaterializationReport {
    pub fn detector_genome(&self) -> Option<EvolutionDetectorGenome> {
        self.genome.clone().or_else(|| {
            self.profile
                .clone()
                .map(|profile| EvolutionDetectorGenome::SuspiciousProcessTree { profile })
        })
    }

    pub fn profile_json(&self) -> Result<Option<serde_json::Value>, serde_json::Error> {
        self.detector_genome()
            .map(|genome| genome.profile_json())
            .transpose()
    }

    pub fn thresholds(&self) -> Option<(f64, f64)> {
        self.detector_genome().map(|genome| genome.thresholds())
    }
}
...
impl DefaultEvolutionDraftingHarness {
    ...
    pub fn materialize_draft(
        ...
        let report = EvolutionMaterializationReport {
            ...
            genome: Some(EvolutionDetectorGenome::SuspiciousProcessTree {
                profile: profile.clone(),
            }),
...
pub fn render_evolution_materialization(report: &EvolutionMaterializationReport) -> String {
    ...
    if let Some(genome) = report.detector_genome() {
        lines.push(format!("Detector genome: {}", genome.strategy()));
        let (high, medium) = genome.thresholds();
        lines.push(format!("Thresholds: high={high:.3} medium={medium:.3}"));
    }
