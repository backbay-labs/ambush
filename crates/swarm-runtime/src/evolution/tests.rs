...
fn classify_unknown_never_reports_a_refutation() {
    let cases = [
        // z3's own wording for rlimit exhaustion, both spellings it uses.
        (
        ...
        (Some("timeout"), EvolutionSolverProofStatus::Timeout),
        (Some("canceled"), EvolutionSolverProofStatus::Timeout),
        (Some("smt tactic failed"), EvolutionSolverProofStatus::Error),
        (None, EvolutionSolverProofStatus::Error),
    ];

    for (reason, expected) in cases {
        let status = super::formal_safety::classify_unknown(reason);
        assert_eq!(status, expected, "reason_unknown={reason:?}");
        assert_ne!(
            status,
    ...
    // the wall clock.
    assert_ne!(
        super::formal_safety::classify_unknown(Some("max. resource limit exceeded")),
        super::formal_safety::classify_unknown(Some("timeout"))
    );
}
