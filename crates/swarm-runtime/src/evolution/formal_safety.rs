...
///
/// z3 reports rlimit exhaustion as `max. resource limit exceeded` and also
/// answers `(get-info :reason-unknown)` with `resource limits reached` depending
/// on the entry point, so both spellings map to `ResourceLimit`. Neither is a
/// refutation.
...
pub(super) fn classify_unknown(reason_unknown: Option<&str>) -> EvolutionSolverProofStatus {
    let Some(reason) = reason_unknown else {
        return EvolutionSolverProofStatus::Error;
    };
    ...
    with_z3_config(&config, || {
        ...
        match result {
            ...
            SatResult::Unknown => {
                ...
                let status = classify_unknown(reason_unknown.as_deref());
