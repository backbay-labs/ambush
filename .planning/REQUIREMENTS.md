...
- [x] **FALSIFY-02**: Each registered test constructs a deliberately-broken variant of the enforcing function and asserts the broken variant permits what the real function denies, proving the positive suite is not vacuous. DELIVERED for all 57 rows: every exact test invokes a registry-bound named case adapter with one probe; the shared protocol executes one real, mirror(None), and mirror(BrokenVariant) operation and asserts real/control denial plus broken permission. A separate five-test compiled contract uses typed counters/roles, and thirteen mutations of the actual protocol prove no-op, omitted/swapped-role, and removed/inverted/vacuous-assertion changes fail. Registry schema 4 binds `CASE_TYPE::real` to an exact fully-qualified production entry and records indirect reachability to private guards. Cargo `--list` and whole-target execution require exact inventories/counts with zero ignored across all five targets. The production-entry binding is structural, not runtime instrumentation, and handwritten-mirror fidelity beyond the probe remains a review claim rather than a mechanical one.
