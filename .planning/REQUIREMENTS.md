...
outcome. The former reverse-subset oracle is rejected. Replacement local
production-mutation and restored-corpus evidence is now recorded under
...
- [x] **GRAPH-01**: `KnowledgeGraphNode` gains `Process`, `File`, and `NetworkFlow` variants carrying raw telemetry identifiers, distinct from the existing detection-level `EngagementNode`. Satisfied 2026-09-08 — `0baa0ef45` (+ comment `69b1a7bef`). `ProcessNode`/`FileNode`/`NetworkFlowNode` added with `KnowledgeNodeKind::{Process,File,NetworkFlow}`, wired through all exhaustive `KnowledgeGraphNode` impl matches and `merge_node()`, with a new persist-across-restart test. **CAVEAT (producer-wiring gap, shared with GRAPH-02):** these nodes only materialize when an observation's indicator map carries new opt-in raw fields (`pid`/`process_key`, `file_path`, `source_ip`+`destination_ip`) that no current normalized telemetry event populates — see GRAPH-02's note.
- [x] **GRAPH-02**: `CausalRelation` (today only `ProcessParentChild` and `NetworkFlowOrigin`) gains `FileWrite`, `FileExecute`, `DnsResolution`, and `CredentialAccess`, emitted from every processed observation. Satisfied 2026-09-08 — `0baa0ef45`. All four emitted from `SphinxAgent::ingest_pheromone()`, pivoted on the new raw `ProcessNode` so the new data model is actually exercised rather than left inert; 6 new unit tests including one proving the whole feature is opt-in (an observation with no raw fields produces zero new nodes/edges). **CAVEAT — PRODUCER-WIRING GAP, NOT YET FIRING ON PRODUCTION TELEMETRY:** the T1 review confirmed that `swarm_core::telemetry`'s `ProcessStartEvent`, `NetworkConnectEvent`, and `FilePersistenceEvent` carry `process_name` (a string), not a raw `pid`/`process_key`, and `DnsQueryEvent` carries no resolved-IP field at all. The model and emission logic are correct and tested against synthetic fixtures, but on real production telemetry as it is normalized TODAY, none of these four edges will fire — no current producer populates the raw identifiers the new logic gates on. This is a disclosed, real limitation, not a defect in this phase's scope (which was model-only per the plan); closing it is follow-on work, likely alongside Phase 297 (Kill-Chain Reconstruction), which consumes these edges.
- [x] **GRAPH-03**: `KnowledgeGraphSnapshot::provenance_paths(from, to, max_hops)` returns bounded-hop paths and becomes the single read path `CorrelationEngine` uses, with a hub-degree cap so high-degree nodes cannot connect unrelated hunts. Satisfied 2026-09-08 — `72c9c2156`. BFS with parent-pointer path reconstruction, undirected traversal (documented), a `PROVENANCE_HUB_DEGREE_CAP = 32` that gates expansion (not reachability) of any node encountered after the initial `from` node; the load-bearing security test constructs two subgraphs bridged only by an inflated-degree hub and proves the bridge is blocked while same-side and hub-as-endpoint queries still work. `CorrelationEngine::graph_provenance_link` is a thin, additive pass-through to `provenance_paths` — the sole traversal API, confirmed by review; migrating `correlate_hunt_at` itself onto the graph is Phase 298 (XHUNT-01), out of this phase's scope.
- [x] **GRAPH-04**: A property test over 200+ randomized `prune_stale` sequences proves GC never orphans an edge, never deletes a retention-window-protected record, and is idempotent. Satisfied 2026-09-08 — `faa5d9dae`. `prune_stale_never_orphans_edges_or_evicts_in_window_records_and_is_idempotent`, 256 randomized cases (`InsertNode`/`InsertEdge`/`AdvanceClock`/`Prune`, weighted, 1-40 ops/sequence), independently re-derives the retention cutoff as an oracle rather than reusing `prune_stale`'s formula by reference. Reviewer traced non-vacuousness directly (independent node/edge timestamp sampling makes the orphan-guard-removed shape genuinely reachable). All 256 cases pass on every run; no defect found in the pre-existing `prune_stale`.
- [x] **GRAPH-05**: A soak test replays 100k synthetic events and asserts post-GC node+edge count stays under a fixed ceiling. Satisfied 2026-09-08 — `faa5d9dae`. 100,000-event replay directly against `KnowledgeGraphSnapshot`'s vectors and the real, unmodified `prune_stale` (persistence side-effects skipped deliberately — same GC function and call pattern, minus the per-tick disk write, to keep the test lane fast), periodic sweeps every 1,000 events. Ceiling `172,866` (`2 * 86,401 + 64`) against an unpruned total of `199,999`; reviewer independently re-derived the arithmetic and confirmed real headroom of only ~65 nodes+edges above the actual ~172,801 survivors — a tight, non-vacuous bound. Isolated wall time ~11.2s (well under the ~2min budget of the scoped test lane).
- [x] **GRAPH-06**: Config validation rejects or loudly warns on `knowledge_retention_days == 0` while `memory.enabled` is true, closing the silent-no-op GC footgun at `sphinx_agent.rs:1097`. Satisfied 2026-09-08 — `faa5d9dae`. The pre-existing hard reject (`MemoryConfig::validate()`, `crates/swarm-core/src/config/state.rs:127-132`, unchanged) plus a new defense-in-depth guard, `SphinxAgent::warn_if_retention_footgun_reachable`, called in `tick()` immediately before `prune_stale`: `debug_assert!`s loudly in debug/test builds and `tracing::warn!`s unconditionally, so a release build that somehow reaches the state still logs rather than silently no-opping forever. **LINE-SLIP RECORDED:** the requirement text cites `sphinx_agent.rs:1097` as the footgun site; at the pre-phase base commit that line was inside `attack_technique_for_node` (confirmed: `git show a7d77129f^:crates/swarm-runtime/src/sphinx_agent.rs` at line 1097), an unrelated lookup function — not the retention/prune logic. The substantive target was always the `prune_stale`/`tick()` `retention_days == 0` no-op path, which this commit closes; the cited line number was simply stale even before this phase's edits shifted it further.
...
| GRAPH-01 | Phase 296 | Satisfied 2026-09-08 — 0baa0ef45 (+ 69b1a7bef); producer-wiring caveat recorded |
| GRAPH-02 | Phase 296 | Satisfied 2026-09-08 — 0baa0ef45; producer-wiring caveat recorded (no current telemetry event carries raw pid/resolved-IP) |
| GRAPH-03 | Phase 296 | Satisfied 2026-09-08 — 72c9c2156 |
