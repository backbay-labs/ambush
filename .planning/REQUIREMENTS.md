...
- [x] **FALSIFY-02**: Each registered test constructs a deliberately-broken variant of the enforcing function and asserts the broken variant permits what the real function denies, proving the positive suite is not vacuous. DELIVERED for all 57 rows: every exact test invokes a registry-bound named case with one probe; the shared protocol executes one macro-owned production call, mirror(None), and mirror(BrokenVariant) operation and asserts real/control denial plus broken permission. A separate five-test compiled contract uses typed counters/roles, and thirteen mutations of the actual protocol prove no-op, omitted/swapped-role, and removed/inverted/vacuous-assertion changes fail. Registry schema 5 and a focused Rust-syntax checker pin the canonical macro invocation, exact production entry, argument/result-projection AST, call mode, and indirect private-guard reachability to a checker-owned authenticated baseline; alias/re-export/shadow, dead-call, substituted-entry, constant-normalizer, and coordinated-baseline attacks are rejected. Cargo `--list` and whole-target execution require exact inventories/counts with zero ignored across all five targets. Handwritten-mirror fidelity beyond the probe remains a review claim rather than a mechanical one.
