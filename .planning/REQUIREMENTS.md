...
- [x] **FALSIFY-02**: Each registered test constructs a deliberately-broken variant of the enforcing function and asserts the broken variant permits what the real function denies, proving the positive suite is not vacuous. DELIVERED for all 57 rows: every exact test invokes the shared typed protocol with one probe and registry-bound case/mutation identities; the protocol executes the real operation, mirror(None), and mirror(BrokenVariant), then asserts real equals the control denial and broken permits. The gate rejects `#[ignore]`/conditional disabling, requires exact Cargo `--list` set equality for all four targets, and requires exact whole-target passed counts with zero failed or ignored. This proves the registered operations ran; it does not mechanically prove broader handwritten-mirror fidelity.
