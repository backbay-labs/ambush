...
**Status:** 7/9 requirements delivered. MAPPING-05 and FALSIFY-04 remain externally blocked on protected-required-check repository settings. The mapping decision is ADR 0012, avoiding the trust ADR 0011 collision; combined trust/security re-derivation remains integration work.
**Plans:** TBD
**Success Criteria**:
1. `docs/assurance/assumptions.toml` parses and enumerates at least 8 named assumptions, each with an owner and dependent invariants.
   MET with 12, parsed by `tomllib`. `ASSUME-STATEFUL-GATE-DETERMINISM` models only local policy rate-window transitions; external adapter outcomes use `ASSUME-EXTERNAL-ADAPTER-BEHAVIOR`. Rows name multiple assumptions where appropriate, and the gate requires exact many-to-many set equality.
2. `docs/assurance/MAPPING.md` carries at least 12 invariant rows spanning `swarm-policy`, `swarm-response`, `swarm-runtime`, and `swarm-spine`, each naming an existing `crate::module::function` path.
   MET with 57 executable rows across the four named crates, plus five enforced omissions. `docs/assurance/universe.toml` freezes the exact IDs/counts, mapped/omitted disjointness, and exact one-surface assignment. The checker strips comments/literals, resolves only modules reachable from each crate root (including inline and `#[path]` modules), and requires each marker to precede an executable decision in the exact function.
...
4. Every MAPPING.md row has a `negative_*.rs` test asserting a broken variant permits what the real function denies; `scripts/check-negative-registry.sh` fails on any missing entry.
   CODE AND WORKFLOW MET at `tools/check-negative-registry.sh`: 57 real attributed tests across four files. Twenty-two adversarial self-tests cover non-tests, ignored and cfg-disabled tests, assertion-free `black_box` bodies, nonexistent modules/types, comment/string-only declarations and mutation evidence, decorative tokens, shared-protocol import/shadow drift, case/variant identity drift, production-shaped `.evaluate`/`black_box`/unrelated-assertion spoofing, and fabricated libtest stdout. The gate requires exact set equality between registry names and Cargo `--list` discovery for each target, then executes each whole target and requires exact passed counts with zero failed or ignored. It does not trust test-controlled output lines. Protected-required-check repository settings remain unverified.
   Each registry row records its neutralization observation, and the field is required non-empty. The review remediation re-ran the disputed sequence, lease-bound and failed-rollback neutralizations and made their recorded left/right output match the reproduced failure. Each test invokes one shared typed protocol with one probe and exact real, mirror(None), and mirror(BrokenVariant) operations; the protocol asserts real equals the unmutated control denial and that the broken operation permits.
   WHAT IT CANNOT CHECK, STATED: that a handwritten mirror is a faithful copy of production beyond the registered probe and operations. The typed differential and review stand in for that broader proof, and the protocol and both gate/document headers state the limit.
