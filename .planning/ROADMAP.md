...
4. Every MAPPING.md row has a `negative_*.rs` test asserting a broken variant permits what the real function denies; `scripts/check-negative-registry.sh` fails on any missing entry.
   CODE AND WORKFLOW MET at `tools/check-negative-registry.sh`: 57 real attributed tests across four files plus a separate five-test compiled protocol contract. The gate reports 85 self-tests (3 clean controls, 82 adversarial), covering non-tests, ignored/cfg-disabled tests, production-shaped `.evaluate`/`black_box`/unrelated-assertion spoofing, caller control flow, protocol aliases/re-exports/shadows, forced or swapped mirror roles, identity-selective protocol returns, production-entry or normalizer drift, constant/ignored/swapped/vacuous predicates, coordinated local data-baseline edits, and fabricated libtest stdout. The gate requires exact set equality from Cargo `--list` and exact whole-target passed counts with zero ignored for all four registered targets and the contract target. Protected check provenance remains external.
   Each registry row records its neutralization observation and binds the shared macro-owned call to an exact fully-qualified public production entry. The local checker digests the complete registered test and shared protocol AST, so setup, call arguments, normalization, mirror operations, and predicates are all within the checked source contract. Mapped markers and real adapters share that public entry; only two Serde conversions are explicitly reviewed boundaries. The protocol executes one real, mirror(None), and mirror(BrokenVariant) operation on one typed probe and asserts the real/control denial and broken permission differential. The co-located checker and digests are tamper-evident, not externally authenticated.
