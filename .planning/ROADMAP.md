...
3. `scripts/check-mapping.sh` is a required CI step and demonstrably fails against a deliberately unmapped `// INVARIANT:` marker.
   ...
   DEMONSTRATED by one clean and twelve adversarial fixtures, including an unmapped marker, coordinated row/marker/assumption deletion, deletion of all omissions, comment/string spoofing, wrong-function adjacency, and an orphan `ghost.rs` module.
4. Every MAPPING.md row has a `negative_*.rs` test asserting a broken variant permits what the real function denies; `scripts/check-negative-registry.sh` fails on any missing entry.
   CODE AND WORKFLOW MET at `tools/check-negative-registry.sh`: 57 real attributed tests across four files. Twenty-one adversarial fixtures cover non-tests, ignored and cfg-disabled tests, assertion-free `black_box` bodies, nonexistent modules/types, comment/string-only declarations and mutation evidence, decorative tokens, and missing real/control/broken calls. The gate executes all four exact Cargo targets and proves every registered name reports `ok` with zero ignored. Protected-required-check repository settings remain unverified.
