...
**Status:** Complete — ACCEPTED 2026-09-08 (merge `954c5cd02`). Candidate `1a5c9003b` (test-only DST harness) rejected; the durable dispatch-intent journal repair (`286-02`) accepted after the full network-enabled regression (blocked in the authoring sandbox) passed here + independent 6-dimension adversarial review (0 blockers/0 majors) + two review fixes.
**Plans:** `286-01-PLAN.md` (superseded/rejected), `286-02-PLAN.md` (accepted repair).
**Review:** `.planning/phases/286-deterministic-simulation-testing/286-REVIEW.md`.
**Success Criteria**:
1. The harness drives real runtime, gate, sandbox effect and persistent substrate paths. Both production runtime authorization entry sites require the same durable intent journal, and composition/reload preserve reservations. **MET: both `authorize_and_execute` paths reserve via `dispatch_once` before any effect (fail-closed without a journal); swarm-runtime lib 585/0 incl. journal unit/composition + `both_execution_entry_points_refuse_live_actions_without_a_dispatch_journal`.**
2. Inject future-drop before dispatch, future-drop after effect but before completion publication, and persistent-substrate close/reopen between policy allowance and publication. Reopen the same storage and redeliver the same immutable request; unresolved outcomes must never dispatch again. **MET: the DST reopens the real journal + substrate and redelivers; a consumed identity never re-dispatches (`completed_success_and_failure_never_reauthorize`); dispatch_integration 21/0, dst_fault_injection 4/0.**
...
4. `SWARM_DST_SEED=<n>` reproduces the exact fault schedule. MAPPING states what the final evidence covers: one host/process recovery model and one logical persistent substrate; signed audit remains distinct from the unsigned local journal. **MET: seed replay matches; MAPPING boundary section documents single-host/single-store + unsigned-local-journal-vs-signed-audit. Phase ACCEPTED.**

The rejected candidate's green corpus was NOT acceptance evidence (reverse-subset
receipt oracle accepted unrecorded effects, dropped denied episodes, no same-request
crash/retry, only four schedules, empty-store reopen). The ACCEPTED repair (286-02)
closes each with a real durable dispatch-intent journal, real crash/reopen/redelivery,
and mutation-falsified oracles (three production mutations each rejected, positives
restored). The full network-enabled regression — which the authoring sandbox could
not run (TCP listeners denied) — passes here: swarm-runtime lib 585, dispatch 21,
DST 4 + 5000-seed nightly, ingest 177, response + runtime-http 0 failures; all 6
static assurance gates + fmt + clippy -D warnings clean. Independent 6-dimension
adversarial review: 0 blockers / 0 majors; two confirmed findings fixed (transient-read
journal poison `a70d5f191`; SIEM retry over-scoping `f289573ea`). Accepted 2026-09-08.
...
**Success Criteria**:
1. `crates/swarm-policy/src/formal_core.rs` holds the approval, severity, rate-limit, and lease predicates as pure functions with no mutex, filesystem, clock, or network call in their bodies. — met: eb64f7c85 (rate-limit core, DCORE-01), f5cb3d8df (governance predicates, DCORE-02), 0f3fa0207 (severity predicates, DCORE-05): `formal_core` now also holds the severity gating that used to sit inline in `static_gate::evaluate` — `severity_floor_denial` (the `static.minimum_severity` and `static.deploy_decoy_min_severity` floors), `human_gate_decision` (the `static.human_gate` hold, which carries the `PolicyHumanGateOnDestructiveAction` invariant, now relocated with it), and the `destructive_action` classifier they both read — so every predicate this criterion enumerates (approval, severity, rate-limit, lease) literally lives in the pure core; all bodies contain no `Mutex`/`fs`/clock/network calls (grep + read).
