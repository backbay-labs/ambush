...
**Status:** Complete 2026-08-14. All four criteria met. Two requirement paths were stale and were corrected rather than created (`scripts/` -> `tools/`), the same correction phase 283 recorded. Decision record: `docs/decisions/0011-invariant-mapping-and-negative-registry.md`.
**Plans:** TBD
**Success Criteria**:
1. `docs/assurance/assumptions.toml` parses and enumerates at least 8 named assumptions, each with an owner and dependent invariants.
   MET with 11, parsed by `tomllib` in the gate rather than by eye. All 8 names MAPPING-01 lists are present, plus ASSUME-DETERMINISTIC-GATE, ASSUME-CONFIG-INTEGRITY and ASSUME-CHAIN-HEAD-DURABILITY. TWO HAVE NO DEPENDENT INVARIANT AND SAY SO: ASSUME-JETSTREAM-DURABILITY (no mapped row reaches the substrate) and ASSUME-SUBPROCESS-ISOLATION (vacuously true today — `grep -rl 'Command::new' crates/*/src` returns one `#[cfg(test)]` file). The gate requires a `no_dependent_invariants_reason` for an empty list; inventing rows to fill the column was refused. It also requires each `invariants` list to EQUAL the set of MAPPING.md rows naming that assumption, in both directions, so the two files cannot drift.
2. `docs/assurance/MAPPING.md` carries at least 12 invariant rows spanning `swarm-policy`, `swarm-response`, `swarm-runtime`, and `swarm-spine`, each naming an existing `crate::module::function` path.
   MET with 21: swarm-policy 5, swarm-response 4, swarm-runtime 6, swarm-spine 6. Existence is RESOLVED, not asserted — `tools/check-mapping.sh` walks crate directory, module file, declared type and declared `fn` for every row, over production text only (everything above the first column-0 `#[cfg(test)]`), so a function that exists only in a test module does not satisfy a row. Observed failing on the real tree: renaming one row's path to `release_lease_renamed` printed `[row-function-absent] row RUNTIME-FAILED-ROLLBACK-KEEPS-LEASE names swarm_runtime::containment::release_lease_renamed but crates/swarm-runtime/src/containment.rs declares no fn release_lease_renamed in production code`.
   THE HONEST CAVEAT IS IN THE TABLE. Four of the six swarm-spine rows are on `verify_chain_link`, which has NO production caller: `grep -rn verify_chain_link crates/` outside its own file and the negative test returns only the `pub use` re-export. They are mapped ahead of the caller deliberately and the row notes say so. That measurement surfaced a real gap: `swarm_runtime::approval::build_vote_envelope_hash` BUILDS an envelope chain and nothing ever verifies it. Reported, not fixed.
3. `scripts/check-mapping.sh` is a required CI step and demonstrably fails against a deliberately unmapped `// INVARIANT:` marker.
   MET at `tools/check-mapping.sh`, PATH CORRECTED for the reason phase 283 recorded: there is no `scripts/` directory and `tools/check-gates-wired.sh` enumerates only `tools/check-*.sh`, so the requirement's path would have made this gate invisible to the gate that catches unrun gates. Wired unconditionally into the `panic-contract` job; `check-gates-wired.sh` confirms it.
   DEMONSTRATED, on the real tree and not only in the fixture. Adding `// INVARIANT: DELIBERATELY-UNMAPPED` above `scope_for_response_action` in `crates/swarm-policy/src/static_gate.rs` gave exit 1 and `[marker-unmapped] // INVARIANT: DELIBERATELY-UNMAPPED in crates/swarm-policy/src/static_gate.rs has no row in docs/assurance/MAPPING.md`. The same case runs as one of 16 fixture cases on every invocation, before the script looks at the real tree at all.
4. Every MAPPING.md row has a `negative_*.rs` test asserting a broken variant permits what the real function denies; `scripts/check-negative-registry.sh` fails on any missing entry.
   MET at `tools/check-negative-registry.sh` (same path correction). 21 rows, 21 tests, across four `crates/*/tests/negative_*.rs` files. Deleting the `SPINE-CHAIN-ISSUER-BOUND` entry gave exit 1 and `[row-unregistered] MAPPING.md row SPINE-CHAIN-ISSUER-BOUND has no entry in docs/assurance/negative-registry.toml`; 15 further fixture cases run on every invocation.
   EVERY MUTATION WAS NEUTRALIZED AND WATCHED TO FAIL. For each row the removed guard was put back — so the "broken" variant is no longer broken — and the test was run and observed failing; the observed text is recorded per row in `negative-registry.toml` and the field is required non-empty. Each test also carries an UNMUTATED CONTROL asserting the mirror reproduces the real function on the same probe, which is what makes the mutated result attributable to the mutation rather than to a sloppy rewrite.
   WHAT IT CANNOT CHECK, STATED: that a mirror is a faithful copy. No static check establishes that; the control and review stand in for it, and both scripts' headers say so instead of implying a guarantee they do not provide.
