...
4. Every MAPPING.md row has a `negative_*.rs` test asserting a broken variant permits what the real function denies; `scripts/check-negative-registry.sh` fails on any missing entry.
   CODE AND WORKFLOW MET at `tools/check-negative-registry.sh`: 57 real attributed tests across four files plus a separate five-test compiled protocol contract. Thirty-six adversarial self-tests cover non-tests, ignored/cfg-disabled tests, production-shaped `.evaluate`/`black_box`/unrelated-assertion spoofing, exact named-adapter/production-entry drift, a real adapter replaced by mirror(None), fabricated libtest stdout, and thirteen mutations of the actual shared protocol (no-op/`if false`, omitted or swapped roles, and removed/inverted/vacuous assertions). The gate requires exact set equality from Cargo `--list` and exact whole-target passed counts with zero ignored for all four registered targets and the contract target. Protected-required-check repository settings remain unverified.
   Each registry row records its neutralization observation and binds `CASE_TYPE::real` to an exact fully-qualified public production entry; private-guard reachability through that entry is explicit. The protocol executes one real, mirror(None), and mirror(BrokenVariant) operation on one typed probe and asserts the real/control denial and broken permission differential.
   WHAT IT CANNOT CHECK, STATED: the entry binding is structural rather than runtime instrumentation, and a handwritten mirror is not mechanically proven faithful beyond the registered probe. The compiled differential, source mutations, and review stand in for those broader proofs.
