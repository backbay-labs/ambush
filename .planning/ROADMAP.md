...
4. Every MAPPING.md row has a `negative_*.rs` test asserting a broken variant permits what the real function denies; `scripts/check-negative-registry.sh` fails on any missing entry.
   CODE AND WORKFLOW MET at `tools/check-negative-registry.sh`: 57 real attributed tests across four files plus a separate five-test compiled protocol contract. Fifty-two adversarial self-tests cover non-tests, ignored/cfg-disabled tests, production-shaped `.evaluate`/`black_box`/unrelated-assertion spoofing, dead, conditional, or unreachable production calls, protocol aliases/re-exports/shadows, renamed mirrors, production-entry or normalizer drift, coordinated indirect-entry and baseline edits, fabricated libtest stdout, and thirteen mutations of the actual shared protocol (no-op/`if false`, omitted or swapped roles, and removed/inverted/vacuous assertions). The gate requires exact set equality from Cargo `--list` and exact whole-target passed counts with zero ignored for all four registered targets and the contract target. Protected-required-check repository settings remain unverified.
   Each registry row records its neutralization observation and binds the shared macro-owned call to an exact fully-qualified public production entry, argument/result-projection AST, and call mode authenticated by a checker-owned digest. Private-guard reachability through that entry is explicit: twelve indirect edges are mechanically traversed and two serde boundaries are identified as reviewed. The protocol executes one real, mirror(None), and mirror(BrokenVariant) operation on one typed probe and asserts the real/control denial and broken permission differential.
