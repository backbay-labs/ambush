...
4. Every MAPPING.md row has a `negative_*.rs` test asserting a broken variant permits what the real function denies; `scripts/check-negative-registry.sh` fails on any missing entry.
   CODE AND WORKFLOW MET at `tools/check-negative-registry.sh`: 57 built-in `#[test]` wrappers across four files plus a separate five-test compiled protocol contract. The gate reports 139 self-tests (3 clean controls, 136 adversarial), covering source/protocol/production-binding evasions plus fake proc macros, compiler and cfg substitution, executable build dependencies, Cargo runner/target redirection, all registered-crate manifest drift, external Cargo-home build.rustc/wrapper/workspace-wrapper/rustflags/linker/runner attacks, ancestor configuration, toolchain redirection, and cwd/PYTHONPATH module shadowing. It pins toolchain, manifests, lock resolution, metadata targets, and package/source artifact identity. Every Cargo command runs through a fresh config-free home with pinned Cargo/rustc and sanitized process settings; a gate-owned isolated-Python wrapper forces and audits one exact test-mode compilation per target, and emitted test binaries run directly under a sanitized environment for exact inventory and passed/ignored counts. Protected check provenance remains external.
