...
4. Every MAPPING.md row has a `negative_*.rs` test asserting a broken variant permits what the real function denies; `scripts/check-negative-registry.sh` fails on any missing entry.
   CODE AND WORKFLOW MET at `tools/check-negative-registry.sh`: 57 built-in `#[test]` wrappers across four files plus a separate five-test compiled protocol contract. The gate reports 121 self-tests (3 clean controls, 118 adversarial), covering non-tests, ignored/cfg-disabled or proc-macro-attributed tests, production-shaped `.evaluate`/`black_box`/unrelated-assertion spoofing, caller control flow, protocol and production-crate aliases/re-exports/shadows/globs, exact crate-root external-crate binding, helper/wrapper drift, dead genuine calls with fabricated results, forced or swapped mirror roles, identity-selective protocol returns, production-entry or normalizer drift, constant/ignored/swapped/vacuous predicates, coordinated local data-baseline edits, fake `tokio-macros` body erasure, fake-compiler and encoded-rustflags source/body substitution, Cargo-runner/libtest stdout fabrication, Cargo target redirection through `autotests = false`, explicit/harness-disabled test targets, library-path/crate-type overrides or custom builds, and manifest drift through path dev-dependencies, default features, or root profiles. The gate pins the complete semantic TOML of the four registered crate manifests plus root execution tables, validates locked Cargo resolution, pins auto-discovered integration-test and production-library metadata to canonical source paths, rejects compiler-affecting environment overrides, binds compiler artifacts to the expected package and source, rechecks execution inputs after compilation, compiles all five targets with `--locked`, and directly invokes each emitted binary for exact inventory and passed/ignored counts. Protected check provenance remains external.
