...
**Status:** Implementation complete 2026-08-14; acceptance open. The gates are wired in the workflow, but protected-required-check repository settings are unverified, so MAPPING-05 and FALSIFY-04 remain unchecked. Decision record: `docs/decisions/0011-invariant-mapping-and-negative-registry.md`.
**Plans:** TBD
**Success Criteria**:
1. `docs/assurance/assumptions.toml` parses and enumerates at least 8 named assumptions, each with an owner and dependent invariants.
   MET with 11, parsed by `tomllib`. `ASSUME-STATEFUL-GATE-DETERMINISM` truthfully models rate-window state transitions rather than claiming verdict purity. Rows name multiple assumptions where appropriate, and the gate requires exact many-to-many set equality.
2. `docs/assurance/MAPPING.md` carries at least 12 invariant rows spanning `swarm-policy`, `swarm-response`, `swarm-runtime`, and `swarm-spine`, each naming an existing `crate::module::function` path.
   MET with 41 executable rows across the four named crates, plus five enforced omissions for surfaces that render no pre-dispatch refusal. The checker lexically removes comments and literals, resolves the exact module/type/function body, and requires each unique marker to precede an executable decision inside that body.
   THE HONEST CAVEAT IS IN THE TABLE. Seven of the ten swarm-spine rows are on `verify_chain_link`, which has NO production caller: `grep -rn verify_chain_link crates/` outside its own file and the negative test returns only the `pub use` re-export. They are mapped ahead of the caller deliberately and the row notes say so. That measurement surfaced a real gap: `swarm_runtime::approval::build_vote_envelope_hash` BUILDS an envelope chain and nothing ever verifies it. Reported, not fixed.
3. `scripts/check-mapping.sh` is a required CI step and demonstrably fails against a deliberately unmapped `// INVARIANT:` marker.
   CODE AND WORKFLOW MET at `tools/check-mapping.sh`, including adversarial comment/string and marker-adjacency fixtures. The workflow invokes it unconditionally. Repository settings have not been checked, so this is not yet evidence that the job is a protected required check.
   DEMONSTRATED, on the real tree and not only in the fixture. Adding `// INVARIANT: DELIBERATELY-UNMAPPED` above `scope_for_response_action` in `crates/swarm-policy/src/static_gate.rs` gave exit 1 and `[marker-unmapped] // INVARIANT: DELIBERATELY-UNMAPPED in crates/swarm-policy/src/static_gate.rs has no row in docs/assurance/MAPPING.md`. The current checker runs one clean control and nine adversarial fixtures before it looks at the real tree.
4. Every MAPPING.md row has a `negative_*.rs` test asserting a broken variant permits what the real function denies; `scripts/check-negative-registry.sh` fails on any missing entry.
   CODE AND WORKFLOW MET at `tools/check-negative-registry.sh`: 41 real attributed tests across four files. Fifteen adversarial fixtures cover non-tests, nonexistent modules/types, comment/string-only declarations and mutation evidence, and decorative tokens. Protected-required-check repository settings remain unverified.
   Each registry row records its neutralization observation, and the field is required non-empty. The review remediation re-ran the disputed sequence, lease-bound and failed-rollback neutralizations and made their recorded left/right output match the reproduced failure. Each test also carries an UNMUTATED CONTROL asserting the mirror reproduces the real function on the same probe, which is what makes the mutated result attributable to the mutation rather than to a sloppy rewrite.
...
| 285. Assumption Registry And Invariant Mapping | v1.79 | 7/9 requirements | Implementation complete; MAPPING-05 and FALSIFY-04 await protected-required-check repository settings | 2026-08-14 |
