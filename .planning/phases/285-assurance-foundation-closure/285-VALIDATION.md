...
| 285-04-03A | 04 | 5 | ASSURE-04, ASSURE-06 | r14 conditional four-account dual public/private NATS relay, 18/18 distinct recovery executions, executed five-value exact-3,000 ms constructor oracle, and 9/9 event-derived observations from the real queue/runner/proxy/CAS/publisher deadline state machine; init/dependency closure remains 03B | `set -e; test "$(git rev-parse HEAD)" = 5ade79141a2f8249ef084d24fcfd5ef12251e48e; test -n "${PHASE285_PLAN04_TASK03A_TREE:-}"; test -n "${PHASE285_WITNESS_INTEGRITY_LAUNCHER_SHA256:-}"; test -n "${PHASE285_WITNESS_INTEGRITY_MANIFEST_SHA256:-}"; test "$(git write-tree)" = "$PHASE285_PLAN04_TASK03A_TREE"; test -z "$(git diff --name-only)"; test -z "$(git ls-files --others --exclude-standard)"; git diff --cached --check 5ade79141a2f8249ef084d24fcfd5ef12251e48e; test "$(git diff --cached --name-only 5ade79141a2f8249ef084d24fcfd5ef12251e48e | LC_ALL=C sort | shasum -a 256 | awk '{print $1}')" = 1decd3b39a6433d9358a10289d6886f67211b2560563b417381273dd1662d614; test "$(git diff --cached --name-only 5ade79141a2f8249ef084d24fcfd5ef12251e48e | wc -l | tr -d ' ')" = 16; bash tools/check-phase285-witness-integrity.sh --integrity-self-test; bash tools/check-phase285-witness-integrity.sh public-dispatcher; bash tools/with-nats-jetstream.sh --self-test; bash tools/with-nats-jetstream.sh env PHASE285_WITNESS_INTEGRITY_LAUNCHER_SHA256="$PHASE285_WITNESS_INTEGRITY_LAUNCHER_SHA256" PHASE285_WITNESS_INTEGRITY_MANIFEST_SHA256="$PHASE285_WITNESS_INTEGRITY_MANIFEST_SHA256" bash tools/check-phase285-witness-integrity.sh full-service-path; cargo test -p swarm-governance --lib --locked --offline -- --test-threads=1; cargo test -p swarm-governance-witness --all-targets --locked --offline --no-run; bash tools/with-nats-jetstream.sh --relay-service-checkpoint env PHASE285_WITNESS_INTEGRITY_LAUNCHER_SHA256="$PHASE285_WITNESS_INTEGRITY_LAUNCHER_SHA256" PHASE285_WITNESS_INTEGRITY_MANIFEST_SHA256="$PHASE285_WITNESS_INTEGRITY_MANIFEST_SHA256" PHASE285_SERVICE_CHECKPOINT_TREE="$PHASE285_PLAN04_TASK03A_TREE" bash tools/check-phase285-witness-integrity.sh --focused-service-checkpoint-recovery; cargo clippy -p swarm-governance -p swarm-governance-witness --all-targets --all-features --locked --offline -- -D warnings; cargo fmt --all -- --check; shellcheck tools/check-phase285-witness-conformance.sh tools/with-nats-jetstream.sh; test "$(git rev-parse HEAD)" = 5ade79141a2f8249ef084d24fcfd5ef12251e48e; test "$(git write-tree)" = "$PHASE285_PLAN04_TASK03A_TREE"; test -z "$(git diff --name-only)"; test -z "$(git ls-files --others --exclude-standard)"; git diff --cached --check 5ade79141a2f8249ef084d24fcfd5ef12251e48e` using only the separately reviewed r14 04-03A freeze | ❌ W0 | ⬜ pending |
