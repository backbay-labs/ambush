...
| 285-04-03A1 | 04 | 5 | ASSURE-04, ASSURE-06 | both shipping public `start` methods delegate to private `start_inner`; cfg(test) builders alone inject the recorder; cheap inherited source preflight proves 1 public signature + 1 private signature + 2 typed parameters + 1 delegation and 44/44 unique mutations; actual-runner receipts bind queue rows; real-constructor receipt joins row 9; `live_nats_grants_proved=false`; exactly 18 ledger, 19 first-library-FQN, 7 internal-live-callsite, 5 live-constructor, 31 unique-source, and 10 boundary controls; no live-grant/relay/recovery claim; exact 8-path digest 3b9e0dcdef3b37beb4dd22cfb576cb0f3d72d45e2c87c54042c548dbc4deacd7 | `bash tools/check-phase285-witness-conformance.sh --self-test store-proxy-source && test "$(git rev-parse HEAD)" = "$PHASE285_PLAN04_TASK03A1_COMMIT" && test "$(git rev-parse "$PHASE285_PLAN04_TASK03A1_COMMIT^{tree}")" = "$PHASE285_PLAN04_TASK03A1_TREE" && test "$(git show "${PHASE285_PLAN04_TASK03A1_COMMIT}:tools/check-phase285-witness-integrity.sh" | shasum -a 256 | awk '{print $1}')" = "$PHASE285_WITNESS_INTEGRITY_LAUNCHER_SHA256" && test "$(git show "${PHASE285_PLAN04_TASK03A1_COMMIT}:tools/fixtures/phase285-witness-integrity.json" | shasum -a 256 | awk '{print $1}')" = "$PHASE285_WITNESS_INTEGRITY_MANIFEST_SHA256" && bash tools/with-nats-jetstream.sh env PHASE285_WITNESS_INTEGRITY_LAUNCHER_SHA256="$PHASE285_WITNESS_INTEGRITY_LAUNCHER_SHA256" PHASE285_WITNESS_INTEGRITY_MANIFEST_SHA256="$PHASE285_WITNESS_INTEGRITY_MANIFEST_SHA256" PHASE285_SERVICE_CHECKPOINT_TREE="$PHASE285_PLAN04_TASK03A1_TREE" bash tools/check-phase285-witness-integrity.sh --focused-service-checkpoint-deadline` plus immutable-commit path/tree, public/full-service, package, clippy, fmt, shell, diff, review, and post-review push gates in 285-04-PLAN.md | ❌ W0 | ⬜ pending |
| 285-04-03A2 | 04 | 6 | ASSURE-04, ASSURE-06 | four-account/five-principal authenticated dual public/private relay plus separate tree-pinned normal/relay exact private/public 3,000/12,000-ms max-one NATS response-grant gates at fixed 2,500/3,500 and 10,500/12,500-ms schedules, with same-request second-publish server rejection; one bounded suppressed reply and successful replay; preserves A1; exact 13-path delta digest 63891e64631df71736d4a21c852c43cec261b257b2508c10db72d2fc4c04c164 and 14-path cumulative digest 4e6629733654f7b4b6f1233bfaaef96a0fede0368620380c8b97f9d75bb18664 | predecessor/candidate-ref/tree/launcher/manifest hash gates, then literal source preflight `bash tools/check-phase285-witness-conformance.sh --self-test store-proxy-source` requiring `store_proxy_source_guard passed=1` and `store_proxy_source_guard_self_test mutations=44 unique=44 passed=1`, then literal normal-harness A1 rerun `bash tools/with-nats-jetstream.sh env PHASE285_WITNESS_INTEGRITY_LAUNCHER_SHA256="$PHASE285_WITNESS_INTEGRITY_LAUNCHER_SHA256" PHASE285_WITNESS_INTEGRITY_MANIFEST_SHA256="$PHASE285_WITNESS_INTEGRITY_MANIFEST_SHA256" PHASE285_SERVICE_CHECKPOINT_TREE="$PHASE285_PLAN04_TASK03A2_TREE" bash tools/check-phase285-witness-integrity.sh --focused-service-checkpoint-deadline`, normal `--focused-service-checkpoint-grants`, relay `--focused-service-checkpoint-relay`, and strict immutable review/push gates in 285-04-PLAN.md | ❌ W0 | ⛔ blocked by accepted A1 checkpoint |
| 285-04-03A3 | 04 | 7 | ASSURE-04, ASSURE-06 | 18/18 physical restart/lost-response rows plus coherent abort successor/binding mutants; preserves A1+A2; exact 6-path delta digest 769825832abfff2e80561b7c4692a0aec750699096f094e3ab5c54765726d4ae and 16-path cumulative digest 1decd3b39a6433d9358a10289d6886f67211b2560563b417381273dd1662d614 | predecessor/candidate-ref/tree/launcher/manifest hash gates, then literal source preflight `bash tools/check-phase285-witness-conformance.sh --self-test store-proxy-source` requiring `store_proxy_source_guard passed=1` and `store_proxy_source_guard_self_test mutations=44 unique=44 passed=1`, then literal normal-harness A1 rerun `bash tools/with-nats-jetstream.sh env PHASE285_WITNESS_INTEGRITY_LAUNCHER_SHA256="$PHASE285_WITNESS_INTEGRITY_LAUNCHER_SHA256" PHASE285_WITNESS_INTEGRITY_MANIFEST_SHA256="$PHASE285_WITNESS_INTEGRITY_MANIFEST_SHA256" PHASE285_SERVICE_CHECKPOINT_TREE="$PHASE285_PLAN04_TASK03A3_TREE" bash tools/check-phase285-witness-integrity.sh --focused-service-checkpoint-deadline`, normal grants, relay recovery/relay, and strict immutable review/push gates in 285-04-PLAN.md | ❌ W0 | ⛔ blocked by accepted A2 checkpoint |
