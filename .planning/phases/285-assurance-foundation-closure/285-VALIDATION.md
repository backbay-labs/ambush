...
The hosted `selector_records` set equals, with no omission or addition, every literal case row in Plan01's Rust registry, the six literal transport rows in this validation file, the literal governance/detector cases plus `cli-migration` registered by Plans05A through 06C, Plan06C's five literal `combined_checkpoint_*` direct controls, and Plan07B's literal `external-test` registry covering every intentional external-environment ignored FQN not already owned by those selectors. The hosted `gate_records` set has exactly these IDs: `plan-schema`, `workspace-tests`, `serial-runtime-ingest-tests`, `strict-clippy`, `format`, `diff-check`, `clean-tree`, `workspace-layering`, `single-governor-key`, `mapping`, `negative-registry`, `fixture-freshness`, `supply-chain`, `sbom`, `witness-dependency-closure`, `deployment-render`, `deployment-live`, and `gates-wired`. Case/target inventories are enumerated before execution; a duplicate, extra, missing, substring-only, filtered-only, skipped, or wrong-filter-count record is invalid.

Plan07B's `bash tools/check-phase285-external-tests.sh --all` owns the literal set difference between compiler-discovered intentional external-environment ignores and all FQNs already owned by Plan01-through-Plan06C selectors. Its registry includes the complete standalone `swarm-pheromone` library and integration JetStream suites. Each row executes by exact FQN in its declared real harness and emits a distinct `SelectorRecordV1` with selector `external-test`. Independent discovery must equal the union of earlier selector-owned ignores and this literal registry; omission, addition, substitution, duplicate ownership, reason/target drift, zero or filtered-only execution, or a synthetic receipt fails.

`workspace-tests` is produced only by `bash tools/check-phase285-workspace-tests.sh`, never by treating a raw successful `cargo test --workspace` exit as sufficient evidence. The orchestrator compiles every workspace test target; validates the exact FQN and stable-reason inventory of intentional external-environment `#[ignore]` tests; and runs only compiler-discovered FQNs not owned by `serial-runtime-ingest-tests`, a Plan01-through-Plan06C selector, or Plan07B's `external-test` registry. It supplies that complete exclusion list explicitly to each test binary so every accepted summary reports `ignored: 0`, then consumes the already-produced confined receipts for each excluded selector/external FQN. Final closure invokes all selector and external lanes first. The exact union of workspace, serial runtime/ingest, selector, and external-test receipts must equal compiler discovery with each FQN owned and executed once. An ignored pure test, ignored-FQN or reason drift, missing/extra/duplicate/substituted test, excluded unowned test, cross-lane overlap, nonzero accepted ignored count, receipt reuse, wrong target/filter count, or selector omission fails the gate. Both checkers self-test each mutation. `GateRecordV1.ignored: 0` therefore describes accepted executions, while external-environment discovery entries are independently executed and bound rather than silently accepted.
...
bash tools/check-phase285-external-tests.sh --all
