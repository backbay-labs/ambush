...
Null GitHub attribution is therefore not a permanent external-email dependency. Before freezing the subject, each unresolved author or committer creates one API-authenticated same-repository commit-comment association; the dispatch bundle carries the exact association records, and the review workflow independently fetches and validates the comments plus raw Git identity digests. An unresolved role without that proof remains blocked before freeze, while an exact valid association produces the same stable GitHub-login principal used by directly resolved commits.
