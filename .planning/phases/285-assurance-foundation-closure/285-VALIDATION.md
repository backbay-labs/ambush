...
| 285-04-03A1 | 04 | 5 | ASSURE-04, ASSURE-06 | real public/private worker state machines plus 9/9 event-derived receipt-anchored deadline rows; no relay/recovery claim; exact 8-path digest 3b9e0dcdef3b37beb4dd22cfb576cb0f3d72d45e2c87c54042c548dbc4deacd7 | `test "$(git rev-parse HEAD)" = 5ade79141a2f8249ef084d24fcfd5ef12251e48e && cargo test -p swarm-governance-witness --lib --locked --offline deadline_state_machine_tests::deadline_state_machine_is_receipt_anchored_and_mutation_sensitive -- --exact --test-threads=1 && bash tools/check-phase285-witness-integrity.sh --focused-service-checkpoint-deadline` plus exact path/tree, package, clippy, fmt, shell, diff, immutable review, and pushed-ref gates in 285-04-PLAN.md | ❌ W0 | ⬜ pending |
| 285-04-03A2 | 04 | 6 | ASSURE-04, ASSURE-06 | four-account/five-principal authenticated dual public/private relay with one bounded suppressed reply and successful replay; preserves A1; exact 13-path delta digest 63891e64631df71736d4a21c852c43cec261b257b2508c10db72d2fc4c04c164 and 14-path cumulative digest 4e6629733654f7b4b6f1233bfaaef96a0fede0368620380c8b97f9d75bb18664 | `test -n "$PHASE285_PLAN04_TASK03A1_COMMIT" && cargo test -p swarm-governance-witness --test service_checkpoint --locked --offline -- service_checkpoint_relay_traces_public_and_private_routes --exact && bash tools/with-nats-jetstream.sh --relay-service-checkpoint bash tools/check-phase285-witness-integrity.sh --focused-service-checkpoint-relay` plus A1 regression, exact path/tree, strict validation, immutable review, and pushed-ref gates | ❌ W0 | ⛔ blocked by accepted A1 checkpoint |
| 285-04-03A3 | 04 | 7 | ASSURE-04, ASSURE-06 | 18/18 physical restart/lost-response rows plus coherent abort successor/binding mutants; preserves A1+A2; exact 6-path delta digest 769825832abfff2e80561b7c4692a0aec750699096f094e3ab5c54765726d4ae and 16-path cumulative digest 1decd3b39a6433d9358a10289d6886f67211b2560563b417381273dd1662d614 | `test -n "$PHASE285_PLAN04_TASK03A2_COMMIT" && cargo test -p swarm-governance-witness --test service_checkpoint --locked --offline -- service_checkpoint_recovers_every_operation_after_restart_or_lost_response --exact && bash tools/with-nats-jetstream.sh --relay-service-checkpoint bash tools/check-phase285-witness-integrity.sh --focused-service-checkpoint-recovery` plus separate A1/A2 regression, exact path/tree, strict validation, immutable review, and pushed-ref gates | ❌ W0 | ⛔ blocked by accepted A2 checkpoint |
