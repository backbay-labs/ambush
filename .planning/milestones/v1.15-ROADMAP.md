...
`v1.14` closed the continuity gap from one ranked review packet back into the existing queue, handoff, and canary path. The runtime could already select one ranked candidate, review it explicitly, and bridge it back into rollout without re-materializing evidence.

`v1.15` widened that review seam without widening autonomy. The runtime now supports portfolio-level comparison across multiple ranked batches or cohorts, explicit operator curation over that portfolio, and governance-ready packet generation that preserves evidence for a future trust-boundary lane. The milestone stayed CLI-first, artifact-first, and fail-closed.
