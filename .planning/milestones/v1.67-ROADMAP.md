...
**Goal:** Zeroize resolved plaintext secrets from heap memory after the runtime finishes using them.
**Requirements:** ZERO-01
**Depends on:** v1.66 milestone complete
**Status:** Complete
**Plans:** 1/1 planned
**Success Criteria**:
1. Plaintext secrets resolved from `@secret:` references or equivalent runtime secret sources are wrapped in zeroizable storage and cleared after use.
2. Secret reload and handoff paths avoid keeping unnecessary long-lived cloned plaintext in heap-backed config or adapter state.
3. Tests cover the shipped secret-lifecycle seams well enough to prove zeroization happens on the supported runtime paths.

### Phase 237: Release Build Hardening

**Goal:** Harden release binaries to reduce memory disclosure and arithmetic-footgun risk.
**Requirements:** ZERO-02
**Depends on:** Phase 236
**Status:** Complete
**Plans:** 1/1 planned
**Success Criteria**:
1. Production release profiles use `panic = "abort"` for the shipped binaries rather than unwinding through sensitive runtime state.
2. Overflow-check expectations are explicit and verified for release builds instead of being left implicit.
3. Hardened release configuration is build-proven on the supported workspace targets without breaking the shipped operator workflows.

### Phase 238: Rotatable Token Lifecycle

**Goal:** Support expiring and rotating bearer tokens without requiring a full runtime restart.
**Requirements:** TOKEN-01
**Depends on:** Phase 237
**Status:** Complete
**Plans:** 1/1 planned
**Success Criteria**:
1. Operator and platform bearer tokens carry explicit expiry or rotation metadata through repo-owned configuration.
2. The runtime can reload or rotate token material without forcing a full process restart.
3. Expired or rotated tokens fail closed with clear audit or status context on the protected HTTP surfaces.

### Phase 239: HTTP Rate Limiting

**Goal:** Enforce configurable per-source rate limits on the shipped HTTP API surfaces.
**Requirements:** TOKEN-02
**Depends on:** Phase 238
**Status:** Complete
**Plans:** 1/1 planned
**Success Criteria**:
1. The shipped HTTP API surfaces enforce per-source burst and sustained rate limits from repo-owned configuration.
2. Rate-limit violations fail closed with bounded response semantics and operator-visible audit context.
3. Tests prove allowed burst behavior, sustained-threshold rejection, and recovery after refill for the protected request paths.
...
---
*Active milestone: v1.67 Secret Zeroization And API Token Lifecycle*
*Next step: Archive v1.67 and promote v1.68.*
*Last updated: 2026-04-13 — Completed release hardening, rotatable bearer lifecycle, and shared HTTP rate limiting across phases 237-239.*
