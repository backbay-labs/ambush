...
- [x] **REFAC-01**: swarmctl CLI logic is extracted into testable library harnesses with the binary reduced to a thin clap wrapper
- [x] **REFAC-02**: operator_http.rs is split into focused route modules behind a dedicated `http/` boundary
- [x] **REFAC-03**: review_workbench.rs is split into focused modules with direct regression coverage
- [x] **REFAC-04**: replay.rs is split into focused modules behind the directory-based `replay/` boundary

### Test Coverage

- [x] **TEST-01**: swarm-runtime coverage is explicitly measured and materially increased
- [x] **TEST-02**: ingest.rs has tests covering event validation, HTTP error cases, and batch processing edge cases
- [x] **TEST-03**: Hot-path modules (pipeline, service, detection) are consolidated into a detection/ submodule with clear boundaries
...
|-------------|-------|--------|
| REFAC-01 | Phase 88 | Satisfied |
| REFAC-02 | Phase 88 | Satisfied |
| REFAC-03 | Phase 88 | Satisfied |
| REFAC-04 | Phase 88 | Satisfied |
| TEST-01 | Phase 89 | Satisfied |
| TEST-02 | Phase 89 | Satisfied |
| TEST-03 | Phase 89 | Satisfied |
