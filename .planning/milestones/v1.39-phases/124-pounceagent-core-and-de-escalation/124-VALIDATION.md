...
|---------|------|------|-------------|-----------|-------------------|-------------|--------|
| 124-01-01 | 124-01 | 1 | POUNCE-03, DEESC-02 | config smoke | `cargo check -p swarm-core -p swarm-policy && cargo test -p swarm-runtime config::tests::loads_repository_ruleset -- --exact` | ✅ existing | ✅ green |
| 124-05-01 | 124-05 | 2 | POUNCE-03, DEESC-02 | compile smoke | `cargo check -p swarm-runtime --lib` | ✅ existing | ✅ green |
| 124-05-02 | 124-05 | 2 | POUNCE-03, DEESC-02 | compile smoke | `cargo check -p swarm-runtime --lib` | ✅ existing | ✅ green |
| 124-02-01 | 124-02 | 3 | DEESC-01 | unit | `cargo test -p swarm-core agent::tests::mode_state_transition_down_clears_triggering_threat_class -- --exact` | ✅ extend | ✅ green |
| 124-02-02 | 124-02 | 3 | DEESC-02 | integration | `cargo test -p swarm-runtime --test escalation_integration concentration_monitor_deescalates_after_cooldown -- --exact` | ✅ extend | ✅ green |
| 124-03-01 | 124-03 | 3 | POUNCE-01, POUNCE-02, POUNCE-03 | test seeding | `rg -n "pounceagent_emits_request_response_for_alert_and_incident|pounceagent_skips_scope_present_in_peer_findings|response_playbook_selects_actions_by_threat_severity_and_confidence" crates/swarm-runtime/tests/pounceagent_integration.rs` | ✅ planned | ✅ green |
| 124-03-02 | 124-03 | 3 | POUNCE-01, POUNCE-02, POUNCE-03 | integration smoke | `cargo test -p swarm-runtime --test pounceagent_integration pounceagent_emits_request_response_for_alert_and_incident -- --exact` | ✅ planned | ✅ green |
| 124-04-01 | 124-04 | 4 | POUNCE-04, POUNCE-05, POLICY-01 | test seeding | `rg -n "request_response_routes_through_authorize_and_execute|pounceagent_dry_run_routes_through_runtime_path|expired_capability_lease_fails_closed_before_execution|receipt_preserves_original_hunt_id_and_lineage_evidence" crates/swarm-runtime/tests/dispatch_integration.rs` | ✅ extend | ✅ green |
| 124-04-02 | 124-04 | 4 | POUNCE-04, POUNCE-05, POLICY-01 | integration smoke | `cargo test -p swarm-runtime --test dispatch_integration request_response_routes_through_authorize_and_execute -- --exact` | ✅ extend | ✅ green |
