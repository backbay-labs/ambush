...
### v1.49 Canonical Runtime Contract And Governance Modes

**Goal:** Rewrite the active architecture, governance, and evolution contract to match what the Rust runtime actually ships, including identity admission, human gates, consensus receipts, degraded governance, and partition behavior.
**Executable phases:** 164-167

- [x] **Phase 164: Canonical Capability Matrix And Source-Of-Truth Reset** - Re-established one active-vs-historical contract set for runtime, agent, config, and deployment surfaces so the planning and docs layer now describes the same system. (CONTRACT-01, CONTRACT-02) (completed 2026-04-10)
- [x] **Phase 165: Governance Modes And Identity Admission Contracts** - Canonicalized local guarded response, human gates, identity admission, and receipt-backed quorum behavior into one operator-readable governance contract. (CONTRACT-03, GOVMODE-01, GOVMODE-02) (completed 2026-04-10)
- [x] **Phase 166: Degraded Governance And Partition Recovery Spec** - Defined fail-closed degraded-governance, contingency lease, partition, and reconciliation behavior against the shipped runtime and serve-mode surfaces. (GOVMODE-03) (completed 2026-04-10)
- [x] **Phase 167: Evolution, Rollout, And Operator Contract Consolidation** - Aligned queue, canary, promotion, proof, and operator-review semantics with the current runtime-owned evolution and approval lanes. (EVOCONTRACT-01) (completed 2026-04-10)

### Phase 164: Canonical Capability Matrix And Source-Of-Truth Reset

**Goal:** Make the docs and planning layer describe the active Rust runtime instead of a mixture of active, historical, and deferred system shapes.
**Requirements:** CONTRACT-01, CONTRACT-02
**Depends on:** Phase 163
**Status:** Complete
**Plans:** 1
**Success Criteria**:
1. One canonical capability matrix distinguishes critical lane, async lane, governance, evolution, and operator surfaces.
2. Active and historical documents are explicitly split without contradictory claims about what is shipped versus deferred.
3. The planning docs, canonical docs, and repo-owned config examples point at the same runtime shape.
...
2. Identity rotation, admission, rejection, and receipt verification are described in one active contract.
3. Config, health, and operator surfaces reference the same governance semantics.
...
**Plans:** 1
**Success Criteria**:
1. Degraded committee health, contingency lease redemption, and partition reconciliation rules are specified against current runtime behavior.
2. Partition-era observability, denied-action handling, and recovery markers are operator-visible and auditable in the contract.
3. The spec identifies what remains bounded and what is explicitly deferred.
...
2. The operator contract explains what is automatic, what is gated, and what remains advisory-only.
3. Evolution status, proof exports, and review surfaces align with the written contract.

## Queued Milestones

### v1.50 Async Enrichment And Correlation Depth

**Goal:** Make the async lane a first-class product surface by adding multi-graph Weaver correlation, investigation scheduling and confidence voting, and deeper behavioral baselines across host, identity, and peer-group scope.
**Executable phases:** 168-171

- [ ] **Phase 168: Weaver Multi-Graph Correlation Foundations** - Extend the current graph-backed correlation lane toward temporal, causal, entity, and semantic traversal with richer explainable evidence. (ASYNC-01, ASYNC-02) (queued)
- [ ] **Phase 169: Investigation Scheduling And Confidence Voting** - Prioritize async work by criticality, freshness, and learned value, and add speculate-plus-vote handling for ambiguous investigations. (SCHED-01, SCHED-02) (queued)
- [ ] **Phase 170: Identity And Peer-Group Behavioral Baselines** - Extend behavioral baselines beyond per-host scope into identity and peer-group learning with durable decay and scope-specific thresholds. (BEHAV-01, BEHAV-02) (queued)
- [ ] **Phase 171: Async Enrichment Surface And Integration Proof** - Surface queue/backpressure/correlation outcomes and prove the bounded detect → investigate → correlate → operator-review path. (ASYNC-03, ASYNC-04) (queued)

### v1.51 Assurance-Gated Evolution And Counterexample Loop

**Goal:** Turn `v1.48` artifacts into real gate inputs by feeding evasion coverage, solver proofs, and counterexamples into queue, canary, promotion, and mutation decisions.
**Executable phases:** 172-175

- [ ] **Phase 172: Assurance Policy And Gate Inputs** - Define assurance policy from coverage floors, solver outcomes, and fail-closed gate thresholds. (ASSURE-01, ASSURE-02) (queued)
- [ ] **Phase 173: Counterexample Harvest And Replay Regeneration** - Convert evasion misses and solver counterexamples into replayable regression and mutation inputs with lineage. (COUNTER-01, COUNTER-02) (queued)
- [ ] **Phase 174: Assurance-Gated Queue, Canary, And Promotion** - Apply assurance policy to the queue, canary, and promotion state machines with fail-closed enforcement. (ASSURE-03) (queued)
- [ ] **Phase 175: Waivers, Review Surface, And Assurance Lineage** - Add signed waivers, bounded operator overrides, and surfaced assurance lineage to review and proof artifacts. (ASSURE-04, ASSURE-05) (queued)
...
| 164. Canonical Capability Matrix And Source-Of-Truth Reset | v1.49 | 1/1 | Pending | - |
| 165. Governance Modes And Identity Admission Contracts | v1.49 | 1/1 | Pending | - |
| 166. Degraded Governance And Partition Recovery Spec | v1.49 | 1/1 | Pending | - |
| 167. Evolution, Rollout, And Operator Contract Consolidation | v1.49 | 1/1 | Pending | - |
| 168. Weaver Multi-Graph Correlation Foundations | v1.50 | 0/1 | Pending | - |
| 169. Investigation Scheduling And Confidence Voting | v1.50 | 0/1 | Pending | - |
| 170. Identity And Peer-Group Behavioral Baselines | v1.50 | 0/1 | Pending | - |
| 171. Async Enrichment Surface And Integration Proof | v1.50 | 0/1 | Pending | - |
| 172. Assurance Policy And Gate Inputs | v1.51 | 0/1 | Pending | - |
| 173. Counterexample Harvest And Replay Regeneration | v1.51 | 0/1 | Pending | - |
| 174. Assurance-Gated Queue, Canary, And Promotion | v1.51 | 0/1 | Pending | - |
| 175. Waivers, Review Surface, And Assurance Lineage | v1.51 | 0/1 | Pending | - |
...
*Next step: `$gsd-autonomous`*
*Last updated: 2026-04-10 — Activated milestones v1.49 through v1.53 and prepared Phase 164*
