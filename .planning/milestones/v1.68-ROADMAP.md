...
**Goal:** Normalize caret insertion and environment-variable substitution before detector evaluation.
**Requirements:** DEOBF-01
**Depends on:** v1.68 milestone complete
**Status:** Pending
**Plans:** 0/1 planned
**Success Criteria**:
1. Command-line detectors evaluate a normalized representation that strips caret-insertion obfuscation and expands environment-variable indirection.
2. The normalization layer preserves auditable lineage between raw and normalized command content.
3. Existing detector families keep their current behavior on already-normalized inputs.

### Phase 245: Unicode Homoglyph And Encoding Normalization

**Goal:** Extend pre-evaluation normalization to Unicode homoglyphs and common encoded command forms.
**Requirements:** DEOBF-02
**Depends on:** Phase 244
**Status:** Pending
**Plans:** 0/1 planned
**Success Criteria**:
1. Detector evaluation sees normalized command content for Unicode homoglyph substitutions and supported encoded-argument forms.
2. The normalization layer stays bounded and explainable instead of introducing opaque free-form rewriting.
3. The new normalization composes with the Phase 244 caret and env-var expansion lane.

### Phase 246: Evasion Catch-Rate Improvement Measurement

**Goal:** Quantify catch-rate improvement from the new normalization layer on evasion-heavy scenarios.
**Requirements:** DEOBF-03
**Depends on:** Phase 245
**Status:** Pending
**Plans:** 0/1 planned
**Success Criteria**:
1. The measured benchmark shows at least 15% catch-rate improvement on the targeted defense-evasion and execution scenarios.
2. The benchmark persists comparable baseline-versus-normalized results on a repo-owned surface.
3. The proof names which detector lanes improved and which scenarios were used.

### Phase 247: Normalization False-Positive Regression Test

**Goal:** Prove the new command-line normalization does not regress benign-baseline false positives.
**Requirements:** DEOBF-04
**Depends on:** Phase 246
**Status:** Pending
**Plans:** 0/1 planned
**Success Criteria**:
1. The benign baseline corpus shows no false-positive regression after the normalization layer lands.
2. The regression proof is executable and repeatable from the repo.
3. The milestone closes with a bounded statement of the detection gain and benign-baseline safety result.
