...
- [x] **Phase 230: Service Module Decomposition** - Split service.rs into lifecycle, orchestration, and request handling. (completed 2026-04-13)
- [x] **Phase 231: RuntimeService Arc Reduction** - Refactor RuntimeService to reduce Arc cloning on the request path. (completed 2026-04-13)

### Phase 228: Config Module Decomposition

**Goal:** Split `swarm-core/src/config.rs` into focused sub-modules without changing the shipped config contract.
**Requirements:** CFGEXT-01
**Depends on:** v1.64 milestone complete
**Status:** Completed 2026-04-13
**Plans:** 1/1 plans complete
**Success Criteria**:
1. `swarm-core/src/config.rs` is extracted into focused sub-modules or a dedicated config crate, with no single config file exceeding 2000 lines.
2. The public config API remains bounded and compatible with the currently shipped runtime, CLI, and operator surfaces.
3. The decomposition leaves the config boundary easier to navigate and reason about than the pre-split monolith.

### Phase 229: Config Compilation Boundary Verification

**Goal:** Prove config changes now rebuild only the crates that actually depend on config state.
**Requirements:** CFGEXT-02
**Depends on:** Phase 228
**Status:** Completed 2026-04-13
**Plans:** 1/1 plans complete
**Success Criteria**:
1. The repo has a concrete verification method showing config-only edits no longer trigger whole-workspace recompilation.
2. The measured rebuild scope is reduced to the crates that genuinely depend on the decomposed config boundary.
3. The verification artifact documents the remaining rebuild edges clearly enough for future crate-extraction work.

### Phase 230: Service Module Decomposition

**Goal:** Split `swarm-runtime/src/service.rs` into focused lifecycle, orchestration, and request-handling modules.
**Requirements:** SVCMOD-01
**Depends on:** Phase 229
**Status:** Completed 2026-04-13
**Plans:** 1/1 plans complete
**Success Criteria**:
1. `swarm-runtime/src/service.rs` is decomposed into focused sub-modules with no single service file exceeding 2000 lines.
2. Lifecycle management, agent orchestration, and request handling each have explicit ownership boundaries instead of one monolithic service surface.
3. The shipped runtime behavior remains unchanged while the service internals become easier to navigate and refactor.

### Phase 231: RuntimeService Arc Reduction

**Goal:** Reduce `Arc` cloning overhead on the request path after the service decomposition lands.
**Requirements:** SVCMOD-02
**Depends on:** Phase 230
**Status:** Completed 2026-04-13
**Plans:** 1/1 plans complete
**Success Criteria**:
1. `RuntimeService` no longer clones wide shared state indiscriminately on the request path.
2. The narrowed ownership model is explicit enough to explain which state remains shared and why.
3. The refactor preserves the shipped request-handling behavior while reducing avoidable overhead in the hot path.
...
---
*Active milestone: v1.65 Config Crate Extraction And service.rs Decomposition*
*Next step: Complete v1.65 and promote v1.66.*
*Last updated: 2026-04-13 — Completed Phases 230 and 231; v1.65 is now fully implemented and ready for archive.*
