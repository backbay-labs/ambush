...
**Goal:** Add a Windows Event Log-backed telemetry bridge that maps core Windows records into the shared telemetry schema.
**Requirements:** TELBR-01
**Depends on:** v1.69 milestone complete
**Status:** Pending
**Plans:** 0/1 planned
**Success Criteria**:
1. A `TelemetryBridge` implementation ingests Windows Event Log records and emits shared telemetry payloads for the currently supported detector families.
2. The bridge is configurable through the repo-owned runtime config surface instead of a one-off demo path.
3. Focused bridge tests prove representative Windows Event Log records map into the expected normalized telemetry events.

### Phase 249: Sysmon Bridge

**Goal:** Add a Sysmon-backed telemetry bridge with normalized process, network, and file event mapping.
**Requirements:** TELBR-02
**Depends on:** Phase 248
**Status:** Pending
**Plans:** 0/1 planned
**Success Criteria**:
1. A `TelemetryBridge` implementation ingests Sysmon events and maps process, network, and file activity into the shared telemetry schema.
2. The bridge reuses the repo-owned bridge-health and ingest patterns instead of inventing a separate runtime path.
3. Focused tests prove representative Sysmon records map into the expected telemetry payloads without schema drift.

### Phase 250: Auditd Bridge

**Goal:** Add a Linux auditd-backed telemetry bridge for syscall and authentication signals.
**Requirements:** TELBR-03
**Depends on:** Phase 249
**Status:** Pending
**Plans:** 0/1 planned
**Success Criteria**:
1. A `TelemetryBridge` implementation ingests auditd records and maps supported syscall and authentication activity into the shared telemetry schema.
2. The auditd bridge preserves enough field context for the existing detector families to evaluate the resulting events without bridge-specific hacks.
3. Focused tests prove representative auditd records normalize into stable telemetry payloads.

### Phase 251: Bridge Health And Metrics Integration

**Goal:** Integrate the new bridges into the shared health, event-count, and lag surfaces.
**Requirements:** TELBR-04
**Depends on:** Phase 250
**Status:** Pending
**Plans:** 0/1 planned
**Success Criteria**:
1. Windows Event Log, Sysmon, and auditd bridges all expose health, processed-event count, and lag metrics consistent with the existing bridge surfaces.
2. Shared runtime status and operator surfaces can report the new bridge states without special-casing one source.
3. Integration proof covers the bridge fleet end to end instead of validating each new adapter in isolation only.
