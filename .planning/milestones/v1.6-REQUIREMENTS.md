# Requirements Archive: v1.6 Bounded Canary And Rollback
...
- [x] **CAN-01**: Team can assign a verified candidate detector to a bounded canary slot without replacing the production baseline
- [x] **CAN-02**: Canary execution emits live detections only within the scoped canary lane and cannot by itself trigger fleet-wide escalation semantics
- [x] **CAN-03**: Canary observation records detection, false-positive, latency, and resource metrics over a configurable live window

### Rollback And Safety

- [x] **RLB-01**: Canary runs automatically roll back when configured metrics diverge beyond thresholds or resource budgets
- [x] **RLB-02**: Operator can manually halt or roll back a canary and retrieve the reason, affected slot, and reverted baseline

### Canary Review

- [x] **PRM-03**: Team can assemble a canary evaluation report that links verification, shadow, and canary evidence into one ready-for-promotion or blocked recommendation
- [x] **PRM-04**: Operator CLI can inspect active or completed canary runs and rollback history by stable ID
...
|-------------|-------|--------|
| CAN-01 | Phase 20 | Complete |
| CAN-02 | Phase 21 | Complete |
| CAN-03 | Phase 21 | Complete |
| RLB-01 | Phase 22 | Complete |
| RLB-02 | Phase 22 | Complete |
| PRM-03 | Phase 22 | Complete |
| PRM-04 | Phase 22 | Complete |
...
*Last updated: 2026-04-03 after milestone v1.6 completion*
