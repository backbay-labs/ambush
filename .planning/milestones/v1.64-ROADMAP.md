...
- [x] **Phase 224: Audit Path Hack Dependencies And Migration Plan** - Catalog all #[path] directives, map which types/functions they expose, and design the re-export strategy. (PATHFIX-01) (completed 2026-04-13)
- [x] **Phase 225: Extract Runtime-Evolution Bridge Types** - Move the former path-hacked source under one crate-owned module tree and keep `swarm-evolution` as a compatibility facade. (PATHFIX-02) (completed 2026-04-13)
- [x] **Phase 226: Remove Remaining Path Directives And Fix Imports** - Delete all `#[path]` lines and normalize runtime/evolution imports to the new crate boundary. (PATHFIX-03) (completed 2026-04-13)
- [x] **Phase 227: Path Hack Removal Integration Proof** - Verify build, tests, and production-target clippy after full removal. (PATHFIX-04) (completed 2026-04-13)

### Phase 224: Audit Path Hack Dependencies And Migration Plan

**Goal:** Catalog the existing `#[path]` bridge surface and define the migration plan that replaces it with normal crate boundaries.
**Requirements:** PATHFIX-01
**Depends on:** v1.63 milestone complete
**Status:** Completed 2026-04-13
**Plans:** 1/1 plans complete
**Success Criteria**:
1. Every `#[path = "../../swarm-evolution/..."]` directive in `swarm-runtime/src/lib.rs` is inventoried with the items it exposes to runtime code.
2. The roadmap identifies which shared types should move or re-export from crate-owned modules instead of compiling evolution files under the runtime crate root.
3. The migration plan calls out the import, visibility, and API-surface risks that later phases must resolve to remove the path hacks safely.

### Phase 225: Extract Runtime-Evolution Bridge Types

**Goal:** Break the runtime/evolution source-inclusion cycle by giving the former path-hacked modules one real crate-owned source tree and preserving `swarm-evolution` as a compatibility facade.
**Requirements:** PATHFIX-02
**Depends on:** Phase 224
**Status:** Completed 2026-04-13
**Plans:** 1/1 plans complete
**Success Criteria**:
1. The former path-hacked evolution modules live under one real crate-owned source tree instead of being compiled into runtime from `../../swarm-evolution/...`.
2. Runtime imports those modules through normal `pub mod` ownership and `swarm-evolution` remains buildable as a compatibility re-export crate.
3. The runtime/evolution boundary no longer relies on source-file inclusion to break the dependency cycle.

### Phase 226: Remove Remaining Path Directives And Fix Imports

**Goal:** Delete the remaining `#[path]` directives and repair the workspace imports to use the new crate boundaries.
**Requirements:** PATHFIX-03
**Depends on:** Phase 225
**Status:** Completed 2026-04-13
**Plans:** 1/1 plans complete
**Success Criteria**:
1. `swarm-runtime/src/lib.rs` no longer contains any `#[path = "../../swarm-evolution/..."]` directives.
2. All affected runtime, evolution, and downstream workspace imports compile through normal crate/module paths after the bridge extraction.
3. Public and internal API usage remains bounded and fail-closed after the path-hack removal rather than relying on implicit source inclusion.

### Phase 227: Path Hack Removal Integration Proof

**Goal:** Prove the affected crates remain healthy and the developer tooling contract is restored after path-hack removal.
**Requirements:** PATHFIX-04
**Depends on:** Phase 226
**Status:** Completed 2026-04-13
**Plans:** 1/1 plans complete
**Success Criteria**:
1. The affected runtime/evolution crates build successfully after the path hacks are removed.
2. The affected crates' library tests pass and their library/bin targets are clippy-clean with no path-hack regressions.
3. The final proof documents that runtime and evolution now interact through normal crate boundaries, restoring IDE navigation and refactoring safety.
...
---
*Active milestone: v1.64 Cross-Crate Path Hack Elimination (all phases complete)*
*Next step: Complete v1.64 milestone and promote v1.65 when ready.*
*Last updated: 2026-04-13 — Completed phases 224-227 and removed the runtime path hacks.*
