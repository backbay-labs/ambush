...
- OPEN, found by the task #18 dataflow sweep, both unfixed and neither on the executing path today: (a) `governance_quorum_threshold(0)` returns 0, so a keyless policy can NEVER report `Partitioned` -- measured `state=Degraded total=0 healthy=0 threshold=0` with a Failed Tom, i.e. `status_report()` reports on governance that does not exist; (b) `ContingencyLease::verify` (`tom_agent.rs:51`) checks the embedded receipt against ITSELF -- signature, Approve decision, proposal hash -- and never against a registered governor, so any ed25519 key mints a lease that passes it. What actually stops a forged lease is the byte-equality check against `state.active_contingency_leases` in `authorize_partition_request`, not `verify()`. Any future caller that treats `verify()` alone as authorization fails open.
