...
if TYPE_CHECKING:
    from cyntra.core.dispatcher.dispatcher import Dispatcher, DispatchResult
...
class PhasedOrchestrator:
    ...
    async def _execute_via_dispatcher(
        ...
        workcell_path = None
        try:
            suggested_template = self._workcell_manager.suggest_template(
                domain="code",
                tags=issue.tags,
            )
            workcell_path = self._workcell_manager.create(
                issue.id,
                speculate_tag=tag,
