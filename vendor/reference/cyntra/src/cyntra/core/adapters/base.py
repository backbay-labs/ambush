...
class ToolchainAdapter(ABC):
    ...
    def _build_prompt(self, manifest: dict, workcell_path: Path | None = None) -> str:
        ...
        if isinstance(strategy_cfg, dict) and strategy_cfg.get("enabled") is True:
            ...
            if isinstance(patterns, dict) and patterns:
                ...
                try:
                    from cyntra.eval.strategy.rubric import CYNTRA_V1_RUBRIC
        ...
        if isinstance(learned_patterns, dict) and learned_patterns:
            ...
            parts.append("*From `.cyntra/learned_context/` consolidation blocks:*")
            parts.append("")
        ...
        kg_context = memory_context.get("knowledge_graph_context")
        if isinstance(kg_context, str) and kg_context.strip():
            parts.append(kg_context.strip())
            parts.append("")
        ...
        if memory_context and memory_context.get("memory_available"):
            ...
            try:
                from cyntra.cognition.memory.membox.retrieval import render_membox_context_section
        ...
        if isinstance(strategy_cfg, dict) and strategy_cfg.get("enabled") is True:
            if strategy_cfg.get("self_report", True):
                ...
                try:
                    from cyntra.eval.strategy.profiler import (
        ...
        if isinstance(genome_id, str) and genome_id.strip() and isinstance(workcell_path, Path):
            try:
                from cyntra.infra.prompts.runtime import (
                    detect_domain,
                    load_prompt_genome,
    ...
    def _resolve_run_dir(self, manifest: dict, workcell_path: Path | None, run_id: str) -> Path:
        ...
        run_dir = repo_root / ".cyntra" / "runs" / run_id
        run_dir.mkdir(parents=True, exist_ok=True)
        return run_dir
    ...
    async def _start_aegis_run(
        ...
        if client and client.is_daemon_available():
            try:
                from cyntra.trust.aegis.client import ExecutionMode, SecurityPolicy as AegisPolicy
    ...
    def _get_patch_info(self, workcell_path: Path, manifest: dict) -> dict:
        ...
        # Resolve diff base from workcell marker first, then integration branch fallbacks.
        ...
        for candidate in (
            marker_data.get("parent_commit"),
            marker_data.get("base_ref"),
            manifest.get("base_ref"),
            os.environ.get("CYNTRA_INTEGRATION_BRANCH"),
            "main",
            "master",
        ):
            if candidate and str(candidate).strip():
                base_candidates.append(str(candidate).strip())
    ...
    def _auto_commit_workcell(self, workcell_path: Path, manifest: dict) -> None:
        ...
        We need committed changes for git diff <base>...HEAD to work.
        """
        ...
        infra_prefixes = (".beads/", ".cyntra/", ".mise.toml", ".workcell", "telemetry.jsonl")
        ...
        subprocess.run(
            ["git", "commit", "-m", f"cyntra: auto-commit changes for {issue_id}",
             "--no-verify"],
            cwd=workcell_path,
            capture_output=True,
            text=True,
