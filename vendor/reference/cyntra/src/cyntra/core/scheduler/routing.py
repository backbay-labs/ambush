...
class KernelConfig:
    ...
    def load(cls, config_path: Path) -> KernelConfig:
        ...
        Also loads .cyntra/config.local.yaml if it exists, merging it on top
    ...
    def from_dict(cls, data: dict[str, Any], config_path: Path | None = None) -> KernelConfig:
        ...
        # Support both:
        # - `gates:` (canonical)
        # - `quality_gates:` (back-compat; used in repo-level .cyntra/config.yaml docs)
        ...
        if beads_path_value:
            config.beads_path = Path(beads_path_value)
        ...
        beads_env = os.environ.get("CYNTRA_BEADS_PATH")
        if beads_env and beads_env.strip():
            config.beads_path = Path(beads_env.strip())

        workcells_env = os.environ.get("CYNTRA_WORKCELLS_DIR")
        if workcells_env and workcells_env.strip():
            config.workcells_dir = Path(workcells_env.strip())

        archives_env = os.environ.get("CYNTRA_ARCHIVES_DIR")
        if archives_env and archives_env.strip():
            config.archives_dir = Path(archives_env.strip())

        max_wc_env = os.environ.get("CYNTRA_MAX_CONCURRENT_WORKCELLS")
