...
from cyntra.infra.gpu_pool.manager import GPUPoolManager
from cyntra.core.scheduler.routing import GpuPoolConfig, KernelConfig
from cyntra.trust.ledger.events import LeaseCreatedEvent
from cyntra.core.providers.base import ExecutionContext, ExecutionProvider, ExecutionResult
from cyntra.core.providers.capabilities import ExecutionCapabilities
from cyntra.core.providers.local.provider import LocalProvider
...
class GPUPoolProvider(ExecutionProvider):
    ...
    def __init__(self, config: dict[str, Any] | None = None):
        ...
        self.kernel_config_path = Path(
            self.config.get("kernel_config_path", ".cyntra/config.yaml")
        )
    ...
    async def health_check(self) -> bool:
        ...
        if isinstance(health, dict) and "backends" in health:
            for backend in health["backends"].values():
