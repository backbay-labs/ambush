...
class ProviderRouter:
    ...
    def _preference_score(self, name: str, manifest: ToolchainManifest) -> float:
        ...
        if name == "workbench" and manifest.task.tags:
            if self.workbench_tags.intersection(set(manifest.task.tags)):
                score += 0.7

        # Prefer attack range provider for explicit range-tagged tasks
        if name == "attack_range" and manifest.task.tags:
            if self.range_tags.intersection(set(manifest.task.tags)):
                score += 0.7
