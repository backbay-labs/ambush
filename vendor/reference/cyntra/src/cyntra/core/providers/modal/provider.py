...
class ModalProvider(ExecutionProvider):
    ...
    def __init__(self, config: dict[str, Any] | None = None):
        ...
        self.app_name = self.config.get("app_name", "cyntra")
    ...
    def _build_function(
        ...
        def execute_toolchain(manifest_dict: dict, context_dict: dict) -> dict:
            ...
            if run_id:
                run_dir = Path(manifest.runtime.working_dir) / ".cyntra" / "runs" / str(run_id)
                os.environ.setdefault("CYNTRA_RUN_ID", str(run_id))
                os.environ.setdefault("CYNTRA_RUN_DIR", str(run_dir))
    ...
    async def _run_gates(
        ...
        for gate in manifest.gates:
            try:
                ...
                env_prefix = (
                    f"CYNTRA_RUN_ID={quote(context.run_id)} "
                    f"CYNTRA_RUN_DIR={quote(str(run_dir))}"
                )
