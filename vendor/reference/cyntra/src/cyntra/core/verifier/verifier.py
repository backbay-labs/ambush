...
from cyntra.core.gates.runner import GateRunner, GateConfig, GateResult
...
from cyntra.cognition.rollouts.builder import write_rollout
...
class Verifier:
    ...
    async def _run_verify_async(self, proof: PatchProof, workcell_path: Path) -> bool:
        ...
        changed_files = self._get_changed_files(proof, workcell_path)
        manifest = self._load_manifest(workcell_path)
        zero_diff_gate: dict[str, Any] | None = None
        if not changed_files:
            zero_diff_reason = self._resolve_zero_diff_allow_reason(manifest)
            if not zero_diff_reason:
                results = {
                    "nonzero-diff": self._normalize_gate_result(
                        name="nonzero-diff",
                        result={
                            "passed": False,
                            "reason": "zero_diff_without_justification",
                            "duration_ms": 0,
                            "blocking": True,
                        },
                        gate_type="policy",
                ...
                return False
            zero_diff_gate = self._normalize_gate_result(
                name="nonzero-diff",
                result={
                    "passed": True,
                    "skipped": True,
                    "reason": f"zero_diff_allowed:{zero_diff_reason}",
                    "duration_ms": 0,
                    "blocking": True,
                },
                gate_type="policy",
        ...
        if zero_diff_gate is not None:
            results["nonzero-diff"] = zero_diff_gate
        if diff_gates:
        ...
        *,
        ...
        for name, cfg in gates.items():
            ...
            try:
                ...
                retries = _coerce_int(
                    cfg_data.get("retries", cfg_data.get("retry", None)),
    ...
    def _get_changed_files(self, proof: PatchProof, workcell_path: Path) -> list[str]:
        candidates: list[str] = []
        repo_probe_succeeded = False
        commands = [
        ...
        if repo_probe_succeeded:
            return []

        patch = proof.patch if isinstance(proof.patch, dict) else {}
        files = patch.get("files_modified")
        if isinstance(files, list):
            normalized = [str(f).strip() for f in files if isinstance(f, str) and f.strip()]
            if normalized:
                return sorted(set(normalized))

        return []

    def _resolve_zero_diff_allow_reason(self, manifest: dict[str, Any]) -> str:
        if os.environ.get("CYNTRA_ALLOW_ZERO_DIFF", "").strip().lower() in {"1", "true", "yes", "on"}:
            return "env:CYNTRA_ALLOW_ZERO_DIFF"

        issue = manifest.get("issue") if isinstance(manifest, dict) else None
        if isinstance(issue, dict):
            issue_reason = str(issue.get("noop_justification") or "").strip()
            if issue_reason:
                return issue_reason
            ...
            if isinstance(issue_tags, list):
                ...
                if "allow-zero-diff" in normalized_tags:
                    return "issue.tags includes allow-zero-diff"

        top_reason = str(manifest.get("noop_justification") or "").strip() if isinstance(manifest, dict) else ""
        if top_reason:
            return top_reason

        top_tags = manifest.get("tags") if isinstance(manifest, dict) else None
        if isinstance(top_tags, list):
            normalized_tags = {str(tag).strip() for tag in top_tags if str(tag).strip()}
            if "allow-zero-diff" in normalized_tags:
    ...
    def _run_secret_detection_gate(
        ...
        for rel_path in scan_files:
            ...
            if candidate.is_absolute():
                ...
                resolved = (workcell_path / candidate)
            try:
    ...
    def _gate_cache_path(self) -> Path:
        return self.config.repo_root / ".cyntra" / "cache" / "gate_cache.json"
    ...
    def _check_skill_maturity_lockdown(self, workcell_path: Path, proof: PatchProof) -> list[str]:
        ...
        try:
            ...
            from cyntra.domain.skills.maturity import SkillMaturity
    ...
    def _infer_skill_id_from_repo_path(self, path: str) -> str | None:
        ...
        if len(parts) >= 6 and parts[:4] == ("kernel", "src", "cyntra", "skills"):
            category = parts[4]
            filename = parts[5]
            if filename.endswith((".yaml", ".yml", ".py")):
                name = Path(filename).stem
    ...
    def _run_psn_skill_tests_gate(
        ...
        *,
        ...
        started = time.perf_counter()
        try:
            from cyntra.domain.skills.registry import SkillRegistry
            from cyntra.domain.skills.runner import SkillRunner
            from cyntra.domain.skills.testing import run_skill_tests
            from cyntra.domain.skills.trace import TraceRecorder
    ...
    def _persist_proof(self, proof: PatchProof, workcell_path: Path) -> None:
        ...
        try:
            ...
            self._persist_run_artifacts(proof, workcell_path)
        except Exception as e:
    ...
    def _persist_run_artifacts(self, proof: PatchProof, workcell_path: Path) -> None:
        """Write run artifacts into ``CYNTRA_RUN_DIR`` for sleeptime ingestion."""
        ...
        if proof.status == "timeout":
            outcome = "timeout"
        elif proof.status == "cancelled":
            outcome = "cancelled"
        elif proof.status in ("success", "partial") and all_passed:
            outcome = "success"

        toolchain = manifest.get("toolchain")
        if not isinstance(toolchain, str) or not toolchain.strip():
            toolchain = metadata.get("toolchain")
        toolchain = str(toolchain) if toolchain else "unknown"

        toolchain_config = manifest.get("toolchain_config")
        toolchain_config = toolchain_config if isinstance(toolchain_config, dict) else {}
        genome_id = toolchain_config.get("prompt_genome_id") or metadata.get("prompt_genome_id")
        if genome_id is not None and not isinstance(genome_id, str):
            genome_id = str(genome_id)

        tags = issue.get("tags") if isinstance(issue, dict) else None
        tags_list = [t for t in tags if isinstance(t, str) and t.strip()] if isinstance(tags, list) else []

        run_manifest: dict[str, object] = {
            "run_id": metadata.get("run_id") or manifest.get("run_id") or workcell_path.name,
            "issue_id": proof.issue_id,
            "workcell_id": proof.workcell_id,
            "job_type": manifest.get("job_type") or "code",
            "toolchain": toolchain,
            "genome_id": genome_id,
            "outcome": outcome,
            "status": proof.status,
        ...
        try:
            ...
            logger.debug("Failed to write run manifest", run_dir=str(run_dir), exc_info=True)

        gate_bools = self._summarize_gate_bools(verification.get("gates") if isinstance(verification, dict) else None)
        if gate_bools is not None:
            try:
                (run_dir / "gates.json").write_text(
                    json.dumps(gate_bools, indent=2, sort_keys=True),
                    encoding="utf-8",
                )
            except Exception:
                logger.debug("Failed to write run gates", run_dir=str(run_dir), exc_info=True)

        # Write changes.patch to support file extraction in sleeptime.
        try:
            patch_text = self._build_changes_patch(proof, workcell_path)
            if patch_text:
                (run_dir / "changes.patch").write_text(patch_text, encoding="utf-8")
        except Exception:
            logger.debug("Failed to write changes.patch", run_dir=str(run_dir), exc_info=True)

        # Best-effort: write tools.jsonl derived from telemetry.jsonl.
        try:
            tools_written = self._write_tools_log(run_dir, workcell_path)
            if not tools_written and isinstance(proof.commands_executed, list):
                tools_written = self._write_tools_log_from_commands(
                    run_dir,
                    proof.commands_executed,
            ...
            logger.debug("Failed to write tools.jsonl", run_dir=str(run_dir), exc_info=True)

        if outcome != "success":
            try:
                error_text = self._format_error_text(proof, verification)
                if error_text:
                    (run_dir / "error.txt").write_text(error_text, encoding="utf-8")
            except Exception:
                logger.debug("Failed to write error.txt", run_dir=str(run_dir), exc_info=True)

    def _summarize_gate_bools(self, gates: Any) -> dict[str, bool] | None:
        if not isinstance(gates, dict) or not gates:
            return None
        simplified: dict[str, bool] = {}
        for name, result in gates.items():
            if not isinstance(name, str) or not name.strip():
                continue
            if isinstance(result, dict) and "passed" in result:
                simplified[name] = bool(result.get("passed", False))
            elif isinstance(result, bool):
                simplified[name] = result
        return simplified or None

    def _build_changes_patch(self, proof: PatchProof, workcell_path: Path) -> str | None:
        patch = proof.patch if isinstance(proof.patch, dict) else {}
        base_commit = patch.get("base_commit")
        head_commit = patch.get("head_commit")
        if isinstance(base_commit, str) and base_commit.strip() and isinstance(head_commit, str) and head_commit.strip():
            rev_range = f"{base_commit.strip()}...{head_commit.strip()}"
        ...
        if result.returncode not in (0, 1):
            return None
        text = result.stdout
        # Avoid writing unbounded diffs into the run directory.
        if len(text) > 2_000_000:
            return text[:2_000_000] + "\n\n# [truncated]\n"
        return text

    def _write_tools_log(self, run_dir: Path, workcell_path: Path) -> int:
        telemetry_candidates = [
            workcell_path / "telemetry.jsonl",
            run_dir / "telemetry.jsonl",
        ]
        telemetry_path = next((p for p in telemetry_candidates if p.exists()), None)
        ...
        def _tool_from_event(event: dict[str, Any]) -> str | None:
            ...
            if event_type == "file_write":
                return "Write"
            if event_type in ("bash_command", "bash_output"):
                return "Bash"
        ...
        with telemetry_path.open("r", encoding="utf-8") as handle, tools_path.open(
            "w", encoding="utf-8"
        ) as out:
            for line in handle:
                line = line.strip()
                ...
                if "error" in event:
                    out_event["error"] = event.get("error")
                out.write(json.dumps(out_event, sort_keys=True) + "\n")
                written += 1

        return written

    def _write_tools_log_from_commands(
        self,
        run_dir: Path,
        ...
        *,
        ...
        # Overwrite (telemetry-derived log may exist but be empty).
        with tools_path.open("w", encoding="utf-8") as out:
            for cmd in commands:
    ...
    def _format_error_text(self, proof: PatchProof, verification: dict[str, Any]) -> str:
        metadata = proof.metadata if isinstance(proof.metadata, dict) else {}
        bits: list[str] = []

        error = metadata.get("error")
        if isinstance(error, str) and error.strip():
            bits.append(error.strip())

        blocking = verification.get("blocking_failures") if isinstance(verification, dict) else None
        if isinstance(blocking, list) and blocking:
            bits.append("")
            bits.append("Blocking failures:")
        ...
        if isinstance(gates, dict):
            ...
            for name, result in gates.items():
                ...
                if isinstance(result, dict) and result.get("passed") is False:
                    summary = result.get("failure_summary") or result.get("reason") or "failed"
                    failed.append((name, summary))
            if failed:
                bits.append("")
                bits.append("Gate failures:")
                for name, summary in failed[:20]:
