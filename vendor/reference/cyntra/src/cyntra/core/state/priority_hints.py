...
Reads/writes `.cyntra/state/priority_hints.json` so the scheduler can boost
or deprioritize issues based on patterns discovered during consolidation.

The file is intentionally separate from .beads/issues.jsonl to avoid
...
class PriorityHintStore:
    """
    Persistent store for priority hints at `.cyntra/state/priority_hints.json`.
