...
from cyntra.core.gates.runner import GateConfig, GateResult
...
def default_blocking_for_gate_type(gate_type: str) -> bool:
    ...
    if normalized in {"llm-eval", "llm_eval", "llm", "informational"}:
        return False
    return True
...
def resolve_logs_dir(work_dir: Path | str | None) -> Path | None:
    ...
    for parent in [candidate, *candidate.parents]:
        logs_dir = parent / ".cyntra" / "logs"
        if logs_dir.exists():
            return logs_dir
    return candidate / ".cyntra" / "logs"
