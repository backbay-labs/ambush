...
from hellcat.core.topology.hsm_schema import TransitionResult
from hellcat.core.topology.orchestrator import (
    OrchestratorConfig,
)
...
from hellcat.infra.observability.events import Event, EventEmitter, EventType

if TYPE_CHECKING:
    from hellcat.core.dispatcher.dispatcher import Dispatcher
    from hellcat.core.scheduler.routing import KernelConfig
    from hellcat.core.scheduler.scheduler import SchedulingSignals
...
class TopologyIntegration:
    ...
    def __init__(
        ...
        if not templates_dir:
            templates_dir = kernel_config.repo_root / ".hellcat" / "topologies"
        ...
        learned_policy = None
        try:
            from hellcat.core.topology.cycle import create_learned_policy
            from hellcat.core.topology.hsm_dynamics import SQLiteDynamicsDB

            weights_path = kernel_config.repo_root / ".hellcat" / "state" / "policy_weights.json"
