"""
Ralph-style Circuit Breaker for Hellcat Kernel.
...
class CircuitBreaker:
    ...
    def record_success(
        ...
        if files_changed == 0:
            ...
            if self._is_known_trap(state_id) and self.config.trap_immediate_trip:
    ...
    def _find_dynamics_report(self) -> Path | None:
        ...
        candidates = [
            Path(".hellcat/dynamics/dynamics_report.json"),
            Path("kernel/.hellcat/dynamics/dynamics_report.json"),
        ]

        # If persist_path is set, use its parent to find repo root
        if self.persist_path:
            repo_root = self.persist_path.parent.parent.parent
            candidates.insert(0, repo_root / ".hellcat" / "dynamics" / "dynamics_report.json")
    ...
    def _is_known_trap(self, state_id: str | None) -> bool:
        ...
        for trap_key, _trap_info in self._known_traps.items():
