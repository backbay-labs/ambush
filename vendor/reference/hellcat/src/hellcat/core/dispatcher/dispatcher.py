...
from hellcat.core.adapters import get_adapter
...
from hellcat.core.manifests.schema import SecurityPolicy
from hellcat.core.routing import first_matching_rule, ordered_toolchain_candidates
...
class Dispatcher:
    ...
    def apply_patch(self, proof: PatchProof, workcell_path: Path) -> bool:
        ...
        lock_path = self.config.repo_root / ".hellcat" / "locks" / "merge.lock"
        ...
        with lock.locked(exclusive=True) as acquired:
            ...
            try:
                ...
                # Stash any local changes so merge can proceed in multi-agent envs
                stashed = False
                if not self._repo_is_clean(self.config.repo_root):
                    stash_result = subprocess.run(
                        ["git", "stash", "push", "--include-untracked", "-m", f"hellcat-merge-{branch}"],
                    ...
                    if stash_result.returncode == 0 and "No local changes" not in stash_result.stdout:
                        stashed = True
                        logger.info("Stashed local changes before merge", branch=branch)
                    elif stash_result.returncode != 0:
                        self._set_merge_metadata(
                            proof, status="failed", error="stash_failed_pre_merge", branch=branch
                        )
                        logger.error("Failed to stash before merge", error=stash_result.stderr.strip(), branch=branch)
                        return False

                current_branch = self._get_current_branch(self.config.repo_root)
                ...
                if checkout.returncode != 0:
                    if stashed:
                        subprocess.run(["git", "stash", "pop"], cwd=self.config.repo_root, capture_output=True, text=True)
                    self._set_merge_metadata(
                        proof,
                        status="failed",
                        error=f"checkout_main_failed: {checkout.stderr.strip()}",
                        branch=branch,
                    )
                    logger.error("Failed to checkout main", error=checkout.stderr)
                    return False
                ...
                if stashed:
                    subprocess.run(["git", "stash", "pop"], cwd=self.config.repo_root, capture_output=True, text=True)
                    logger.info("Restored stashed changes after merge", branch=branch)
    ...
    def _build_manifest(
        ...
        run_id = workcell_id
        run_dir = self.config.repo_root / ".hellcat" / "runs" / run_id
        ...
        try:
            from hellcat.infra.prompts.runtime import detect_domain, load_prompt_genome
            from hellcat.infra.prompts.selector import select_prompt_genome_id
    ...
    def _build_quality_gates(self, tags: list[str]) -> dict[str, Any]:
        ...
        if asset_tags or "gate:realism" in gate_tags:
            ...
            gates["fab-realism"] = {
                ...
                "command": f"python -m hellcat.fab.gate --asset {{asset_path}} --config {gate_config_id} --out {{output_dir}}",
            }
    ...
    def _build_strategy_config(self, *, issue: Issue, toolchain: str) -> dict[str, Any] | None:
        ...
        ab_salt = str(getattr(routing, "ab_test_salt", "") or "").strip() or "hellcat.strategy.ab.v1"

        bucket_value = int(
            hashlib.sha256(f"{ab_salt}:{issue.id}".encode()).hexdigest(), 16
        ) % 10_000
    ...
    def _get_dataset_optimal_patterns(
        ...
        db_path = self.config.repo_root / ".hellcat" / "dynamics" / "hellcat.db"
