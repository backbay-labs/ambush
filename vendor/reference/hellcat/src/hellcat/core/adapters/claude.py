...
if TYPE_CHECKING:
    from hellcat.core.adapters.pty_output_parser import TurnResult
    from hellcat.core.topology.claude_compiler import ClaudeCompiler
    from hellcat.core.topology.planner import TopologyPlanner
...
class ClaudeAdapter(ToolchainAdapter):
    ...
    def _get_topology_planner(self, workcell_path: Path | None = None) -> TopologyPlanner | None:
        ...
        try:
            ...
            if self.topologies_dir:
                ...
                # Look for .hellcat/topologies in the workcell or parent repos
                for candidate in [
                    workcell_path / ".hellcat" / "topologies",
                    workcell_path.parent / ".hellcat" / "topologies",
                    workcell_path.parent.parent / ".hellcat" / "topologies",
                ]:
                    if candidate.exists():
    ...
    async def _execute_interactive(
        ...
        from hellcat.core.adapters.pty_session import PTYSession
