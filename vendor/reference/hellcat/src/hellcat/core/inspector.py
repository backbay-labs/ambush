...
Inspects the current state of Hellcat modules, skills, gates, and subsystems.
...
class KernelInspector:
    """Inspects Hellcat kernel state for integration verification."""

    def __init__(self, repo_root: Path):
        self.repo_root = repo_root
        self.kernel_src = repo_root / "kernel" / "src" / "hellcat"
    ...
    def _inspect_memory(self) -> SubsystemInfo:
        ...
        db_file = self.repo_root / ".hellcat" / "memory" / "hellcat-mem.db"
    ...
    def _inspect_dynamics(self) -> SubsystemInfo:
        ...
        # Check for actual DB file
        db_file = self.repo_root / ".hellcat" / "dynamics" / "hellcat.db"
        details["db_file_exists"] = db_file.exists()
        if db_file.exists():
            details["db_file_size_kb"] = round(db_file.stat().st_size / 1024, 2)
    ...
    for sub in [
        ...
        if sub:
            ...
            for k, v in sub.details.items():
                ...
                elif k.endswith("_status") and v == "stub":
