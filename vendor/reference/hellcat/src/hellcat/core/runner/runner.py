...
try:
    from hellcat.cognition.dynamics.state_t1 import build_state_t1
except ImportError:
    build_state_t1 = None  # type: ignore[assignment]
try:
    from hellcat.cognition.dynamics.transition_db import TransitionDB
except ImportError:
    TransitionDB = None  # type: ignore[assignment,misc]
try:
    from hellcat.cognition.sleeptime import SleeptimeConfig, SleeptimeOrchestrator
except ImportError:
    SleeptimeConfig = None  # type: ignore[assignment,misc]
    SleeptimeOrchestrator = None  # type: ignore[assignment,misc]
try:
    from hellcat.cognition.strategy.profiles import DomainProfileStore
except ImportError:
    DomainProfileStore = None  # type: ignore[assignment,misc]
from hellcat.core.control.exploration_controller import ExplorationController
from hellcat.core.dispatcher import Dispatcher, DispatchResult

try:
    from hellcat.core.memory_integration import KernelMemoryBridge
except ImportError:
    KernelMemoryBridge = None  # type: ignore[assignment,misc]
try:
    from hellcat.core.planner_integration import KernelPlannerIntegration, PlannerSelection
except ImportError:
    KernelPlannerIntegration = None  # type: ignore[assignment,misc]
    PlannerSelection = None  # type: ignore[assignment,misc]
try:
    from hellcat.core.ralph import RalphConfig, RalphIntegration
except ImportError:
    RalphConfig = None  # type: ignore[assignment,misc]
    RalphIntegration = None  # type: ignore[assignment,misc]
from hellcat.core.scheduler import (
    SchedulePlan,
    Scheduler,
    SchedulingSignals,
...
from hellcat.core.sentinel import create_sentinel_scheduler_from_config
from hellcat.core.state.manager import StateManager

try:
    from hellcat.core.strategy_integration import StrategyIntegration
except ImportError:
    StrategyIntegration = None  # type: ignore[assignment,misc]
from hellcat.core.topology import create_topology_integration
...
try:
    from hellcat.core.providers.e2b.provider import E2B_AVAILABLE, E2BProvider
except ImportError:
    E2B_AVAILABLE = False
...
from hellcat.infra.observability.swarm_summary import (
    LatencyEnvelope,
    SwarmEpisode,
    WorkCategories,
)
...
if TYPE_CHECKING:
    from hellcat.cognition.planner.action_space import ActionSpace
    from hellcat.core.state.models import BeadsGraph, Issue
...
class SpeculateDispatchPlan:
    candidates: list[str]
    selection: Any  # PlannerSelection when available
    parallelism: int
...
class KernelRunner:
    ...
    def __init__(
        ...
        if universe_id:
            from hellcat.eval.universe import UniverseLoadError, load_universe
            ...
            from hellcat.eval.universe.policy import apply_universe_policies
        ...
        if KernelMemoryBridge is not None:
            try:
                self.memory_bridge = KernelMemoryBridge(
                    db_path=self.config.repo_root / ".hellcat" / "memory" / "hellcat-mem.db"
                )
            except Exception:
                logger.debug("KernelMemoryBridge unavailable")

        # Dynamics transition DB for routing improvement - created early for scheduler
        self.transition_db = None
        if TransitionDB is not None:
            try:
                dynamics_db_path = self.config.repo_root / ".hellcat" / "dynamics" / "hellcat.db"
                self.transition_db = TransitionDB(dynamics_db_path)
            except Exception:
                logger.debug("TransitionDB unavailable")

        self.strategy_integration = None
        if StrategyIntegration is not None and self.transition_db is not None:
            try:
                self.strategy_integration = StrategyIntegration(self.transition_db)
            except Exception:
                logger.debug("StrategyIntegration unavailable")

        # Domain profiles for token estimation and toolchain preferences
        self.domain_profile_store = None
        if DomainProfileStore is not None and self.transition_db is not None:
            try:
                profiles_cache_dir = self.config.repo_root / ".hellcat" / "strategy"
                ...
                logger.debug("DomainProfileStore unavailable")

        # Scheduler (pure core) and signals builder
        learned_context_dir = self.config.repo_root / ".hellcat" / "learned_context"
        ...
        if KernelPlannerIntegration is not None:
            try:
                self.planner_integration = KernelPlannerIntegration(self.config)
            except Exception:
                logger.debug("KernelPlannerIntegration unavailable")
        self.workcell_manager = None
        if WorkcellManager is not None:
            try:
                self.workcell_manager = WorkcellManager(self.config, self.config.repo_root)
            except Exception:
                logger.debug("WorkcellManager unavailable")
        ...
        self.sleeptime = None
        if SleeptimeConfig is not None and SleeptimeOrchestrator is not None:
            try:
                ...
                logger.debug("SleeptimeOrchestrator unavailable")

        # Ralph integration for autonomous loop features
        self.ralph = None
        if RalphConfig is not None and RalphIntegration is not None:
            try:
                ...
                logger.debug("RalphIntegration unavailable")
    ...
    async def run_async(self) -> None:
        ...
        console.print("\n[bold blue]Hellcat[/bold blue] starting...")
    ...
    async def _run_cycle(self) -> bool:
        ...
        if self.config.speculation.enabled and ready_set:
            ...
            if should_speculate:
                try:
                    from hellcat.cognition.planner.artifacts import collect_history_candidates
                except ImportError:
                    collect_history_candidates = None  # type: ignore[assignment]

                if collect_history_candidates is not None:
            ...
            if self.target_issue and self.single_cycle:
                ...
                if issue:
                    ...
                    if not history_candidates:
                        try:
                            from hellcat.cognition.planner.artifacts import (
    ...
    def _cap_speculate_parallelism(
        ...
        *,
        ...
        from hellcat.core.routing import speculate_parallelism as routing_parallelism
    ...
    async def _dispatch_e2b_async(
        ...
        try:
            ...
            from hellcat.core.manifests.schema import RuntimeConfig, TaskSpec, ToolchainManifest
            ...
            if result.status == "completed":
                ...
                self._handle_e2b_failure(
                    issue,
                    e2b_workcell_id,
                    error_summary=gate_summary or "E2B gate_failed",
                    result=result,
                )
            else:
        ...
        *,
        ...
        if not ralph_decision.allow:
            ...
            if ralph_decision.wait_seconds > 0:
                from hellcat.core.rate_limiter import format_wait_time
        ...
        try:
            ...
            try:
                ...
                topology_result = SimpleNamespace(
                    success=False,
                    error="Unexpected exception in topology execution",
                    total_duration_ms=0,
                    total_tokens=0,
                    final_output=None,
                    all_files_written=[],
    ...
    async def _dispatch_single_async(
        ...
        *,
        ...
        if self._should_use_e2b(issue):
            console.print("  [dim]Provider:[/dim] E2B cloud sandbox")
        ...
        try:
            ...
            try:
                from hellcat.cognition.planner.artifacts import system_state_snapshot
            except ImportError:
                system_state_snapshot = None  # type: ignore[assignment]
            ...
            system_state = (
                ...
                if system_state_snapshot is not None
                else {}
            )
    ...
    def _build_speculate_dispatch_plan(
        ...
        *,
        ...
        from hellcat.core.routing import speculate_toolchains
    ...
    async def _dispatch_speculate_async(
        ...
        *,
        ...
        try:
            ...
            {path.name: toolchain for toolchain, _, path in workcells}
            ...
            try:
                from hellcat.core.verifier.synthesis import SpeculationSynthesizer
    ...
    def _maybe_update_issue_with_next_steps(
        ...
        from hellcat.core.runner.next_steps import (
    ...
    def _planner_universe_context(self) -> tuple[str, dict[str, Any], ActionSpace]:
        ...
        """
        try:
            from hellcat.cognition.planner.action_space import action_space_for_swarms
        except ImportError:
            action_space_for_swarms = None  # type: ignore[assignment]
        ...
        action_space = action_space_for_swarms(swarm_ids) if action_space_for_swarms is not None else None
    ...
    def _emit_swarm_episode(
        ...
        start_ms = int(start_time * 1000) if start_time is not None else now_ms - duration_ms
