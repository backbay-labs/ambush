...
class LocalProvider(SandboxProvider):
    ...
    async def execute(self, ctx: ExecutionContext) -> ExecutionResult:
        ...
        if aegis and aegis.is_daemon_available():
            try:
                from hellcat.trust.aegis.client import ExecutionMode, SecurityPolicy
                ...
                # - Env override: HELLCAT_AEGIS_MODE=local|microvm
        ...
        run_dir = (work_dir / ".hellcat" / "runs" / run_id).resolve()
    ...
    async def _run_command(
        ...
        if aegis and aegis.is_daemon_available():
            try:
                from hellcat.trust.aegis.client import CommandEventData, EventType, ExecutionEvent
        ...
        try:
            ...
            if aegis and aegis.is_daemon_available():
                try:
                    from hellcat.trust.aegis.client import (
                        CommandEventData,
                        EventType,
                        ExecutionEvent,
                    )
