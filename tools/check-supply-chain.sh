...
# Two workspace roots plus one excluded production path dependency make both the
# workspace inventory and the exact non-development dependency graph measurable.
# Additional parser fixtures close graph, enum, duplicate-key, and Python-json
# nonstandard-number acceptance rather than assuming plausible JSON is evidence.
...
mkdir -p \
  ...
  "$SBOM_FIXTURE_WORKSPACE/dependency/src" \
  "$VALID_SBOM_DIR"
...
edition = "2021"
TOML
cat >"$SBOM_FIXTURE_WORKSPACE/dependency/Cargo.toml" <<'TOML'
[package]
name = "dependency"
version = "3.0.0"
edition = "2021"
...
printf '%s\n' '' >"$SBOM_FIXTURE_WORKSPACE/dependency/src/lib.rs"
...
"$SBOM_FIXTURE_CYCLONEDX_BIN" cyclonedx \
  --manifest-path "$SBOM_FIXTURE_WORKSPACE/Cargo.toml" \
  --format json --spec-version 1.5 --target all --quiet
cp "$SBOM_FIXTURE_WORKSPACE/fixture/fixture.cdx.json" "$VALID_SBOM_DIR/fixture.cdx.json"
cp "$SBOM_FIXTURE_WORKSPACE/omitted/omitted.cdx.json" "$VALID_SBOM_DIR/omitted.cdx.json"
find "$SBOM_FIXTURE_WORKSPACE" -mindepth 2 -maxdepth 2 -name '*.cdx.json' -delete
...
cp -R "$SBOM_FIXTURE_WORKSPACE/dependency" "$ALIAS_SHADOW_ROOT/dependency"
...
fn main() {
    ...
    let source = env::var("SUPPLY_CHAIN_ALIAS_SBOM_SOURCE")
        .expect("SUPPLY_CHAIN_ALIAS_SBOM_SOURCE");
    let path = env::var("SUPPLY_CHAIN_ALIAS_SBOM_PATH")
        .expect("SUPPLY_CHAIN_ALIAS_SBOM_PATH");
    fs::copy(source, path).expect("copy alias-forged SBOM");
}
RS
...
"$SBOM_FIXTURE_CYCLONEDX_BIN" cyclonedx \
  --manifest-path "$ALIAS_SHADOW_ROOT/Cargo.toml" \
  --format json --spec-version 1.5 --target all --quiet
ALIAS_VALID_SBOM_SOURCE="$WORK_DIR/alias-valid-fixture.cdx.json"
cp "$ALIAS_SHADOW_ROOT/fixture/fixture.cdx.json" "$ALIAS_VALID_SBOM_SOURCE"
find "$ALIAS_SHADOW_ROOT" -mindepth 2 -maxdepth 2 -name '*.cdx.json' -delete
...
(
  ...
  SUPPLY_CHAIN_ALIAS_MARKER="$ALIAS_MARKER" \
    SUPPLY_CHAIN_ALIAS_SBOM_SOURCE="$ALIAS_VALID_SBOM_SOURCE" \
...
MISSING_GRAPH_EDGE_DIR="$WORK_DIR/missing-graph-edge-sbom"
MISSING_GRAPH_COMPONENT_DIR="$WORK_DIR/missing-graph-component-sbom"
INVALID_COMPONENT_ENUM_DIR="$WORK_DIR/invalid-component-enum-sbom"
UNKNOWN_COMPONENT_FIELD_DIR="$WORK_DIR/unknown-component-field-sbom"
FORGED_TARGET_COMPONENT_DIR="$WORK_DIR/forged-target-component-sbom"
mkdir -p \
  ...
  "$WRONG_VERSION_DIR" \
  "$WRONG_PURL_DIR" \
  "$MISSING_GRAPH_EDGE_DIR" \
  "$MISSING_GRAPH_COMPONENT_DIR" \
  "$INVALID_COMPONENT_ENUM_DIR" \
  "$UNKNOWN_COMPONENT_FIELD_DIR" \
  "$FORGED_TARGET_COMPONENT_DIR"
...
for mutation_dir in \
  ...
  "$INVALID_COMPONENT_ENUM_DIR" \
  "$UNKNOWN_COMPONENT_FIELD_DIR" \
  "$FORGED_TARGET_COMPONENT_DIR"; do
  cp "$VALID_SBOM_DIR/fixture.cdx.json" "$mutation_dir/fixture.cdx.json"
  cp "$VALID_SBOM_DIR/omitted.cdx.json" "$mutation_dir/omitted.cdx.json"
done
jq '(.metadata.component["bom-ref"]) as $root
  | (.dependencies[] | select(.ref == $root).dependsOn) = []' \
  "$MISSING_GRAPH_EDGE_DIR/fixture.cdx.json" \
  >"$MISSING_GRAPH_EDGE_DIR/fixture.cdx.json.tmp"
mv "$MISSING_GRAPH_EDGE_DIR/fixture.cdx.json.tmp" \
  "$MISSING_GRAPH_EDGE_DIR/fixture.cdx.json"
jq 'del(.components[0])' \
  "$MISSING_GRAPH_COMPONENT_DIR/fixture.cdx.json" \
  >"$MISSING_GRAPH_COMPONENT_DIR/fixture.cdx.json.tmp"
mv "$MISSING_GRAPH_COMPONENT_DIR/fixture.cdx.json.tmp" \
  "$MISSING_GRAPH_COMPONENT_DIR/fixture.cdx.json"
jq '.components[0].type = "cryptographic-asset"' \
  "$INVALID_COMPONENT_ENUM_DIR/fixture.cdx.json" \
  >"$INVALID_COMPONENT_ENUM_DIR/fixture.cdx.json.tmp"
mv "$INVALID_COMPONENT_ENUM_DIR/fixture.cdx.json.tmp" \
  "$INVALID_COMPONENT_ENUM_DIR/fixture.cdx.json"
jq '.components[0].forged = true' \
  "$UNKNOWN_COMPONENT_FIELD_DIR/fixture.cdx.json" \
  >"$UNKNOWN_COMPONENT_FIELD_DIR/fixture.cdx.json.tmp"
mv "$UNKNOWN_COMPONENT_FIELD_DIR/fixture.cdx.json.tmp" \
  "$UNKNOWN_COMPONENT_FIELD_DIR/fixture.cdx.json"
jq '.metadata.component.components[0].name = "forged"' \
  "$FORGED_TARGET_COMPONENT_DIR/fixture.cdx.json" \
  >"$FORGED_TARGET_COMPONENT_DIR/fixture.cdx.json.tmp"
mv "$FORGED_TARGET_COMPONENT_DIR/fixture.cdx.json.tmp" \
  "$FORGED_TARGET_COMPONENT_DIR/fixture.cdx.json"
expect_sbom_rejection \
...
expect_sbom_rejection \
  "missing-dependency-graph-edge" "$MISSING_GRAPH_EDGE_DIR" \
  "dependency graph disagrees with locked cargo metadata"
expect_sbom_rejection \
  "missing-dependency-component" "$MISSING_GRAPH_COMPONENT_DIR" \
  "component refs disagree with locked Cargo resolve closure"
expect_sbom_rejection \
  "invalid-cyclonedx-1.5-component-enum" "$INVALID_COMPONENT_ENUM_DIR" \
  "is not allowed by the pinned CycloneDX 1.5 component-type enum"
expect_sbom_rejection \
  "unknown-component-field" "$UNKNOWN_COMPONENT_FIELD_DIR" \
  "unknown=['forged']"
expect_sbom_rejection \
  "forged-root-target-component" "$FORGED_TARGET_COMPONENT_DIR" \
  "metadata.component.components disagree with Cargo production-target inventory"
...
echo "SBOM validation fixtures ok: pinned all-target graph control accepted; spoofed graph, schema, and malformed JSON refused"
