...
# The `perch_daemon_request` skip is the "one dispatch function" exemption, and
# it has to be inside awk: the printf below emits `file:line`, not the source
# line, so a `grep -v` on awk's OUTPUT (as the skeleton had it) filters a stream
# the needle never appears in and the exemption silently never fires. The
# dispatch function and the two wrappers that call it are the only lines that
# name it, which is why the wrappers are kept on one line each.
w2="$(awk '
  $0 ~ /^[[:space:]]*(\/\/|\/\/!|\*)/ { next }
  /PERCH_DAEMON_WRITES/ { next }
  /perch_daemon_request/ { next }
