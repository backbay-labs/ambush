...
# The one shape R3 exists to PRESCRIBE: a source count that cannot be rendered
# without its agent count, because both are members of one inseparable pair.
# Without this the rule fires on the very function whose job is to make the two
# numbers impossible to separate (shared/viz/sourceAttribution.ts). Mirrors
# PAINT_OK_RE: a narrow, named exemption for the compliant form, not a file
# allowlist -- an allowlist would exempt a file's future violations too.
SOURCES_OK_RE='agents[[:space:]]*[?]?:[[:space:]]*number'
...
scan_stream() {
  ...
  awk -v label="$label" \
      ...
      -v srcnum="$SOURCES_NUM_RE" -v srcok="$SOURCES_OK_RE" \
      -v buzzvar="$BUZZ_VAR_RE" '
    ...
    }
    {
      ...
      if (line ~ srcnum && line !~ srcok)
                          printf "%s:%d:R3-sources-number:%s\n", label, FNR, $0
...
function counts(): { sources: number; agents: number } { return { sources: 0, agents: 0 }; }
...
if [ "$#" -gt 0 ]; then
  ...
  # In-repository now: one checkout, one desktop tree. `PERCH_DESKTOP_ROOT`
  # still overrides for anyone scanning an external checkout, but the default
  # is this repository's own desktop app, so the gate runs with no environment.
  PERCH_DESKTOP_ROOT="${PERCH_DESKTOP_ROOT:-$ROOT_DIR/workspace}"
  ROOTS=(
...
if [ -n "${HITS//[$'\n']/}" ]; then
  ...
  echo "R3 sources:num  -> render law 2: take sourceIds, or a NAMED absence; never a bare count
                   (a `sources: number` beside an `agents: number` on the same line is the
                    prescribed pair and is not a violation)" >&2
