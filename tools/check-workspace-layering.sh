...
def main(argv):
    ...
    # ---- RULE 2: DECLARED. A TCB crate may name ONLY an allow-listed crate. -
    #
    # THIS IS AN ALLOW-LIST, NOT A DERIVED DENY-LIST, AND THE FIRST VERSION GOT
    # THAT WRONG IN A WAY THAT MATTERED. It banned anything in a `downstream`
    # set derived as "reaches the TCB but is not in the TCB's own normal
    # closure". That set is computed from the very edges the rule polices, so
    # ADDING THE FORBIDDEN EDGE IS WHAT REMOVED ITS TARGET FROM SCOPE: giving
    # swarm-spine a NORMAL dependency on swarm-pheromone put swarm-pheromone
    # into the closure, so the rule stopped seeing it and the gate exited 0 --
    # while the same pair as a BUILD dependency was caught. Measured evadable
    # that way: 7 of 14 crates for swarm-policy and 7 for swarm-spine.
    #
    # The header also justified skipping a normal-kind fixture case by claiming
    # such an edge is always a cycle cargo refuses to resolve. That is true only
    # of the three crates TCBOUND-03 happens to name (swarm-cli, swarm-runtime,
    # swarm-runtime-http, which all depend back on swarm-policy); it is false
    # for the seven above, and the relock succeeded for every one of them.
    #
    # Reachability cannot express this rule at all, because the TCB is not a
    # flat layer: swarm-crypto sits BELOW swarm-core while swarm-policy and
    # swarm-spine sit above it, so "everything that reaches the TCB" wrongly
    # includes swarm-core itself and would fire on the shipped tree.
    #
    # So the permitted set is written down. The whole TCB workspace-dependency
    # surface is six edges, each of which was a deliberate decision, and adding
    # a seventh should be one too -- the same reasoning, and the same shape, as
    # the allowlist in tools/check-visibility-baseline.sh. An edge that is not
    # on this list fails the build in EVERY dependency kind, and the list cannot
    # be satisfied by adding the edge.
    TCB_ALLOWED_WORKSPACE_DEPS = {
        # swarm-crypto is a leaf primitive and names no workspace crate at all.
        # Keep it empty: the emptiest row in the table is the strongest claim.
        "swarm-crypto": set(),
        "swarm-policy": {"swarm-core"},
        # swarm-response and swarm-whisker are the two rows worth re-reading at
        # review time. They are why ADR 0009 records the TCB as NOT
        # transport-free on the resolved graph: swarm-spine -> swarm-response ->
        # reqwest -> hyper. RULE 3 baselines that leak by name; this row is
        # where it enters.
        "swarm-spine": {
            "swarm-core",
            "swarm-crypto",
    ...
    if set(TCB_ALLOWED_WORKSPACE_DEPS) != set(TCB):
        raise Vacuity(
            "TCB_ALLOWED_WORKSPACE_DEPS does not have exactly one row per TCB "
            f"crate: {sorted(TCB_ALLOWED_WORKSPACE_DEPS)} vs {sorted(TCB)}. A "
            "missing row would silently permit every dependency of that crate."
        )
    workspace_names = set(member_ids)
    for crate in sorted(TCB):
        allowed = TCB_ALLOWED_WORKSPACE_DEPS[crate]
        stale = allowed - workspace_names
        if stale:
            raise Vacuity(
                f"TCB_ALLOWED_WORKSPACE_DEPS['{crate}'] names {sorted(stale)}, "
                "which is not a workspace member; an exemption must not outlive "
                "the crate it exempts"
            )
        for dep_name, kind in sorted(declared(member_ids[crate])):
            ...
            if dep_name not in allowed:
                report.violation(
                    ...
                    f"{crate} declares workspace crate '{dep_name}' as a {kind} "
                    "dependency and it is not on the TCB allow-list; the TCB may "
                    "name only crates below it, and adding one is a deliberate, "
                    "reviewable act (ADR 0009, TCBOUND-03). If this edge is "
                    "correct, add it to TCB_ALLOWED_WORKSPACE_DEPS with the "
                    "reason, and check whether it drags a transport in under "
                    "RULE 3.",
                )
...
fx_spine_normal_pheromone() { break_fixture_dep "$FIXTURE_DIR" swarm-spine swarm-pheromone normal; }
...
fixture_case "swarm-policy taking swarm-runtime as a DEV dependency is caught" 1 \
  "LAYERING-VIOLATION[tcb-declared-downstream] swarm-policy declares workspace crate 'swarm-runtime' as a dev dependency" \
  fx_policy_dev_runtime

# RULE 2, build kind.
fixture_case "swarm-spine taking swarm-pheromone as a BUILD dependency is caught" 1 \
  "LAYERING-VIOLATION[tcb-declared-downstream] swarm-spine declares workspace crate 'swarm-pheromone' as a build dependency" \
  fx_spine_build_pheromone

# RULE 2, NORMAL kind. THE CASE THE FIRST VERSION OF THIS GATE DID NOT HAVE, and
# the reason RULE 2 is now an allow-list. Its header argued a normal edge here is
# always a cycle cargo refuses to resolve, so no fixture case was written; that
# holds only for crates which depend back on swarm-policy (swarm-cli,
# swarm-runtime, swarm-runtime-http) and was false for seven others. With the
# derived deny-list, THIS EXACT MUTATION EXITED 0 while the build-kind variant
# directly above exited 1 -- adding the edge put its target into the closure the
# rule derived its scope from. Do not delete this case; it is the regression
# test for a gate that passed the common form of the thing it bans.
fixture_case "swarm-spine taking swarm-pheromone as a NORMAL dependency is caught" 1 \
  "LAYERING-VIOLATION[tcb-declared-downstream] swarm-spine declares workspace crate 'swarm-pheromone' as a normal dependency" \
  fx_spine_normal_pheromone
