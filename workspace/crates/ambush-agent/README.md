...
[ACP](https://agentclientprotocol.com) is the Agent Client Protocol — JSON-RPC 2.0 over stdio between a client (Zed, JetBrains, ambush-acp, …) and an agent. [MCP](https://modelcontextprotocol.io) is how the agent talks to its tools.

`ambush-agent` is the agent.
...
```bash
# Build
cargo build --release -p ambush-agent

# Run against Anthropic
AMBUSH_AGENT_PROVIDER=anthropic \
ANTHROPIC_API_KEY=sk-ant-... \
ANTHROPIC_MODEL=claude-sonnet-4-5 \
  ./target/release/ambush-agent

# Or any OpenAI-compatible endpoint
AMBUSH_AGENT_PROVIDER=openai \
...
# Or OpenRouter
AMBUSH_AGENT_PROVIDER=openrouter \
...
← {"jsonrpc":"2.0","id":1,"result":{
    ...
    "agentInfo":{"name":"ambush-agent","version":"0.1.0"}
  }}
...
|---|---|---|
| `AMBUSH_AGENT_PROVIDER` | — | Required. `anthropic`, `openai`, `openrouter`, `databricks`, or `databricks_v2`. No implicit fallback — the agent errors at startup when this is unset. |
...
| `AMBUSH_AGENT_SYSTEM_PROMPT` | built-in | Inline system prompt. |
| `AMBUSH_AGENT_SYSTEM_PROMPT_FILE` | — | File path. Mutually exclusive with the above. |
| `AMBUSH_AGENT_MAX_ROUNDS` | `0` | Tool-loop iteration cap. 0 = unlimited. |
| `AMBUSH_AGENT_MAX_OUTPUT_TOKENS` | `65536` | Desired per-call ceiling. Set this at or below the served model's output limit for each agent deployment. Proactive handoff is independently based on 90% of `AMBUSH_AGENT_MAX_CONTEXT_TOKENS`. |
| `AMBUSH_AGENT_MAX_TOKEN_RECOVERIES` | `3` | Retries after a successful response is truncated at the output-token limit. `0` disables recovery; the finite value and `AMBUSH_AGENT_MAX_ROUNDS` prevent infinite retries. |
| `AMBUSH_AGENT_MAX_CONTEXT_TOKENS` | `200000` | Provider context window used by the handoff gate. |
| `AMBUSH_AGENT_MAX_HANDOFFS` | `10` | Max context handoffs per session before falling back to truncation. |
| `AMBUSH_AGENT_LLM_TIMEOUT_SECS` | `240` | Max seconds with no response bytes before abandoning an LLM call (per-read inactivity, not wall-clock). |
| `AMBUSH_AGENT_TOOL_TIMEOUT_SECS` | `660` | Per-tool call timeout in seconds |
| `AMBUSH_AGENT_MAX_PARALLEL_TOOLS` | `8` | Max concurrent tool calls per turn (1 = sequential) |
| `AMBUSH_AGENT_MAX_SESSIONS` | unlimited | Max concurrent ACP sessions. Sessions are cheap; default has no cap. |
| `AMBUSH_AGENT_MAX_LINE_BYTES` | `4194304` | 4 MiB. Hard cap on inbound JSON-RPC frames. |
| `AMBUSH_AGENT_MAX_HISTORY_BYTES` | `1048576` | 1 MiB. Old turns are evicted past this. |
| `AMBUSH_AGENT_MAX_TOOL_RESULT_TEXT_BYTES` | `51200` | 50 KiB. Per-result cap on tool-output text; oversize is middle-elided (head + tail kept) with an inline marker. Images are exempt. |
| `AMBUSH_AGENT_REQUIRE_REPLY` | `0` (`1` on mesh) | `1` enables the [reply guard](#reply-guard) — remind the model to publish when a turn is about to end with nothing posted to Ambush. Desktop defaults it to `1` for Ambush shared-compute agents. |


## Reply Guard

Off by default, except on Ambush shared-compute (mesh) agents, where Ambush Desktop
sets `AMBUSH_AGENT_REQUIRE_REPLY=1` automatically. With it enabled, a turn that is
about to end without any recognized attempt to post to Ambush gets a reminder that
its assistant text is invisible to humans, and is rerolled.

This exists because a Ambush agent's reasoning and tool output are not shown to
...
Setting `AMBUSH_AGENT_REQUIRE_REPLY=0` on the agent, persona, or global env opts a
...
- whose qualified name ends in `__shell` — i.e. the bare tool name is exactly
  `shell`, which is `ambush-dev-mcp`'s shell tool and any other server's, and
...
posted. Text that merely quotes a send (`echo "ambush messages send"`) matches, so
...
`AMBUSH_AGENT_STOP_MAX_REJECTIONS` — the outer cap on every end-turn objection.
...
`ambush-agent` speaks a few HTTP dialects. Pick with `AMBUSH_AGENT_PROVIDER`.

| Provider | `AMBUSH_AGENT_PROVIDER` | Endpoint (auto) | Tested with |
|---|---|---|---|
...
If `AMBUSH_AGENT_PROVIDER=anthropic` is selected without `ANTHROPIC_API_KEY`, `AMBUSH_AGENT_PROVIDER=openai` is selected without `OPENAI_COMPAT_API_KEY`, or `AMBUSH_AGENT_PROVIDER=openrouter` is selected without `OPENROUTER_API_KEY`, the agent returns an error — there is no implicit fallback to another provider.
...
- Retryable statuses (429 and typed `provider_overloaded` 503) honor the documented `Retry-After` header (clamped to a small ceiling — see `RETRY_AFTER_CAP_SECS` in `llm.rs` — since the sleep happens outside `AMBUSH_AGENT_LLM_TIMEOUT_SECS`); 502 and untyped 503 retry with jittered backoff instead. `401` is treated as an expired/invalid key and refreshed once, while `402` (no credits) and `403` (guardrail/moderation/permission) fail immediately without retry.
...
| Frame size | `AMBUSH_AGENT_MAX_LINE_BYTES` (default 4 MiB). Oversize → connection killed. |
...
| Session isolation | Unlimited concurrent sessions by default (configurable via `AMBUSH_AGENT_MAX_SESSIONS`). One prompt per session at a time. Each session gets its own MCP servers. |
...
| Limit | Default | Where |
|---|---|---|
| Inbound JSON-RPC frame | 4 MiB | `AMBUSH_AGENT_MAX_LINE_BYTES` |
| Single prompt | 1 MiB | `MAX_PROMPT_BYTES` |
| History window | 1 MiB | `AMBUSH_AGENT_MAX_HISTORY_BYTES` |
...
| Tool result text | 50 KiB | `AMBUSH_AGENT_MAX_TOOL_RESULT_TEXT_BYTES` |
...
| Loop rounds | 0 (unlimited) | `AMBUSH_AGENT_MAX_ROUNDS` |
| LLM read inactivity timeout | 240 s | `AMBUSH_AGENT_LLM_TIMEOUT_SECS` |
| Tool call timeout | 660 s | `AMBUSH_AGENT_TOOL_TIMEOUT_SECS` |
