...
async fn handshake(h: &mut Harness) -> String {
    ...
    assert_eq!(init["result"]["agentInfo"]["name"], "ambush-agent");
    assert_eq!(
...
/// ambush-agent must emit a `usage_update` notification with `accumulatedInputTokens`
...
/// When the Anthropic parser produces an input-sum overflow, ambush-agent must
