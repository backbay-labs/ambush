...
//! exercised manually via the `ambush-agent auth databricks` subcommand
//! (see `lib.rs::auth_subcommand`).
//!
//! The second test module (further down) is an ACP-level envelope
//! regression: it spawns the real `ambush-agent` binary with
...
use ambush_agent::auth::{PkceOAuthConfig, PkceOAuthTokenSource, TokenSource};
...
//
// Boots the real ambush-agent binary with `DATABRICKS_TOKEN` set (so the
...
impl AgentHarness {
    ...
    async fn spawn_provider_with_options(
        ...
        cmd.env("AMBUSH_AGENT_PROVIDER", provider)
            ...
            .env("AMBUSH_AGENT_MAX_ROUNDS", "2")
            .env("AMBUSH_AGENT_MAX_SESSIONS", max_sessions.to_string())
            .env("AMBUSH_AGENT_MCP_INIT_TIMEOUT_SECS", "2")
            .stdin(Stdio::piped())
            .stdout(Stdio::piped())
            .stderr(Stdio::null())
            .kill_on_drop(true);
...
async fn model_discovery_surfaces_rejected_static_token_as_auth_failure() {
    use ambush_agent::config::{Config, Provider};
    use ambush_agent::discover_databricks_models;
