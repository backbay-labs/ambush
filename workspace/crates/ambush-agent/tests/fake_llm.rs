//! Integration test: fake LLM HTTP server + ambush-agent subprocess.
//!
...
impl Harness {
    async fn spawn(base_url: &str) -> Self {
        let bin = env!("CARGO_BIN_EXE_ambush-agent");
        let mut cmd = tokio::process::Command::new(bin);
        cmd.env("AMBUSH_AGENT_PROVIDER", "openai")
            .env("OPENAI_COMPAT_API_KEY", "test")
            .env("OPENAI_COMPAT_MODEL", "fake-model")
            .env("OPENAI_COMPAT_BASE_URL", base_url)
            .env("AMBUSH_AGENT_LLM_TIMEOUT_SECS", "5")
            .env("AMBUSH_AGENT_TOOL_TIMEOUT_SECS", "5")
            .env("AMBUSH_AGENT_MAX_ROUNDS", "4")
            .stdin(Stdio::piped())
            .stdout(Stdio::piped())
            .stderr(Stdio::inherit())
            .kill_on_drop(true);
        let mut child = cmd.spawn().expect("spawn ambush-agent");
...
async fn init_session(h: &mut Harness) -> String {
    ...
    assert_eq!(r["result"]["agentInfo"]["name"], "ambush-agent");
...
async fn rejects_oversized_line() {
    ...
    cmd.env("AMBUSH_AGENT_PROVIDER", "openai")
        ...
        .env("AMBUSH_AGENT_MAX_LINE_BYTES", "256")
        .stdin(Stdio::piped())
        .stdout(Stdio::piped())
        .stderr(Stdio::null())
        .kill_on_drop(true);
...
async fn system_prompt_reaches_llm_system_role() {
    ...
    let default_prompt = "You are ambush-agent";
    assert!(
...
async fn system_prompt_absent_no_canary() {
    ...
    assert!(
        system_content.contains("You are ambush-agent"),
...
/// Wait for the `activeRunId` advert ambush-agent emits at prompt start and
...
/// completion_tokens). ambush-agent maps these to `accumulatedInputTokens` /
...
/// ambush-agent must treat this turn's total as Unknown and poison the cumulative.
...
/// ambush-agent must emit `_goose/unstable/session/update` with `sessionUpdate:
...
/// When the provider returns a response with no `usage` block, ambush-agent must
...
/// (so token counts are observed), ambush-agent must still emit the usage
