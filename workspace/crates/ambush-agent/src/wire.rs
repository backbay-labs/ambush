...
pub enum Inbound {
    ...
    /// to a request ambush-agent issued. Today the only such request is
...
/// one. `expected_run_id` must match the run id ambush-agent advertised via
...
/// session use `model_id` instead of the configured `AMBUSH_AGENT_MODEL`.
#[derive(Debug, Deserialize)]
#[serde(rename_all = "camelCase")]
...
pub fn classify(msg: &Value) -> Inbound {
    ...
    match (method, id) {
        ...
        // Bare responses (id present, no method) answer a request ambush-agent
...
/// `session/request_permission`. ambush-acp's answering side selects the option
...
/// is applied.
///
/// - **v2** (what ambush-agent negotiates with current ambush-acp): tool context
...
/// method goose uses for custom usage and status events.  Used by ambush-agent
/// to emit the `usage_update` payload so ambush-acp's `UsageTracker` can treat
/// ambush-agent and goose symmetrically.
...
/// exists to prevent.
///
/// All counts are SESSION-cumulative, matching goose, so ambush-acp's
...
/// | `contextLimit` | `u64` | `0` — ambush-agent has no context limit tracking |
...
pub fn usage_update_payload(
    ...
    let mut update = json!({
        ...
        // contextLimit: 0 (ambush-agent has no context limit tracking).
...
mod tests {
    ...
    /// v2 (what ambush-agent negotiates with current ambush-acp): tool context is
    ...
    /// `optionId == kind` so ambush-acp's `kind`-based selector and this side's
    ...
    /// request ambush-agent issued; it routes to the broker with its `result`.
    #[test]
    ...
    /// fails closed. ambush-agent never leaves the waiter hanging on an error.
    #[test]
