...
/// `cache_namespace` is the directory under `~/.config/ambush-agent/oauth/`
...
pub struct PkceOAuthConfig {
    ...
    /// `~/.config/ambush-agent/oauth/<cache_namespace>/`. Production code
...
impl PkceOAuthTokenSource {
    ...
    pub(crate) async fn try_bearer_no_browser(&self) -> Result<String, AgentError> {
        ...
        Err(AgentError::LlmAuth(
            "no cached Databricks token; run `ambush-agent auth databricks` first".into(),
        ))
    }
}
...
/// write-path property: a world-readable cache left by an older ambush-agent
...
fn callback_outcome(
    ...
    let page = match result {
        Ok(_) => "<h2>Ambush: signed in</h2><p>You can close this window.</p>",
        Err(_) => "<h2>Ambush auth failed</h2><p>You can close this window and try again.</p>",
    }
    .to_string();
    (result, page)
}
...
mod tests {
    ...
    async fn test_save_repairs_preexisting_loose_mode_file() {
        ...
        // Simulate a world-readable cache written by an older ambush-agent.
    ...
    async fn test_bearer_cache_hit_tightens_preexisting_loose_mode_file() {
        ...
        // A world-readable cache left by an older ambush-agent must be tightened
