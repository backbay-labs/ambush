...
const PASSTHROUGH_ENV: &[&str] = &[
    ...
    "SSL_CERT_FILE",
    "SSL_CERT_DIR",
    // Ambush identity — dev-mcp writes NOSTR_PRIVATE_KEY to a keyfile then
    // removes it from its own env (children never see it). AMBUSH_PRIVATE_KEY
    // and AMBUSH_RELAY_URL are kept for the ambush CLI. AMBUSH_AUTH_TAG is a
    ...
    // spawn ambush-agent without declaring it.
...
mod content_tests {
    ...
    #[test]
    fn passthrough_includes_ambush_owner_attestation() {
        assert!(PASSTHROUGH_ENV.contains(&"AMBUSH_AUTH_TAG"));
    }
