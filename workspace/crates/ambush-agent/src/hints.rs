...
mod tests {
    ...
    fn build_hints_section_combined() {
        ...
        let skill_dir = cwd.join(".agents/skills/ambush-cli");
        std::fs::create_dir_all(&skill_dir).unwrap();
        std::fs::write(
            skill_dir.join("SKILL.md"),
            "---\nname: ambush-cli\ndescription: CLI reference for Ambush managed agents\n---\nUse `ambush` to manage agents.\n",
        )
        .unwrap();
        ...
        assert!(
            result.contains("ambush-cli: CLI reference for Ambush managed agents"),
            "missing skill bullet"
        );
        // Body must NOT be inlined — lazy loading only.
        assert!(
            !result.contains("Use `ambush` to manage agents."),
