...
/// Blossom kind:24242 verbs Ambush currently accepts.
...
/// [`ambush_core::tenant::normalize_host`] rule so the comparison agrees with how
...
mod tests {
    ...
    fn build_get_auth(keys: &Keys, tags: Vec<Tag>) -> nostr::Event {
        EventBuilder::new(Kind::from(24242), "Get ambush-media")
            .tags(tags)
            .sign_with_keys(keys)
            .unwrap()
    }
    ...
    fn test_server_tag_enforcement() {
        ...
        assert!(matches!(
            verify_blossom_upload_auth(&event, &sha256, Some("ambush.example.com"), 600),
            Err(MediaError::ServerMismatch)
        ));
