...
mod tests {
    ...
    async fn delete_object_versions_empty_input_short_circuits_before_folding() {
        let storage = MediaStorage::new(&storage_config("ambush_dev", "ambush_dev_secret"))
  ...
  <Version><Key>k-versioned</Key><VersionId>v1</VersionId><Size>10</Size></Version>
    ...
    fn storage_config(access: &str, secret: &str) -> crate::config::MediaConfig {
        crate::config::MediaConfig {
            ...
            s3_bucket: "ambush-media".to_string(),
    ...
    fn client_constructor_applies_both_addressing_styles() {
        let path = MediaStorage::new(&storage_config("ambush_dev", "ambush_dev_secret"))
            .expect("path-style client");
        assert!(path.bucket.is_path_style());
        assert_eq!(path.bucket.url(), "http://localhost:9000/ambush-media");

        let mut virtual_config = storage_config("ambush_dev", "ambush_dev_secret");
        ...
        assert_eq!(
            ...
            "http://ambush-media.localhost:9000"
        );
    }

    #[test]
    fn partial_static_keys_are_rejected() {
        let err = match MediaStorage::new(&storage_config("ambush_dev", "")) {
        ...
        let err = match MediaStorage::new(&storage_config("", "ambush_dev_secret")) {
            Ok(_) => panic!("partial static creds must not silently use credential chain"),
            Err(err) => err,
        };
        assert!(
