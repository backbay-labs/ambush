...
//!
//! Run it against the docker-compose MinIO (creds `ambush_dev`/`ambush_dev_secret`):
//!
//! ```bash
//! docker compose up -d minio minio-init
//! cargo test -p ambush-media --test versioned_minio -- --ignored --nocapture
//! ```
//!
//! The test creates and removes its own bucket. The MinIO container name is
//! overridable with `AMBUSH_MINIO_CONTAINER`; credentials/endpoint/region/addressing
//! use the same `AMBUSH_S3_*` env vars as `static_creds_minio`.

use std::process::Command;

use ambush_media::config::MediaConfig;
use ambush_media::storage::{MediaStorage, ObjectVersionKind, ObjectVersionRef};
...
fn minio_config(bucket: String) -> MediaConfig {
    MediaConfig {
        s3_endpoint: env_or("AMBUSH_S3_ENDPOINT", "http://localhost:9000"),
        s3_access_key: env_or("AMBUSH_S3_ACCESS_KEY", "ambush_dev"),
        s3_secret_key: env_or("AMBUSH_S3_SECRET_KEY", "ambush_dev_secret"),
        s3_bucket: bucket,
        s3_region: env_or("AMBUSH_S3_REGION", "us-east-1"),
        s3_addressing_style: env_or("AMBUSH_S3_ADDRESSING_STYLE", "path")
            .parse()
...
fn run_mc(args: &[String]) -> Result<(), String> {
    let container = env_or("AMBUSH_MINIO_CONTAINER", "ambush-minio");
...
async fn list_all_versions(
    ...
    prefix: &str,
    max_keys: usize,
) -> Vec<ambush_media::storage::ObjectVersionEntry> {
...
fn refs_from(entries: &[ambush_media::storage::ObjectVersionEntry]) -> Vec<ObjectVersionRef> {
    entries
        .iter()
...
async fn never_versioned_bucket_lists_null_versions_and_exact_delete_empties_listing() {
    let bucket = format!("ambush-media-never-versioned-{}", std::process::id());
...
async fn versioned_bucket_exact_version_delete_reaches_final_list_versions_emptiness() {
    let bucket = format!("ambush-media-versioned-{}", std::process::id());
    let bucket_path = format!("local/{bucket}");
    let config = minio_config(bucket.clone());
    mc_alias(&config.s3_access_key, &config.s3_secret_key).expect("configure mc alias");
    run_mc(&[
