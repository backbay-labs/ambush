...
//! bucket `ambush-media`, endpoint `http://localhost:9000`):
//!
//! ```bash
//! docker compose up -d minio minio-init
//! cargo test -p ambush-media --test static_creds_minio -- --ignored
//! ```
//!
//! Overridable via `AMBUSH_S3_ENDPOINT` / `AMBUSH_S3_ACCESS_KEY` /
//! `AMBUSH_S3_SECRET_KEY` / `AMBUSH_S3_BUCKET` / `AMBUSH_S3_REGION` /
//! `AMBUSH_S3_ADDRESSING_STYLE`. The default remains `path` for MinIO.

use ambush_media::config::MediaConfig;
use ambush_media::storage::MediaStorage;
...
fn minio_config() -> MediaConfig {
    MediaConfig {
        ...
        s3_bucket: std::env::var("AMBUSH_S3_BUCKET").unwrap_or_else(|_| "ambush-media".to_string()),
        s3_region: std::env::var("AMBUSH_S3_REGION").unwrap_or_else(|_| "us-east-1".to_string()),
