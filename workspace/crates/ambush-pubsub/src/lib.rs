#![deny(unsafe_code)]
#![warn(missing_docs)]
//! `ambush-pubsub` — Redis pub/sub fan-out, presence tracking, and typing indicators.
//!
//! # Architecture
//!
//! ```text
//! ambush-relay process
//!   │
//!   ├── deadpool-redis pool → PUBLISH, SET, ZADD, etc.
//!   │
//!   └── dedicated redis::aio::PubSub connection (NOT from pool)
//!         └── dynamic SUBSCRIBE ambush:{community}:channel:{id} / ambush:{community}:global
...
pub struct ChannelEvent {
    ...
    pub community_id: ambush_core::CommunityId,
...
/// Central pub/sub manager for a Ambush relay instance.
...
impl PubSubManager {
    ...
    /// `ambush:{community}:channel:{id}` / `ambush:{community}:global`, and
