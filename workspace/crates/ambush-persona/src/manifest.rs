...
//! (OPS metadata) and tells Ambush where to find personas, hooks, and MCP
//! config.
//!
//! ```json
//! {
...
pub struct Engines {
    /// Semver range the Ambush runtime must satisfy (e.g. `">=0.9.0"`).
    #[serde(skip_serializing_if = "Option::is_none", alias = "ambush")]
...
/// validator issues advisory warnings for Ambush-unknown keys.
...
mod tests {
    ...
    fn parse_full_manifest() {
        ...
        assert_eq!(m.engines.unwrap().ambush.as_deref(), Some(">=0.9.0"));
