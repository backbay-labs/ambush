...
//! - Ambush-agent personas emit AMBUSH_AGENT_MODEL, AMBUSH_AGENT_PROVIDER
...
use ambush_persona::resolve::resolve_pack;
...
#[test]
fn resolve_pack_ambush_agent_persona_emits_ambush_agent_vars() {
    let tmp = tempfile::tempdir().unwrap();
    let root = tmp.path();
...
"#,
    ...
    assert_eq!(
        env.get("AMBUSH_AGENT_MODEL"),
        Some(&"gpt-4o"),
        "should emit AMBUSH_AGENT_MODEL=gpt-4o"
    );
    assert_eq!(
        env.get("AMBUSH_AGENT_PROVIDER"),
        Some(&"openai"),
        "should emit AMBUSH_AGENT_PROVIDER=openai"
    );

    // Must NOT contain GOOSE_* keys
    assert!(
        !env.contains_key("GOOSE_MODEL"),
        "ambush-agent runtime must not emit GOOSE_MODEL"
    );
    assert!(
        !env.contains_key("GOOSE_PROVIDER"),
        "ambush-agent runtime must not emit GOOSE_PROVIDER"
    );
}
...
"#,
    ...
    // Ambush-agent persona
    fs::write(
        root.join("agents/ambush-bot.persona.md"),
        r#"---
name: "ambush-bot"
display_name: "Ambush Bot"
description: "An ambush-agent runtime bot"
runtime: "ambush-agent"
model: "openai:gpt-4o"
---
You are an ambush bot.
"#,
    )
    .unwrap();
    ...
    let ambush = pack
        .personas
        .iter()
        .find(|p| p.name == "ambush-bot")
        .expect("ambush-bot should exist");
    ...
    assert!(
        !goose_env.contains_key("AMBUSH_AGENT_MODEL"),
        "goose persona must not emit AMBUSH_AGENT_MODEL"
    );
    assert!(
        !goose_env.contains_key("AMBUSH_AGENT_PROVIDER"),
        "goose persona must not emit AMBUSH_AGENT_PROVIDER"
    );

    // Ambush-agent persona gets AMBUSH_AGENT_* env vars
    let ambush_env: std::collections::HashMap<_, _> = ambush
        .runtime_env_vars
        .iter()
        .map(|(k, v)| (k.as_str(), v.as_str()))
        .collect();
    assert_eq!(ambush_env.get("AMBUSH_AGENT_MODEL"), Some(&"gpt-4o"));
    assert_eq!(ambush_env.get("AMBUSH_AGENT_PROVIDER"), Some(&"openai"));
    assert!(
        !ambush_env.contains_key("GOOSE_MODEL"),
        "ambush-agent persona must not emit GOOSE_MODEL"
    );
    assert!(
        !ambush_env.contains_key("GOOSE_PROVIDER"),
        "ambush-agent persona must not emit GOOSE_PROVIDER"
    );
}
