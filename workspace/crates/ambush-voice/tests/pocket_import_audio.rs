...
fn evidence_dir() -> PathBuf {
    std::env::var_os("AMBUSH_VOICE_EVIDENCE_DIR")
        .map(PathBuf::from)
        .unwrap_or_else(|| {
            Path::new(env!("CARGO_MANIFEST_DIR")).join("../../target/ambush-voice-evidence")
        })
}
...
#[test]
#[ignore = "requires AMBUSH_POCKET_MODEL_DIR and runs the installed Pocket ONNX model"]
fn objective_import_synthesis_delete_and_mary_fallback() {
    let model_dir = required_path("AMBUSH_POCKET_MODEL_DIR");
