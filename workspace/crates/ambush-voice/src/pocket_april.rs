...
impl AprilPocketTts {
    ...
    pub(crate) fn synth_chunk(
        ...
        // EXPERIMENTAL (latency bench): phase timing, enabled by AMBUSH_TTS_PHASE_LOG=1.
        let phase_log = std::env::var("AMBUSH_TTS_PHASE_LOG").is_ok_and(|v| v == "1");
...
mod tests {
    ...
    fn switching_between_equal_length_voices_reconditions_the_flow_state() {
        let dir = std::env::var("AMBUSH_POCKET_TEST_MODEL_DIR")
            .expect("set AMBUSH_POCKET_TEST_MODEL_DIR to the verified April bundle");
        let style_a =
