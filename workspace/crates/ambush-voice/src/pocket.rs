//! April 2026 Pocket TTS engine for Ambush Desktop.
//!
...
//! Ambush selects the upstream three-graph INT8 variant while retaining the
...
/// sessions via `AMBUSH_TTS_THREADS`. Default preserves production's 1.
fn tts_num_threads() -> usize {
    std::env::var("AMBUSH_TTS_THREADS")
        .ok()
...
/// Load Ambush Desktop's pinned April INT8 model.
...
mod tests {
    ...
    #[test]
    #[ignore = "requires AMBUSH_POCKET_TEST_MODEL_DIR"]
    fn production_api_emits_non_silent_april_int8_pcm() {
        let dir = std::env::var("AMBUSH_POCKET_TEST_MODEL_DIR")
            .expect("set AMBUSH_POCKET_TEST_MODEL_DIR to an April INT8 model directory");
