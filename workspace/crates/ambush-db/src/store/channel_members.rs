...
pub use ambush_core::channel::MemberRole;
...
impl LockedMemberSnapshot {
    ...
    pub async fn replace_member_event(
        ...
        channel_id: Uuid,
        event: &nostr::Event,
    ) -> Result<(ambush_core::StoredEvent, bool)> {
        ...
        let kind = ambush_core::kind::event_kind_i32(event);
        if kind != 39002 {
            return Err(DbError::InvalidData(
        ...
        if let Some((existing_ts, existing_id)) = existing {
            ...
            {
                return Ok((
                    ambush_core::StoredEvent::with_received_at(
                        event.clone(),
                        Utc::now(),
                        Some(channel_id),
                        false,
