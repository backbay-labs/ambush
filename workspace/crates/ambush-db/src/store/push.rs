...
pub struct BatchedMatch {
    ...
    pub event: ambush_core::StoredEvent,
    /// Attempt number, starting at one for the first claim.
    pub attempt: i32,
}
...
where
    ...
    Fut: std::future::Future<Output = Result<Vec<ambush_core::StoredEvent>>>,
{
...
mod tests {
    ...
    async fn setup_pool() -> PgPool {
        let database_url = std::env::var("AMBUSH_TEST_DATABASE_URL")
            ...
            .unwrap_or_else(|_| "postgres://ambush:ambush_dev@localhost:5432/ambush".into()); // sadscan:disable np.postgres.1 -- local test-only credentials
