...
///
/// Reads `AMBUSH_MAX_COMMUNITIES_PER_OWNER` once (cached for the process
...
pub fn max_communities_per_owner() -> i64 {
    ...
    *LIMIT.get_or_init(|| {
        effective_owner_limit(
            std::env::var("AMBUSH_MAX_COMMUNITIES_PER_OWNER")
                .ok()
                .as_deref(),
        )
    })
...
impl Db {
    ...
    pub async fn nip43_membership_snapshot_needs_reconciliation(
        ...
        let snapshot = self
            .query_events(&crate::event::EventQuery {
                kinds: Some(vec![ambush_core::kind::KIND_NIP43_MEMBERSHIP_LIST as i32]),
    ...
    pub async fn publish_nip43_membership_locked(
        ...
        let kind_i32 = ambush_core::kind::KIND_NIP43_MEMBERSHIP_LIST as i32;
