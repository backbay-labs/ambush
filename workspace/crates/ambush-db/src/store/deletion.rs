...
/// the instant it is checked. The value is arbitrary (ASCII `ambushdel1`) but
...
impl DeletionStore {
    ...
    ///
    /// Probe the deployed catalog rather than SQLx's migration ledger. Ambush also
...
mod postgres_tests {
    ...
    async fn store() -> (Db, DeletionStore) {
        let database_url = std::env::var("AMBUSH_TEST_DATABASE_URL")
            ...
            .unwrap_or_else(|_| "postgres://ambush:ambush_dev@localhost:5432/ambush".to_string()); // sadscan:disable np.postgres.1 -- local test-only credentials
    ...
    async fn preclaim_setup_failure_is_durable_without_a_lease() {
        ...
        let blocked = store
            .block_preclaim_setup(
                ...
                "AMBUSH_S3_ENDPOINT is required",
            )
            .await
        ...
        assert_eq!(
            ...
            Some("AMBUSH_S3_ENDPOINT is required")
        );
        assert_eq!(
    ...
    async fn destructive_stages_serialize_with_migrations_and_fail_closed_on_new_scoped_tables() {
        ...
        let probe_db = format!("ambush_lock_probe_{}", Uuid::new_v4().simple());
    ...
    async fn desired_state_schema_bootstrap_progresses_beyond_fencing() {
        ...
        let admin = PgPool::connect(&base_url)
            .await
            .expect("connect admin database");
        let probe_db = format!("ambush_desired_state_{}", Uuid::new_v4().simple());
        sqlx::query(AssertSqlSafe(format!("CREATE DATABASE {probe_db}")))
            .execute(&admin)
            .await
            .expect("create probe database");
