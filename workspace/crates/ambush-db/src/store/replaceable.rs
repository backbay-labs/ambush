...
async fn replace_parameterized_event_in_transaction_impl(
    ...
    let kind_i32 = ambush_core::kind::event_kind_i32(event);
    ...
    let read_state_t_tag_count = event
        ...
        .count();
    let is_nip_rs = kind_i32 == ambush_core::kind::KIND_READ_STATE as i32
        && d_tag_count == 1
        && has_exact_d_tag
    ...
    let is_ambush_mesh_status = kind_i32 == ambush_core::kind::KIND_BOOKMARK_SET as i32
        && d_tag.starts_with("buzz-mesh-member-status:")
        && event.tags.iter().any(|tag| {
            let parts = tag.as_slice();
            parts.len() == 2 && parts[0] == "k" && parts[1] == "buzz-mesh-status"
        });
    let hard_delete_superseded = is_nip_rs || is_ambush_mesh_status;
...
mod tests {
    ...
    const TEST_DB_URL: &str = "postgres://ambush:ambush_dev@localhost:5432/ambush"; // sadscan:disable np.postgres.1 -- local test-only credentials
    ...
    async fn nip_rs_replacement_hard_deletes_payload_and_watermark_rejects_replay() {
        ...
        let old = EventBuilder::new(
            Kind::Custom(ambush_core::kind::KIND_READ_STATE as u16),
            "old",
        )
        .tags(tags.clone())
        .custom_created_at(Timestamp::from(base))
    ...
    async fn parameterized_replacement_in_existing_transaction_honors_revision_and_rollback() {
        ...
        let event = |content: &str, timestamp: u64| {
            EventBuilder::new(
                Kind::Custom(ambush_core::kind::KIND_PROJECT as u16),
                content,
            )
        ...
        .bind(community.as_uuid())
        .bind(ambush_core::kind::KIND_PROJECT as i32)
        .bind(keys.public_key().to_bytes())
        .bind(&d_tag)
        .fetch_one(&db.pool)
        .await
    ...
    async fn parameterized_replacement_rolls_back_when_mention_indexing_fails() {
        ...
        let base = Timestamp::now().as_secs();
        let old = EventBuilder::new(Kind::Custom(ambush_core::kind::KIND_PROJECT as u16), "old")
            .tags(tags())
            .custom_created_at(Timestamp::from(base))
            .sign_with_keys(&keys)
            .expect("sign old project");
        let new = EventBuilder::new(Kind::Custom(ambush_core::kind::KIND_PROJECT as u16), "new")
            .tags(tags())
            .custom_created_at(Timestamp::from(base + 1))
            .sign_with_keys(&keys)
    ...
    async fn mesh_status_replacement_keeps_one_physical_row() {
        ...
        for (offset, content) in [(0, "running"), (1, "running-again"), (2, "stopped")] {
            let event = EventBuilder::new(
                Kind::Custom(ambush_core::kind::KIND_BOOKMARK_SET as u16),
                content,
            )
            .tags(tags.clone())
