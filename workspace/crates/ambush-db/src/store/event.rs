...
async fn insert_event_on(
    ...
    // Cast chain: nostr Kind (u16) → i32 (Postgres INT column). Safe: all Ambush kinds fit in i32.
...
impl Db {
    ...
    /// seam is gated on `AMBUSH_REPLICA_READ_MAX_AGE_MS` (default off): when
...
mod tests {
    ...
    async fn coordinate_delete_spares_head_newer_than_the_deletion() {
        ...
        let keys = Keys::generate();
        let kind = ambush_core::kind::KIND_PROJECT as i32;
