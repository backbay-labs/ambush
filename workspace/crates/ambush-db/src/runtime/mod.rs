...
use ambush_core::{CommunityId, StoredEvent};
            ...
            ReadSessionInner::Replica { tx, writer } => {
                match event::query_events_on(tx, q).await {
                    ...
                    Err(e) => {
                        ...
                        // Deliberately not a `ambush_db_route_decision` event:
                        // the page's route was already recorded, and the
                        // offload metric must stay one-event-per-request.
                        metrics::counter!("ambush_db_read_session_degraded").increment(1);
                        writer.clone()
                    }
                }
            }
...
pub(crate) enum RouteDecision {
    ...
    /// failure emits exactly one `ambush_db_route_decision` event per request
...
impl RoutePredicate {
    ...
    ///
    /// `routing_enabled` is whether `AMBUSH_REPLICA_READ_MAX_AGE_MS` is set
...
/// Map the configured read budget (`AMBUSH_REPLICA_READ_MAX_AGE_MS`) to the
...
pub struct DbConfig {
    ...
    /// `AMBUSH_DB_READ_POOL_SIZE`). `None` inherits [`Self::max_connections`].
    ...
    /// `AMBUSH_REPLICA_READ_MAX_AGE_MS`). `0` disables bounded-staleness
...
impl Default for DbConfig {
    ...
    fn default() -> Self {
        Self {
            database_url: "postgres://ambush:ambush_dev@localhost:5432/ambush".to_string(), // sadscan:disable np.postgres.1
...
impl Db {
    ...
    /// the migration decision. On a relay with `AMBUSH_AUTO_MIGRATE` off, the
    ...
    > {
        ...
        .await
        {
            ...
            // `ambush_db_route_decision{decision="writer",reason="reader_acquire_timeout"}`
            ...
            // Runbook: correlate with `ambush_db_read_pool_active` / `_max`
            ...
            // sample (AMBUSH_POOL_METRICS_INTERVAL_SECS, default 10s) while
    ...
    /// the writer's: `ambush_db_read_pool_active / ambush_db_read_pool_max` is
    /// the operator's utilisation signal for tuning `AMBUSH_DB_READ_POOL_SIZE`,
    ...
    pub async fn insert_event_with_serving_write_guard(
        ...
        if ambush_core::kind::is_ephemeral(kind_u32) {
