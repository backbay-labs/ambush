...
const TEST_DB_URL: &str = "postgres://ambush:ambush_dev@localhost:5432/ambush";
...
async fn database_guard_covers_legacy_writer_and_nip09_deletion() {
    ...
    let a = EventBuilder::new(Kind::Custom(ambush_core::kind::KIND_READ_STATE as u16), "A")
        .tags(tags.clone())
        .custom_created_at(Timestamp::from(base))
        .sign_with_keys(&keys)
        .expect("sign A");
    let x = EventBuilder::new(Kind::Custom(ambush_core::kind::KIND_READ_STATE as u16), "X")
        .tags(tags.clone())
        .custom_created_at(Timestamp::from(base + 1))
        .sign_with_keys(&keys)
        .expect("sign X");
    let b = EventBuilder::new(Kind::Custom(ambush_core::kind::KIND_READ_STATE as u16), "B")
        .tags(tags.clone())
        .custom_created_at(Timestamp::from(base + 2))
        .sign_with_keys(&keys)
        .expect("sign B");
    let c = EventBuilder::new(Kind::Custom(ambush_core::kind::KIND_READ_STATE as u16), "C")
        .tags(tags)
    ...
    async fn legacy_insert(
        ...
        sqlx::query(
            ...
            .bind(ambush_core::kind::KIND_READ_STATE as i32)
...
/// `routing_enabled = false` (AMBUSH_REPLICA_READ_MAX_AGE_MS unset)
...
/// ceiling, not the writer's — `ambush_db_read_pool_active / _max` is the
...
/// directly, because `ambush_db_route_decision` is emitted by `route_read`
...
async fn routed_fallback_spends_one_acquire_budget_when_aurora_cache_is_cold() {
    ...
    let reasons: std::collections::HashMap<(String, String), u64> = snapshotter
        ...
        .into_vec()
        .into_iter()
        .filter(|(key, ..)| key.key().name() == "ambush_db_route_decision")
        .map(|(key, _, _, value)| {
            let metrics_util::debugging::DebugValue::Counter(n) = value else {
                panic!("ambush_db_route_decision must be a counter");
            };
...
/// `AMBUSH_AUTO_MIGRATE` off: an armed GUC with no enforcing trigger must
...
async fn fence_probe_refuses_to_start_without_verified_floor_guard() {
    ...
    // Sabotage B: trigger dropped entirely (the AMBUSH_AUTO_MIGRATE=off /
