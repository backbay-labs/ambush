...
pub(crate) fn record_pool_acquire(role: PoolRole, outcome: Outcome, elapsed: Duration) {
    metrics::histogram!(
        "ambush_db_pool_acquire_wait_seconds",
        "pool_role" => role.as_str(),
        "outcome" => outcome.as_str(),
    )
    .record(elapsed.as_secs_f64());
    metrics::counter!(
        "ambush_db_pool_acquisitions_total",
        "pool_role" => role.as_str(),
        "outcome" => outcome.as_str(),
    )
    .increment(1);
    ...
    F: Future<Output = sqlx::Result<T>>,
{
    ...
    let outcome = result
        ...
        .unwrap_or_else(Outcome::from_sqlx_error);
    metrics::histogram!(
        "ambush_db_advisory_lock_wait_seconds",
        "lock_type" => lock_type.as_str(),
        "outcome" => outcome.as_str(),
    )
    .record(started.elapsed().as_secs_f64());
    metrics::counter!(
        "ambush_db_advisory_lock_acquisitions_total",
        "lock_type" => lock_type.as_str(),
        "outcome" => outcome.as_str(),
    )
    .increment(1);
...
impl Drop for TransactionTimer {
    fn drop(&mut self) {
        metrics::histogram!(
            "ambush_db_transaction_duration_seconds",
...
mod tests {
    ...
    async fn transaction_timer_observe_classifies_result_outcomes() {
        ...
        for (operation, outcome) in [
            ...
            assert!(keys.contains(&(
                "ambush_db_transaction_duration_seconds".to_owned(),
                [
    ...
    async fn pool_acquire_records_success_timeout_and_error_with_wait_time() {
        let database_url = std::env::var("TEST_DATABASE_URL")
            .unwrap_or_else(|_| "postgres://ambush:ambush_dev@localhost:5432/ambush".to_owned()); // sadscan:disable np.postgres.1 -- local test-only credentials
        ...
        for (key, _, _, value) in snapshotter.snapshot().into_vec() {
            if key.key().name() != "ambush_db_pool_acquire_wait_seconds" {
                continue;
            }
            let DebugValue::Histogram(samples) = value else {
    ...
    async fn advisory_lock_records_success_contention_timeout_and_error() {
        ...
        let mut outcomes = BTreeMap::<(String, String), Vec<f64>>::new();
        for (key, _, _, value) in snapshotter.snapshot().into_vec() {
            if key.key().name() != "ambush_db_advisory_lock_wait_seconds" {
                continue;
            }
            let DebugValue::Histogram(samples) = value else {
