//! Embedded SQLx migrations for Ambush.
//!
...
/// Run all pending Ambush database migrations.
///
...
mod tests {
    ...
    /// URL. Only the shapes this test emits (`AMBUSH_TEST_DATABASE_URL` /
    ...
    fn migration_execution_cannot_bypass_schema_destruction_lock() {
        ...
        // exemption honest.
        let push_gateway_exception = crates_dir.join("ambush-push-gateway/src/postgres.rs");
        let push_gateway_migrations = crates_dir.join("ambush-push-gateway/migrations");
        for entry in
            std::fs::read_dir(&push_gateway_migrations).expect("read push gateway migrations")
        {
            ...
            assert!(
                ...
                "{} defines community-scoped data; its migrator would bypass the \
                 schema/destruction lock and must move under ambush-db migrations",
                path.display()
            );
        }
        let mut files = Vec::new();
            ...
            if *path == this_file {
                ...
                assert_eq!(
                    ...
                    "{} embeds or runs a SQLx migrator outside the schema/destruction \
                     ...
                     ambush_db migration::run_migrations",
                    path.display()
                );
            }
        }
    ...
    async fn cancelled_migration_cannot_expose_shared_lock_while_ddl_backend_lives() {
        ...
        let base_url = std::env::var("AMBUSH_TEST_DATABASE_URL")
            .or_else(|_| std::env::var("DATABASE_URL"))
            .unwrap_or_else(|_| TEST_DB_URL.to_owned());
        let admin = PgPool::connect(&base_url)
            .await
            .expect("connect admin database");
        let probe_db = format!("ambush_lock_cancel_{}", uuid::Uuid::new_v4().simple());
    ...
    async fn admin_schema_parity_between_desired_state_and_migrations() {
        ...
        let desired_db = format!("ambush_admin_desired_{}", uuid::Uuid::new_v4().simple());
        let migrated_db = format!("ambush_admin_migrated_{}", uuid::Uuid::new_v4().simple());
