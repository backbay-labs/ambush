//! `ambush-pair` — NIP-AB device pairing interop testing CLI.
//!
//! # Usage
//!
//! ```text
//! ambush-pair source --relay wss://relay.example.com [--nsec nsec1...]
//! ambush-pair target [--relay wss://relay.example.com]
...
use ambush_core::kind::KIND_PAIRING;
use ambush_core::pairing::session::PairingSession;
use ambush_core::pairing::{
...
async fn cmd_source(relay_url: String, nsec: Option<String>) -> Result<(), CliError> {
    ...
    if !confirmed {
        ...
        if let Some(abort_event) =
            session.abort(ambush_core::pairing::types::AbortReason::SasMismatch)?
        {
