...
CLI tool for testing the [NIP-AB device pairing protocol](../ambush-core/src/pairing/NIP-AB.md) end-to-end. Exercises the full protocol over a live Nostr relay — designed for interop testing and NIP submission, not production use.

## Quick Start

```bash
cargo build --release -p ambush-pairing-cli

# Terminal 1 — source (holds the secret)
./target/release/ambush-pair source --relay wss://relay.damus.io

# Terminal 2 — target (receives the secret)
./target/release/ambush-pair target --show-secret
...
```
ambush-pair source --relay <RELAY_URL> [--nsec <BECH32_NSEC>]
```
...
```
ambush-pair target [--relay <OVERRIDE_URL>] [--show-secret]
```
...
```
ambush-pair test-vectors
```

## Testing Against a Local Ambush Relay

The CLI supports NIP-42 authentication, so it works with Ambush relays out of the box.

### Prerequisites

- Docker running (for Postgres, Redis, etc.)
- Ambush relay built: `cargo build --release -p ambush-relay`
...
screen -dmS relay bash -c "./target/release/ambush-relay 2>&1 | tee /tmp/ambush-relay.log"
...
```bash
# Terminal 1
./target/release/ambush-pair source --relay ws://localhost:3000
