#![deny(unsafe_code)]
#![warn(missing_docs)]
//! `ambush-workflow` — Workflow engine for Ambush.
//!
...
use ambush_core::kind::{event_kind_u32, is_workflow_execution_kind, KIND_REACTION};
use ambush_core::tenant::CommunityId;
use ambush_db::workflow::RunStatus;
use ambush_db::Db;
...
pub struct WorkflowEngine {
    ...
    pub(crate) workflow_cache:
        moka::sync::Cache<(CommunityId, Uuid), Arc<Vec<ambush_db::workflow::WorkflowRecord>>>,
}
...
/// Build a [`executor::TriggerContext`] from a [`ambush_core::StoredEvent`].
///
...
pub fn build_trigger_context(event: &ambush_core::StoredEvent) -> executor::TriggerContext {
    let kind_u32 = event_kind_u32(&event.event);
...
/// marker. Delegates to the shared [`ambush_core::nip10`] parser so this stays in
...
fn event_is_reply(event: &nostr::Event) -> bool {
    ambush_core::nip10::parse_thread_markers(&event.tags)
        .reply
        .is_some()
}
...
fn trigger_matches_event(trigger: &TriggerDef, kind_u32: u32) -> bool {
    use ambush_core::kind::{KIND_REACTION, KIND_STREAM_MESSAGE, KIND_STREAM_MESSAGE_DIFF};
    match trigger {
...
"#;
    ...
    fn trigger_matches_stream_message() {
        ...
        assert!(trigger_matches_event(
            &trigger,
            ambush_core::kind::KIND_STREAM_MESSAGE
        ));
        assert!(!trigger_matches_event(
            &trigger,
...
"#;
    ...
    fn schedule_trigger_never_matches_events() {
        ...
        assert!(!trigger_matches_event(
            &trigger,
            ambush_core::kind::KIND_WORKFLOW_TRIGGERED
        ));
    }
    ...
    fn workflow_execution_kinds_do_not_match_any_trigger() {
        ...
        for kind in ambush_core::kind::KIND_WORKFLOW_TRIGGERED
            ..=ambush_core::kind::KIND_WORKFLOW_APPROVAL_DENIED
        {
            assert!(
    ...
    fn make_message_event() -> ambush_core::StoredEvent {
        ...
        let event = EventBuilder::new(Kind::Custom(9), "hello world")
            ...
            .sign_with_keys(&keys)
            .expect("sign");
        ambush_core::StoredEvent::new(event, Some(Uuid::new_v4()))
    }

    /// Create a reaction event with an `e` tag pointing to a target message.
    fn make_reaction_event() -> (ambush_core::StoredEvent, String) {
    ...
    fn build_trigger_context_is_reply_true_for_threaded_message() {
        ...
        let event = EventBuilder::new(Kind::Custom(9), "a threaded reply")
            ...
            .sign_with_keys(&keys)
            .expect("sign");
        let stored = ambush_core::StoredEvent::new(event, Some(Uuid::new_v4()));
        let ctx = build_trigger_context(&stored);
    ...
    fn build_trigger_context_no_channel_id() {
        ...
        let stored = ambush_core::StoredEvent::new(event, None);
        let ctx = build_trigger_context(&stored);
    ...
    fn build_trigger_context_ignores_actor_tag() {
        ...
        let event = EventBuilder::new(Kind::Custom(9), "forged actor")
            ...
            .expect("sign");
        let stored = ambush_core::StoredEvent::new(event, Some(uuid::Uuid::new_v4()));
    ...
    async fn setup_db() -> ambush_db::Db {
        let database_url = std::env::var("AMBUSH_TEST_DATABASE_URL")
            ...
            .unwrap_or_else(|_| {
                let local_test_database = "postgres://ambush:ambush_dev@localhost:5432/ambush"; // sadscan:disable np.postgres.1
                local_test_database.to_owned()
            });
        ambush_db::Db::new(&ambush_db::DbConfig {
            database_url,
            ..Default::default()
        })
        .await
    ...
    async fn setup_channel(
        ...
        {
            ambush_db::CreateCommunityWithOwnerResult::Created(rec) => rec.id,
    ...
    fn message_event(channel_id: Uuid) -> ambush_core::StoredEvent {
        let keys = nostr::Keys::generate();
        let event = nostr::EventBuilder::new(nostr::Kind::Custom(9), "hello")
            .sign_with_keys(&keys)
            .expect("sign");
        ambush_core::StoredEvent::new(event, Some(channel_id))
    }
