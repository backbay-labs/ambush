//! Canonical `ambush://` deep links for Ambush entities.
//!
//! Ambush Desktop renders these links as rich preview cards in chat and
...
/// A validated `ambush://message` deep link.
...
/// Parse a `ambush://message?channel=<uuid>&id=<event>[&thread=<root>]` link.
///
...
pub fn parse_message_link(input: &str) -> Result<MessageLink, CliError> {
    let url = url::Url::parse(input.trim())
        .map_err(|_| CliError::Usage("invalid Ambush message link".into()))?;
        ...
        || url.fragment().is_some()
    {
        return Err(CliError::Usage(
            "expected an ambush://message link without credentials or a fragment".into(),
        ));
    }
    ...
    for (key, value) in url.query_pairs() {
        let slot = match key.as_ref() {
            ...
            _ => {
                return Err(CliError::Usage(
                    "Ambush message link contains an unsupported query parameter".into(),
                ))
            }
        };
        if slot.replace(value.into_owned()).is_some() {
            return Err(CliError::Usage(format!(
                "Ambush message link contains more than one {key} parameter"
            )));
        }
    }
    ...
    let channel = channel
        ...
        .ok_or_else(|| CliError::Usage("Ambush message link is missing channel".into()))?;
    let message = message
        .filter(|value| !value.is_empty())
        .ok_or_else(|| CliError::Usage("Ambush message link is missing id".into()))?;
    if thread.as_deref() == Some("") {
        return Err(CliError::Usage(
            "Ambush message link contains an empty thread parameter".into(),
        ));
    }

    let channel_id = uuid::Uuid::parse_str(&channel)
        .map_err(|_| {
            CliError::Usage("Ambush message link contains an invalid channel UUID".into())
        })?
        .to_string();
...
fn canonical_event_id(value: &str, parameter: &str) -> Result<String, CliError> {
    if value.len() != 64 || !value.chars().all(|character| character.is_ascii_hexdigit()) {
        return Err(CliError::Usage(format!(
            "Ambush message link contains an invalid {parameter} event ID"
        )));
    }
    Ok(value.to_ascii_lowercase())
}

/// Whether a d-tag can be expressed in a `ambush://` link.
///
...
/// Build a `ambush://repo` link for a repository announcement (kind 30617).
pub fn repo_link(owner: &str, repo_id: &str) -> String {
    format!("ambush://repo?owner={owner}&d={repo_id}")
}

/// Build a `ambush://project` link for a project announcement (kind 30621).
pub fn project_link(owner: &str, project_id: &str) -> String {
    format!("ambush://project?owner={owner}&d={project_id}")
}

/// Build a `ambush://pr` link for a pull request event (kind 1618).
pub fn pull_request_link(event_id: &str, owner: &str, repo_id: &str) -> String {
    format!("ambush://pr?id={event_id}&owner={owner}&d={repo_id}")
}

/// Build a `ambush://issue` link for an issue event (kind 1621).
pub fn issue_link(event_id: &str, owner: &str, repo_id: &str) -> String {
    format!("ambush://issue?id={event_id}&owner={owner}&d={repo_id}")
}
...
mod tests {
    ...
    fn parses_message_link_with_thread_root() {
        let parsed = parse_message_link(&format!(
            "ambush://message?channel={CHANNEL}&id={MESSAGE}&thread={THREAD}"
        ))
        .unwrap();
    ...
    fn parses_message_link_without_thread_root() {
        let parsed =
            parse_message_link(&format!("ambush://message?channel={CHANNEL}&id={MESSAGE}"))
                .unwrap();
    ...
    fn rejects_message_link_that_could_change_connection_context() {
        for link in [
            format!("ambush://message?channel={CHANNEL}&id={MESSAGE}&relay=other"),
            format!("ambush://user:secret@message?channel={CHANNEL}&id={MESSAGE}"),
            format!("ambush://message?channel={CHANNEL}&id={MESSAGE}#fragment"),
        ] {
    ...
    fn rejects_duplicate_or_malformed_message_link_identifiers() {
        for link in [
            format!("ambush://message?channel={CHANNEL}&channel={CHANNEL}&id={MESSAGE}"),
            format!("ambush://message?channel=not-a-uuid&id={MESSAGE}"),
            format!("ambush://message?channel={CHANNEL}&id=not-an-event"),
            format!("ambush://message?channel={CHANNEL}&id={MESSAGE}&thread="),
        ] {
            assert!(parse_message_link(&link).is_err(), "accepted {link}");
        }
    }
