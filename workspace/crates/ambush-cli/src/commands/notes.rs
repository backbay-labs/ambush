//! `ambush notes` — NIP-23 long-form editable notes (kind:30023).
//!
...
pub async fn fetch_own_note(client: &AmbushClient, slug: &str) -> Result<Option<Event>, CliError> {
...
pub async fn fetch_by_slug(client: &AmbushClient, slug: &str) -> Result<Vec<Event>, CliError> {
    let filter = serde_json::json!({
        "kinds": [KIND_LONG_FORM],
        "#d": [slug],
        "limit": 50,
...
pub async fn cmd_rm(client: &AmbushClient, slug: &str) -> Result<(), CliError> {
...
pub async fn dispatch(cmd: crate::NotesCmd, client: &AmbushClient) -> Result<(), CliError> {
    use crate::NotesCmd;
    match cmd {
        NotesCmd::Set {
            name,
