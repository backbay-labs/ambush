...
use crate::client::{normalize_write_response, AmbushClient};
use crate::error::CliError;
use ambush_sdk::CustomEmoji;
...
const CUSTOM_EMOJI_SET_D_TAG: &str = ambush_sdk::CUSTOM_EMOJI_SET_D_TAG;
...
/// own kind:30030 set (d=`ambush:custom-emoji`).
async fn cmd_list(client: &AmbushClient) -> Result<(), CliError> {
    let filter = serde_json::json!({
        "kinds": [ambush_sdk::kind::KIND_EMOJI_SET],
...
async fn fetch_own_emoji(client: &AmbushClient) -> Result<Vec<CustomEmoji>, CliError> {
...
async fn publish_own_set(client: &AmbushClient, emojis: &[CustomEmoji]) -> Result<(), CliError> {
    let builder = ambush_sdk::build_custom_emoji_set(emojis)
...
async fn cmd_set(client: &AmbushClient, shortcode: &str, url: &str) -> Result<(), CliError> {
...
async fn cmd_rm(client: &AmbushClient, shortcode: &str) -> Result<(), CliError> {
    let normalized = ambush_sdk::normalize_custom_emoji_shortcode(shortcode)
        .map_err(|e| CliError::Other(format!("invalid shortcode: {e}")))?;
    let mut emojis = fetch_own_emoji(client).await?;
...
pub async fn dispatch(cmd: crate::EmojiCmd, client: &AmbushClient) -> Result<(), CliError> {
    use crate::EmojiCmd;
    match cmd {
