...
use client::AmbushClient;
use error::CliError;
use nostr::Keys;
use uuid::Uuid;

/// Run the Ambush CLI from raw arguments (including `argv[0]`).
///
...
///
/// ```ignore
/// let code = ambush_cli::run_from_args(std::env::args()).await;
    ...
    S: Into<std::ffi::OsString> + Clone,
{
    ...
    // features across the workspace and enables *both* ring (from ambush-acp/ambush-dev-mcp)
    ...
    // ambush-dev-mcp delegates to run_from_args, it has already installed ring; the
...
#[command(
    name = "ambush",
    about = "Ambush CLI — interact with a Ambush relay",
    long_about = "\
Ambush CLI — interact with a Ambush relay

Configuration (flags override env vars):
  AMBUSH_RELAY_URL     Relay base URL        [default: http://localhost:3000]
  AMBUSH_PRIVATE_KEY   Nostr private key (hex or nsec)  [required]
  AMBUSH_AUTH_TAG      NIP-OA auth tag JSON  [optional]
...
struct Cli {
    /// Relay URL (http:// or https://). Overrides AMBUSH_RELAY_URL env var.
    #[arg(
        long,
        env = "AMBUSH_RELAY_URL",
        default_value = "http://localhost:3000"
    )]
    relay: String,

    /// Nostr private key (hex or nsec). This is the CLI's identity.
    #[arg(long, env = "AMBUSH_PRIVATE_KEY", hide_env_values = true)]
    private_key: Option<String>,

    /// NIP-OA auth tag JSON (owner attestation). Injected into every signed event.
    #[arg(long, env = "AMBUSH_AUTH_TAG", hide_env_values = true)]
...
#[derive(Subcommand)]
pub enum AgentsCmd {
    /// Open a prefilled create-agent form in the owner's Ambush Desktop
    DraftCreate {
    ...
    /// Open a prefilled edit-agent form in the owner's Ambush Desktop
    DraftUpdate {
...
running under AMBUSH_AUTH_TAG signs as itself, so it can only ever satisfy \
the self path (target == signer) — not the owner-of-agent path for another \
identity.\n\n\
Examples:\n  \
ambush agents archive <PUBKEY> --reason retired\n  \
ambush agents archive <PUBKEY> --reason bot-rebuilt --replaced-by <NEW_PUBKEY>"
    )]
    Archive {
...
Examples:\n  \
ambush agents unarchive <PUBKEY> --reason returned"
    )]
    Unarchive {
        /// Target identity pubkey (hex)
        target_pubkey: String,
...
pub enum MessagesCmd {
    ...
    #[command(
        after_help = "Examples:\n  ambush messages send --channel <UUID> --content \"hello\"\n  ambush messages send --channel <UUID> --content \"@alice check this\"\n  echo \"hello from stdin\" | ambush messages send --channel <UUID> --content -"
    )]
    Send {
        /// Channel UUID (from 'ambush channels list')
        #[arg(long)]
        channel: String,
    ...
    #[command(
        after_help = "Examples:\n  ambush messages get --channel <UUID>\n  ambush messages get --channel <UUID> --limit 50 --kinds 1,1984"
    )]
    Get {
        /// Channel UUID
        #[arg(long)]
    ...
    /// Get the containing thread for a message or Ambush message link
    #[command(
        after_help = "Examples:\n  ambush messages thread --channel <UUID> --event <EVENT_ID>\n  ambush messages thread --link 'ambush://message?channel=<UUID>&id=<EVENT_ID>&thread=<ROOT_ID>'"
    )]
    Thread {
        ...
        #[arg(long, required_unless_present = "link", conflicts_with = "link")]
        event: Option<String>,
        /// Canonical ambush://message deep link; uses the configured relay and identity
    ...
    #[command(
        after_help = "Examples:\n  ambush messages search --query checkout\n  ambush messages search --author npub1... --since 1783497600\n  ambush messages search --author Aaron --query checkout --limit 20"
    )]
    Search {
...
pub enum ChannelsCmd {
    ...
    #[command(
        after_help = "Examples:\n  ambush channels list\n  ambush channels list --visibility open"
    )]
    List {
    ...
    #[command(
        after_help = "Examples:\n  ambush channels search --query composer\n  ambush channels search --query ambush-chat-composer --exact\n  ambush channels search --query design --include-archived"
    )]
    Search {
    ...
    #[command(
        after_help = "Examples:\n  ambush channels create --name general --type stream --visibility open\n  ambush channels create --name design --type forum --visibility open --description \"Design discussions\"\n  ambush channels create --name standup --type stream --visibility open --ttl 3600  # ephemeral, archived after 1h idle\n  ambush channels create --name project-x --template \"Ambush Team\"  # type/visibility/canvas/roster from the template; explicit flags override"
    )]
    Create {
        /// Channel name
        #[arg(long)]
    ...
    #[command(
        after_help = "Examples:\n  ambush channels update --channel <uuid> --name general\n  ambush channels update --channel <uuid> --visibility open\n  ambush channels update --channel <uuid> --visibility private"
    )]
    Update {
        /// Channel UUID
        #[arg(long)]
...
pub enum WorkflowsCmd {
    ...
    #[command(
        after_help = "Examples:\n  ambush workflows trigger --workflow <UUID>\n  ambush workflows trigger --workflow <UUID> --inputs '{\"key\":\"value\"}'"
    )]
    Trigger {
        /// Workflow UUID
        #[arg(long)]
...
pub enum NotesCmd {
    ...
    #[command(
        after_help = "Examples:\n  echo '# Hello' | ambush notes set --name hello --title 'Hello' --content -\n  ambush notes set --name hello --tag onboarding --content - < draft.md"
    )]
    Set {
...
pub enum ReposCmd {
    ...
    Create {
        ...
        relays: Vec<String>,
        /// Channel UUID to bind the repo to. The `ambush-channel` tag is the
        /// git ACL: without it the relay 404s every clone/fetch/push until
        /// the author runs `ambush repos bind` (issue #3527).
        #[arg(long)]
        channel: Option<String>,
    },
    ...
    ///
    /// The `ambush-channel` tag on the announcement is the git ACL: the relay
...
pub enum ProjectsCmd {
    ...
    Create {
        ...
        slug: String,
        /// Member repository coordinate: bare Ambush repo id (e.g. `ambush`) or full
        ...
        #[arg(long)]
        description: Option<String>,
        /// Associated Ambush channel UUID
        #[arg(long)]
        channel: Option<String>,
    ...
    /// Draft a project-linked channel for owner review in Ambush Desktop
    ...
    Update {
        ...
        /// Set the associated Ambush channel UUID
        #[arg(long, group = "mutation")]
        channel: Option<String>,
...
pub enum PatchesCmd {
    ...
    #[command(
        after_help = "Examples:\n  git format-patch -1 HEAD --stdout | ambush patches send --repo-owner <hex> --repo-id myrepo --patch-file - --root\n  ambush patches send --repo-owner <hex> --repo-id myrepo --patch-file 0001-fix.patch --reply-to <prev-patch-id>"
    )]
    Send {
...
pub enum PrCmd {
    ...
    #[command(
        after_help = "Examples:\n  ambush pr open --repo-owner <hex> --repo-id myrepo --subject 'Fix bug' --body-file - --commit $(git rev-parse HEAD) --clone https://relay/git/owner/myrepo --branch-name fix-bug\n  ambush pr update --repo-owner <hex> --repo-id myrepo --pr <event> --pr-author <hex> --commit $(git rev-parse HEAD) --clone https://relay/git/owner/myrepo"
    )]
    Open {
        /// Repo owner pubkey (64-char hex)
        #[arg(long)]
...
pub enum IssuesCmd {
    ...
    Create {
        ...
        /// `AMBUSH_GIT_ORIGIN_CHANNEL_ID`) names a project home.
        #[arg(long)]
        repo_owner: Option<String>,
        ...
        /// this project when none exists. Defaults to `AMBUSH_GIT_ORIGIN_CHANNEL_ID`.
        #[arg(long)]
        channel: Option<String>,
        /// Issue title
...
/// Subcommands for `ambush mem`.
#[derive(Subcommand)]
pub enum MemCmd {
    /// List non-tombstoned memory entries
    Ls {
        /// Owner pubkey (hex). Overrides AMBUSH_AUTH_TAG.
        #[arg(long)]
        owner: Option<String>,
    ...
    Patch {
        ...
        /// against. Hashes the exact UTF-8 bytes returned by `ambush mem get`,
        /// not normalized lines. Run `ambush mem hash <slug>` to capture this
        /// before editing.
        #[arg(long)]
...
/// `AMBUSH_RELAY_URL` — moderation commands are community-global and carry no
...
pub enum ModerationCmd {
    ...
    #[command(
        after_help = "Examples:\n  ambush moderation reports\n  ambush moderation reports --status open --limit 20"
    )]
    Reports {
    ...
    #[command(
        after_help = "Examples:\n  ambush moderation resolve --report <REPORT_EVENT_ID> --status dismissed --action dismiss\n  ambush moderation resolve --report <REPORT_EVENT_ID> --status resolved --action ban --reason \"rule 3\""
    )]
    Resolve {
    ...
    #[command(
        after_help = "Examples:\n  ambush moderation ban --pubkey <HEX>\n  ambush moderation ban --pubkey <HEX> --expires-in 604800 --reason \"repeated spam\""
    )]
    Ban {
    ...
    #[command(
        after_help = "Examples:\n  ambush moderation timeout --pubkey <HEX> --expires-in 3600\n  ambush moderation timeout --pubkey <HEX> --expires-at 1783500000 --reason \"cool off\""
    )]
    Timeout {
        /// Target member pubkey (hex)
        #[arg(long)]
...
/// Normalize hand-authored `AMBUSH_AUTH_TAG` input to strict JSON.
///
...
async fn run(cli: Cli) -> Result<(), CliError> {
    ...
    let private_key_str = cli.private_key.ok_or_else(|| {
        CliError::Auth("AMBUSH_PRIVATE_KEY is required (use --private-key or set env var)".into())
    })?;
    let keys = Keys::parse(&private_key_str)
        .map_err(|e| CliError::Key(format!("invalid AMBUSH_PRIVATE_KEY: {e}")))?;

    // NIP-OA: parse and verify the auth tag if provided.
    //
    // `AMBUSH_AUTH_TAG` is hand-authored configuration, so the unquoted raw
    ...
    let (auth_tag, auth_tag_json) = match cli.auth_tag {
        Some(ref input) if !input.is_empty() => {
            ...
            let tag = ambush_sdk::nip_oa::parse_auth_tag(&json)
                .map_err(|e| CliError::Auth(format!("AMBUSH_AUTH_TAG is malformed: {e}")))?;
            ambush_sdk::nip_oa::verify_auth_tag(&json, &keys.public_key()).map_err(|e| {
                CliError::Auth(format!(
                    "AMBUSH_AUTH_TAG verification failed for pubkey {}: {e}",
                    keys.public_key().to_hex()
                ))
            })?;
            // Canonical wire form derives from the parsed-and-verified tag
            // (same shape as ambush-acp's RestClient), never from raw input.
            let canonical = serde_json::to_string(tag.as_slice()).map_err(|e| {
                CliError::Auth(format!("AMBUSH_AUTH_TAG serialization failed: {e}"))
            })?;
    ...
    let client = AmbushClient::new(relay_url, keys, auth_tag, auth_tag_json)?;
...
mod tests {
    ...
    fn messages_thread_accepts_link_or_explicit_identifiers() {
        ...
        let link = format!("ambush://message?channel={channel}&id={event}");

        assert!(
            Cli::try_parse_from(["ambush", "messages", "thread", "--link", link.as_str(),]).is_ok()
        );
    ...
    fn messages_thread_rejects_partial_or_mixed_targets() {
        ...
        assert!(Cli::try_parse_from(["ambush", "messages", "thread"]).is_err());
        assert!(
            Cli::try_parse_from(["ambush", "messages", "thread", "--channel", channel]).is_err()
        );
        assert!(Cli::try_parse_from([
            "ambush",
            "messages",
    ...
    fn set_status_clear_rejects_text_and_emoji() {
        for extra in [["--text", "busy"], ["--emoji", "🎶"]] {
            let args = ["ambush", "users", "set-status", "--clear"]
                .into_iter()
                .chain(extra);
            assert!(
    ...
    fn set_status_requires_text_or_clear() {
        assert!(Cli::try_parse_from(["ambush", "users", "set-status"]).is_err());
        assert!(
            Cli::try_parse_from(["ambush", "users", "set-status", "--emoji", "🎶"]).is_err(),
            "--emoji alone must not imply a status"
        );
        assert!(Cli::try_parse_from(["ambush", "users", "set-status", "--clear"]).is_ok());
    }
    ...
    fn projects_update_no_mutation_is_rejected_by_clap() {
        ...
        assert!(
            Cli::try_parse_from(["ambush", "projects", "update", "my-slug"]).is_err(),
