...
mod tests {
    ...
    fn provider_config() -> ProviderConfig {
        config::parse(&serde_json::json!({
            "namespace": "ambush-agents-test",
            "image": format!("ghcr.io/backbay-labs/ambush-sprig@sha256:{}", "a".repeat(64)),
        }))
        .unwrap()
    }
    ...
    fn pod() -> Pod {
        ...
        build_pod(
            ...
            &cfg,
            "gen00001",
            &intent_template(&cfg, ["AMBUSH_RELAY_URL".to_string()]).fingerprint(),
        )
    }
    ...
    fn pod_carries_identity_marker_and_recorded_intent() {
        ...
        assert_eq!(meta.namespace.as_deref(), Some("ambush-agents-test"));
    ...
    fn secret_is_immutable_marked_and_holds_the_env() {
        ...
        let env: BTreeMap<String, String> =
            [("AMBUSH_RELAY_URL".to_string(), "wss://r".to_string())].into();
    ...
    fn recorded_fingerprint_matches_a_fresh_computation() {
        let cfg = provider_config();
        let keys = ["AMBUSH_RELAY_URL".to_string(), "GOOSE_MODE".to_string()];
    ...
    fn generation_does_not_change_the_recorded_fingerprint() {
        let cfg = provider_config();
        let keys = ["AMBUSH_RELAY_URL".to_string()];
