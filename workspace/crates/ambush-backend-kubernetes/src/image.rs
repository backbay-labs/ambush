...
//! fails closed with a named field. The published `ghcr.io/backbay-labs/ambush-sprig`
...
mod tests {
    ...
    fn accepts_digest_pinned_reference() {
        let d = "a".repeat(64);
        let r = parse(&format!("ghcr.io/backbay-labs/ambush-sprig@sha256:{d}")).unwrap();
        assert_eq!(
            r.as_str(),
            format!("ghcr.io/backbay-labs/ambush-sprig@sha256:{d}")
        );
    }
    ...
    fn strips_tag_from_tag_plus_digest_form() {
        ...
        let tagged = parse(&format!(
            "ghcr.io/backbay-labs/ambush-sprig:v1.2@sha256:{d}"
        ))
        .unwrap();
        let plain = parse(&format!("ghcr.io/backbay-labs/ambush-sprig@sha256:{d}")).unwrap();
    ...
    fn registry_port_is_not_mistaken_for_a_tag() {
        let d = "c".repeat(64);
        let r = parse(&format!("localhost:5000/ambush-sprig@sha256:{d}")).unwrap();
        assert_eq!(
            r.as_str(),
            format!("localhost:5000/ambush-sprig@sha256:{d}")
        );
    }
    ...
    fn port_and_tag_together_drops_only_the_tag() {
        let d = "d".repeat(64);
        let r = parse(&format!("localhost:5000/ambush-sprig:dev@sha256:{d}")).unwrap();
        assert_eq!(
            r.as_str(),
            format!("localhost:5000/ambush-sprig@sha256:{d}")
        );
    ...
    fn rejects_every_tag_only_reference() {
        for bad in [
            "ghcr.io/backbay-labs/ambush-sprig:latest",
            "ghcr.io/backbay-labs/ambush-sprig:v1.2.3",
            "ghcr.io/backbay-labs/ambush-sprig:sha-abc1234",
            "ghcr.io/backbay-labs/ambush-sprig",
            "localhost:5000/ambush-sprig",
        ] {
