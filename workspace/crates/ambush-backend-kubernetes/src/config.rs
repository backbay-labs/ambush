...
/// `AMBUSH_ACP_EXIT_AFTER_INACTIVITY` are one knob, not two.
...
pub const DEFAULT_IMAGE: &str = "ghcr.io/backbay-labs/ambush-sprig:sha-6530b58@sha256:17facfc7608d8ddb33bc056c9aaba1098f4ef6abe5655702fbfd7584d1f74d76";
...
/// A fresh `ambush-agents-<rand6>` namespace default.
///
    ...
    serde_json::json!({
        ...
        "properties": {
            ...
            "image": {
                "type": "string",
                "title": "Agent image",
                "description": "Digest-pinned image containing the ambush-acp runtime ABI, e.g. ghcr.io/backbay-labs/ambush-sprig@sha256:<digest>. Tags alone are not accepted: this pod holds the agent's private key.",
...
mod tests {
    ...
    fn digest_ref() -> String {
        format!(
            "ghcr.io/backbay-labs/ambush-sprig@sha256:{}",
            "a".repeat(64)
        )
    }

    fn minimal() -> serde_json::Value {
        serde_json::json!({"namespace": "ambush-agents-abc123", "image": digest_ref()})
    }
    ...
    fn image_is_required_and_must_be_digest_pinned() {
        ...
        cfg["image"] = "ghcr.io/backbay-labs/ambush-sprig:latest".into();
    ...
    fn generated_namespaces_are_fresh_and_valid() {
        ...
        assert!(a.starts_with("ambush-agents-"));
        assert_eq!(a.len(), "ambush-agents-".len() + 6);
    }
    ...
    fn schema_default_image_round_trips_through_parse() {
        ...
        let cfg = serde_json::json!({"namespace": "ambush-agents-abc123", "image": default});
        let parsed = parse(&cfg).unwrap();
        assert_eq!(
            parsed.image.as_str(),
            "ghcr.io/backbay-labs/ambush-sprig@sha256:17facfc7608d8ddb33bc056c9aaba1098f4ef6abe5655702fbfd7584d1f74d76"
        );
    }
