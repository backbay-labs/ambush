...
fn run(request: &str) -> (String, i32) {
    let mut child = Command::new(env!("CARGO_BIN_EXE_ambush-backend-kubernetes"))
...
fn info_response_carries_the_contract_fields() {
    ...
    assert!(
        default.starts_with("ambush-agents-"),
    ...
    assert!(
        image_default.starts_with("ghcr.io/backbay-labs/ambush-sprig:")
