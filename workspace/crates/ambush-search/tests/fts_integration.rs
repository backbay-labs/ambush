...
//!
//! Run with a local PG: `AMBUSH_TEST_DATABASE_URL=postgres://ambush:ambush_dev@localhost:5432/ambush cargo test -p ambush-search --tests -- --include-ignored`
//!
...
use ambush_search::{ChannelScope, SearchQuery, SearchService};
...
async fn setup() -> (PgPool, String) {
    let url = std::env::var("AMBUSH_TEST_DATABASE_URL").unwrap_or_else(|_| TEST_DB_URL.to_string());
...
async fn teardown(pool: PgPool, schema: &str) {
    ...
    let admin_pool = PgPoolOptions::new()
        .max_connections(1)
        .connect(
            &std::env::var("AMBUSH_TEST_DATABASE_URL").unwrap_or_else(|_| TEST_DB_URL.to_string()),
        )
        .await
...
async fn search_finds_event_in_same_community() {
    ...
    insert_event(
        ...
        pk,
        9,
        "hello wonderland — ambush everyone",
        None,
        1700000000,
    )
    .await;
...
async fn kind0_search_by_display_name_works_without_flattening() {
    ...
    insert_event(
        ...
        pk,
        0,
        r#"{"display_name":"Alice Wonderland","name":"alice","nip05":"alice@ambush.app","about":"hello"}"#,
        None,
        1700000000,
    )
    .await;
    ...
    let svc = SearchService::new(pool.clone());
    for q in ["wonderland", "alice", "alice@ambush.app"] {
        let r = svc
            .search(&SearchQuery {
                community: c,
                q: q.to_string(),
...
async fn short_kind0_prefix_prioritizes_exact_lexeme_on_a_noisy_page() {
    ...
    let first_page = svc
        .search(&SearchQuery {
            ...
            page: 1,
            per_page: 3,
            mode: ambush_search::SearchMode::Prefix,
        })
        .await
...
async fn p_gated_persistent_kinds_have_storage_null_tsvector() {
    ...
    let persistent: Vec<u32> = P_GATED_KINDS
        .iter()
        .copied()
        .filter(|&k| !ambush_core::kind::is_ephemeral(k))
        .collect();
    assert!(
        !persistent.is_empty(),
