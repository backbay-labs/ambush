...
use ambush_auth::{AuthService, Nip98ReplayGuard};
use ambush_core::tenant::TenantContext;
use ambush_core::CommunityId;
use ambush_db::Db;
use ambush_media::MediaStorage;
use ambush_pubsub::cache_invalidation::CacheInvalidation;
use ambush_pubsub::conn_control::ConnControl;
use ambush_pubsub::rate_limiter::RedisRateLimiter;
use ambush_pubsub::{PubSubManager, RedisNip98ReplayGuard};
...
pub(crate) async fn run_registered_community_connection<Check, CheckFuture, Run, RunFuture>(
    ...
    CheckFuture: Future<Output = Result<bool, ambush_db::DbError>>,
...
) -> (usize, Vec<(CommunityId, ambush_db::DbError)>)
where
...
impl ConnectionManager {
    ...
    /// (`AMBUSH_DRAIN_JITTER_MS` unset or `0`). It is synchronous and returns as
    ...
    ///
    /// This is the jittered drain, used only when `AMBUSH_DRAIN_JITTER_MS > 0`.
            ...
            match conn.tx.try_send(msg) {
                ...
                Err(tokio::sync::mpsc::error::TrySendError::Full(_)) => {
                    ...
                    if count >= conn.grace_limit {
                        ...
                        metrics::counter!("ambush_ws_backpressure_disconnects_total").increment(1);
                        conn.cancel.cancel();
                    } else {
...
pub struct AppState {
    ...
    pub audit_tx: Option<mpsc::Sender<ambush_audit::NewAuditEntry>>,
    ...
    /// See `crates/ambush-conformance/` and `crate::conformance` for the
    /// schema, emitter helpers, and the independent checker.
    pub tracer: Arc<dyn ambush_conformance::Tracer>,
...
impl AppState {
    ...
    pub fn new(
        ...
        let (audit_tx, mut audit_rx) = mpsc::channel::<ambush_audit::NewAuditEntry>(1000);
        ...
        let state = Self {
            ...
            // `crates/ambush-test-client` once those land).
    ...
    pub async fn is_member_cached(
        ...
        channel_id: Uuid,
        pubkey: &[u8],
    ) -> Result<bool, ambush_db::DbError> {
        let key = (community_id, channel_id, pubkey.to_vec());
        if let Some(cached) = self.membership_cache.get(&key) {
            metrics::counter!("ambush_membership_cache_hits_total").increment(1);
            return Ok(cached);
        }
        metrics::counter!("ambush_membership_cache_misses_total").increment(1);
    ...
    pub async fn disconnect_community_clusterwide(
        &self,
        tenant: &TenantContext,
    ) -> Result<usize, ambush_pubsub::PubSubError> {
        let closed = self
    ...
    pub async fn get_accessible_channel_ids_cached(
        ...
        community_id: CommunityId,
        pubkey: &[u8],
    ) -> Result<Vec<Uuid>, ambush_db::DbError> {
        let key = (community_id, pubkey.to_vec());
        if let Some(cached) = self.accessible_channels_cache.get(&key) {
            metrics::counter!("ambush_accessible_channels_cache_hits_total").increment(1);
            return Ok(cached);
        }
        metrics::counter!("ambush_accessible_channels_cache_misses_total").increment(1);
        let result = self
            .db
    ...
    pub async fn channel_visibility_cached(
        ...
        community_id: CommunityId,
        channel_id: Uuid,
        prefetched: Option<&ambush_db::channel::ChannelRecord>,
    ) -> Result<String, ambush_db::DbError> {
...
async fn log_audit_entry(audit: &ambush_audit::AuditService, entry: ambush_audit::NewAuditEntry) {
    let t = std::time::Instant::now();
    if let Err(e) = audit.log(entry).await {
        metrics::counter!("ambush_audit_log_errors_total").increment(1);
        tracing::error!("Audit log failed: {e}");
    } else {
        metrics::histogram!("ambush_audit_log_seconds").record(t.elapsed().as_secs_f64());
    }
}
...
mod tests {
    ...
    fn shared_counter_between_direct_and_fanout() {
        ...
        let conn = ConnectionState {
            conn_id,
            tenant: ambush_core::tenant::TenantContext::resolved(
                ambush_core::tenant::CommunityId::from_uuid(Uuid::nil()),
    ...
    async fn disconnect_pubkey_is_fenced_to_the_banning_community() {
        ...
        let community_a = ambush_core::tenant::CommunityId::from_uuid(Uuid::from_u128(0xa));
        let community_b = ambush_core::tenant::CommunityId::from_uuid(Uuid::from_u128(0xb));
    ...
    async fn drain_all_sends_restart_close_and_cancels_every_conn() {
        ...
        let (mut ctrl_a, cancel_a) = register(ambush_core::tenant::CommunityId::from_uuid(
            Uuid::from_u128(0xa),
        ));
        let (mut ctrl_b, cancel_b) = register(ambush_core::tenant::CommunityId::from_uuid(
            Uuid::from_u128(0xb),
        ));
