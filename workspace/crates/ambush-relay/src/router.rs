...
pub fn build_router(state: Arc<AppState>) -> Router {
    ...
    let api_router = Router::new()
        ...
        // Mesh demo echo probe — testbed-only; 404 unless AMBUSH_MESH=on and
        // AMBUSH_MESH_DEMO_ECHO=on (see api::mesh_demo).
...
fn build_cors_layer(cors_origins: &[String]) -> CorsLayer {
    ...
    if origins.is_empty() {
        tracing::error!(
            "AMBUSH_CORS_ORIGINS set but no valid origins could be parsed — \
...
mod tests {
    ...
    fn status_payload_exposes_source_and_build_identity() {
        ...
        assert_eq!(payload["service"], "ambush-relay");
