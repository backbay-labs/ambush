...
use ambush_core::kind::KIND_STREAM_MESSAGE;
use ambush_core::tenant::CommunityId;
use ambush_workflow::action_sink::{ActionSink, ActionSinkError};
use chrono::Utc;
...
impl ActionSink for RelayActionSink {
    fn send_message(
        ...
        Box::pin(async move {
            ...
            let tenant = ambush_core::tenant::TenantContext::resolved(community_id, host);
            ...
            let channel = state
                ...
                .await
                .map_err(|e| match &e {
                    ambush_db::DbError::ChannelNotFound(_) | ambush_db::DbError::NotFound(_) => {
            ...
            //    - `ambush:workflow` tag prevents recursive workflow triggering
            ...
            let mut tags = vec![
                ...
                Tag::parse(["ambush:workflow", "true"])
            ...
            // the `root` + `reply` pair — matching `ambush_sdk::builders::thread_tags`
            ...
            let thread_meta = Some(match &thread_meta_owned {
                ...
                None => ambush_db::event::ThreadMetadataParams {
...
mod integration_tests {
    ...
    //!   `cargo test -p ambush-relay --lib workflow_sink -- --ignored`
    use super::*;
    use ambush_core::channel::{ChannelType, ChannelVisibility, MemberRole};
    use ambush_db::CreateCommunityWithOwnerResult;
    use std::sync::Arc;
