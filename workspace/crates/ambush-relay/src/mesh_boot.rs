//! Relay startup wiring for the inter-relay mesh (`AMBUSH_MESH` seam).
//!
//! [`boot_mesh`] is the ONLY place the relay constructs mesh machinery. It
//! returns `None` — and touches nothing — when `AMBUSH_MESH=off`, so mesh-off
//! deployments stay byte-identical to a relay built before this module
//! existed. When enabled, it:
//!
//! 1. binds the iroh endpoint on `AMBUSH_MESH_BIND_ADDR` (boot-unique keypair =
...
use ambush_relay_mesh::endpoint::MeshEndpoint;
use ambush_relay_mesh::gossip::GossipRecord;
use ambush_relay_mesh::registry::{ReadyRecord, ReadyRegistry};
use ambush_relay_mesh::{
...
///   either the `AMBUSH_MESH_DEMO_ECHO` consumer (testbed evidence runs: echo
...
/// `AMBUSH_MESH_DEMO_ECHO` consumer: echo every validated `Data` frame back to
...
const PROTO_VERSION: u16 = ambush_relay_mesh::WIRE_VERSION as u16;
...
/// `AMBUSH_MESH_ADVERTISE_ADDR` (explicit, classic-LB shapes) →
...
fn advertise_addrs(endpoint: &MeshEndpoint) -> Vec<String> {
    if let Ok(addr) = std::env::var("AMBUSH_MESH_ADVERTISE_ADDR") {
...
/// Boot the mesh, or return `None` when `AMBUSH_MESH=off`.
///
...
/// sets `AMBUSH_MESH=on` wants the mesh or wants to know why not; silently
...
pub async fn boot_mesh(
    ...
    if !config.mesh.enabled {
        tracing::info!("mesh disabled (AMBUSH_MESH is not 'on') — single-instance behavior");
        return Ok(None);
    }
    ...
    ambush_relay_mesh::runtime::spawn_registry_heartbeat(
        registry.clone(),
        ready_record,
...
mod tests {
    ...
    /// AMBUSH_MESH=off must be a hard no-op: no endpoint bind, no Redis write,
    ...
    /// Blocker fix (Wren review of 8b077fdb): absent `AMBUSH_MESH`, the mesh is
    /// OFF — an env-untouched image upgrade must not bind or write Redis.
    #[test]
    fn mesh_defaults_off_when_env_absent() {
        // `Config::from_env` in the test env has no AMBUSH_MESH set unless a
        // caller exported it; assert the fail-safe reading.
        if std::env::var("AMBUSH_MESH").is_ok() {
        ...
        assert!(
            !config.mesh.enabled,
            "AMBUSH_MESH absent must mean mesh off"
        );
    }
    ...
    fn session_hello(sender: RuntimeId, profile: Profile) -> StreamHello {
        StreamHello {
            sender,
            role: ambush_relay_mesh::StreamRole::Session {
                fenced: FencedHeader {
    ...
    async fn wired_datagram_consumer_shares_the_handle_fence() {
        ...
        impl ambush_relay_mesh::RelayPeerTransport for NoopTransport {
