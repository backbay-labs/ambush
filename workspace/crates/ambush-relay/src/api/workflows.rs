...
async fn authorize_workflow_read(
    ...
    let workflow = state
        ...
        .map_err(|error| match error {
            ambush_db::error::DbError::NotFound(_) => {
...
fn run_json(run: &ambush_db::workflow::WorkflowRunRecord) -> Value {
    serde_json::json!({
        "id": run.id,
...
fn approval_json(approval: &ambush_db::workflow::ApprovalRecord) -> Value {
    serde_json::json!({
...
mod tests {
    ...
    fn approval_wire_does_not_expose_hash_as_token() {
        let approval = ambush_db::workflow::ApprovalRecord {
            token: vec![0xab; 32],
            ...
            status: ambush_db::workflow::ApprovalStatus::Pending,
