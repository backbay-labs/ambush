...
use ambush_auth::{LimitType, Nip98ReplayGuard, DEFAULT_REPLAY_TTL_SECS};
use ambush_core::TenantContext;
...
pub(crate) async fn enforce_http_admission(
    ...
    .await
    {
        ...
        Err(crate::admission::AdmissionError::Exceeded { reset_in_secs }) => {
            metrics::counter!("ambush_admission_rejections_total", "transport" => "http", "reason" => "quota").increment(1);
            Err(api_error(
        ...
        Err(crate::admission::AdmissionError::Unavailable) => {
            metrics::counter!("ambush_admission_rejections_total", "transport" => "http", "reason" => "unavailable").increment(1);
            Err(api_error(
...
pub(crate) fn verify_bridge_auth_with_options(
        ...
        .and_then(|s| s.strip_prefix("Nostr "))
    {
        ...
        let pubkey = ambush_auth::verify_nip98_event(&event_json, url, method, body)
...
fn extract_ambush_channel(raw: &Value) -> Option<&str> {
    raw.get("#ambush-channel")
...
fn extract_search_mode(raw: &Value) -> ambush_search::SearchMode {
    match raw
        .get("search_mode")
        .or_else(|| raw.get("searchMode"))
        .and_then(Value::as_str)
    {
        Some("prefix") => ambush_search::SearchMode::Prefix,
        _ => ambush_search::SearchMode::FullText,
    }
}
...
const WINDOW_AUX_KINDS: [u32; 4] = [
    ambush_core::kind::KIND_DELETION,
    ambush_core::kind::KIND_REACTION,
    ambush_core::kind::KIND_NIP29_DELETE_EVENT,
    ambush_core::kind::KIND_STREAM_MESSAGE_EDIT,
];
...
/// (`ambush_db::DEFAULT_MAX_PAGE_LIMIT`) so each page is one full query.
const AUX_PAGE_LIMIT: i64 = ambush_db::DEFAULT_MAX_PAGE_LIMIT;
...
fn build_aux_query(
    community: ambush_core::CommunityId,
    target_ids: Vec<String>,
    kinds: &[u32],
) -> ambush_db::EventQuery {
    let mut query = ambush_db::EventQuery::for_community(community);
...
enum AuxReader<'a> {
    Session(&'a mut ambush_db::ReadSession),
    Routed(&'a ambush_db::Db, &'static str),
    #[cfg(test)]
    Fake(&'a mut (dyn FnMut(&ambush_db::EventQuery) -> Vec<ambush_core::StoredEvent> + Send)),
}

impl AuxReader<'_> {
    async fn fetch(
        &mut self,
        query: &ambush_db::EventQuery,
    ) -> ambush_db::Result<Vec<ambush_core::StoredEvent>> {
        match self {
...
async fn query_all_pages(
    mut query: ambush_db::EventQuery,
    page_limit: i64,
...
async fn handle_channel_window_filter(
    state: &AppState,
    tenant: &ambush_core::TenantContext,
    raw: &Value,
    filter: &nostr::Filter,
    accessible_channels: &[uuid::Uuid],
    events: &mut Vec<Value>,
) -> Result<(), (StatusCode, Json<Value>)> {
    use ambush_core::kind::{KIND_THREAD_SUMMARY, KIND_WINDOW_BOUNDS};
...
fn event_in_accessible_channel(se: &ambush_core::StoredEvent, accessible: &[uuid::Uuid]) -> bool {
    match se.channel_id {
...
async fn submit_event_authed(
    ...
    let kind_u32 = ambush_core::kind::event_kind_u32(&event);
    let auth = IngestAuth::Http {
        pubkey,
        scopes: ambush_auth::Scope::all_known(), // Pure Nostr: full scopes, channel access via membership
...
async fn query_events_authed(
    ...
    for (idx, (raw, filter)) in raw_filters.iter().zip(filters.iter()).enumerate() {
        ...
        for feed_type in &feed_types {
            ...
            for se in type_events {
                ...
                if !ambush_core::filter::reader_authorized_for_event(&se.event, &authed_pubkey_hex)
                {
                    continue;
                }
            ...
            for hop_kinds in [&WINDOW_AUX_KINDS[..], &WINDOW_AUX_DELETE_KINDS[..]] {
                ...
                for se in aux_events {
                    if !seen_aux.insert(se.event.id)
                        ...
                        || !ambush_core::filter::reader_authorized_for_event(
                            &se.event,
                            &authed_pubkey_hex,
                        )
                    {
    ...
    let mut catchall_queries: Vec<(usize, ambush_db::EventQuery)> = Vec::new();
    for (idx, (raw, filter)) in raw_filters.iter().zip(filters.iter()).enumerate() {
        if handled.contains(&idx) {
            continue;
        }
        ...
        if let Some(channel) = extract_ambush_channel(raw) {
            query.custom_tag = Some(("ambush-channel".into(), channel.into()));
        }
        ...
        match filter_events {
            Ok(stored_events) => {
                for se in stored_events {
                    ...
                    if !ambush_core::filter::filters_match(std::slice::from_ref(filter), &se) {
                        continue;
                    }
                    ...
                    .await
                {
                    Ok(stored_events) => {
                        if crate::handlers::req::count_fallback_exceeded(stored_events.len()) {
                            metrics::counter!("ambush_count_fallback_rejections_total")
                                .increment(1);
                            return Err(api_error(
                                StatusCode::BAD_REQUEST,
                        ...
                        for se in stored_events {
                            if !ambush_core::filter::filters_match(
                                std::slice::from_ref(filter),
                                &se,
                            ) {
                                continue;
...
fn search_hit_accepted(
    filter: &nostr::Filter,
    stored: &ambush_core::StoredEvent,
    accessible_channels: &[uuid::Uuid],
    reader_pubkey_hex: &str,
) -> bool {
    if !ambush_core::filter::filters_match(std::slice::from_ref(filter), stored) {
        return false;
    }
    ...
    if !ambush_core::filter::reader_authorized_for_event(&stored.event, reader_pubkey_hex) {
        return false;
    }
    true
}
...
async fn handle_bridge_search(
    ...
    filters: &[nostr::Filter],
    accessible_channels: &[uuid::Uuid],
    tenant: &ambush_core::tenant::TenantContext,
    reader_pubkey_hex: &str,
    pubkey_bytes: &[u8],
) -> Result<Json<Value>, (StatusCode, Json<Value>)> {
    ...
    for (raw, filter) in raw_filters.iter().zip(filters) {
        ...
        let filter_channel_scope =
            if let Some(vs) = filter.generic_tags.get(&h_tag).filter(|vs| !vs.is_empty()) {
                ...
                ambush_search::ChannelScope::Channels(valid)
            } else {
                channel_scope.clone()
            };
        ...
        let search_query = ambush_search::SearchQuery {
            community: tenant.community(),
            q: search_text,
        ...
        let event_map: std::collections::HashMap<[u8; 32], &ambush_core::StoredEvent> =
            stored_events
                .iter()
...
pub async fn workflow_webhook(
    ...
    let def: ambush_workflow::WorkflowDef = serde_json::from_value(workflow.definition.clone())
        .map_err(|e| super::internal_error(&format!("corrupt workflow definition: {e}")))?;

    if !matches!(def.trigger, ambush_workflow::TriggerDef::Webhook) {
        return Err(api_error(
            StatusCode::BAD_REQUEST,
    ...
    let mut trigger_ctx = ambush_workflow::executor::TriggerContext {
        channel_id: workflow
            .channel_id
            .map(|ch| ch.to_string())
            .unwrap_or_default(),
    ...
    if !workflow.enabled || workflow.status != ambush_db::workflow::WorkflowStatus::Active {
    ...
    tokio::spawn(async move {
        let def: ambush_workflow::WorkflowDef = match serde_json::from_value(def_value) {
            Ok(d) => d,
            Err(e) => {
                ...
                if let Err(db_err) = db
                    .update_workflow_run(
                        community_id,
                        run_id,
                        ambush_db::workflow::RunStatus::Failed,
                        0,
                        &serde_json::json!([]),
                        Some(ambush_db::workflow::WorkflowRunFailure {
        ...
        let result = ambush_workflow::executor::execute_from_step(
            &engine,
            community_id,
            run_id,
            &def,
...
async fn synthesize_presence(
    pubsub: &ambush_pubsub::PubSubManager,
    relay_keypair: &nostr::Keys,
    tenant: &ambush_core::tenant::TenantContext,
    filters: &[nostr::Filter],
) -> Option<Result<Vec<Value>, (StatusCode, Json<Value>)>> {
    use ambush_core::kind::{KIND_PRESENCE_SNAPSHOT, KIND_PRESENCE_UPDATE};
...
fn report_json(r: &ambush_db::moderation::ReportRecord) -> Value {
    let (target_kind, target) = match &r.target {
        ambush_db::moderation::ReportTarget::Event(id) => ("event", hex::encode(id)),
        ambush_db::moderation::ReportTarget::Pubkey(pk) => ("pubkey", hex::encode(pk)),
        ambush_db::moderation::ReportTarget::Blob(sha) => ("blob", hex::encode(sha)),
    };
    serde_json::json!({
        "id": r.id,
...
fn action_json(a: &ambush_db::moderation::ActionRecord) -> Value {
    serde_json::json!({
        "id": a.id,
...
fn ban_json(b: &ambush_db::moderation::BanRecord) -> Value {
    serde_json::json!({
...
mod tests {
    ...
    fn fresh_tenant(host: &str) -> TenantContext {
        TenantContext::resolved(
            ambush_core::CommunityId::from_uuid(uuid::Uuid::new_v4()),
            host,
        )
    }
    ...
    async fn synthesize_presence_surfaces_redis_failure_as_error_response() {
        use ambush_core::kind::KIND_PRESENCE_SNAPSHOT;
        ...
        let pubsub = ambush_pubsub::PubSubManager::new("redis://127.0.0.1:1", dead_pool)
            .await
    ...
    fn aux_event(keys: &Keys, created_at: u64, content: &str) -> ambush_core::StoredEvent {
        let ev = EventBuilder::new(Kind::Custom(7), content)
            .custom_created_at(nostr::Timestamp::from(created_at))
            .sign_with_keys(keys)
            .unwrap();
        ambush_core::StoredEvent::new(ev, None)
    }
    ...
    async fn query_all_pages_drains_past_the_page_clamp() {
        ...
        let mut fetch = |q: &ambush_db::EventQuery| {
    ...
    async fn query_all_pages_stops_at_one_short_page() {
        ...
        let query = build_aux_query(tenant.community(), vec!["root".into()], &WINDOW_AUX_KINDS);
        let mut calls = 0;
        let mut fetch = |_q: &ambush_db::EventQuery| {
            calls += 1;
            Vec::new()
        };
    ...
    fn bridge_search_mode_extension_defaults_to_full_text() {
        assert_eq!(
            ...
            ambush_search::SearchMode::FullText
        );
        assert_eq!(
    ...
    fn bridge_search_mode_extension_accepts_prefix_snake_or_camel_case() {
        assert_eq!(
            ...
            ambush_search::SearchMode::Prefix
        );
        assert_eq!(
    ...
    async fn nip98_replay_guard_rejects_cross_pod_replay_on_bridge_path() {
        let pool = redis_pool();
        let pod_a = ambush_pubsub::RedisNip98ReplayGuard::new(pool.clone());
        let pod_b = ambush_pubsub::RedisNip98ReplayGuard::new(pool);
    ...
    async fn nip98_replay_guard_rejects_same_pod_same_community_replay() {
        let pool = redis_pool();
        let pod = ambush_pubsub::RedisNip98ReplayGuard::new(pool);
    ...
    /// `ambush-auth/src/nip98_replay.rs:70-73`).
    ///
    ...
    /// `ambush_auth::nip98::tests::make_nip98_event` so the bridge tests don't
    /// reach into ambush-auth's test scope.
    ...
    /// `ambush moderation reports` / `audit` 401'd on a NIP-98 URL mismatch in
    ...
    fn verify_nip42_rejects_event_signed_for_wrong_communitys_host() {
        ...
        assert!(
            matches!(err, ambush_auth::AuthError::RelayUrlMismatch),
    ...
    fn engram_envelope(agent: &Keys, owner_hex: &str) -> ambush_core::StoredEvent {
    ...
    #[test]
    fn extract_ambush_channel_requires_one_string_value() {
        assert_eq!(
            extract_ambush_channel(&serde_json::json!({"#ambush-channel": ["channel-a"]})),
            Some("channel-a")
        );
        assert_eq!(
            extract_ambush_channel(
                &serde_json::json!({"#ambush-channel": ["channel-a", "channel-b"]})
            ),
            None
        );
        assert_eq!(
            extract_ambush_channel(&serde_json::json!({"#ambush-channel": [42]})),
            None
        );
    }
    ...
    fn extract_page_offset_sizes_pages_from_clamped_limit() {
        let clamped = ambush_db::DEFAULT_MAX_PAGE_LIMIT;
    ...
    fn event_accessible_no_channel() {
        ...
        let ev = EventBuilder::new(Kind::Custom(1), "test")
            .sign_with_keys(&keys)
            .unwrap();
        let se = ambush_core::StoredEvent::new(ev, None);
    ...
    fn event_accessible_matching_channel() {
        ...
        let mut se = ambush_core::StoredEvent::new(ev, None);
    ...
    fn search_hit_rejects_dm_visibility_for_kindless_ids_third_party() {
        ...
        let ev = EventBuilder::new(
            Kind::Custom(ambush_core::kind::KIND_DM_VISIBILITY as u16),
            "",
        )
        .tags([d_tag, p_tag])
        .sign_with_keys(&relay)
        .expect("sign snapshot");
        let stored = ambush_core::StoredEvent::new(ev.clone(), None);
    ...
    // increments ambush_events_rejected_total{transport="http"}.  They are
    ...
    const TEST_DB_URL: &str = "postgres://ambush:ambush_dev@localhost:5432/ambush"; // sadscan:disable np.postgres.1
    ...
    async fn bridge_handler_test_state() -> Option<Arc<crate::state::AppState>> {
        ...
        let media_storage = ambush_media::MediaStorage::new(&config.media).ok()?;
    ...
    /// Collect ambush_events_rejected_total with (transport, reason) labels from
    ...
    fn http_reject_counts(
        ...
        snapshotter
            ...
            .into_vec()
            .into_iter()
            .filter(|(key, ..)| key.key().name() == "ambush_events_rejected_total")
            .map(|(key, _, _, value)| {
                let metrics_util::debugging::DebugValue::Counter(n) = value else {
                    panic!("ambush_events_rejected_total must be a counter");
                };
    ...
    /// increment ambush_events_rejected_total{transport="http",reason="invalid"}.
    ///
    ...
    fn submit_event_relay_only_kind_increments_http_transport_counter() {
        ...
        let relay_only_event = EventBuilder::new(
            Kind::Custom(ambush_core::kind::KIND_NIP43_MEMBERSHIP_LIST as u16),
            "",
        )
