...
//!
//! The `ambush-channel` tag on a kind:30617 announcement IS the git ACL: the
...
//!
//! Only the *first* `ambush-channel` tag is considered, and it must carry a
...
//! who can append a second `ambush-channel` tag would pick the channel.
//!
...
pub enum RepoBinding {
    /// No `ambush-channel` tag at all. The announcement author (the only
    /// identity that can rebind — 30617 is keyed by `(author, d)`) may be
    /// offered remediation; everyone else gets the generic denial.
    NotBound,
    /// First `ambush-channel` tag carries a valid UUID.
    Bound(Uuid),
    /// First `ambush-channel` tag exists but its value is not a UUID.
...
pub fn resolve_repo_binding(event: &nostr::Event) -> RepoBinding {
    let Some(first) = event
        .tags
        .iter()
        .find(|t| t.as_slice().first().map(String::as_str) == Some("ambush-channel"))
    else {
...
mod tests {
    ...
    fn extracts_valid_uuid() {
        ...
        let event = announcement(vec![
            ...
            Tag::parse(["ambush-channel", &ch.to_string()]).unwrap(),
        ]);
    ...
    fn malformed_and_empty_values_are_broken_not_absent() {
        ...
        let empty = announcement(vec![
            Tag::parse(["d", "repo"]).unwrap(),
            Tag::parse(["ambush-channel"]).unwrap(),
        ]);
    ...
    fn fails_closed_on_ambiguous_duplicate_bindings() {
        ...
        let valid_first = announcement(vec![
            Tag::parse(["d", "repo"]).unwrap(),
            Tag::parse(["ambush-channel", &ch.to_string()]).unwrap(),
            Tag::parse(["ambush-channel", &other.to_string()]).unwrap(),
        ]);
