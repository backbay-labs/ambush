...
/// - `AMBUSH_HOOK_URL` — internal policy endpoint (http://127.0.0.1:{port}/internal/git/policy)
/// - `AMBUSH_HOOK_SECRET` — per-push HMAC secret
/// - `AMBUSH_REPO_ID` — repo identifier (d-tag)
/// - `AMBUSH_COMMUNITY_ID` — server-resolved community UUID for the git HTTP request
/// - `AMBUSH_PUSHER_PUBKEY` — authenticated pusher's hex pubkey
///
...
# Ambush pre-receive hook — FAIL-CLOSED
...
: "${AMBUSH_REPO_ID:?error: AMBUSH_REPO_ID not set}"
: "${AMBUSH_REPO_OWNER:?error: AMBUSH_REPO_OWNER not set}"
: "${AMBUSH_COMMUNITY_ID:?error: AMBUSH_COMMUNITY_ID not set}"
: "${AMBUSH_PUSHER_PUBKEY:?error: AMBUSH_PUSHER_PUBKEY not set}"
: "${AMBUSH_HOOK_URL:?error: AMBUSH_HOOK_URL not set}"
: "${AMBUSH_HOOK_SECRET:?error: AMBUSH_HOOK_SECRET not set}"
...
SIGNATURE=$(printf '%s' "$HMAC_INPUT" | openssl dgst -sha256 -hmac "$AMBUSH_HOOK_SECRET" -hex 2>/dev/null | sed 's/.*= //')
if [ -z "$SIGNATURE" ]; then
...
SAFE_REPO_ID=$(printf '%s' "$AMBUSH_REPO_ID" | sed 's/\\/\\\\/g; s/"/\\"/g')
BODY="{\"repo_id\":\"${SAFE_REPO_ID}\",\"repo_owner\":\"${AMBUSH_REPO_OWNER}\",\"community_id\":\"${AMBUSH_COMMUNITY_ID}\",\"pusher_pubkey\":\"${AMBUSH_PUSHER_PUBKEY}\",\"ref_updates\":[${REFS}],\"timestamp\":${TIMESTAMP},\"signature\":\"${SIGNATURE}\"}"
