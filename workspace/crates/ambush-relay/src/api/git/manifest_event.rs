...
//!   ]
//!
//! Ambush extension: a `p` tag carrying the pusher's pubkey (or the repo
//! owner on creation events) so subscribers can filter by author of state
//! transition. Not part of NIP-34 but consistent with the rest of ambush's
...
pub struct RefStateInputs<'a> {
    ...
    /// Pubkey to include in the `p` tag (ambush extension). On push, this is
...
pub fn build_ref_state_event(
    ...
    // p-tag: ambush extension (pusher or owner pubkey).
