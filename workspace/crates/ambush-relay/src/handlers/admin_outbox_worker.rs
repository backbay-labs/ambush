...
use ambush_db::relay_admin_actions::OutboxRecord;
...
async fn resolve_tenant(
    ...
    task_type: &str,
) -> Result<ambush_core::tenant::TenantContext, String> {
    let host = state
        .db
    ...
    Ok(ambush_core::tenant::TenantContext::resolved(
        community_id,
        host,
    ))
}
...
async fn deliver_tombstone(state: &Arc<AppState>, row: &OutboxRecord) -> Result<(), String> {
    ...
    let community_id = ambush_core::CommunityId::from_uuid(community_uuid);
