...
//!    participant-hash-idempotent DM model (`ambush-db/src/dm.rs`).
...
use ambush_core::kind::{event_kind_u32, KIND_STREAM_MESSAGE};
...
mod tests {
    ...
    fn tenant() -> TenantContext {
        TenantContext::resolved(
            ambush_core::CommunityId::from_uuid(Uuid::new_v4()),
            "example.org",
        )
    }
