...
use ambush_core::tenant::{normalize_host, TenantContext};
use url::{Host, Url};
...
/// present, the owner is (re)bootstrapped via [`ambush_db::Db::bootstrap_owner`]
...
pub async fn provision_community(
    ...
    if request.create_only {
        ...
        {
            ambush_db::CreateCommunityWithOwnerResult::Created(record) => record,
            ambush_db::CreateCommunityWithOwnerResult::HostExists => {
                return Err("community already exists".to_string());
            }
            ambush_db::CreateCommunityWithOwnerResult::LimitReached => {
                return Err(
...
mod tests {
    ...
    fn host_valid_bare_domain() {
        assert!(validate_host("acme.communities.ambush.example.com").is_ok());
    }
