...
use ambush_db::admin_moderation::AdminReportDetail;
use ambush_db::relay_admin_actions::{AdminActionRecord, ClaimResult};
...
impl From<ambush_db::DbError> for ResolutionError {
    fn from(e: ambush_db::DbError) -> Self {
    ...
    loop {
        ...
        if rec.step_marker.is_none() {
            ...
            let lease_token = if let Some(token) = held_lease {
                ...
                match lease {
                    ambush_db::relay_admin_actions::LeaseResult::Acquired(token) => token,
                    ambush_db::relay_admin_actions::LeaseResult::Contended => {
                    ...
                    ambush_db::relay_admin_actions::LeaseResult::NotLeasable => {
...
async fn run_atomic_mutation(
    ...
    let raw: anyhow::Result<bool> = match ctx.action {
        ...
        "kick" => {
            ...
            {
                ambush_db::relay_admin_actions::KickWithMarkerResult::Removed => Ok(true),
                ambush_db::relay_admin_actions::KickWithMarkerResult::AlreadyMarked => Ok(false),
                ambush_db::relay_admin_actions::KickWithMarkerResult::AlreadyGone => Err(
...
mod tests {
    use super::*;
    use ambush_db::admin_moderation::{AdminReport, AdminReportedMessage};
