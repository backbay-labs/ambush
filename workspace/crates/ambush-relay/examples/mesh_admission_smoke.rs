...
//!
//! Proves the security claim of Ambush's membership-derived mesh admission:
...
//!      same builder calls Ambush desktop makes with a resolved member roster.
...
//! corrupt each other — exactly how Ambush runs it in production anyway (one
//! desktop = one node).
//!
//! Hardware-gated, not CI — loads a real model. Run with:
//!   cargo run -p ambush-relay --example mesh_admission_smoke
use std::io::BufRead;
...
fn orchestrate() -> anyhow::Result<()> {
    ...
    let scratch =
        std::env::temp_dir().join(format!("ambush-mesh-admission-{}", std::process::id()));
