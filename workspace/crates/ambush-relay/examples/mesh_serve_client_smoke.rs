...
//!
//! Proves the full Ambush shared compute serve→consume path on a single box,
...
//!      (the `client::start` path a Ambush shared compute agent uses), binding
...
//!   cargo run -p ambush-relay --example mesh_serve_client_smoke
//!
