//! Relay-driven mesh lifecycle smoke — the full Ambush join story, CI-shaped.
//!
...
//!      ambush-relay (kind:13534 roster via ambush-admin); a third is not.
...
//! cargo build --profile ci -p ambush-admin
//! AMBUSH_ADMIN_BIN=target/ci/ambush-admin \
//!   cargo run --profile ci -p ambush-relay --example mesh_relay_lifecycle_smoke
//! ```
...
/// (`KIND_AMBUSH_MESH_MEMBER_STATUS` in the desktop coordinator).
const KIND_MESH_STATUS: u16 = 30_003;
/// NIP-43 membership roster snapshot.
const KIND_MEMBERSHIP: u16 = 13_534;
const STATUS_D_TAG_PREFIX: &str = "ambush-mesh-member-status";
...
fn member_endpoint_binding_bytes(member_pubkey: &str, endpoint_tokens: &[String]) -> Vec<u8> {
    ...
    format!(
        "ambush-mesh-owner-endpoint-binding-v1:{}:{}",
        member_pubkey.trim().to_ascii_lowercase(),
...
async fn role_serve() -> anyhow::Result<()> {
    ...
    let keys = Keys::parse(&env("AMBUSH_MEMBER_NSEC")?)?;
    ...
    let mut relay = AmbushTestClient::connect(&relay_ws_url(), &keys)
        .await
    ...
    let cfg = serve::EmbeddedServeConfig::builder()
        ...
        // auto-discover. The Ambush relay is the only discovery surface.
        .publish(false)
        .auto_join(false)
...
async fn role_stranger() -> anyhow::Result<()> {
    ...
    match AmbushTestClient::connect(&relay_ws_url(), &keys).await {
        Err(error) => {
...
fn orchestrate() -> anyhow::Result<()> {
    ...
    let admin =
        std::env::var("AMBUSH_ADMIN_BIN").unwrap_or_else(|_| "target/ci/ambush-admin".to_string());
    anyhow::ensure!(
        std::path::Path::new(&admin).exists(),
        "ambush-admin binary not found at {admin} (set AMBUSH_ADMIN_BIN)"
    );

    let scratch =
        std::env::temp_dir().join(format!("ambush-mesh-lifecycle-{}", std::process::id()));
    ...
    // MEMBERSHIP: A and B become relay members via ambush-admin (publishes the
    ...
    for (label, keys) in [("A", &member_a), ("B", &member_b)] {
        let status = Command::new(&admin)
            ...
            .status()?;
        anyhow::ensure!(status.success(), "ambush-admin add-member {label} failed");
        eprintln!(
    ...
    let mut serve_child = Command::new(&exe)
        ...
        .env("AMBUSH_MEMBER_NSEC", secret_hex(&member_a))
    ...
    let mut client_child = Command::new(&exe)
        ...
        .env("AMBUSH_MEMBER_NSEC", secret_hex(&member_b))
    ...
    let mut stranger_child = Command::new(&exe)
        ...
        .env("AMBUSH_MEMBER_NSEC", secret_hex(&stranger))
