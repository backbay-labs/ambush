...
//!   P4 agentic tool use     — agent + ambush-dev-mcp writes a file on disk.
//!
...
//! `ambush-agent` binary with the exact env vars the relay-mesh preset ships.
//!
//! Hardware-gated, not CI. Run:
//!   cargo build --release -p ambush-agent -p ambush-dev-mcp
//!   cargo run -p ambush-relay --example mesh_agent_e2e
...
async fn run() -> anyhow::Result<()> {
    ...
    // mode that broke relay-mesh agents when ambush-agent's default 32768
    // budget met a 32k-context model). 150k output: passes ambush-agent's own
    ...
    // P4: agentic tool use via ambush-dev-mcp — write a real file inside the
    ...
    let prompt = format!(
        "Use your developer tools to create {marker_name} in the current working directory containing exactly the text AMBUSH_OK (no quotes, no newline commentary). Then confirm."
    );
    let mcp = vec![("dev".to_string(), repo_bin("ambush-dev-mcp")?)];
    let (r, marker) =
        agent_chat_with_marker(&base, "mesh", None, &prompt, &mcp, &marker_name).await;
    let file_ok = std::fs::read_to_string(&marker)
        .map(|c| c.contains("AMBUSH_OK"))
        .unwrap_or(false);
    match r {
        Ok(text) => record(
            "P4 agentic tool use",
...
/// Spawn the real ambush-agent with relay-mesh preset env and drive one ACP
...
async fn agent_chat_in_isolated_home(
    ...
    let agent = match repo_bin("ambush-agent") {
        Ok(agent) => agent,
    ...
    command
        ...
        // model, key, and chat API. Not AMBUSH_AGENT_REQUIRE_REPLY, which needs
        // Ambush's publish tools to mean anything.
        .env("AMBUSH_AGENT_PROVIDER", "openai")
        .env("AMBUSH_AGENT_MODEL", model)
        .env("OPENAI_COMPAT_BASE_URL", base)
        .env("OPENAI_COMPAT_MODEL", model)
        .env("OPENAI_COMPAT_API_KEY", "ambush-mesh-local")
        .env("OPENAI_COMPAT_API", "chat")
        .env("AMBUSH_AGENT_MAX_OUTPUT_TOKENS", "4096")
        // No AMBUSH_AGENT_THINKING_EFFORT: apply_relay_mesh_env() deliberately
    ...
    if let Some(value) = max_output_tokens {
        command.env("AMBUSH_AGENT_MAX_OUTPUT_TOKENS", value);
    }
