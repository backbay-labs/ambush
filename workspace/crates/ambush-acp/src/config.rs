//! Configuration for the ambush-acp harness.
//!
...
/// while running long sub-tools (e.g. an ambush-agent running another agent, or
...
/// Override via `--idle-timeout` / `AMBUSH_ACP_IDLE_TIMEOUT`.
pub(crate) const DEFAULT_IDLE_TIMEOUT_SECS: u64 = 900;

/// Default absolute wall-clock cap per agent turn (2 hours).
/// Override via `--max-turn-duration` / `AMBUSH_ACP_MAX_TURN_DURATION`.
...
/// CLI args for `ambush-acp models` — query available models from an agent.
///
...
pub struct AuthAgentArgs {
    /// Agent binary to spawn (e.g. "goose", "claude-agent-acp", "codex-acp").
    #[arg(long, env = "AMBUSH_ACP_AGENT_COMMAND", default_value = "goose")]
    pub agent_command: String,

    /// Arguments passed to the agent binary.
    #[arg(
        long,
        env = "AMBUSH_ACP_AGENT_ARGS",
        default_value = "acp",
        value_delimiter = ','
    )]
...
/// CLI args for `ambush-acp auth-methods` — query adapter-advertised login methods.
#[derive(Debug, Parser)]
#[command(
    name = "ambush-acp auth-methods",
...
/// CLI args for `ambush-acp authenticate` — start an adapter-owned login flow.
#[derive(Debug, Parser)]
#[command(
    name = "ambush-acp authenticate",
...
#[command(
    name = "ambush-acp",
    about = "ACP harness that bridges Ambush events to AI agents"
)]
pub struct CliArgs {
    #[arg(long, env = "AMBUSH_RELAY_URL", default_value = "ws://localhost:3000")]
    ...
    #[arg(long, env = "AMBUSH_ACP_AGENT_OWNER")]
    ...
    #[arg(long, env = "AMBUSH_ACP_MCP_COMMAND", default_value = "")]
    pub mcp_command: String,

    /// Idle timeout: max seconds of silence before killing a turn.
    /// Resets on any agent stdout activity.
    #[arg(long, env = "AMBUSH_ACP_IDLE_TIMEOUT")]
    pub idle_timeout: Option<u64>,

    /// Absolute wall-clock cap per turn (safety valve).
    #[arg(long, env = "AMBUSH_ACP_MAX_TURN_DURATION", default_value_t = DEFAULT_MAX_TURN_DURATION_SECS)]
    pub max_turn_duration: u64,

    /// Deprecated: alias for --idle-timeout. If both set, --idle-timeout wins.
    #[arg(long, env = "AMBUSH_ACP_TURN_TIMEOUT", hide = true)]
    pub turn_timeout: Option<u64>,

    #[arg(
        long,
        env = "AMBUSH_ACP_SYSTEM_PROMPT",
    ...
    #[arg(
        long,
        env = "AMBUSH_ACP_HEARTBEAT_PROMPT_FILE",
        conflicts_with = "heartbeat_prompt"
    )]
    pub heartbeat_prompt_file: Option<PathBuf>,

    #[arg(long, env = "AMBUSH_ACP_INITIAL_MESSAGE")]
    pub initial_message: Option<String>,

    #[arg(
        long,
        env = "AMBUSH_ACP_SUBSCRIBE",
        default_value = "mentions",
        value_enum
    )]
    pub subscribe: SubscribeMode,

    #[arg(long, env = "AMBUSH_ACP_KINDS", value_delimiter = ',')]
    pub kinds: Option<Vec<u32>>,

    #[arg(long, env = "AMBUSH_ACP_CHANNELS", value_delimiter = ',')]
    pub channels: Option<Vec<String>>,

    #[arg(long, env = "AMBUSH_ACP_NO_MENTION_FILTER")]
    pub no_mention_filter: bool,

    #[arg(long, env = "AMBUSH_ACP_CONFIG", default_value = "./ambush-acp.toml")]
    pub config: PathBuf,

    #[arg(long, env = "AMBUSH_ACP_DEDUP", default_value = "queue", value_enum)]
    pub dedup: DedupMode,
    ...
    #[arg(
        long,
        env = "AMBUSH_ACP_MULTIPLE_EVENT_HANDLING",
        default_value = "steer",
        value_enum
    )]
    pub multiple_event_handling: MultipleEventHandling,

    #[arg(long, env = "AMBUSH_ACP_NO_IGNORE_SELF")]
    ...
    /// when the relay confirms no core engram exists). The `ambush mem` CLI
    ...
    /// Pass `--no-memory` / `AMBUSH_ACP_NO_MEMORY=true` to disable.
    #[arg(
        long,
        env = "AMBUSH_ACP_MEMORY",
    ...
    #[arg(long, env = "AMBUSH_ACP_NO_MEMORY", conflicts_with = "memory")]
    pub no_memory: bool,

    /// Disable the `<base>` platform-context section prepended to every prompt.
    /// When set, agents receive only the persona `<system>` prompt with no Ambush orientation.
    #[arg(long, env = "AMBUSH_ACP_NO_BASE_PROMPT")]
    ...
    #[arg(
        long,
        env = "AMBUSH_ACP_BASE_PROMPT_FILE",
    ...
    /// Use `ambush-acp models` to discover available model IDs.
    #[arg(long, env = "AMBUSH_ACP_MODEL")]
    ...
    #[arg(long, env = "AMBUSH_ACP_EFFORT_LEVEL")]
    ...
    #[arg(long, env = "AMBUSH_ACP_SESSION_TITLE")]
    ...
    #[arg(
        long,
        env = "AMBUSH_ACP_PERMISSION_MODE",
    ...
    #[arg(long, env = "AMBUSH_ACP_RESPOND_TO_ALLOWLIST", value_delimiter = ',')]
    ...
    #[arg(long, env = "AMBUSH_ACP_IDLE_POOL_SLEEP", default_value_t = 0)]
...
pub struct Config {
    ...
    /// `--no-memory` / `AMBUSH_ACP_NO_MEMORY` opt-out.
    ...
    /// Per-persona env vars to inject at agent spawn time (e.g., GOOSE_PROVIDER, GOOSE_MODEL, AMBUSH_AGENT_MODEL).
...
///
/// Ambush is the only guard here: Codex's own `normalize_thread_name` merely
...
fn default_agent_args(command: &str) -> Option<Vec<String>> {
    match normalize_agent_command_identity(command).as_str() {
        ...
        | "claudecode" | "ambush-agent" => Some(Vec::new()),
        _ => None,
    }
}

/// Per-runtime environment defaults applied when Ambush owns the agent process.
///
...
/// startup budget (see block/ambush#3355). Skip that unrelated global startup
...
///
/// Codex sandboxes MCP subprocesses (including `ambush-cli`) behind a Seatbelt sandbox
/// that blocks all outbound network by default. Without this env var, `ambush-cli`
...
pub fn propagate_legacy_env_vars() {
    for (legacy, canonical) in [
        ("AMBUSH_ACP_PRIVATE_KEY", "AMBUSH_PRIVATE_KEY"),
        ("AMBUSH_ACP_API_TOKEN", "AMBUSH_API_TOKEN"),
    ] {
...
impl Config {
    ...
    pub fn from_args(mut args: CliArgs) -> Result<Self, ConfigError> {
        ...
        let idle_timeout_secs = {
            let raw = match (args.idle_timeout, args.turn_timeout) {
                (Some(idle), Some(_turn)) => {
                    tracing::warn!(
                        "--turn-timeout / AMBUSH_ACP_TURN_TIMEOUT is deprecated and ignored \
                         when --idle-timeout / AMBUSH_ACP_IDLE_TIMEOUT is also set"
                    );
                    idle
                }
                (Some(idle), None) => idle,
                (None, Some(turn)) => {
                    tracing::warn!(
                        "--turn-timeout / AMBUSH_ACP_TURN_TIMEOUT is deprecated; \
                         use --idle-timeout / AMBUSH_ACP_IDLE_TIMEOUT instead"
                    );
                    turn
                }
            ...
            for s in &raw {
                RespondTo::from_str(s.trim(), true).map_err(|_| {
                    ConfigError::ConfigFile(format!(
                        "invalid value in AMBUSH_ACP_ALLOWED_RESPOND_TO: '{s}' \
        ...
        // opens the Seatbelt network sandbox for ambush-cli (an MCP subprocess). No-op
...
mod tests {
    ...
    fn test_config(mode: SubscribeMode) -> Config {
        Config {
            ...
            config_path: PathBuf::from("./ambush-acp.toml"),
    ...
    fn test_mentions_mode_default_kinds() {
        ...
        for ch in &channels {
            ...
            assert!(kinds.contains(&ambush_core::kind::KIND_STREAM_MESSAGE));
            assert!(kinds.contains(&ambush_core::kind::KIND_WORKFLOW_APPROVAL_REQUESTED));
            assert!(kinds.contains(&ambush_core::kind::KIND_STREAM_REMINDER));
        }
    }
    ...
    #[test]
    fn normalizes_ambush_agent_args_to_empty() {
        assert_eq!(
            normalize_agent_args("ambush-agent", Vec::new()),
            Vec::<String>::new()
        );
        assert_eq!(
            normalize_agent_args("ambush-agent", vec!["acp".into()]),
            Vec::<String>::new()
        );
    }
    ...
    fn default_agent_env_recognizes_hermes_identities() {
        ...
        for command in ["goose", "codex-acp", "claude-agent-acp", "ambush-agent", ""] {
            assert!(
    ...
    fn codex_network_env_non_codex_agent_returns_none() {
        ...
        assert!(codex_network_env("ambush-agent", "wss://relay.example.com").is_none());
    }
    ...
    fn test_load_rules_valid_toml() {
        let dir = std::env::temp_dir().join("ambush-acp-test-valid");
...
"#,
    ...
    fn test_load_rules_empty_name_rejected() {
        let dir = std::env::temp_dir().join("ambush-acp-test-empty-name");
        let path = dir.join("rules.toml");
        std::fs::create_dir_all(&dir).unwrap();
        std::fs::write(
            &path,
...
"#,
    ...
    fn test_load_rules_duplicate_name_rejected() {
        let dir = std::env::temp_dir().join("ambush-acp-test-dup-name");
        let path = dir.join("rules.toml");
        std::fs::create_dir_all(&dir).unwrap();
        std::fs::write(
            &path,
...
"#,
    ...
    fn test_load_rules_invalid_filter_rejected() {
        let dir = std::env::temp_dir().join("ambush-acp-test-bad-filter");
        let path = dir.join("rules.toml");
        std::fs::create_dir_all(&dir).unwrap();
...
"#,
    ...
    fn test_load_rules_channel_scope_typo_rejected() {
        let dir = std::env::temp_dir().join("ambush-acp-test-scope-typo");
        let path = dir.join("rules.toml");
        std::fs::create_dir_all(&dir).unwrap();
        std::fs::write(
            &path,
...
"#,
    ...
    fn test_load_rules_too_many_rules_rejected() {
        let dir = std::env::temp_dir().join("ambush-acp-test-too-many");
        let path = dir.join("rules.toml");
        std::fs::create_dir_all(&dir).unwrap();
    ...
    fn test_load_rules_filter_too_long_rejected() {
        let dir = std::env::temp_dir().join("ambush-acp-test-long-filter");
        let path = dir.join("rules.toml");
        std::fs::create_dir_all(&dir).unwrap();
    ...
    fn inactivity_exit_defaults_disabled_and_accepts_cli_value() {
        let key = "0".repeat(64);
        let default = CliArgs::parse_from(["ambush-acp", "--private-key", &key]);
    ...
    fn lazy_pool_defaults_off() {
        let key = "0".repeat(64);
        assert!(!CliArgs::parse_from(["ambush-acp", "--private-key", &key]).lazy_pool);
    }
    ...
    fn lazy_pool_cli_flag_enables_deferred_startup() {
        let key = "0".repeat(64);
        let args =
            CliArgs::try_parse_from(["ambush-acp", "--private-key", &key, "--lazy-pool=true"]);
        assert!(args.is_err(), "bool flags do not take an explicit value");
        assert!(
            CliArgs::parse_from(["ambush-acp", "--private-key", &key, "--lazy-pool"]).lazy_pool
        );
    }
    ...
    fn test_permission_mode_value_enum_camel_case_aliases() {
        ...
        // (e.g. AMBUSH_ACP_PERMISSION_MODE=bypassPermissions). The #[value(alias)]
    ...
    fn test_multiple_event_handling_default_is_steer() {
        ...
        let args = CliArgs::parse_from(["ambush-acp", "--private-key", &"0".repeat(64)]);
    ...
    // --- AMBUSH_ACP_ALLOWED_RESPOND_TO gate ---
    ...
    fn allowed_respond_to_rejects_invalid_mode_string() {
        ...
        let msg = result.unwrap_err().to_string();
        assert!(
            msg.contains("invalid value in AMBUSH_ACP_ALLOWED_RESPOND_TO"),
    ...
    //
    // These tests exercise the actual wiring: AMBUSH_ACP_ALLOWED_RESPOND_TO in the
    ...
    fn compose_session_title_drops_the_channel_when_the_agent_name_fills_the_cap() {
        ...
        assert_eq!(compose_session_title(&agent, Some("ambush-dev")), agent);
    }
