...
//! model, or credentials), it spawns `ambush-acp` with a setup-mode payload
...
///
/// ambush-acp is a separate crate and must NOT depend on desktop types —
...
pub(crate) const SETUP_PAYLOAD_ENV_VAR: &str = "AMBUSH_ACP_SETUP_PAYLOAD";
...
/// (`managed_agents/readiness.rs`). Desktop serializes it; `ambush-acp`
...
impl SetupPayload {
    ...
    /// 2. A fenced `ambush:config-nudge` sentinel block containing the structured
    ...
    fn nudge_body(&self) -> String {
        let prose = if self.requirements.is_empty() {
            ...
            let footer = if has_doctor_requirement {
                ...
                // Mixed: some Ambush-managed fields, some external config.
                "Open Edit Agent in the Ambush app for the Ambush-managed fields; fix the external CLI config files manually and restart the agent.".to_string()
            } else {
                // All Ambush-managed — original footer unchanged.
                "Open Edit Agent in the Ambush app to set these.".to_string()
            };
...
pub(crate) async fn run_setup_listener(config: Config, payload: SetupPayload) -> Result<()> {
    tracing::info!(
        ...
        "ambush-acp entering setup mode"
    );

    let pubkey_hex = config.keys.public_key().to_hex();

    // Parse AMBUSH_AUTH_TAG for relay membership / NIP-OA.
    ...
    loop {
        let Some(ambush_event) = relay.next_event().await else {
        ...
        {
            handle_setup_membership(&mut relay, &ambush_event, &config, &rules, &channel_ids).await;
            continue;
        }
        ...
        let is_dm = crate::is_dm_channel(ambush_event.channel_id, &channel_info).await;
        ...
        .await
        {
            ...
            tracing::info!(
                ...
                event_id = %ambush_event.event.id,
...
async fn handle_setup_membership(
    ...
    ambush_event: &crate::relay::AmbushEvent,
    config: &Config,
    rules: &[filter::SubscriptionRule],
    _initial_channel_ids: &[Uuid],
) {
    let kind_u32 = ambush_event.event.kind.as_u16() as u32;
    let channel_id = ambush_event.channel_id;
...
async fn publish_setup_nudge(
    ...
    let event_builder = ambush_sdk::build_message(
        channel_id,
        &body,
        thread_ref.as_ref(),
...
mod tests {
    ...
    fn setup_payload_deserializes_git_bash_requirement() {
        let payload: SetupPayload = serde_json::from_str(
            r#"{"agent_name":"Ambush Agent","agent_pubkey":"test","requirements":[{"surface":"git_bash"}]}"#,
        )
        .unwrap();
        assert!(matches!(
    ...
    fn nudge_body_git_bash_copy_points_to_agent_runtimes() {
        let payload = SetupPayload {
            agent_name: "Ambush Agent".to_string(),
            agent_pubkey: "test".to_string(),
    ...
    fn nudge_body_mixed_requirements_uses_split_footer() {
        // Mixed list: one Ambush-managed env key + one external config invalid.
    ...
    fn nudge_body_all_ambush_managed_retains_original_footer() {
        ...
        let body = payload.nudge_body();
        assert!(
            body.contains("Open Edit Agent in the Ambush app to set these."),
    ...
    fn nudge_body_sentinel_round_trips_payload() {
        ...
        let fence_open = "```ambush:config-nudge\n";
        let fence_close = "\n```";
        let start = body
            .rfind(fence_open)
    ...
    //
    // These tests prove the desktop→ambush-acp→sentinel path preserves the
    // `availability` field. They simulate what actually happens at runtime:
    // desktop serializes a `cli_login` JSON blob → ambush-acp parses it via
