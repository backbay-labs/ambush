...
/// name (e.g., `ambush-acp --verbose models`) are not supported.
...
/// Timeout for `ambush-acp authenticate`. Browser-based vendor auth can require
...
async fn publish_presence(
    ...
    use ambush_core::kind::KIND_PRESENCE_UPDATE;
...
///
/// Priority:
/// 1. `AMBUSH_AUTH_TAG` env var — NIP-OA attestation signed by the owner.
///    Verified against the agent's own pubkey to extract the owner pubkey.
/// 2. `--agent-owner` CLI flag / `AMBUSH_ACP_AGENT_OWNER` env var.
fn resolve_agent_owner(config: &Config) -> Option<String> {
    // Try AMBUSH_AUTH_TAG first (NIP-OA attestation).
    if let Ok(auth_tag) = std::env::var("AMBUSH_AUTH_TAG") {
        if !auth_tag.is_empty() {
            let agent_pk = config.keys.public_key();
            match ambush_sdk::nip_oa::verify_auth_tag(&auth_tag, &agent_pk) {
                Ok(owner_pk) => {
                    let owner_hex = owner_pk.to_hex().to_ascii_lowercase();
                    tracing::info!("owner resolved from AMBUSH_AUTH_TAG: {owner_hex}");
                    return Some(owner_hex);
                }
                Err(e) => {
                    tracing::warn!("AMBUSH_AUTH_TAG verification failed: {e} — falling back");
                }
            }
        }
    }
...
async fn check_sibling_via_profile(
    ...
    for tag in tags {
        ...
        match ambush_sdk::nip_oa::verify_auth_tag(&tag_json, &agent_pk) {
            Ok(_) => {
...
/// keyed to `OBSERVER_MAX_PLAINTEXT_LEN` in ambush-core/observer.rs:25) is what actually
...
/// already-small frame. Reusing that string would mean changing ambush-core's
/// `encrypt_observer_payload` signature or adding a parallel encrypt path; both
/// are out of this change's scope (ambush-core stays untouched). The clean `&mut`
...
async fn publish_relay_observer_event(
    ...
    let builder = match ambush_sdk::build_agent_observer_frame(
        owner_pubkey_hex,
        agent_pubkey_hex,
...
fn handle_relay_observer_control_event(
    ...
    if let Err(e) = ambush_core::verify_event(&event) {
...
async fn tokio_main() -> Result<()> {
    ...
    tracing_subscriber::fmt()
        .with_env_filter(
            EnvFilter::try_from_default_env().unwrap_or_else(|_| EnvFilter::new("ambush_acp=info")),
        )
        .compact()
        .init();
    ...
    // model, or provider), it spawns ambush-acp with AMBUSH_ACP_SETUP_PAYLOAD set.
    ...
    {
        tracing::info!("ambush-acp: setup payload present, entering setup-listener mode");
        return setup_mode::run_setup_listener(config, payload).await;
    }

    tracing::info!("ambush-acp starting: {}", config.summary());
    ...
    // Parse AMBUSH_AUTH_TAG into a nostr::Tag for NIP-OA relay membership delegation.
    let relay_auth_tag: Option<nostr::Tag> = std::env::var("AMBUSH_AUTH_TAG")
        .ok()
        .filter(|s| !s.is_empty())
        .and_then(|s| ambush_sdk::nip_oa::parse_auth_tag(&s).ok());
    ...
    // Priority: AMBUSH_AUTH_TAG (NIP-OA attestation) → --agent-owner flag.
            ...
            RespondTo::OwnerOnly => {
                tracing::warn!(
                    "respond-to=owner-only but no owner is set — all events will be \
                     dropped. Set AMBUSH_AUTH_TAG or --agent-owner, or use --respond-to=anyone."
                );
            }
            RespondTo::Allowlist => {
                tracing::warn!(
    ...
    let runtime_start_nonce = std::env::var("AMBUSH_MANAGED_AGENT_START_NONCE").unwrap_or_default();
    ...
    if !config.memory_enabled {
        tracing::info!(
            target: "engram::core",
            "NIP-AE core memory injection disabled (re-enable by removing --no-memory / AMBUSH_ACP_NO_MEMORY)"
        );
    }
    ...
    loop {
        ...
        let pool_event: Option<PoolEvent> = {
            ...
            tokio::select! {
                ...
                ambush_event = relay.next_event() => {
                    let _ = result_rx; // end split borrow before relay handling
                    match ambush_event {
                        Some(ambush_event) => {
                            let kind_u32 = ambush_event.event.kind.as_u16() as u32;

                            if kind_u32 == KIND_MEMBER_ADDED_NOTIFICATION
                                || kind_u32 == KIND_MEMBER_REMOVED_NOTIFICATION
                            {
                                let ch = ambush_event.channel_id;
                                let ts = ambush_event.event.created_at.as_secs();
                                let eid = ambush_event.event.id.to_hex();
                            ...
                            if config.ignore_self && ambush_event.event.pubkey.to_hex() == pubkey_hex {
                                tracing::debug!(channel_id = %ambush_event.channel_id, "dropping self-authored event");
                                continue;
                            }
                            ...
                            if is_shutdown {
                                ...
                                if let Some(owner) = owner {
                                    if ambush_event.event.pubkey.to_hex() == *owner {
                                        tracing::info!(
                                            channel_id = %ambush_event.channel_id,
                                            sender = %ambush_event.event.pubkey.to_hex(),
                                ...
                                if let Some(owner) = owner_cache.get() {
                                    if ambush_event.event.pubkey.to_hex() == *owner {
                                        ...
                                        if fired {
                                            ...
                                            let invalidated = pool.invalidate_channel_sessions(ambush_event.channel_id);
                                            tracing::info!(
                                                channel_id = %ambush_event.channel_id,
                                                invalidated,
                            ...
                            {
                                let author = ambush_event.event.pubkey.to_hex();
                                // DM hardening: resolve channel type (fail-closed
                                // to DM) so allowlist/anyone modes cannot be
                                // exercised by non-owner authors inside DMs.
                                let is_dm =
                                    is_dm_channel(ambush_event.channel_id, &ctx.channel_info).await;
                                ...
                                if !allowed {
                                    tracing::debug!(
                                        channel_id = %ambush_event.channel_id,
                                        author = %ambush_event.event.pubkey.to_hex(),
                            ...
                            let matched = filter::match_event(&ambush_event.event, ambush_event.channel_id, &rules, &pubkey_hex).await;
                            let prompt_tag = match matched {
                                Some(m) => m.prompt_tag,
                                None => {
                                    tracing::debug!(channel_id = %ambush_event.channel_id, kind = ambush_event.event.kind.as_u16(), "event matched no rule — dropping");
                                    continue;
                                }
                            };
                            // Capture author pubkey before queue.push() moves
                            // ambush_event.event (needed for mode gate below).
                            let author_hex = ambush_event.event.pubkey.to_hex();
                            let event_id_hex = ambush_event.event.id.to_hex();
                            ...
                            let event_for_steer = ambush_event.event.clone();
                            let prompt_tag_for_steer = prompt_tag.clone();
                            let accepted = queue.push(QueuedEvent {
                                channel_id: ambush_event.channel_id,
                                event: ambush_event.event,
                            ...
                            if accepted && queue.is_channel_in_flight(ambush_event.channel_id) {
    ...
    tracing::info!("ambush-acp stopped");
    Ok(())
}
...
fn try_native_steer(
    ...
    // `queue::format_prompt` uses internally for `[Ambush event: …]`
...
fn handle_prompt_result(
    ...
    // which live in ambush-agent's session state and are what `llm: (model) …`
...
mod agent_draft_prompt_tests {
    ...
    fn shared_base_prompt_teaches_portable_agent_drafts() {
        ...
        assert!(prompt.contains("ambush agents draft-create"));
    ...
    fn shared_base_prompt_teaches_real_newlines_for_multiline_messages() {
        ...
        assert!(prompt.contains("ambush messages send ... --content -"));
    }
    ...
    fn shared_base_prompt_teaches_not_to_duplicate_projects() {
        let prompt = include_str!("base_prompt.md");
        assert!(prompt.contains("do **not** run `ambush projects create`"));
        assert!(prompt.contains("ambush issues create --channel"));
        assert!(prompt.contains("is not a Ambush repository"));
    }

    #[test]
    fn shared_base_prompt_teaches_single_command_mentions_and_preflight() {
        let prompt = include_str!("base_prompt.md");
        assert!(prompt.contains("use the person's **exact display name as shown in Ambush**"));
        ...
        assert!(prompt.contains(
            "add them explicitly with `ambush channels add-member` only when authorized"
        ));
...
fn default_heartbeat_prompt() -> String {
    ...
    format!(
        "[System: Heartbeat]\nTime: {now}\n\n\
         ...
         Your tasks:\n\
         1. Run `ambush feed get --types needs_action` to check for pending workflow approvals or\n\
            high-priority requests addressed to you.\n\
         2. Run `ambush feed get --types mentions` to check for unanswered @mentions.\n\
         3. If you find actionable items, address them using the appropriate CLI commands\n\
            (e.g., `ambush workflows approve --token <UUID>`, `ambush messages send`,\n\
            `ambush messages send --reply-to <event-id>`).\n\
         4. If there are no pending actions or mentions, end your turn immediately.\n\n\
         Do not run `ambush channels list` or `ambush messages search` unless you have a specific reason.\n\
...
/// `ambush-acp auth-methods` — spawn an adapter, initialize it, print authMethods.
...
/// `ambush-acp authenticate` — invoke one adapter-owned auth method.
...
fn build_mcp_servers(config: &Config) -> Vec<McpServer> {
    ...
    vec![McpServer {
        ...
        env: {
            let mut env = vec![
                EnvVar {
                    name: "AMBUSH_RELAY_URL".into(),
                    value: config.relay_url.clone(),
                },
                EnvVar {
                    name: "AMBUSH_PRIVATE_KEY".into(),
            ...
            // Forward AMBUSH_AUTH_TAG (NIP-OA owner attestation credential)
            // so the MCP server can attach it to every signed event.
            if let Ok(auth_tag) = std::env::var("AMBUSH_AUTH_TAG") {
                if !auth_tag.is_empty() {
                    env.push(EnvVar {
                        name: "AMBUSH_AUTH_TAG".into(),
                        value: auth_tag,
                    });
                }
            }
            ...
            if let Ok(display_name) = std::env::var("AMBUSH_ACP_DISPLAY_NAME") {
                if !display_name.is_empty() {
                    env.push(EnvVar {
                        name: "AMBUSH_ACP_DISPLAY_NAME".into(),
                        value: display_name,
                    });
                }
            }
...
mod build_mcp_servers_tests {
    ...
    fn test_config() -> Config {
        Config {
            ...
            config_path: std::path::PathBuf::from("./ambush-acp.toml"),
    ...
    fn session_new_mcp_server_has_required_fields() {
        ...
        assert!(
            names.contains(&"AMBUSH_RELAY_URL"),
            "missing AMBUSH_RELAY_URL; got {names:?}"
        );
        assert!(
            names.contains(&"AMBUSH_PRIVATE_KEY"),
            "missing AMBUSH_PRIVATE_KEY; got {names:?}"
        );
    }

    #[test]
    fn session_new_mcp_server_forwards_ambush_auth_tag() {
        let _guard = ENV_LOCK.lock().unwrap();
        std::env::set_var("AMBUSH_AUTH_TAG", "test-attestation-tag");
        let config = test_config();
        let servers = build_mcp_servers(&config);
        std::env::remove_var("AMBUSH_AUTH_TAG");

        let server = &servers[0];
        let auth_tag_env = server.env.iter().find(|e| e.name == "AMBUSH_AUTH_TAG");
        assert!(
            auth_tag_env.is_some(),
            "AMBUSH_AUTH_TAG should be forwarded when set"
        );
        assert_eq!(auth_tag_env.unwrap().value, "test-attestation-tag");
    }

    #[test]
    fn session_new_mcp_server_skips_empty_ambush_auth_tag() {
        let _guard = ENV_LOCK.lock().unwrap();
        std::env::set_var("AMBUSH_AUTH_TAG", "");
        let config = test_config();
        let servers = build_mcp_servers(&config);
        std::env::remove_var("AMBUSH_AUTH_TAG");

        let server = &servers[0];
        let has_auth_tag = server.env.iter().any(|e| e.name == "AMBUSH_AUTH_TAG");
        assert!(
            !has_auth_tag,
            "empty AMBUSH_AUTH_TAG should not be forwarded"
        );
    }
    ...
    fn test_display_name_set_is_forwarded_to_mcp_server() {
        let _guard = ENV_LOCK.lock().unwrap();
        std::env::set_var("AMBUSH_ACP_DISPLAY_NAME", "Duncan");
        let config = test_config();
        let servers = build_mcp_servers(&config);
        std::env::remove_var("AMBUSH_ACP_DISPLAY_NAME");

        let entry = servers[0]
            .env
            .iter()
            .find(|e| e.name == "AMBUSH_ACP_DISPLAY_NAME");
        assert_eq!(
    ...
    fn test_display_name_unset_omits_the_key_entirely() {
        ...
        assert!(
            !servers[0]
                .env
                .iter()
                .any(|e| e.name == "AMBUSH_ACP_DISPLAY_NAME"),
    ...
    fn test_display_name_empty_omits_the_key_entirely() {
        let _guard = ENV_LOCK.lock().unwrap();
        std::env::set_var("AMBUSH_ACP_DISPLAY_NAME", "");
        let config = test_config();
        let servers = build_mcp_servers(&config);
        std::env::remove_var("AMBUSH_ACP_DISPLAY_NAME");
...
mod observer_payload_trim_tests {
    ...
    fn test_multi_block_prompt_retains_every_section_header_after_elision() {
        ...
        // {text: "[Agent Memory — core]…"}, … {text: "[Ambush event: …]…<huge>"}].
        ...
        // [Ambush event] header fell into the elided middle and the count collapsed
        // to 1).
        let sections = [
        ...
        let event_block = texts
            .iter()
            .find(|t| t.starts_with("[Ambush event: @mention]"))
            .unwrap();
        assert!(
