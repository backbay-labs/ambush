//! Harness-side Ambush relay client.
//!
//! Connects to the Ambush relay via NIP-01 WebSocket, authenticates via NIP-42,
...
//! - Forwards `AmbushEvent`s through an `mpsc` channel
...
/// Override with `AMBUSH_ACP_EVENT_BUFFER` env var at startup.
...
fn event_channel_capacity() -> usize {
    std::env::var("AMBUSH_ACP_EVENT_BUFFER")
        .ok()
...
impl RestClient {
    ...
    /// lookup needs `#ambush-channel`, which this path serializes verbatim.
...
///
/// Connects to the Ambush relay, authenticates via NIP-42, and streams
...
pub struct HarnessRelay {
    ...
    event_rx: mpsc::Receiver<Option<AmbushEvent>>,
...
impl HarnessRelay {
    ...
    pub async fn connect(
        ...
        let (event_tx, event_rx) = mpsc::channel::<Option<AmbushEvent>>(event_channel_capacity());
    ...
    pub async fn discover_channels(&self) -> Result<HashMap<Uuid, ChannelInfo>, RelayError> {
        ...
        let member_filter = nostr::Filter::new()
            .kind(Kind::Custom(
                ambush_core::kind::KIND_NIP29_GROUP_MEMBERS as u16,
            ))
    ...
    pub async fn next_event(&mut self) -> Option<AmbushEvent> {
...
async fn run_background_task(
    ...
    event_tx: mpsc::Sender<Option<AmbushEvent>>,
            ...
            match relay_msg {
                RelayMessage::Event {
                    ...
                    if subscription_id == OBSERVER_CONTROL_SUB_ID {
                        ...
                        match event_tx.try_send(Some(ambush_event)) {
                            Ok(()) => {
...
/// sends (`crates/ambush-relay/src/handlers/auth.rs`).
///
...
mod tests {
    ...
    fn make_observer_frame(keys: &Keys) -> Event {
        ...
        ambush_sdk::build_agent_observer_frame(
