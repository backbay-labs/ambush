...
pub struct AcpClient {
    ...
    ///
    /// Both goose and ambush-agent emit `session_info_update` with this field;
    ...
    /// run clears (goose/ambush-agent emit `activeRunId: null` at end of turn).
    ...
    /// Usage tracker for goose/ambush-agent's cumulative notification format.
...
///
/// Returns `Some(json_string)` when `has_generated_codex_config` is true (Ambush injected a
...
pub(crate) fn build_codex_config_env(
    ...
    // Without an explicit Ambush-generated overlay signal, skip the merge entirely.
...
/// `_meta.goose.activeRunId`. Emitted by goose and ambush-agent only.
...
/// `outcome` value meaning the steer was applied to the turn Ambush is waiting
/// on, which therefore keeps running.
const STEER_OUTCOME_INJECTED: &str = "injected";

/// `outcome` value meaning the turn Ambush was steering had already finished, so
...
fn build_client_capabilities() -> serde_json::Value {
    serde_json::json!({
        // Signal to ACP adapters that Ambush can hand users to terminal-native
        // auth flows. Adapters decide which auth methods to expose; Ambush does
...
impl AcpClient {
    ...
    pub async fn spawn(
        ...
        // Per-persona env vars (e.g., GOOSE_PROVIDER, AMBUSH_AGENT_PROVIDER).
    ...
    ///   (ACP protocol v2, ambush-agent, goose unused).
    ...
    /// `["/cmd args", "<ambush context>"]` instead of one wrapped block.
    ...
    ///
    /// Both goose and ambush-agent emit `session_info_update`; other agents
    ...
    /// Consume per-turn usage for NIP-AM publishing. Goose/ambush-agent is an
    ...
    /// Notify the usage tracker that ambush-acp just spawned a new session.
    ///
    ...
    /// Must be called only when ambush-acp created the session via `session/new`;
                            ...
                            if let Some((steer_id, _, _)) = pending_steer.as_ref() {
                                if *id == serde_json::json!(*steer_id) {
                                    ...
                                    let ack = if let Some(error) = msg.get("error") {
                                        ...
                                        // turn Ambush awaits is still running —
    ...
    fn handle_session_update(&mut self, msg: &serde_json::Value) -> bool {
        ...
        match update_type {
            ...
            "session_info_update" => {
                // Both goose and ambush-agent emit `session_info_update` with
                ...
                // alongside it at the params level. Goose and ambush-agent both
...
///
/// - **`Field`** — bare `systemPrompt` field (ACP protocol v2, ambush-agent).
...
mod tests {
    ...
    fn session_new_mcp_server_has_required_fields() {
        ...
        assert_eq!(
            ...
            Some("AMBUSH_RELAY_URL")
        );
    }

    #[test]
    fn session_prompt_request_format() {
        let prompt_text = "[Ambush @mention]\nChannel: test\nFrom: npub1...\nMessage: hello";
        let msg = serde_json::json!({
            "jsonrpc": "2.0",
            "id": 2u64,
            "method": "session/prompt",
    ...
    async fn spawn_named_and_read_child_env(
        ...
        let dir =
            std::env::temp_dir().join(format!("ambush-acp-env-probe-{}", uuid::Uuid::new_v4()));
    ...
    /// Ambush-owned Hermes processes get the configured-MCP isolation default,
    ...
    async fn goose_system_prompt_request_uses_set_contract() {
        ...
        assert_eq!(received["params"]["key"], "ambush");
    ...
    /// `SessionInfoUpdate` schema), matching what goose and ambush-agent
    /// emit on the wire.
    ...
    fn capture_path(name: &str) -> std::path::PathBuf {
        let dir = std::env::temp_dir().join("ambush-acp-steer-capture");
    ...
    /// Test 1b: no `_meta` at all (goose, ambush-agent, any older adapter) must
    ...
    /// Ambush maps `SteerAck::Success` to `queue.remove_event`, so decoding
    ...
    /// the turn Ambush was steering had already ended and the adapter began a
    ...
    fn build_codex_config_env_generated_only_single_entry_with_signal_true_merges_with_parent() {
        // No persona: Ambush injects one CODEX_CONFIG; signal=true.
    ...
    fn build_codex_config_env_persona_only_signal_false_returns_none() {
        // Persona set CODEX_CONFIG; Ambush did not inject a generated overlay (signal=false).
