//! Event queue state machine for ambush-acp.
//!
...
/// to [`ambush_core::nip10`] so ACP anchoring reads ancestry exactly as relay
...
/// checking whether the **first** prompt content block starts with `/`. Ambush
...
pub fn extract_slash_command(content: &str, known_names: &[&str]) -> Option<String> {
    ...
    loop {
        if rest.starts_with("nostr:npub1") || rest.starts_with("nostr:nprofile1") {
            ...
            // token of the characters Ambush allows in plain @mentions.
            let name_len = names
                .iter()
                .find_map(|name| {
...
fn append_reply_instruction(s: &mut String, event_id: &str) {
    s.push_str(&format!(
        "\nIMPORTANT: For ordinary replies in this turn, use `--reply-to {event_id}` \
         on `ambush messages send` so the conversation stays threaded. \
...
fn append_new_thread_reply_instruction(s: &mut String, event_id: &str) {
    s.push_str(&format!(
        "\nIMPORTANT: This is a new top-level message. For ordinary replies in \
         this turn, use `--reply-to {event_id}` on `ambush messages send` — the \
...
fn append_project_home(s: &mut String, channel_info: Option<&PromptChannelInfo>, channel_id: Uuid) {
    ...
    s.push_str(&format!(
        "\nThis channel is that project's home. Tasks, repositories, and files created here belong to this project. Do not run `ambush projects create`. Create a repository with `ambush repos create --id <id> --name \"…\" --channel {channel_id}`. Create tasks with `ambush issues create --channel {channel_id} --subject \"…\" --content \"…\"`."
    ));
}
...
fn format_context_hints(
    ...
    if is_dm {
        ...
        let ctx_hint = if complete_conversation_context && is_reply {
            ...
            "Thread context included below. Use `ambush messages thread --channel <UUID> --event <ID>` for full history if truncated."
        } else if has_conversation_context {
            "Conversation context included below. Use `ambush messages get --channel <UUID>` for full history if truncated."
        } else if conversation_context_had_delivered_events && is_reply {
            "Earlier thread context was already delivered in this session. Use `ambush messages thread --channel <UUID> --event <ID>` to re-read the reply chain."
        } else if conversation_context_had_delivered_events {
            "Earlier conversation context was already delivered in this session. Use `ambush messages get --channel <UUID>` to re-read it."
        } else if is_reply {
            "Use `ambush messages thread --channel <UUID> --event <ID>` to fetch the reply chain."
        } else {
            "Use `ambush messages get --channel <UUID>` for conversation context."
        };
        let mut s = format!(
            "Scope: dm\n\
             Channel: {channel_display}\n\
        ...
        let ctx_hint = if complete_conversation_context {
            ...
            "Earlier thread context was already delivered in this session. Use `ambush messages thread --channel <UUID> --event <ID>` to re-read it."
        } else {
            "Use `ambush messages thread --channel <UUID> --event <ID>` to fetch thread context."
        };
        let mut s = format!(
            "Scope: thread\n\
             Channel: {channel_display}"
        ...
        append_project_home(&mut s, channel_info, channel_id);
        s.push_str(
            "\nHint: Use `ambush messages get --channel <UUID>` for recent messages if needed.",
        );
        if let Some(event_id) = reply_anchor {
...
/// 3. `<ambush-event>` / `<ambush-events>` — the triggering event(s)
///
...
mod tests {
    ...
    fn test_format_prompt_single() {
        ...
        assert!(prompt.contains("<ambush-event type=\"@mention\">\n"));
    ...
    fn test_format_prompt_batch() {
        ...
        assert!(prompt.contains("<context>"));
        assert!(prompt.contains("<ambush-events count=\"3\">"));
    ...
    fn test_thread_context_retrieval_hint_only_when_needed() {
        ...
        assert!(!complete_prompt.contains("ambush messages thread"));
        ...
        .join("\n\n");
        assert!(prompt_with_prior_delivery.contains("ambush messages thread"));
        ...
        assert!(truncated_prompt.contains("ambush messages thread"));
        ...
        assert!(missing_root_prompt.contains("ambush messages thread"));
    }
    ...
    fn test_thread_context_retrieval_hint_requires_batch_coverage() {
        ...
        assert!(mixed_prompt.contains("ambush messages thread"));
        ...
        assert!(!same_thread_prompt.contains("ambush messages thread"));
    }
    ...
    fn test_format_prompt_with_dm_context() {
        ...
        assert!(!prompt.contains("ambush messages get"));
    ...
    fn test_format_prompt_dm_reply_with_complete_thread_context_omits_retrieval_hint() {
        ...
        assert!(!prompt.contains("ambush messages thread"));
        assert!(!prompt.contains("full history"));
    ...
    fn test_format_prompt_dm_non_reply_hints_get_messages() {
        ...
        assert!(prompt.contains("Scope: dm"));
        assert!(
            prompt.contains("ambush messages get"),
            "DM non-reply hint should mention `ambush messages get`"
        );
        assert!(
            !prompt.contains("ambush messages thread"),
            "DM non-reply should NOT mention `ambush messages thread`"
        );
    }
    ...
    fn test_reply_instruction_allows_explicit_root_post_requests() {
        ...
        assert!(
            !prompt.contains("on EVERY `ambush messages send` call"),
    ...
    fn test_append_project_home_names_the_project_and_blocks_duplicates() {
        ...
        assert!(
            s.contains("do not run `ambush projects create`")
                || s.contains("Do not run `ambush projects create`")
        );
        assert!(s.contains("ambush issues create --channel 11111111-1111-4111-8111-111111111111"));
        assert_eq!(
            s.lines()
