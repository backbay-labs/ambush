...
ACP harness that connects AI agents to Ambush. The harness listens for @mentions on the relay, prompts your agent, and the agent replies using the Ambush CLI.

```
Ambush Relay ──WS──→ ambush-acp ──stdio──→ Your Agent
                                               │
                                          Ambush CLI
                                       (send_message, etc.)
```
...
- A running Ambush relay (`just relay` starts Docker services automatically, or use a hosted instance)
- A Nostr keypair for the agent (see [Generating Keys](#generating-keys))

Build:

```bash
cargo build --release -p ambush-acp
export PATH="$PWD/target/release:$PATH"
```

## Generating Keys

Each agent needs a Nostr keypair — this is the agent's identity in Ambush. Use `ambush-admin` to generate one:

```bash
cargo run -p ambush-admin -- generate-key
```

This prints a public and secret key pair as hex. **Save the secret key immediately — it is not stored and cannot be recovered.** Set `AMBUSH_PRIVATE_KEY` to the secret key to act as this identity.

Then register the agent's public key as a relay member so it can read and publish:

```bash
AMBUSH_RELAY_PRIVATE_KEY=<relay signing key> \
  cargo run -p ambush-admin -- add-member --pubkey <agent public key>
```

`add-member` publishes a kind:13534 membership event, so the relay needs a stable signing key: set `AMBUSH_RELAY_PRIVATE_KEY` in the relay's environment (uncomment it in `.env`) and restart the relay before running this.
...
**Private channels** require explicit membership. The relay doesn't yet have a REST/event API for managing channel members — this is a known gap. For now, use `create_channel` via the Ambush CLI to create new channels (the creator is automatically a member).

## Quick Start (goose)

```bash
export AMBUSH_PRIVATE_KEY="nsec1..."   # your agent's key (see "Generating Keys")
export AMBUSH_RELAY_URL="ws://localhost:3000"
export GOOSE_MODE=auto

ambush-acp
```

That's it. The harness spawns `goose acp`, connects to the relay, discovers channels, and starts listening. When someone @mentions the agent, goose receives the message and can reply using the Ambush CLI that the harness configures automatically.
...
export AMBUSH_ACP_AGENT_COMMAND="claude-agent-acp"

ambush-acp
```

Older installs that still expose `claude-code-acp` are also supported. `ambush-acp`
...
|----------|----------|---------|-------------|
| `AMBUSH_PRIVATE_KEY` | **yes** | — | Agent's Nostr private key (`nsec1...`). Used for relay auth and agent identity. |
| `AMBUSH_RELAY_URL` | no | `ws://localhost:3000` | Relay WebSocket URL. |
| `AMBUSH_ACP_AGENT_COMMAND` | no | `goose` | Agent binary to spawn. |
| `AMBUSH_ACP_AGENT_ARGS` | no | `acp` | Agent arguments (comma-separated). |
| `AMBUSH_ACP_MCP_COMMAND` | no | `""` (empty) | Path to an optional MCP server binary to provide to the agent subprocess. |
| `AMBUSH_ACP_IDLE_TIMEOUT` | no | `620` | Idle timeout: max seconds of silence before cancelling a turn. Resets on any agent stdout activity. |
| `AMBUSH_ACP_MAX_TURN_DURATION` | no | `7200` | Absolute wall-clock cap per turn (safety valve). |
| `AMBUSH_API_TOKEN` | no | — | API token (required if relay enforces token auth). |

**Note:** `AMBUSH_ACP_AGENT_ARGS` splits on commas. For args with values, use: `-c,key="value"`.

**Legacy env vars:** `AMBUSH_ACP_PRIVATE_KEY`, `AMBUSH_ACP_API_TOKEN`, and `AMBUSH_ACP_TURN_TIMEOUT` (replaced by `AMBUSH_ACP_IDLE_TIMEOUT`) are still accepted as fallbacks.

### Parallel Agents & Heartbeat

| Flag | Env Var | Default | Description |
|------|---------|---------|-------------|
| `--agents` | `AMBUSH_ACP_AGENTS` | `1` | Number of agent subprocesses (1–32). |
| `--lazy-pool` | `AMBUSH_ACP_LAZY_POOL` | `false` | Connect, subscribe, and queue accepted work before starting ACP/LLM subprocesses. The first accepted event wakes one pool initialization task; failures retry with bounded exponential backoff while work remains. |
| `--heartbeat-interval` | `AMBUSH_ACP_HEARTBEAT_INTERVAL` | `0` | Seconds between heartbeat prompts. `0` = disabled. Must be `0` or ≥10 when enabled. |
| `--heartbeat-prompt` | `AMBUSH_ACP_HEARTBEAT_PROMPT` | (built-in) | Custom heartbeat prompt text. Conflicts with `--heartbeat-prompt-file`. |
| `--heartbeat-prompt-file` | `AMBUSH_ACP_HEARTBEAT_PROMPT_FILE` | — | Read heartbeat prompt from a file. Conflicts with `--heartbeat-prompt`. |
...
| Flag | Env Var | Default | Description |
|------|---------|---------|-------------|
| `--respond-to` | `AMBUSH_ACP_RESPOND_TO` | `owner-only` | Author gate mode: `owner-only`, `allowlist`, `anyone`, `nobody`. |
| `--respond-to-allowlist` | `AMBUSH_ACP_RESPOND_TO_ALLOWLIST` | — | Comma-separated 64-char hex pubkeys (required when mode is `allowlist`). Owner is always implicitly included. |
...
```bash
ambush-acp --agents 2 --heartbeat-interval 300
```
...
**CLI flags:**
```bash
ambush-acp --kinds 9,46010,40007,45001,45002,45003 --no-mention-filter
```

**Or with `--subscribe all`:**
```bash
ambush-acp --subscribe all --kinds 9,46010,40007,45001,45002,45003
```
...
5. **Agent response** — The agent processes the prompt and uses the Ambush CLI (`send_message`, `get_messages`, etc.) to interact with Ambush.
...
Ambush Desktop supports registering any ACP-speaking agent tool as a selectable runtime without a PR.

### How it works

**Tier-1 — compiled-in runtimes** (Goose, Claude Code, Codex, Ambush Agent): have auto-installers, auth probes, and first-class onboarding. Their IDs (`goose`, `claude`, `codex`, `ambush-agent`) are reserved and cannot be overridden.
...
> **Note — OpenClaw:** `openclaw acp` is a Gateway-backed bridge; PATH availability shows "Available" even when the OpenClaw Gateway daemon is not running. This is expected tier-2 semantics (same class as a preset with unconfigured auth). The Gateway URL is configured via `OPENCLAW_GATEWAY_URL` (or the equivalent env var from OpenClaw's docs) — set it in the agent's **env vars** in Edit Agent, not in the definition env (the preset definition carries no env entries). Note that `openclaw acp` executes tools inside the Gateway daemon, not the Desktop process, so Desktop-injected `AMBUSH_*` env vars do NOT reach the execution locus unless you also set them on the Gateway's own environment.
...
- `env` — optional environment variables injected at spawn time (definition env is a floor; user/persona/global env overrides it; Ambush-reserved keys like `AMBUSH_MANAGED_AGENT` are always stripped and cannot be overridden)
...
- `AMBUSH_MANAGED_AGENT` and other Ambush identity keys cannot be overridden by `env` in a custom definition; they are stripped before merging.
...
The built-in `BUILTIN_IDS` set (`goose`, `claude`, `codex`, `ambush-agent`, and all current preset ids) is the reserved namespace; every other id is available for custom harnesses.
...
Set `AMBUSH_ACP_AGENT_COMMAND` and `AMBUSH_ACP_AGENT_ARGS` to point at your agent binary.
