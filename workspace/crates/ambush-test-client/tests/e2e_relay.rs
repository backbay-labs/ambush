//! End-to-end integration tests for the Ambush relay.
//!
...
use ambush_test_client::{AmbushTestClient, RelayMessage, TestClientError};
...
fn test_owner_keys() -> Keys {
    std::env::var("AMBUSH_TEST_OWNER_PRIVATE_KEY")
        .ok()
...
async fn e2e_db_pool() -> sqlx::Pool<sqlx::Postgres> {
    let database_url = std::env::var("DATABASE_URL").unwrap_or_else(|_| {
        "postgres://ambush:ambush_dev@localhost:5432/ambush".to_string() // sadscan:disable np.postgres.1
    });
...
async fn test_connect_and_authenticate() {
    ...
    let client = AmbushTestClient::connect(&url, &keys)
        .await
...
async fn test_client_submitted_nip43_membership_snapshots_are_rejected() {
    ...
    let mut ws = AmbushTestClient::connect(&url, &keys)
        .await
        .expect("connect");
    let ok = ws
...
async fn test_send_event_and_receive_via_subscription() {
    ...
    let mut client_a = AmbushTestClient::connect(&url, &keys_a)
        .await
    ...
    let mut client_b = AmbushTestClient::connect(&url, &keys_b)
        .await
...
async fn test_large_event_frame_below_configured_limit_is_accepted() {
    ...
    let mut client = AmbushTestClient::connect(&url, &keys)
        .await
        .expect("connect");
...
async fn test_unauthenticated_rejected() {
    ...
    let mut client = AmbushTestClient::connect_unauthenticated(&url)
        .await
...
async fn test_multiple_concurrent_clients() {
    ...
    let mut clients: Vec<AmbushTestClient> =
        futures_util::future::try_join_all(keys.iter().map(|k| AmbushTestClient::connect(&url, k)))
            .await
...
async fn subscribe_until_eose(client: &mut AmbushTestClient, sid: &str, filter: Filter) {
    loop {
        client
            .subscribe(sid, vec![filter.clone()])
            .await
...
async fn test_pubkey_mismatch_rejected() {
    ...
    let mut client = AmbushTestClient::connect(&url, &keys_a)
        .await
...
async fn test_nip29_put_user_default_policy_allows() {
    ...
    let mut ws = AmbushTestClient::connect(&url, &channel_owner_keys)
        .await
...
async fn test_unarchive_emits_member_added_notification() {
    ...
    let mut ws = AmbushTestClient::connect(&url, &owner_keys)
        .await
...
async fn test_nip29_put_user_self_add_bypasses_policy() {
    ...
    // Connect as agent.
    let mut ws = AmbushTestClient::connect(&url, &agent_keys)
        .await
...
async fn test_membership_notification_emitted_on_add() {
    ...
    let mut agent_client = AmbushTestClient::connect(&url, &agent_keys)
        .await
        .expect("connect as agent");
...
async fn create_private_channel_ws(client: &mut AmbushTestClient, keys: &Keys) -> String {
...
async fn test_private_channel_any_member_can_invite() {
    ...
    let mut owner_client = AmbushTestClient::connect(&url, &owner_keys)
        .await
        .expect("connect as owner");
    ...
    for (actor_role, actor_keys) in &actors {
        let mut actor_client = AmbushTestClient::connect(&url, actor_keys)
            .await
...
async fn test_private_channel_admin_can_invite() {
    ...
    let mut admin_client = AmbushTestClient::connect(&url, &admin_keys)
        .await
...
async fn test_private_channel_non_member_cannot_invite() {
    ...
    let mut outsider_client = AmbushTestClient::connect(&url, &outsider_keys)
        .await
...
async fn test_private_channel_member_cannot_grant_admin() {
    ...
    let mut member_client = AmbushTestClient::connect(&url, &member_keys)
        .await
...
async fn test_reply_ingest_pushes_live_thread_summary() {
    ...
    async fn recv_summary(client: &mut AmbushTestClient) -> nostr::Event {
        loop {
            match client
                .recv_event(Duration::from_secs(5))
                .await
...
async fn member_role(url: &str, keys: &Keys, channel_id: &str, pubkey_hex: &str) -> Option<String> {
    let mut ws = AmbushTestClient::connect(url, keys).await.expect("connect");
...
async fn test_nip29_put_user_cannot_demote_owner() {
    ...
    let mut ws = AmbushTestClient::connect(&url, &attacker_keys)
        .await
...
async fn test_nip29_owner_demotion_recovery_paths() {
    ...
    let put_user = |signer: Keys, target_hex: String, role: &'static str| {
        ...
        let url = url.clone();
        async move {
            let mut ws = AmbushTestClient::connect(&url, &signer)
                .await
                .expect("connect");
    ...
    let mut ws = AmbushTestClient::connect(&url, &victim_keys)
        .await
        .expect("connect");
...
async fn test_nip29_put_user_without_role_tag_preserves_role() {
    ...
    let mut ws = AmbushTestClient::connect(&url, &owner_a)
        .await
    ...
    let mut ws = AmbushTestClient::connect(&url, &owner_b)
        .await
...
async fn test_nip29_relay_rejects_role_change_by_unprivileged_actor() {
    ...
    let mut ws = AmbushTestClient::connect(&url, &attacker)
        .await
        .expect("connect as attacker");
...
async fn test_nip29_relay_rejects_last_owner_self_demotion() {
    ...
    let mut ws = AmbushTestClient::connect(&url, &owner)
        .await
        .expect("connect as owner");
