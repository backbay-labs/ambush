...
//!   cargo test -p ambush-test-client --test e2e_git -- --ignored --nocapture
//! ```
...
use ambush_media::S3AddressingStyle;
...
/// named by the announcement's `ambush-channel` tag, so every repo these tests
...
/// Run `git` with the Ambush credential helper and isolated config.
...
/// Run `git` with the Ambush credential helper and isolated config. Asserts the
...
impl GitS3Probe {
    ...
    fn from_env() -> Self {
        ...
        // relay. Unit/live MinIO probes in ambush-relay keep explicit local
        // fixtures and do not need provider overrides.
        let endpoint = std::env::var("AMBUSH_S3_ENDPOINT")
            .unwrap_or_else(|_| "http://localhost:9000".to_string());
        let access_key =
            std::env::var("AMBUSH_S3_ACCESS_KEY").unwrap_or_else(|_| "ambush_dev".to_string());
        let secret_key = std::env::var("AMBUSH_S3_SECRET_KEY")
            .unwrap_or_else(|_| "ambush_dev_secret".to_string());
        let bucket_name =
            std::env::var("AMBUSH_S3_BUCKET").unwrap_or_else(|_| "ambush-media".to_string());
        let region_name =
            std::env::var("AMBUSH_S3_REGION").unwrap_or_else(|_| "us-east-1".to_string());
    ...
    fn pointer_key(owner: &str, repo: &str) -> String {
        ...
        if let Ok(community) = std::env::var("AMBUSH_E2E_GIT_COMMUNITY_ID") {
...
fn git_s3_probe_builds_both_addressing_styles() {
    ...
    assert_eq!(path.url(), "https://storage.example/ambush-media");
    ...
    assert_eq!(virtual_hosted.url(), "https://ambush-media.storage.example");
}
...
async fn git_clone_push_fetch_force_roundtrip() {
    ...
    // The `ambush-channel` binding is the repo's ACL: without it the read gate
    ...
    let announce = EventBuilder::new(Kind::from(30617), "")
        .tags(vec![
            ...
            Tag::parse(["ambush-channel", &channel]).unwrap(),
        ])
...
async fn git_concurrent_push_one_wins_and_repo_recovers() {
    ...
    let tmp = tempdir_named("ambush-e2e-git-concurrent");
    let url = format!("{}/git/{}/{}", relay_http_url(), owner_hex, repo);
