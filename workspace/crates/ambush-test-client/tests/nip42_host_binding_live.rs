...
//! # Run one binary, AMBUSH_HEALTH_PORT=8180 AMBUSH_METRICS_PORT=9202,
//! # AMBUSH_RECONCILE_CHANNELS=false, AMBUSH_GIT_CONFORMANCE_PROBE=false
//! ```
//!
//! Each test is `#[ignore]` so it only runs explicitly:
//! `cargo test -p ambush-test-client --test nip42_host_binding_live -- --ignored --test-threads=1`
