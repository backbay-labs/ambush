...
async fn test_team_catalog_foreign_sees_only_shared() {
    ...
    let mut author = AmbushTestClient::connect(&url, &author_keys)
        .await
    ...
    let mut foreign = AmbushTestClient::connect(&url, &foreign_keys)
        .await
...
async fn test_team_catalog_live_fanout_and_unshare_retracts() {
    ...
    let delivered = loop {
        match foreign.recv_event(Duration::from_secs(5)).await {
            ...
            Ok(_) => continue,
            Err(ambush_test_client::TestClientError::Timeout) => break false,
