#![deny(unsafe_code)]
#![warn(missing_docs)]
//! `ambush-core` — zero-I/O foundation types for the Ambush relay.
//!
//! Provides [`StoredEvent`], filter matching, kind constants, and event
//! verification. All other Ambush crates depend on this one.
...
pub mod invite;
/// Ambush kind number registry — custom event type constants.
pub mod kind;
