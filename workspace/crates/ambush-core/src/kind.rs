//! Ambush V2 kind number registry.
//!
//! This module is the authoritative source for Ambush kind numbers.
...
/// NIP-01: Channel metadata (replaceable). Not used by Ambush today.
...
/// Ambush custom one-time identity binding proof (ephemeral, not stored).
...
// NEW: Ambush command kinds (Pure Nostr plan)
...
/// `crates/ambush-search/tests/fts_integration.rs`).
///
...
/// visibility pushdown in `ambush-db`.
///
...
/// Ambush product feedback submission. Accepted at ingest, sidecarred to the
...
// Ambush community moderation commands (mod-signed, processed like 9030-series:
...
/// Ambush: Set the workspace profile (icon). Admin/owner-signed command.
...
// Not using NIP-90 kinds (5000–6999) — Ambush requires auth chains (depth ≤ 3, breadth ≤ 10).
...
/// or the Ambush workspace-profile admin command (9033).
...
/// Returns `true` if `kind` is a Ambush command kind that requires transactional execution.
...
/// Safe: all Ambush kinds fit in i32 (max 65535 < i32::MAX).
...
// Compile-time: all Ambush kind constants fit in nostr's u16-backed Kind.
