...
//!
//! This module defines the core data types for the Ambush git permission system.
//! The permission model: channel role = repo role; `ambush-protect` tags on
...
/// announcement with no `ambush-channel` binding.
///
...
/// Maximum number of `ambush-protect` tags per repo.
...
/// A single protection rule parsed from a `ambush-protect` tag on kind:30617.
///
/// Format: `["ambush-protect", "<ref-pattern>", "<rule>", ...]`
...
/// Errors from parsing a `ambush-protect` tag.
#[derive(Debug, Clone, PartialEq, Eq)]
...
impl fmt::Display for RuleParseError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::TooFewValues => write!(f, "ambush-protect tag needs pattern + at least one rule"),
...
/// Parse a single `ambush-protect` tag into a `ProtectionRule`.
///
/// Tag format: `["ambush-protect", "<pattern>", "<rule1>", "<rule2>", ...]`
/// The first element ("ambush-protect") should already be stripped — pass
/// the remaining values starting with the pattern.
/// Parse a single `ambush-protect` tag (simple API, discards unknown rules).
...
/// Parse a single `ambush-protect` tag, returning unknown rules for logging.
...
/// Parse all `ambush-protect` tags from a kind:30617 event's tag list.
///
/// Returns an error if any `ambush-protect` tag is structurally malformed.
...
pub fn parse_protection_tags(tags: &[Vec<String>]) -> Result<ParsedProtection, RuleParseError> {
    ...
    for tag in tags {
        if tag.first().map(|s| s.as_str()) != Some("ambush-protect") {
            continue;
        }
...
/// Built-in default minimum role for an operation when no `ambush-protect` tag matches.
