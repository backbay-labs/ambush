...
boundary** in `crates/ambush-relay/src/handlers/{ingest,req,event}.rs`.
...
cargo test -p ambush-conformance --lib

# 2. Replay fixtures (5 tests). Three JSONL traces in
#    crates/ambush-conformance/tests/fixtures/ are committed for reviewer
...
#    AMBUSH_CONFORMANCE_UPDATE=1 cargo test -p ambush-conformance --test replay_fixtures
cargo test -p ambush-conformance --test replay_fixtures

# 3. EmitGuard coverage-breach self-test (2 tests in
#    crates/ambush-relay/src/conformance/mod.rs). Proves the Drop guard
#    records `ImplBug` when no emit reaches the tracer, and stays
#    silent when an emit did. The seam-name string flows through.
cargo test -p ambush-relay --lib conformance::
