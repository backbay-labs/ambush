# Trace Schema (`ambush-conformance`)
...
- **`sanitized_error { reason }`** where `reason ∈ { restricted,
  ...
  `crates/ambush-relay/src/conformance/mod.rs`) goes non-exhaustive and
  CI catches it.
...
| File | What it emits |
|------|---------------|
| `crates/ambush-relay/src/conformance/mod.rs` | helpers + `EmitGuard` + `sanitized_reason_for` |
| `crates/ambush-relay/src/conformance/tracers.rs` | `NoopTracer` (prod default), `JsonlTracer` |
| `crates/ambush-relay/src/handlers/ingest.rs` | `AuthCheck`, `WriteInsert`, `WriteInsertGlobal`, `WriteDuplicate`, outer-wrapper `SanitizedError` |
| `crates/ambush-relay/src/handlers/req.rs` | **held back** — additive patch for integration onto Max's req.rs work |

## Where the checker lives

| File | What it does |
|------|--------------|
| `crates/ambush-conformance/src/lib.rs` | schema + `Tracer` trait |
| `crates/ambush-conformance/src/transitions.rs` | spec `Next` re-implementation |
| `crates/ambush-conformance/src/checker.rs` | replay engine: `IllegalTransition` / `StateMismatch` / `NonInterference` / `CoverageBreach` |
...
`crates/ambush-conformance/src/checker.rs::tests` proving the gate bites
