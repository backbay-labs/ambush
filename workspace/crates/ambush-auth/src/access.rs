...
//!
//! Defines [`ChannelAccessChecker`] so `ambush-auth` can enforce access
//! without depending on `ambush-db` directly.
...
///
/// Implemented by the database layer (`ambush-db`) in production. The `ambush-auth`
