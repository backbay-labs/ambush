...
//! `ambush-relay` / `ambush-pubsub`. Fixed-window counter algorithm.
//!
...
///
/// The Redis-backed production implementation lives in `ambush-relay` / `ambush-pubsub`.
...
/// key is community-prefixed (`ambush:{community}:ratelimit:{pubkey}:{suffix}`). The
...
/// `ambush:{community}:ratelimit:{pubkey_hex}:{suffix}`.
///
...
/// Redis key for IP-based rate limit: `ambush:ratelimit:ip:{ip}:conn`.
///
/// Operator-global by design — see [`RateLimiter`] docs.
pub fn ip_rate_limit_key(ip: &IpAddr) -> String {
    format!("ambush:ratelimit:ip:{}:conn", ip)
}
...
mod tests {
    ...
    fn rate_limit_key_includes_community_prefix() {
        ...
        let expected_prefix = format!("ambush:{}:ratelimit:", ctx.community());
        assert!(
    ...
    fn ip_rate_limit_key_format() {
        ...
        assert_eq!(
            ...
            "ambush:ratelimit:ip:192.168.1.1:conn"
        );
    }
