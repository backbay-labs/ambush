#![deny(unsafe_code)]
#![warn(missing_docs)]
//! `ambush-auth` — Authentication and authorization for the Ambush relay.
//!
//! ## Auth paths
//!
...
///
/// Uses `SHA-256("ambush-test-key:{username}")` as the secret key material.
...
pub fn derive_pubkey_from_username(username: &str) -> Result<nostr::PublicKey, AuthError> {
    ...
    let seed = format!("ambush-test-key:{username}");
