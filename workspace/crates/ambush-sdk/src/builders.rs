...
///
/// Shortcodes are case-insensitive in Ambush's relay-global set; lowercase
...
/// Build a Ambush-native delete event (kind 9005).
...
/// Build a Ambush-native delete event (kind 9005) with optional moderation metadata.
...
pub const CUSTOM_EMOJI_SET_D_TAG: &str = "ambush:custom-emoji";
...
/// "ambush:custom-emoji")`. Replaces the caller's prior set. The workspace
...
pub fn build_update_channel(
    ...
    if name
        .map(ambush_core::channel::canonical_channel_name)
...
/// Maximum byte length of a project `ambush-channel` tag value.
pub const PROJECT_CHANNEL_MAX: usize = 256;
/// Maximum byte length of a project `ambush-visibility` tag value.
...
///
/// Rules enforced (matches relay `ambush-db` ingest logic):
...
/// 7. Singleton metadata: each of `name`, `description`, `ambush-channel`,
///    `ambush-visibility` appears at most once.
/// 8. Metadata byte lengths: `name` ≤256, `description` ≤2048,
///    `ambush-channel` ≤256, `ambush-visibility` ≤256.
...
pub fn build_project(
    ...
    if let Some(ch) = channel {
        uuid::Uuid::parse_str(ch).map_err(|_| {
            SdkError::InvalidInput(format!("ambush-channel must be a valid UUID (got {ch:?})"))
        })?;
    }
    ...
    if let Some(vis) = visibility {
        if vis != "listed" && vis != "unlisted" {
            return Err(SdkError::InvalidInput(format!(
                "ambush-visibility must be 'listed' or 'unlisted' (got {vis:?})"
            )));
        }
    }
    ...
    if let Some(ch) = channel {
        tags.push(tag(&["ambush-channel", ch])?);
    }
    if let Some(vis) = visibility {
        tags.push(tag(&["ambush-visibility", vis])?);
    }
...
mod tests {
    ...
    fn agent_observer_frame_happy_path() {
        ...
        let recipient = keys();
        let agent = keys();
        let encrypted = ambush_core::observer::encrypt_observer_payload(
            &sender,
    ...
    fn repo_announcement_with_tags_preserves_metadata_and_canonicalizes_d() {
        let tags = vec![
            ...
            Tag::parse(["ambush-channel", "channel-id"]).unwrap(),
        ...
        assert!(has_tag(&ev, "ambush-channel", "channel-id"));
    ...
    fn git_patch_rejects_whitespace_only_content() {
        // Regression: a failed `git format-patch | ambush patches send
    ...
    fn build_project_emitted_envelope_has_correct_shape() {
        ...
        // name, description, ambush-channel, ambush-visibility present.
        ...
        let vis_tags: Vec<_> = all_tags
            .iter()
            .filter(|t| t[0] == "ambush-visibility")
            .collect();
        ...
        assert_eq!(a_tags[0][1], format!("30617:{OWNER64}:ambush"));
    }
