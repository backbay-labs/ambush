//! ambush-relay-mesh — the inter-relay QUIC mesh.
//!
...
//! mesh-free: when `AMBUSH_MESH=off` or no peers exist, the relay never
...
// seams below; huddle fan-out lives in ambush-relay's audio module (Dawn).
...
pub struct MeshConfig {
    /// `AMBUSH_MESH` — `on` (default when replicas can exist) | `off` kill
    /// switch. When off, the relay must behave exactly like single-instance.
    pub enabled: bool,
    /// UDP bind for the iroh endpoint (`AMBUSH_MESH_BIND_ADDR`, default
...
pub enum MeshError {
    ...
    #[error("mesh is disabled (AMBUSH_MESH=off)")]
    Disabled,
    #[error("transport: {0}")]
    Transport(String),
    #[error("redis: {0}")]
