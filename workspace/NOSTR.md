# Using Third-Party Nostr Clients with Ambush

Ambush is a Nostr relay that speaks NIP-29 (relay-based groups) natively. Third-party Nostr clients connect directly to `ambush-relay` using NIP-29 and NIP-42 authentication. The old NIP-28 compatibility proxy has been removed.

## Community scope

Ambush treats the relay URL/domain as authoritative for the community. Today's
...
export AMBUSH_PUBKEY_ALLOWLIST=true
...
PGPASSWORD=ambush_dev psql -h localhost -U ambush -d ambush -c \
...
| **Edits (kind:40003)** | ⚠️ | Works on the wire but Ambush-only — no standard NIP-29 client renders these |
| **Rich content (kind:40002)** | ⚠️ | Works on the wire but Ambush-only — no standard NIP-29 client renders these |
...
When `AMBUSH_PUBKEY_ALLOWLIST=true`, NIP-42 connections that authenticate with only a pubkey
...
| Kind | Tags | Content |
|------|------|---------|
| **39000** | `d=<uuid>`, `name`, `closed` (always); `about` (if description non-empty); `private` (if applicable); `hidden` (DM channels only) | Group metadata. **Note:** `closed` is always emitted per NIP-29 convention (Ambush channels require explicit membership), but open channels are still readable/writable by non-members at runtime. The tag reflects the membership model, not access enforcement. |
...
|--------|----------|:--------:|-------|
| **AmbushTestClient** | Rust (repo) | Automated E2E | Full NIP-29 flow: discovery (39000/39001/39002), kind:9 send/receive, reactions, deletions, h-tag enforcement |
...
When `AMBUSH_REQUIRE_RELAY_MEMBERSHIP=true`, every authenticated connection is checked against the
...
Use `ambush-admin` — the operator CLI shipped in the relay image — to manage relay membership.
...
Or invoke `ambush-admin` directly inside the container:

```bash
docker compose exec relay ambush-admin add-member --pubkey npub1abc...
docker compose exec relay ambush-admin add-member --pubkey npub1abc... --role admin
docker compose exec relay ambush-admin remove-member --pubkey npub1abc...
docker compose exec relay ambush-admin list-members
```
...
| `AMBUSH_RELAY_PRIVATE_KEY` | Hex private key — required to sign kind:13534 events |
...
|----------|:--------:|---------|-------------|
| `AMBUSH_PUBKEY_ALLOWLIST` | ❌ | `false` | Enable pubkey allowlist for NIP-42 pubkey-only auth |
| `AMBUSH_RELAY_PRIVATE_KEY` | ❌ | random | Hex secret key for relay signing (discovery events, system messages) |
| `AMBUSH_REQUIRE_AUTH_TOKEN` | ❌ | `false` | Require authenticated NIP-42 for all connections |
...
- [`docs/nips/`](docs/nips/) — Ambush's own NIP extension documents.
