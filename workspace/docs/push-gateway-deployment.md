# Ambush Push Gateway deployment

`ambush-push-gateway` is the standalone public APNs last hop intended for `push.ambush.xyz`. Build it with `Dockerfile.push-gateway`; do not run it in the relay image or give relays APNs credentials.

## Network and health

- Public listener: `AMBUSH_PUSH_BIND_ADDR` (default `0.0.0.0:8080`). Route `https://push.ambush.xyz` to this port.
- Private health listener: `AMBUSH_PUSH_HEALTH_ADDR` (default `0.0.0.0:8081`). Probe `/_liveness` and `/_readiness`; do not expose this port publicly. The chart has no pod-ingress allowance for 8081; Kubernetes node/kubelet-origin probe traffic is exempt from NetworkPolicy. Add a narrowly selected monitoring source only if the target CNI requires pod-origin health scraping.
...
| `AMBUSH_PUSH_PUBLIC_DELIVERY_URL` | Exact externally signed URL, normally `https://push.ambush.xyz/v1/deliveries/apns`. |
| `AMBUSH_PUSH_MAX_GRANT_LIFETIME_SECONDS` | Maximum delegation capability lifetime (`1..=31536000`). |
| `AMBUSH_PUSH_MAX_INSTALLATION_LIFETIME_SECONDS` | Maximum encrypted-token installation lifetime (default 90 days, max one year). Clients must renew before expiry. |
| `AMBUSH_PUSH_APP_ATTEST_ROOT_CERT_PATH` | Read-only mounted Apple App Attest root certificate PEM. |
| `AMBUSH_PUSH_DOGFOOD_APP_ATTEST_APP_ID` | Exact server-owned Apple App Attest application identifier (`TEAMID.bundle-id`). |
| `AMBUSH_PUSH_DOGFOOD_APNS_TOPIC` | Server-owned APNs topic. Never accepted from a client. |
| `AMBUSH_PUSH_DOGFOOD_APNS_ENVIRONMENT` | `production` or `sandbox`, selected by deployment configuration. |
| `AMBUSH_PUSH_DOGFOOD_APNS_CERT_PATH` | Read-only certificate/private-key PEM. |
| `AMBUSH_PUSH_GRANT_KEYS` | Capability AEAD keyring, `id:base64-32-bytes[,predecessor...]`; current key first. |
| `AMBUSH_PUSH_TOKEN_KEYS` | Independent token-custody AEAD keyring in the same format. Never reuse grant keys. |

The canonical `push.ambush.xyz` MVP serves the dogfood application identity
(`com.backbay.ambush.dogfood.mobile`). App Attest must cryptographically validate
...
`ambush-ios-dogfood`. The chart value
`profiles.dogfood.appAttestAppId` is rendered as
`AMBUSH_PUSH_DOGFOOD_APP_ATTEST_APP_ID`; the gateway rejects startup when it is
...
environment rather than baking an Apple team identifier into this repository.
...
Optional endpoint quota policy variables are `AMBUSH_PUSH_ENDPOINT_QUOTA_WINDOW_SECONDS` (default `10`, max `86400`) and `AMBUSH_PUSH_ENDPOINT_QUOTA_MAX_DELIVERIES` (default `10`, max `10000`). These are Ambush policy hypotheses, not Apple-published limits; tune under load while retaining a hard ceiling.
...
|---|---|---|---|
| `PushGatewayConfigurationFault` | any `configuration_fault` outcomes for 10m | critical | The APNs certificate/topic/environment is unhealthy. Check `AMBUSH_PUSH_DOGFOOD_APNS_*` configuration. No endpoints are being invalidated. |
...
Relay push is an explicit deployment opt-in through `AMBUSH_PUSH_ENABLED=true`;
the established strict boolean parser rejects unknown values and the default is
false. When enabled, an absent `AMBUSH_PUSH_GATEWAY_DELIVERY_URL` selects the exact
canonical URL `https://push.ambush.xyz/v1/deliveries/apns`; operators can provide
...
| Metric | Type | Labels | Meaning |
|---|---|---|---|
| `ambush_push_enabled` | gauge | — | `1` only when the deployment opt-in is active. |
| `ambush_push_match_jobs_total` | counter | `result` = `matched` \| `unmatched` \| `error` \| `context_error` | Accepted message events evaluated by the matcher. |
| `ambush_push_match_queue_seconds` | histogram | — | Relay receipt to matcher evaluation latency. |
| `ambush_push_wakes_total` | counter | `result` = `enqueued` \| `duplicate` \| `inactive_lease` | Durable wake-enqueue outcomes. |
| `ambush_push_wake_enqueue_errors_total` | counter | — | Set-wise outbox transactions that failed. |
| `ambush_push_wake_queue_seconds` | histogram | — | First-attempt outbox enqueue-to-worker latency. |
| `ambush_push_gateway_requests_total` | counter | — | Relay requests that reached the gateway transport seam. |
| `ambush_push_gateway_request_seconds` | histogram | — | Relay-observed gateway request latency. |
| `ambush_push_deliveries_total` | counter | `outcome` (static closed set) | Accepted, retried, suppressed, exhausted, invalid, or failed relay delivery outcomes. |
...
delivery run only when `AMBUSH_PUSH_ENABLED=true`. End-to-end use still requires
...
selected internal relay deployments set `AMBUSH_PUSH_ENABLED=true`. Every iOS
...
enroll `com.backbay.ambush.mobile` or another bundle identifier merely by changing
...
`origin/main` commit, build that tag through the normal mobile release pipeline,
and wait for the signed `com.backbay.ambush.dogfood.mobile` artifact to appear in
...
profiles for both `com.backbay.ambush.dogfood.mobile` and
`com.backbay.ambush.dogfood.mobile.NotificationService`; an app-only profile does
...
updating Apple portal records is a separately authorized release step.
...
Set `AMBUSH_PUSH_ENABLED=false` on the enabled relays to stop advertisement, lease
...
gh attestation verify \
  oci://ghcr.io/backbay-labs/ambush-push-gateway@sha256:<64-lowercase-hex> \
  --owner block
```

Only after that command succeeds, set the exact digest as `image.digest`; the chart then renders `ghcr.io/backbay-labs/ambush-push-gateway@sha256:...` and ignores the mutable tag. `values-production.yaml` is an intentionally invalid production-input contract: deployment CI must inject this verified `image.digest`, the provisioned dogfood Apple application identifier, an environment-owned Gateway parent reference, and the actual PostgreSQL network. Schema validation rejects the artifact when any remains empty; the render guard proves both rejection and a fully injected render.
...
Kubernetes does not restart pods when referenced Secret bytes change. AEAD or APNs certificate rotation therefore requires an explicit rolling restart after the secret manager update (for example, `kubectl rollout restart deployment/<release>-ambush-push-gateway`) and readiness verification before removing predecessor keys. Service-account token automount is disabled.

## Gateway chart release

The gateway chart has a collision-free release lane separate from the main
`ambush` chart. To publish version `X.Y.Z`, update both `version` and `appVersion`
in `deploy/charts/ambush-push-gateway/Chart.yaml`, validate the chart, and open a
same-repository PR whose branch is exactly `push-chart-release/X.Y.Z`:

```bash
deploy/charts/ambush-push-gateway/tests/render.sh
git switch -c push-chart-release/X.Y.Z
git add deploy/charts/ambush-push-gateway/Chart.yaml
...
`oci://ghcr.io/backbay-labs/ambush/charts/ambush-push-gateway`. A manually pushed
