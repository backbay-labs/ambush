...
- Slice 0 — HTTPS relay git clone URLs (`{relay-origin}/git/<pubkey>/<repo>`)
  render as Ambush repository preview cards in chat
  (`desktop/src/shared/lib/linkPreview.ts`).
- Slice 1 — `ambush://pr|issue|repo|project` deep links: `entityLink.ts`
...
- Slice 2 — OS-level deep links: the `repo`/`project`/`pr`/`issue` hosts in
  ...
  clicks.
- Slice 3 (create-command part) — `crates/ambush-cli/src/links.rs`, `link`
...
preview card ("GitHub · PR backbay-labs/ambush #4020") below the message. Those cards
...
Ambush-hosted entities have no equivalent. There is **no link format at all**
for an Ambush repository, project, pull request, or issue:

- The only rich deep link today is `ambush://message?channel=…&id=…`
...
- `ambush pr open` / `ambush issues create` return raw event ids; there is no URL
  in their output and no guidance in the agent base prompt
  (`crates/ambush-acp/src/base_prompt.md`) for referencing Ambush work items in
  chat. Agents can only say "PR up" with a hex id.
- The relay-served web client only has `/repos/$repoId`; no PR/issue pages.

So an agent that opens a PR on an Ambush-hosted repository cannot produce
anything clickable, while the same agent opening a GitHub PR gets a card for
free.

## Goals

1. A canonical, shareable link format for Ambush repositories, projects, pull
   requests, and issues.
...
- Web (browser) pages for PRs/issues — the web client has no such views yet,
  so links are app-only, same as `ambush://message` today.
- Cross-community links. Like `ambush://message`, links are interpreted against
...
Extend the existing `ambush://` scheme, mirroring `ambush://message`:

```
ambush://repo?owner=<pubkey-hex>&d=<repo-dtag>[&tab=<tab>]
ambush://project?owner=<pubkey-hex>&d=<project-dtag>[&tab=<tab>]
ambush://pr?id=<event-id-hex>&owner=<pubkey-hex>&d=<repo-dtag>
ambush://issue?id=<event-id-hex>&owner=<pubkey-hex>&d=<repo-dtag>
```
...
the canonical `ambush://repo?owner=…&d=…` deep link (the raw transport URL is
...
no such routes), which is why they use the `ambush://` scheme above: it is
community-relative by construction, matches the established `ambush://message`
...
1. **Autolinked bare URL** (`<ambush://pr?…>` or bare in text): render an
   **attachment card** below the message in the existing `AttachmentGroup`,
   exactly like GitHub cards. Provider label `Ambush`, type label
   `PR` / `issue` / `repo` / `project`.
2. **Explicitly labeled markdown link** (`[fix the tooltip](ambush://pr?…)`):
...
Unlike GitHub (title derived from URL path only), Ambush entities live on the
...
`ambush://` pattern (new `SupportedLinkPreviewKind` members
`ambush-pull-request`, `ambush-issue`, `ambush-repository`, `ambush-project`), or —
...
- `ambush pr open` → `{ event_id, accepted, message, link }`
- `ambush issues create` → same
- `ambush repos create` → link built from owner pubkey + `d`-tag
- `ambush projects create` → same

The builder lives in one Rust helper (e.g. `crates/ambush-cli/src/links.rs`)
...
`ambush pr get` / `ambush issues get` / `ambush repos get` also include `link` in
their output so agents can link to existing entities, not just ones they
just created.

## Agent guidance

One addition to `crates/ambush-acp/src/base_prompt.md`, next to the existing
...
0. **HTTPS clone-URL repo cards** *(done, this branch)* — recognize relay
   `/git/<pubkey>/<repo>` URLs in `linkPreview.ts`, `Ambush` provider card
   with the `AmbushMark` logo, href normalized to the `ambush://repo` deep link
   for in-app navigation.
1. **Link core + cards** *(done, this branch)* — `entityLink.ts`, detection
   in `linkPreview.ts`, `Ambush` card variant in
