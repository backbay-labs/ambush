...
Run from the `backbay-labs/ambush` repository root on the mesh-enabled branch.
...
`~/.ambush-dev`, and local Docker volumes; it preserves the installed Ambush app's
data, production keyring, and `~/.ambush`. The first dev page load also clears
...
If using `mesh-dev-fresh`, the clean window opens at **Welcome to Ambush**. Join
the seeded local community before continuing:

1. Click **Join a community**.
2. Use any local name, such as **Local Ambush**.
...
Stop only stale Ambush/MeshLLM processes shown by that command. Do not leave a
...
take several minutes. Wait for the Ambush window to open and for the terminal to
...
**ambush-agent** as the runtime, **Ambush shared compute** as the LLM provider,
...
Ambush publishes member-signed discovery notes through an ordinary relay-supported
...
tokens in the note. Current Ambush membership controls which node identities are
...
`AMBUSH_MESH_IROH_RELAYS` defaults to Iroh's production relay set. Set it to `0`
...
MeshLLM—not the Ambush relay—carries inference over direct QUIC or its encrypted
...
is an upstream protocol change and is not claimed by the Ambush-side checks above.
