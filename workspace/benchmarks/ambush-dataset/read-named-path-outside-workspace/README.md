...
reading it. See backbay-labs/ambush#6261.

## Environment

`python:3.12-slim-bookworm` with `HOME=/home/ambush`, so `~` in the instruction
...
Reads the post-agent `/logs/artifacts/ambush-evidence.json` snapshot plus the
...
```bash
just benchmark \
  --path benchmarks/ambush-dataset/read-named-path-outside-workspace \
  --attempts 1 \
...
`../harbor-ambush-orchestra/tests/test_read_named_path_outside_workspace_verifier.py`.
