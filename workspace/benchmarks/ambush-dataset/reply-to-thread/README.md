...
> the behavior under test, and it must come from `ambush-acp`'s production base
...
container's shell. `AmbushOrchestraAgent` launches the real `ambush-acp` /
`ambush-agent` stack against a dedicated relay, and the agent works entirely
through Ambush. Agent timeout 300s; the manifest's `trial_budget` is the
effective clock.

## Verifier

Reads the post-agent `/logs/artifacts/ambush-evidence.json` snapshot (written by
`AmbushContainerRuntime._collect_evidence` after the agent stops, so the agent
...
shipped: the Oracle agent replaces `AmbushOrchestraAgent`, so no relay trial is
...
`../harbor-ambush-orchestra/tests/test_reply_to_thread_verifier.py` (run from
`benchmarks/harbor-ambush-orchestra`: `uv run --extra dev pytest -q`).
