...
gets a real Ambush notification instead of a message they have to notice.

> **The instruction deliberately says nothing about mentioning anyone.** The
> mention is the behavior under test and must come from `ambush-acp`'s production
...
`ambush-agent` stack against a dedicated relay. Agent timeout 300s.

## Verifier

Reads the post-agent `/logs/artifacts/ambush-evidence.json` snapshot. Every
...
```bash
just benchmark \
  --path benchmarks/ambush-dataset/user-mention \
  --attempts 1 \
...
`../harbor-ambush-orchestra/tests/test_user_mention_verifier.py`.
