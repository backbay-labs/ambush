"""Thin subprocess wrapper over the ``ambush`` CLI — the production client path."""
...
class AmbushCliError(RuntimeError):
    """A ambush CLI invocation failed."""


class AmbushCli:
    """Run ambush CLI commands as one relay identity (key + NIP-OA auth tag)."""
    ...
    def run(self, *args: str) -> Any:
        """Run a ambush subcommand and return its parsed JSON stdout."""
        ...
        try:
            completed = subprocess.run(
                ...
                check=False,
                env={
                    "AMBUSH_RELAY_URL": self._relay_url,
                    "AMBUSH_PRIVATE_KEY": self._secret_key,
                    "AMBUSH_AUTH_TAG": self._auth_tag,
                    "PATH": _path(),
                },
            )
        except (OSError, subprocess.TimeoutExpired) as error:
            raise AmbushCliError(f"ambush {args[0]}: {error}") from error
        if completed.returncode != 0:
            raise AmbushCliError(
                f"ambush {' '.join(args)} exited {completed.returncode}: "
        ...
        try:
            ...
            raise AmbushCliError(
                f"ambush {args[0]} returned non-JSON output: {completed.stdout[:200]!r}"
            ) from error
    ...
    def create_private_channel(self, name: str, description: str) -> str:
        ...
        if not channel_id:
            raise AmbushCliError(f"channel create returned no channel_id: {response}")
        return channel_id
