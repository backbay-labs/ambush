"""Channel-per-trial Ambush provisioning against a local benchmark stack."""
...
from harbor_ambush_orchestra.task_fixtures import DirectoryEntry, fixture_for

from .ambush_cli import AmbushCli
...
def provisioner_from_dict(config: dict[str, object]) -> AmbushTrialProvisioner:
    """Harbor CLI factory for a JSON-decoded testbed configuration."""
    return AmbushTrialProvisioner(TestbedConfig(**config))


class AmbushTrialProvisioner:
    """Implements the TrialHandle v1.1 contract against a live Ambush relay.

    Guarantees (contract PLANS/HARBOR_AMBUSH_TRIALHANDLE_CONTRACT.md):
    ...
    def _provision(
        ...
        # mirroring production Ambush, where a human owns the channel their
        # agents work in.
    ...
    def _seed_directory(
        self, task_name: str | None, observer: AmbushCli
    ...
    def _stable_credential(
        ...
        digest = hashlib.sha256(
            b"ambush-benchmark-directory-v1\0"
    ...
    def _cli_for(self, credential: AgentCredential) -> AmbushCli:
        return AmbushCli(
