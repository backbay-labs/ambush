...
def test_ambush_task_metadata_defines_the_expected_layers():
    tasks = benchmark.ambush_tasks_for_path(benchmark.AMBUSH_DATASET_ROOT)
...
def test_ambush_task_metadata_rejects_missing_or_unknown_layers(
    tmp_path, monkeypatch, layer
):
    dataset = tmp_path / "ambush-dataset"
    task = dataset / "example"
    task.mkdir(parents=True)
    metadata = "" if layer is None else f'evaluation_layer = "{layer}"\n'
    (task / "task.toml").write_text(
        'schema_version = "1.3"\n\n'
        '[task]\nname = "ambush-native/example"\n\n'
        f'[metadata]\n{metadata}difficulty = "easy"\n'
    )
    monkeypatch.setattr(benchmark, "AMBUSH_DATASET_ROOT", dataset)

    with pytest.raises(SystemExit, match="evaluation_layer|metadata"):
        benchmark.ambush_tasks_for_path(dataset)
...
def test_layer_selects_metadata_and_uses_its_default_attempts(layer, attempts):
    args = benchmark.parse_args(
        ["--path", str(benchmark.AMBUSH_DATASET_ROOT), "--layer", layer]
    )
    (run,) = benchmark.plan_benchmark_runs(args)

    assert run.attempts == attempts
    selected = benchmark.ambush_tasks_for_path(benchmark.AMBUSH_DATASET_ROOT)
...
def test_layer_selectors_resolve_through_harbor_local_dataset_filter(layer):
    ...
    configs = asyncio.run(
        DatasetConfig(
            path=benchmark.AMBUSH_DATASET_ROOT,
...
def test_omitted_layer_splits_ambush_dataset_into_two_jobs():
    args = benchmark.parse_args(["--path", str(benchmark.AMBUSH_DATASET_ROOT)])
...
def test_single_ambush_task_infers_its_layer_default():
    task_path = benchmark.AMBUSH_DATASET_ROOT / "cross-thread-requests"
...
def test_explicit_attempts_override_keeps_one_mixed_ambush_job():
    args = benchmark.parse_args(
        ["--path", str(benchmark.AMBUSH_DATASET_ROOT), "--attempts", "7"]
    )
    (run,) = benchmark.plan_benchmark_runs(args)
    ...
    layered = benchmark.parse_args(
        [
            "--path",
            str(benchmark.AMBUSH_DATASET_ROOT),
            "--layer",
            "workflow",
            "-k",
            "2",
...
def test_layer_dry_run_constructs_exact_task_selectors_and_attempts():
    ...
    binaries = {"ambush": Path("host-bin/ambush")}
    agent_binaries = {
...
def test_env_file_wires_owner_and_ports(state_dir):
    ...
    assert env["AMBUSH_HTTP_PORT"] == str(benchmark.RELAY_HTTP_PORT)
    assert env["AMBUSH_PG_HOST_PORT"] == str(benchmark.PG_HOST_PORT)
    assert env["AMBUSH_REQUIRE_RELAY_MEMBERSHIP"] == "true"


def test_compose_command_isolates_the_project(state_dir):
    command = benchmark.compose_command("up", "-d")
    assert command[:2] == ["docker", "compose"]
    assert command[command.index("--project-name") + 1] == "ambush-benchmark"
