...
Gated by AMBUSH_TESTBED_LIVE=1 with stack coordinates in the environment:
  AMBUSH_TESTBED_RELAY_HTTP   (default http://localhost:3000)
  AMBUSH_TESTBED_RELAY_WS     (default ws://host.docker.internal:3000)
  AMBUSH_TESTBED_OWNER_KEY    relay owner secret key (hex)
  AMBUSH_TESTBED_PG_DSN       benchmark Postgres DSN
"""
...
from harbor_ambush_testbed.ambush_cli import AmbushCli, AmbushCliError
from harbor_ambush_testbed.provisioner import (
...
pytestmark = pytest.mark.skipif(
    os.environ.get("AMBUSH_TESTBED_LIVE") != "1",
    reason="live testbed suite; set AMBUSH_TESTBED_LIVE=1 with a running stack",
)


@pytest.fixture()
def provisioner() -> AmbushTrialProvisioner:
    owner_key = os.environ.get("AMBUSH_TESTBED_OWNER_KEY")
    dsn = os.environ.get("AMBUSH_TESTBED_PG_DSN")
    if not owner_key or not dsn:
        pytest.fail("AMBUSH_TESTBED_OWNER_KEY and AMBUSH_TESTBED_PG_DSN are required")
    return AmbushTrialProvisioner(
        TestbedConfig(
            relay_http_url=os.environ.get(
                "AMBUSH_TESTBED_RELAY_HTTP", "http://localhost:3000"
            ),
            relay_ws_url=os.environ.get(
                "AMBUSH_TESTBED_RELAY_WS", "ws://host.docker.internal:3000"
            ),
...
def cli_for(provisioner: AmbushTrialProvisioner, credential) -> AmbushCli:
...
def test_create_is_idempotent_and_isolated(provisioner, manifest):
    ...
    try:
        ...
        with pytest.raises(AmbushCliError, match="private"):
    ...
    provisioner.teardown(handle_a)
    with psycopg.connect(os.environ["AMBUSH_TESTBED_PG_DSN"]) as conn:
        row = conn.execute(
