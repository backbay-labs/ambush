...
from harbor_ambush_orchestra import (
    AgentCredential,
...
async def test_missing_integrations_fail_explicitly(tmp_path, manifest_data):
    agent = AmbushOrchestraAgent(logs_dir=tmp_path, manifest=manifest_data)
...
async def test_cli_runtime_construction_from_json(tmp_path, manifest_data):
    ...
    agent = AmbushOrchestraAgent(
        ...
        ambush_acp_binary="/pinned/ambush-acp",
        ambush_agent_binary="/pinned/ambush-agent",
        ambush_dev_mcp_binary="/pinned/ambush-dev-mcp",
        ambush_cli_binary="/pinned/ambush",
    )
    assert agent.runtime.artifact_root == tmp_path
    assert agent.runtime.endpoints["frontier/rev"].provider == "anthropic"
    assert agent.runtime.ambush_acp_binary == "/pinned/ambush-acp"
    assert agent.runtime.ambush_agent_binary == "/pinned/ambush-agent"
    assert agent.runtime.ambush_dev_mcp_binary == "/pinned/ambush-dev-mcp"
    assert agent.runtime.ambush_cli_binary == "/pinned/ambush"
