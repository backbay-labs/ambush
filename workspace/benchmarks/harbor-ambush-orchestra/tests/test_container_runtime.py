...
from harbor_ambush_orchestra.container_runtime import (
    REMOTE_BIN,
    REMOTE_EVIDENCE,
    REMOTE_LOGS,
    THINKING_EFFORT,
...
from harbor_ambush_orchestra.manifest import ExperimentManifest
from harbor_ambush_orchestra.provisioning import (
    AgentCredential,
    FixtureActor,
    TrialHandle,
)
from harbor_ambush_orchestra.task_fixtures import fixture_for
...
def test_prompt_hash_and_identity_override_are_fail_closed(tmp_path):
    ...
    endpoint = EndpointLaunchConfig(
        "anthropic", "ANTHROPIC_API_KEY", {"AMBUSH_ACP_MCP_COMMAND": "evil"}
    )
...
async def test_collects_task_declared_channel_membership(tmp_path, monkeypatch):
    ...
    async def ambush_json(credential_arg, trial_arg, *args):
    ...
    monkeypatch.setattr(rt, "_ambush_json", ambush_json)
...
async def test_install_stack_uploads_the_pinned_stack(tmp_path):
    binaries = {}
    for name in ("ambush-acp", "ambush-agent", "ambush-dev-mcp"):
        path = tmp_path / name
        path.write_text("#!binary")
        binaries[name] = str(path)
    rt = runtime(
        tmp_path,
        ambush_acp_binary=binaries["ambush-acp"],
        ambush_agent_binary=binaries["ambush-agent"],
        ambush_dev_mcp_binary=binaries["ambush-dev-mcp"],
    )
    environment = Environment()
    await rt._install_stack(environment)
    assert {target for _, target in environment.uploads} == {
        f"{REMOTE_BIN}/ambush-acp",
        f"{REMOTE_BIN}/ambush-agent",
        f"{REMOTE_BIN}/ambush-dev-mcp",
    }
...
async def test_install_stack_requires_binaries_on_disk(tmp_path):
    rt = runtime(tmp_path, ambush_acp_binary=str(tmp_path / "missing"))
...
async def test_forwarder_bridges_the_canonical_relay_address(tmp_path):
    from harbor_ambush_orchestra.container_runtime import FORWARDER
...
async def test_launch_wires_the_desktop_environment(tmp_path, configured, expected):
    ...
    environment = Environment(
        responses={"ambush-acp": ExecResult(stdout="4242\n", stderr="", return_code=0)}
    )
    ...
    assert f"{REMOTE_BIN}/ambush-acp" in command
    # The real product wiring: acp spawns ambush-agent, which gets ambush-dev-mcp.
    assert env["AMBUSH_ACP_AGENT_COMMAND"] == f"{REMOTE_BIN}/ambush-agent"
    assert env["AMBUSH_ACP_MCP_COMMAND"] == f"{REMOTE_BIN}/ambush-dev-mcp"
    assert env["AMBUSH_RELAY_URL"] == trial.relay_ws_url
    assert env["AMBUSH_PRIVATE_KEY"] == orch.nostr_secret_key
    assert env["NOSTR_PRIVATE_KEY"] == orch.nostr_secret_key
    assert env["AMBUSH_AGENT_NO_HINTS"] == "1"
    assert env["AMBUSH_AGENT_MAX_ROUNDS"] == expected
    assert env["AMBUSH_ACP_SYSTEM_PROMPT_FILE"].endswith("orch-1.system-prompt.md")
    # The composed prompt was uploaded into the container.
    assert any(
        target == env["AMBUSH_ACP_SYSTEM_PROMPT_FILE"]
        for _, target in environment.uploads
    )


def test_runtime_validates_construction_bounds(tmp_path):
    # 0 is legal and means unbounded (AMBUSH_AGENT_MAX_ROUNDS=0); the trial
...
async def test_wait_for_agents_ready_requires_every_channel_subscription(tmp_path):
    ...
    from harbor_ambush_orchestra.container_runtime import _Agent
...
async def test_send_mentions_by_pubkey_so_task_text_stays_inert(tmp_path, monkeypatch):
    ...
    async def ambush_json(credential, trial, *args):
        calls.append(args)
        return {}
...
async def test_wait_for_done_requires_orchestrator_authorship(tmp_path, monkeypatch):
    ...
    async def ambush_json(credential, *args, **kwargs):
...
async def test_solo_turn_end_completes_without_done_message(tmp_path, monkeypatch):
    ...
    async def ambush_json(*args, **kwargs):
        return []
...
def test_turn_status_parses_completed_batch_and_successful_steer_receipts():
    ...
    output = "\n".join(
        [
            ...
            f"turn delivered Ambush events for channel test: {batch_event_id}",
    ...
    assert AmbushContainerRuntime._parse_turn_status(output) == (
        1,
        1,
...
async def test_collect_evidence_uploads_verifier_artifact(tmp_path, monkeypatch):
    ...
    evidence = json.loads((trial_dir / "ambush-evidence.json").read_text())
...
async def test_failed_evidence_snapshot_records_the_reason(tmp_path, monkeypatch):
    ...
    assert "relay unreachable" in (trial_dir / "ambush-evidence-error.txt").read_text()
    assert not (trial_dir / "ambush-evidence.json").exists()


def test_runtime_logging_keeps_readiness_and_turn_completion_signals(tmp_path):
    rt = runtime(tmp_path)
    assert rt._rust_log(None) == "ambush_acp=info,pool::prompt=info"
    assert rt._rust_log("custom=debug") == (
        "custom=debug,ambush_acp=info,pool::prompt=info"
    )
...
async def test_stop_agents_sweeps_the_uploaded_stack(tmp_path):
    ...
    await AmbushContainerRuntime._stop_agents(environment, agents)
...
async def test_thinking_effort_reaches_the_agent(tmp_path, pinned, expected):
    ...
    assert env["AMBUSH_AGENT_THINKING_EFFORT"] == expected
