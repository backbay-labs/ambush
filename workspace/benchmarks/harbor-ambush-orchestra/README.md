# Harbor Ambush Orchestra

A stock-Harbor custom agent that runs a manifest-defined team through the real
Ambush stack. Harbor sees one `AmbushOrchestraAgent`; behind that adapter, one
...
`ambush-acp` → `ambush-agent` → `ambush-dev-mcp` process tree the desktop app
launches: the production MCP toolset (shell, file tools, todo) with the
`ambush` CLI on the shell's PATH. No Harbor fork or patch is required.
...
```bash
uv run --project benchmarks/harbor-ambush-orchestra/testbed harbor run --yes -p <TASK_OR_DIRECTORY> --agent harbor_ambush_orchestra:AmbushOrchestraAgent --agent-kwarg manifest=<CONDITION.yaml> --agent-kwarg provisioner_factory=harbor_ambush_testbed:provisioner_from_dict --agent-kwarg provisioner_config=<PROVISIONER.json> --agent-kwarg endpoint_config=<ENDPOINTS.json> --agent-kwarg artifact_root=benchmarks/harbor-ambush-orchestra --agent-kwarg ambush_acp_binary=<LINUX_BIN>/ambush-acp --agent-kwarg ambush_agent_binary=<LINUX_BIN>/ambush-agent --agent-kwarg ambush_dev_mcp_binary=<LINUX_BIN>/ambush-dev-mcp --agent-kwarg ambush_cli_binary=target/debug/ambush --agent-kwarg run_id="bench-$(date -u +%Y%m%dT%H%M%SZ)" --agent-timeout-multiplier 15 --n-concurrent 1
```

`ambush_acp_binary`/`ambush_agent_binary`/`ambush_dev_mcp_binary` must be **Linux**
...
any Linux base image works). `ambush_cli_binary` is the **host** CLI the harness
...
Each trial gets fresh keys and a private Ambush channel. The provisioner archives
rather than deletes that channel, leaving the relay/Postgres event timeline
and the per-agent acp/agent logs (downloaded into the trial's `ambush/`
artifacts) available for analysis.

### Ambush-native tasks

The local [`benchmarks/ambush-dataset`](../ambush-dataset) suite — a sibling
directory of this harness, not a subdirectory of it — scores Ambush product
...
```bash
just benchmark \
  --path benchmarks/ambush-dataset/reply-to-thread \
  --manifest benchmarks/harbor-ambush-orchestra/manifests/ambush-native-solo-luna.yaml \
  --endpoint-config benchmarks/harbor-ambush-orchestra/testbed/endpoints/openai-live.json \
  --n-concurrent 1
```

Ambush-native tasks declare one of two evaluation layers in `task.toml`.
...
`--path benchmarks/ambush-dataset --layer regression` or `--layer workflow`;
task identities stay unchanged.

When the Ambush dataset root is passed without `--layer` or `--attempts`, the
...
The default condition is `ambush-native-solo-luna.yaml` — one solo agent on
...
`anthropic-live.json`. Swap in `ambush-native-solo-sonnet.yaml` (no
...
Replace the path with `benchmarks/ambush-dataset/create-channel-invite-users`
...
`/logs/artifacts/ambush-evidence.json`. The task verifier reads that post-agent
...
cause is written to the trial's `ambush/ambush-evidence-error.txt`.
...
the tasks whose graded Ambush behavior is deliberately absent from
...
stack (`ambush-benchmark` compose project — relay :3600, Postgres :5633, secrets
...
just benchmark --path benchmarks/ambush-dataset --layer regression   # Ambush k=1
just benchmark --path benchmarks/ambush-dataset --layer workflow     # Ambush k=3
...
relay membership list and opens the Ambush desktop app logged in as them, so
...
uv run --project benchmarks/harbor-ambush-orchestra/testbed \
    benchmarks/harbor-ambush-orchestra/scripts/run_leaderboard.py \
    --dataset terminal-bench/terminal-bench-2-1 \
    --attempts 5 \
    --manifest benchmarks/harbor-ambush-orchestra/manifests/<TEAM>.yaml \
    --endpoint-config benchmarks/harbor-ambush-orchestra/testbed/endpoints/<ENDPOINTS>.json \
...
```bash
cd benchmarks/harbor-ambush-orchestra
