#!/usr/bin/env python3
"""One-command benchmark: bring up the Ambush stack in Docker and run it.
...
Ambush-native tasks use their ``evaluation_layer`` metadata: regression runs
default to 1 attempt and workflow runs default to 3. The script owns
everything around the run:

- A dedicated ``ambush-benchmark`` compose project reusing the production
...
- ``--gui`` adds that user to the relay membership list and opens the Ambush
...
Run inside the testbed environment (the just recipe does this):
    ...
    uv run --project benchmarks/harbor-ambush-orchestra/testbed \
        benchmarks/harbor-ambush-orchestra/scripts/benchmark.py [--gui] [...]
"""
...
COMPOSE_PROJECT = "ambush-benchmark"
COMPOSE_FILES = (
...
GUI_BUNDLE_IDENTIFIER = "com.backbay.ambush.app.benchmark"

DEFAULT_DATASET = "terminal-bench/terminal-bench-2-1"
DEFAULT_ATTEMPTS = 5
AMBUSH_DATASET_ROOT = REPO_ROOT / "benchmarks" / "ambush-dataset"
...
AGENT_BINARIES = ("ambush-acp", "ambush-agent", "ambush-dev-mcp")
...
class AmbushTask:
    """The identity and evaluation layer declared by one Ambush task."""
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Ambush evaluation layer to run (selected from task metadata)",
    )
    parser.add_argument(
        "--attempts",
        "-k",
        type=int,
        default=None,
        help="Runs per problem (default: Terminal-Bench 5, Ambush regression 1, "
        "Ambush workflow 3)",
    )
    parser.add_argument(
        "--manifest",
    ...
    parser.add_argument(
        "--gui",
        action="store_true",
        help="Open the Ambush desktop app as the benchmark user to watch the run live",
    )
    parser.add_argument(
        "--fresh",
        action="store_true",
...
def _read_ambush_task(task_toml: Path) -> AmbushTask:
    ...
    try:
        ...
        raise SystemExit(
            f"invalid Ambush task metadata in {task_toml}: {error}"
        ) from error
    if not isinstance(name, str) or not name:
        raise SystemExit(f"invalid Ambush task name in {task_toml}: expected a string")
    ...
    return AmbushTask(name=name, layer=layer, path=task_toml.parent)


def ambush_tasks_for_path(path: Path | None) -> tuple[AmbushTask, ...] | None:
    """Return validated Ambush tasks, or ``None`` for an unrelated problem set."""
    if path is None:
        return None
    root = AMBUSH_DATASET_ROOT.resolve()
    ...
    if not task_files:
        raise SystemExit(f"no Ambush tasks found under {path}")
    tasks = tuple(_read_ambush_task(task_file) for task_file in task_files)
    names = [task.name for task in tasks]
    if len(names) != len(set(names)):
        raise SystemExit(f"duplicate Ambush task names found under {path}")
    return tasks


def _matches_task(task: AmbushTask, pattern: str) -> bool:
    return fnmatch.fnmatchcase(task.name, pattern) or fnmatch.fnmatchcase(
        task.path.name, pattern
    )


def select_ambush_tasks(
    tasks: tuple[AmbushTask, ...],
    *,
    layer: str | None,
    include: list[str],
    exclude: list[str],
    ...
    if not selected:
        ...
        raise SystemExit(f"no Ambush tasks selected{detail}")
    return selected
...
def plan_benchmark_runs(
    ...
    tasks = ambush_tasks_for_path(args.path)
    if args.layer and tasks is None:
        raise SystemExit(
            "--layer is only valid with --path under benchmarks/ambush-dataset"
        )
    if tasks is None:
        return (
            _copy_run_args(
...
def load_state() -> dict[str, str]:
    ...
    from harbor_ambush_testbed.keys import generate_keypair, keypair_from_secret
...
def print_user_identity(state: dict[str, str]) -> None:
    ...
    nothing beyond it)."""
    from harbor_ambush_testbed.keys import encode_nsec
...
def write_env_file(state: dict[str, str]) -> Path:
    ...
    lines = {
        "AMBUSH_IMAGE": os.environ.get(
            "AMBUSH_IMAGE", "ghcr.io/backbay-labs/ambush:main"
        ),
        "AMBUSH_DOMAIN": "localhost",
        "RELAY_URL": f"ws://localhost:{RELAY_HTTP_PORT}",
        "AMBUSH_MEDIA_BASE_URL": f"http://localhost:{RELAY_HTTP_PORT}/media",
        "AMBUSH_MEDIA_SERVER_DOMAIN": "localhost",
        "AMBUSH_CORS_ORIGINS": f"http://localhost:{RELAY_HTTP_PORT}",
        "AMBUSH_REQUIRE_AUTH_TOKEN": "true",
        "AMBUSH_REQUIRE_RELAY_MEMBERSHIP": "true",
        "AMBUSH_ALLOW_NIP_OA_AUTH": "true",
        "AMBUSH_AUTO_MIGRATE": "true",
        "AMBUSH_GIT_CONFORMANCE_PROBE": "true",
        "RUST_LOG": "ambush_relay=info,ambush_db=info,ambush_auth=info",
        "RELAY_OWNER_PUBKEY": state["owner_pubkey"],
        "AMBUSH_RELAY_PRIVATE_KEY": state["relay_private_key"],
        "AMBUSH_GIT_HOOK_HMAC_SECRET": state["git_hook_hmac_secret"],
        "POSTGRES_DB": "ambush",
        "POSTGRES_USER": "ambush",
        "POSTGRES_PASSWORD": state["postgres_password"],
        "REDIS_PASSWORD": state["redis_password"],
        "AMBUSH_S3_ACCESS_KEY": state["s3_access_key"],
        "AMBUSH_S3_SECRET_KEY": state["s3_secret_key"],
        "AMBUSH_S3_BUCKET": "ambush-media",
        "AMBUSH_HTTP_PORT": str(RELAY_HTTP_PORT),
        "AMBUSH_PG_HOST_PORT": str(PG_HOST_PORT),
        "AMBUSH_METRICS_HOST_PORT": str(METRICS_HOST_PORT),
    }
...
def postgres_dsn(state: dict[str, str]) -> str:
    return f"postgresql://ambush:{state['postgres_password']}@127.0.0.1:{PG_HOST_PORT}/ambush"
...
def ensure_binaries() -> dict[str, Path]:
    """Find the host ambush CLI, building it once if missing."""
    try:
        return run_leaderboard.find_binaries(None)
    except SystemExit:
        print("host ambush CLI missing — building (cargo build, first run only)...")
    cargo = REPO_ROOT / "bin" / "cargo"
    subprocess.run(
        [str(cargo), "build", "-p", "ambush-cli"],
        cwd=REPO_ROOT,
        check=True,
    )
    return run_leaderboard.find_binaries(None)
...
def ensure_agent_binaries() -> Path:
    ...
    ambush-acp → ambush-agent → ambush-dev-mcp stack, so the binaries must be
...
def launch_gui(state: dict[str, str]) -> subprocess.Popen:
    """Open the Ambush desktop app logged in as the benchmark user.

    The relay runs closed (membership required), so the user pubkey is first
    added to the relay membership list via ambush-admin inside the container —
    ...
    for name in (
        ...
        if not stub.exists():
            stub.touch()
    real_cli = sidecar_dir / f"ambush-{triple}"
    real_cli.write_bytes(binaries["ambush"].read_bytes())
    real_cli.chmod(0o755)

    print(
        f"Opening Ambush GUI as the benchmark user ({state['user_pubkey'][:16]}…).\n"
    ...
    # workspace's relay URL overrides AMBUSH_RELAY_URL by design. Reusing the
    ...
    tauri_config = json.dumps(
        {"identifier": GUI_BUNDLE_IDENTIFIER, "productName": "Ambush Benchmark"}
    )
    return subprocess.Popen(
        ...
        env={
            **os.environ,
            "AMBUSH_RELAY_URL": f"ws://localhost:{RELAY_HTTP_PORT}",
            "AMBUSH_PRIVATE_KEY": state["user_secret_key"],
        },
    )
...
def leaderboard_argv(
    ...
    argv += [
        ...
        # AMBUSH_BENCHMARK_DOCKER_HOST if your engine exposes the host
        # differently.
        "--relay-gateway",
        (
            f"{os.environ.get('AMBUSH_BENCHMARK_DOCKER_HOST', 'host.docker.internal')}"
            f":{RELAY_HTTP_PORT}"
        ),
        "--n-concurrent",
