...
importable:
    ...
    uv run --project benchmarks/harbor-ambush-orchestra/testbed \
        ...
        --agent-bin-dir <DIR with Linux ambush-acp/ambush-agent/ambush-dev-mcp>
"""
...
AGENT_IMPORT = "harbor_ambush_orchestra:AmbushOrchestraAgent"
PROVISIONER_FACTORY = "harbor_ambush_testbed:provisioner_from_dict"
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        required=True,
        help="JSON config for the Ambush relay/Postgres provisioner",
    )
    parser.add_argument(
        "--ambush-bin-dir",
        type=Path,
        default=None,
        help="Directory with the host ambush CLI (default: repo target/release, then target/debug)",
    )
    parser.add_argument(
        "--agent-bin-dir",
        type=Path,
        required=True,
        help="Directory with Linux builds of ambush-acp/ambush-agent/ambush-dev-mcp "
...
def find_binaries(bin_dir: Path | None) -> dict[str, Path]:
    ...
    raise SystemExit(
        f"ambush binaries not found (need {', '.join(BINARIES)}; searched {searched}). "
        "Build them with `cargo build` or pass --ambush-bin-dir."
    )
...
def build_command(
    ...
    kwargs = {
        ...
        "ambush_acp_binary": agent_binaries["ambush-acp"],
        "ambush_agent_binary": agent_binaries["ambush-agent"],
        "ambush_dev_mcp_binary": agent_binaries["ambush-dev-mcp"],
        "ambush_cli_binary": binaries["ambush"],
...
def write_metadata_template(args: argparse.Namespace, job_dir: Path) -> Path:
    ...
    metadata = {
        "agent_url": "https://github.com/backbay-labs/ambush",
        "agent_display_name": f"Ambush Orchestra ({manifest.get('condition', 'team')})",
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.dry_run:
        ...
        bin_dir = args.ambush_bin_dir or PACKAGE_ROOT.parents[1] / "target" / "release"
        ...
        return 0
    binaries = find_binaries(args.ambush_bin_dir)
