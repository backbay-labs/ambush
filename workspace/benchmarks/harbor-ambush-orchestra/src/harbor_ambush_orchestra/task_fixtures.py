"""Public setup declarations for Ambush-native benchmark tasks."""
...
_CREATE_CHANNEL_FIXTURE = AmbushTaskFixture(
    directory=tuple(
        [
...
_USER_MENTION_FIXTURE = AmbushTaskFixture(
    user_display_name=USER_MENTION_DISPLAY_NAME,
    requires_evidence=True,
)

_NARRATIVE_AGENT_NAMES_FIXTURE = AmbushTaskFixture(
    directory=(
...
_INTERLEAVED_AGENT_REPORTS_FIXTURE = AmbushTaskFixture(
    directory=(
...
_CROSS_THREAD_REQUESTS_FIXTURE = AmbushTaskFixture(
    scripted_messages=(
        ScriptedMessage(
            label="beta-request",
            actor="user",
...
_AMBIGUOUS_USER_MENTION_FIXTURE = AmbushTaskFixture(
    directory=(
        DirectoryEntry(
            "Taylor Morgan Lee",
            "user",
...
_FIXTURES = {
    ...
    REPLY_TO_THREAD_TASK: AmbushTaskFixture(requires_evidence=True),
    READ_NAMED_PATH_TASK: AmbushTaskFixture(requires_evidence=True),
    MULTILINE_MESSAGE_TASK: AmbushTaskFixture(
...
def fixture_for(task_name: str | None) -> AmbushTaskFixture:
    """Return the declared setup for a task, or an empty setup."""
    return _FIXTURES.get(task_name or "", AmbushTaskFixture())
