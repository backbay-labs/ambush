"""Run the production Ambush agent stack inside the Harbor task container.

Each provisioned identity is a full ``ambush-acp`` → ``ambush-agent`` →
``ambush-dev-mcp`` process tree launched *inside* the task container — the same
binaries and the same MCP toolset (shell, file tools, the ``ambush`` CLI on
PATH) that the desktop app gives a Ambush agent. The harness stays outside:
...
from .evidence import build_ambush_evidence
...
DEFAULT_MAX_AGENT_ROUNDS = (
    0  # 0 = unbounded (AMBUSH_AGENT_MAX_ROUNDS=0); the trial budget is the clock
)
...
# Container-side layout for the uploaded Ambush stack.
REMOTE_ROOT = "/opt/ambush"
REMOTE_BIN = f"{REMOTE_ROOT}/bin"
REMOTE_PROMPTS = f"{REMOTE_ROOT}/prompts"
REMOTE_LOGS = f"{REMOTE_ROOT}/logs"
REMOTE_EVIDENCE = "/logs/artifacts/ambush-evidence.json"
...
DELIVERY_RECEIPT_MARKER = "turn delivered Ambush events for channel"
...
class RuntimeLaunchError(RuntimeError):
    """Raised when a Ambush agent process cannot be launched or exits early."""
...
class AmbushContainerRuntime:
    """Launch one production Ambush agent stack per identity in the container."""
    ...
    def __init__(
        ...
        *,
        ...
        ambush_acp_binary: str = "ambush-acp",
        ambush_agent_binary: str = "ambush-agent",
        ambush_dev_mcp_binary: str = "ambush-dev-mcp",
        ambush_cli_binary: str = "ambush",
        relay_gateway: str = "",
        ...
        self.ambush_acp_binary = ambush_acp_binary
        self.ambush_agent_binary = ambush_agent_binary
        self.ambush_dev_mcp_binary = ambush_dev_mcp_binary
        # Host build used for user/provisioning operations only:
        self.ambush_cli_binary = ambush_cli_binary
    ...
    async def run(
        ...
        *,
        ...
        trial_dir = self.logs_dir / "ambush"
        ...
        try:
            ...
            # The task arrives exactly as it would in production Ambush: a
        ...
        if fixture_for(trial.task_name).requires_evidence and not evidence_exported:
            raise RuntimeLaunchError(
                "failed to export ambush-evidence.json; the trial has no "
        ...
        return RuntimeResult(
            metadata={
                ...
                "ambush_evidence_exported": evidence_exported,
    ...
    async def _install_stack(self, environment: BaseEnvironment) -> None:
        ...
        uploads = {
            f"{REMOTE_BIN}/ambush-acp": self.ambush_acp_binary,
            f"{REMOTE_BIN}/ambush-agent": self.ambush_agent_binary,
            f"{REMOTE_BIN}/ambush-dev-mcp": self.ambush_dev_mcp_binary,
        }
    ...
    async def _launch_agent(
        ...
        *,
        ...
        command = (
            f"{shlex.quote(f'{REMOTE_BIN}/ambush-acp')} </dev/null "
        ...
        *,
        ...
        return {
            **endpoint.env,
            ...
            "AMBUSH_RELAY_URL": trial.relay_ws_url,
            "AMBUSH_PRIVATE_KEY": credential.nostr_secret_key,
            # Desktop parity: the GUI also sets NOSTR_PRIVATE_KEY on ambush-acp
            # so ambush-dev-mcp's shim can wire git auth/signing for the agent.
            "NOSTR_PRIVATE_KEY": credential.nostr_secret_key,
            "AMBUSH_AUTH_TAG": credential.nostr_auth_tag,
            "AMBUSH_ACP_AGENT_COMMAND": f"{REMOTE_BIN}/ambush-agent",
            "AMBUSH_ACP_AGENT_ARGS": "",
            "AMBUSH_ACP_MCP_COMMAND": f"{REMOTE_BIN}/ambush-dev-mcp",
            "AMBUSH_ACP_CHANNELS": trial.channel_id,
            "AMBUSH_ACP_SUBSCRIBE": "mentions",
            "AMBUSH_ACP_RESPOND_TO": "anyone",
            "AMBUSH_ACP_NO_MEMORY": "true",
            "AMBUSH_ACP_SYSTEM_PROMPT_FILE": remote_prompt,
            "AMBUSH_AGENT_PROVIDER": endpoint.provider,
            "AMBUSH_AGENT_MODEL": credential.llm_endpoint,
            "AMBUSH_AGENT_THINKING_EFFORT": (
                agent_class.generation.thinking_effort or THINKING_EFFORT
            ),
            "AMBUSH_AGENT_MAX_OUTPUT_TOKENS": str(
                agent_class.generation.max_output_tokens
            ),
            "AMBUSH_AGENT_MAX_CONTEXT_TOKENS": str(
                agent_class.generation.context_window_tokens
            ),
            "AMBUSH_AGENT_MAX_ROUNDS": str(
            ...
            "AMBUSH_AGENT_NO_HINTS": "1",
            endpoint.api_key_env: credential.llm_api_key,
        }

    @staticmethod
    def _rust_log(configured: str | None) -> str:
        # ``ambush_acp=info`` carries the subscription-readiness line; the turn
        ...
        required = "ambush_acp=info,pool::prompt=info"
    ...
    async def _wait_for_done(
        ...
        while True:
            ...
            polls += 1
            messages = await self._ambush_json(
                trial.user,
                trial,
                "messages",
                "get",
    ...
    async def _turn_ended(environment: BaseEnvironment, agent: _Agent) -> bool:
        _, ends = await AmbushContainerRuntime._turn_counts(environment, agent)
        return ends > 0
    ...
    async def _turn_counts(
        ...
        starts, ends, _ = await AmbushContainerRuntime._turn_status(environment, agent)
        return starts, ends
    ...
    async def _turn_status(
        ...
        return AmbushContainerRuntime._parse_turn_status(result.stdout or "")
    ...
    async def _collect_evidence(
        ...
        *,
        ...
        try:
            ...
            evidence = build_ambush_evidence(
                trial=trial,
                messages=messages,
                task_event_id=task_event_id,
            ...
            evidence_path = trial_dir / "ambush-evidence.json"
    ...
    def _record_evidence_error(trial_dir: Path, reason: str) -> None:
        ...
        try:
            (trial_dir / "ambush-evidence-error.txt").write_text(
    ...
    async def _collect_observed_channels(
        ...
        for name in names:
            matches = await self._ambush_json(
                orchestrator,
                trial,
                "channels",
                "search",
            ...
            for match in matches:
                ...
                if not isinstance(channel_id, str) or not channel_id:
                    continue
                members = await self._ambush_json(
                    orchestrator,
                    trial,
                    "channels",
                    "members",
    ...
    # -- Ambush CLI as the trial user / provisioning identities -------------------
    ...
    async def _send(
        ...
        *,
        ...
        return await self._ambush_json(credential, trial, *args)
    ...
    async def _send_scripted_messages(
        ...
        *,
        ...
        Messages are sent back-to-back so Ambush's normal queueing and batching
    ...
    async def _ambush_json(
        self, credential: AgentCredential, trial: TrialHandle, *args: str
    ) -> Any:
        process = await asyncio.create_subprocess_exec(
            self.ambush_cli_binary,
            *args,
            stdout=asyncio.subprocess.PIPE,
            stderr=asyncio.subprocess.PIPE,
            env={
                **os.environ,
                "AMBUSH_RELAY_URL": self._user_relay_url(trial),
                "AMBUSH_PRIVATE_KEY": credential.nostr_secret_key,
                "AMBUSH_AUTH_TAG": credential.nostr_auth_tag,
            },
        )
        ...
        if process.returncode != 0:
            raise RuntimeLaunchError(
                f"ambush {shlex.join(args)} exited {process.returncode}: "
                f"{stderr.decode(errors='replace').strip()}"
            )
        try:
            return json.loads(stdout)
        except json.JSONDecodeError as error:
            raise RuntimeLaunchError("ambush returned invalid JSON") from error
    ...
    def _user_relay_url(trial: TrialHandle) -> str:
        ...
        return AmbushContainerRuntime._cli_relay_url(trial.relay_ws_url)
    ...
    def _compose_system_prompt(
        ...
        *,
        ...
        The analogue of a production Ambush workspace's team context: each agent
        ...
        lines = [
            ...
            f"The team coordinates in Ambush channel `{trial.channel_id}`.",
            (
    ...
    def _reject_identity_overrides(endpoint: EndpointLaunchConfig) -> None:
        forbidden = {
            ...
            "AMBUSH_AUTH_TAG",
            "AMBUSH_ACP_CHANNELS",
            "AMBUSH_ACP_MCP_COMMAND",
            "AMBUSH_ACP_AGENT_COMMAND",
        }
