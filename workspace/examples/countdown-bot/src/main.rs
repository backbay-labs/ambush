...
const BOT_ABOUT: &str =
    "A tiny non-AI Ambush reference bot that replies to !countdown and countdown-style !fib.";
...
impl Config {
    fn from_env() -> Result<Self> {
        let relay_url =
            std::env::var("AMBUSH_RELAY_URL").unwrap_or_else(|_| DEFAULT_RELAY_URL.to_string());
        let channel_id = required_env("AMBUSH_CHANNEL_ID")?;
        let bot_keys = Keys::parse(&required_env("AMBUSH_BOT_PRIVATE_KEY")?)
            .context("AMBUSH_BOT_PRIVATE_KEY must be an nsec or hex private key")?;

        let auth_mode =
            std::env::var("AMBUSH_BOT_AUTH_MODE").unwrap_or_else(|_| "standalone".to_string());
        let owner_auth_tag = match auth_mode.as_str() {
            "standalone" => None,
            "owner-attested" => {
                let tag_json = match std::env::var("AMBUSH_AUTH_TAG") {
                    Ok(value) if !value.trim().is_empty() => value,
                    _ => {
                        let owner_keys = Keys::parse(&required_env("AMBUSH_OWNER_PRIVATE_KEY")?)
                            .context(
                                "AMBUSH_OWNER_PRIVATE_KEY must be an nsec or hex private key",
                            )?;
                        ambush_sdk::nip_oa::compute_auth_tag(
                            &owner_keys,
                            &bot_keys.public_key(),
                            "",
                        )?
                ...
                let owner = ambush_sdk::nip_oa::verify_auth_tag(&tag_json, &bot_keys.public_key())
                    .context("AMBUSH_AUTH_TAG is not valid for AMBUSH_BOT_PRIVATE_KEY")?;
                eprintln!("owner-attested auth tag verified; owner={}", owner.to_hex());
                Some(ambush_sdk::nip_oa::parse_auth_tag(&tag_json)?)
            }
            other => {
                bail!(
                    "AMBUSH_BOT_AUTH_MODE must be 'standalone' or 'owner-attested', got {other:?}"
                )
            }
        };
...
async fn publish_profile(ws: &mut Ws, config: &Config) -> Result<()> {
    let builder = ambush_sdk::builders::build_profile(
...
async fn maybe_reply(
    ...
    let builder = ambush_sdk::builders::build_message(
