...
Ambush has three independent release lanes. Desktop and relay use release PRs.
...
| Relay | `just release-relay` | `ghcr.io/backbay-labs/ambush` container image |
...
exact candidate tag. The mobile handoff to the private mobile build pipeline
...
1. **`just release-relay`** runs locally on `main`, creates or updates a
   `relay-release/<version>` PR, bumps `crates/ambush-relay/Cargo.toml`,
...
1. **Publish a candidate.** From a clean checkout whose `origin` is the
   canonical `backbay-labs/ambush` repository, run
   ...
   `mobile-vX.Y.Z-rc.N` tag there through the dedicated `ambush-release-bot`
...
2. **Build the exact tag.** Enter the candidate tag as `mobile_ref` in the
   private Ambush mobile Buildkite pipeline. OSS CI deliberately cannot trigger
...
| Relay | `crates/ambush-relay/Cargo.toml` |
...
```sh
gh workflow run signed-macos-canary.yml --repo backbay-labs/ambush --ref main
```
...
create or move tags, and cannot update `ambush-desktop-latest` or `latest.json`.

Download the artifact from the completed run:

```sh
gh run download <run-id> --repo backbay-labs/ambush --name <artifact-name>
```
...
(or use `gh run rerun <run-id> --failed --repo backbay-labs/ambush`). A rerun
...
2. **`ambush-desktop-latest`**: the rolling auto-updater release. Its
...
`ambush-desktop-latest/latest.json`. Same-version retries succeed only when the
...
- **Write access** to the `backbay-labs/ambush` GitHub repository
- An `origin` remote whose configured URL is the canonical `backbay-labs/ambush`
  repository
...
- Release tag ruleset [`14378754`](https://github.com/backbay-labs/ambush/rules/14378754)
  active for `desktop-v*` and `mobile-v*`, with creation, update, deletion, and
  non-fast-forward protections and `ambush-release-bot` as its sole always-bypass
  actor
- The `ambush-release-bot` App credentials configured for GitHub Actions
- The following **GitHub Actions variables and secrets** configured for the
  desktop release lane:

  | Name | Kind | Purpose |
  |------|------|---------|
  | `AMBUSH_RELEASE_TAGGER_CLIENT_ID` | Variable | GitHub App client ID used to create protected release tags |
  | `AMBUSH_RELEASE_TAGGER_PRIVATE_KEY` | Secret | GitHub App private key |
  ...
  | `AMBUSH_UPDATER_PUBLIC_KEY` or `SPROUT_UPDATER_PUBLIC_KEY` | Secret | Tauri updater public key |
...
must be installed on `backbay-labs/ambush`, have Contents write and Metadata read, and
...
Confirm `ambush-release-bot` remains the sole always-bypass actor for the active
...
Verify that the `ambush-desktop-latest` release exists and contains a
