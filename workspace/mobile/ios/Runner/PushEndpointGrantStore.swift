...
final class AmbushPushEndpointGrantKeychainStore: AmbushPushEndpointGrantStore {
  private static let service = "ambush.push.endpoint-grants"
  ...
  func records() throws -> [AmbushPushEndpointGrantRecord] {
    ...
    do {
      return try JSONDecoder().decode([AmbushPushEndpointGrantRecord].self, from: data)
    } catch {
      throw NSError(
        domain: "AmbushPushEndpointGrantStore",
        code: 1,
  ...
  private func pendingEnrollments() throws -> [AmbushPushPendingEnrollmentRecord] {
    ...
    do {
      return try JSONDecoder().decode([AmbushPushPendingEnrollmentRecord].self, from: data)
    } catch {
      throw NSError(
        domain: "AmbushPushEndpointGrantStore",
        code: 2,
...
extension AmbushPushEndpointGrantRecord {
