...
final class AmbushPushSnapshotBridge {
  private let appGroupIdentifier: String?
  private let endpointGrantStore: AmbushPushEndpointGrantKeychainStore
  private let keychainAccessGroup: String?
  private let queue = DispatchQueue(
    label: "com.backbay.ambush.push-snapshot",
    qos: .utility
  )
  private lazy var store: AmbushPushPresentationCacheStore? = {
    guard let appGroupIdentifier,
      let container = FileManager.default.containerURL(
        forSecurityApplicationGroupIdentifier: appGroupIdentifier
      )
    else { return nil }
    return AmbushPushPresentationCacheStore(containerURL: container)
  }()

  init(
    appGroupIdentifier: String?,
    endpointGrantStore: AmbushPushEndpointGrantKeychainStore,
  ...
  private func syncCommunities(_ arguments: [String: Any], result: @escaping FlutterResult) {
    guard let communities = arguments["communities"] as? [[String: Any]],
      ...
      communities.count <= AmbushPushPresentationCacheStore.maximumCommunities
    else {
      result(
        FlutterError(
    ...
    queue.async { [weak self] in
      do {
        ...
        try AmbushPushKeychain.replace(
  ...
  static func relayMetadataPubkey(
    relayURL: String,
    grants: [AmbushPushEndpointGrantRecord]
  ) -> String? {
    guard let origin = AmbushPushPresentationCacheStore.canonicalRelayOrigin(relayURL) else {
      return nil
    }
    return grants.filter {
      $0.appProfile == AmbushDevPushEnrollmentDriver.appProfile
        && AmbushPushPresentationCacheStore.canonicalRelayOrigin($0.relayOrigin) == origin
    }.max {
  ...
  private func cacheProfiles(_ rawArguments: Any?, result: @escaping FlutterResult) {
    guard let arguments = rawArguments as? [String: Any],
      ...
      rawEvents.count <= AmbushPushPresentationCacheStore.maximumProfiles
    else {
      result(
        FlutterError(
          code: "invalid_arguments",
    ...
    queue.async { [weak self] in
      do {
        ...
        try store?.updateProfiles(
          ...
          updates: events.map { AmbushPushProfileCacheUpdate(event: $0) }
        )
        Self.complete(result, value: nil)
      } catch {
        Self.complete(
  ...
  private func cacheChannels(_ rawArguments: Any?, result: @escaping FlutterResult) {
    guard let arguments = rawArguments as? [String: Any],
      ...
      rawMetadataEvents.count <= AmbushPushPresentationCacheStore.maximumChannels,
      rawMembershipEvents.count <= AmbushPushPresentationCacheStore.maximumChannels
    else {
      result(
        FlutterError(
          code: "invalid_arguments",
  ...
  private func community(id: String) -> PushLeaseCommunity? {
    guard let appGroupIdentifier,
      ...
      let data = try? Data(contentsOf: container.appendingPathComponent(AmbushPushPresentationCacheStore.fileName)),
      let snapshot = try? JSONDecoder().decode(AmbushPushPresentationCacheSnapshot.self, from: data)
    else { return nil }
