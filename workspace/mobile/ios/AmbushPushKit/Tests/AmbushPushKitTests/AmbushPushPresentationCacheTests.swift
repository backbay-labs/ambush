...
struct AmbushPushPresentationCacheTests {
  ...
  func verifiedProfilePrecedenceAndAvatar() throws {
    ...
    let store = AmbushPushPresentationCacheStore(
    ...
    let needsAvatar = try store.updateProfiles(
      ...
      updates: [AmbushPushProfileCacheUpdate(event: event)]
    )
  ...
  func verifiedInlineRasterProfileAndAvatar() throws {
    let directory = try temporaryDirectory()
    defer { try? FileManager.default.removeItem(at: directory) }
    let store = AmbushPushPresentationCacheStore(containerURL: directory)
  ...
  func malformedAndUnverifiedProfileFallback() throws {
    ...
    try store.updateProfiles(
      communityID: "community-a",
      relayOrigin: "https://relay.example",
      updates: [AmbushPushProfileCacheUpdate(event: named)]
    )
    ...
    try store.updateProfiles(
      communityID: "community-a",
      relayOrigin: "https://relay.example",
      updates: [AmbushPushProfileCacheUpdate(event: malformed)]
    )
    ...
    try store.updateProfiles(
      communityID: "community-a",
      relayOrigin: "https://relay.example",
      updates: [AmbushPushProfileCacheUpdate(event: tampered)]
    )
    ...
    #expect(
      cached.memberDigests
        == [firstMember, secondMember].map {
          AmbushPushPresentationIdentity.channelMember(
            communityID: "community-a",
            channelID: "opaque-channel",
            pubkey: $0
          )
  ...
  func oversizedChannelBatchIsIgnored() throws {
    ...
    try store.updateChannels(
      ...
      metadataEvents: Array(
        ...
        count: AmbushPushPresentationCacheStore.maximumChannels + 1
      ),
      membershipEvents: []
    )
  ...
  func globalMembershipDigestBound() throws {
    let membersPerChannel = AmbushPushPresentationCacheStore.maximumMembersPerChannel
    let channelCount =
      AmbushPushPresentationCacheStore.maximumTotalMemberDigests / membersPerChannel + 1
    let digests = (0..<membersPerChannel).map { String(format: "%064x", $0 + 1) }
    let snapshot = AmbushPushPresentationCacheSnapshot(
      channels: (0..<channelCount).map { index in
        AmbushPushCachedChannel(
          communityID: "community-a",
          relayOrigin: "https://relay.example",
    ...
    let encoded = try AmbushPushPresentationCacheStore.encodedBoundedSnapshot(snapshot)
    let decoded = try JSONDecoder().decode(AmbushPushPresentationCacheSnapshot.self, from: encoded)
    let totalDigests = decoded.channels.reduce(0) { $0 + ($1.memberDigests?.count ?? 0) }

    #expect(totalDigests == AmbushPushPresentationCacheStore.maximumTotalMemberDigests)
  ...
  func oversizedMembershipFallback() throws {
    ...
    let memberCount = AmbushPushPresentationCacheStore.maximumMembersPerChannel + 25
    let membership = try signedEvent(
      privateKey: relayKey,
      kind: 39_002,
    ...
    #expect(
      cached.memberCount == AmbushPushPresentationCacheStore.maximumMembersPerChannel + 1
    )
    #expect(cached.memberDigests == nil)
  ...
  func communityPruning() throws {
    ...
    try store.updateProfiles(
      communityID: "removed",
      relayOrigin: "https://relay.example",
      updates: [AmbushPushProfileCacheUpdate(event: profile)]
    )
    try store.updateChannels(
      communityID: "removed",
      relayOrigin: "https://relay.example",
  ...
  func boundedEviction() {
    var snapshot = AmbushPushPresentationCacheSnapshot(
      profiles: (0...AmbushPushPresentationCacheStore.maximumProfiles).map { index in
        AmbushPushCachedProfile(
          communityID: "community-a",
          relayOrigin: "https://relay.example",
      ...
      channels: (0...AmbushPushPresentationCacheStore.maximumChannels).map { index in
        AmbushPushCachedChannel(
          communityID: "community-a",
          relayOrigin: "https://relay.example",
          channelID: "channel-\(index)",
    ...
    AmbushPushPresentationCacheStore.enforceBounds(&snapshot)

    #expect(snapshot.profiles.count == AmbushPushPresentationCacheStore.maximumProfiles)
    #expect(snapshot.channels.count == AmbushPushPresentationCacheStore.maximumChannels)
  ...
  func encodedCacheByteBound() throws {
    ...
    let avatar =
      ...
      + Data(
        repeating: 0,
        count: AmbushPushPresentationCacheStore.maximumAvatarBytes - 8
      )
    let snapshot = AmbushPushPresentationCacheSnapshot(
    ...
    let decoded = try JSONDecoder().decode(
      AmbushPushPresentationCacheSnapshot.self,
      from: encoded
    )

    #expect(encoded.count <= AmbushPushPresentationCacheStore.maximumSnapshotBytes)
  ...
  func displayNameUTF8Bound() throws {
    ...
    let metadata = AmbushPushPresentationCacheStore.profileMetadata(event)
  ...
  private func temporaryDirectory() throws -> URL {
    let url = FileManager.default.temporaryDirectory
      .appendingPathComponent("ambush-push-cache-\(UUID().uuidString)", isDirectory: true)
    try FileManager.default.createDirectory(at: url, withIntermediateDirectories: true)
    return url
  }

  private func loadSnapshot(_ directory: URL) throws -> AmbushPushPresentationCacheSnapshot {
    let data = try Data(
      contentsOf: directory.appendingPathComponent(AmbushPushPresentationCacheStore.fileName)
    )
    return try JSONDecoder().decode(AmbushPushPresentationCacheSnapshot.self, from: data)
  }
