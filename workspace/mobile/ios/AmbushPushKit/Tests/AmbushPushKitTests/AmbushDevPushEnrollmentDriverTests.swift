...
final class AmbushDevPushEnrollmentDriverTests: XCTestCase {
  ...
  fileprivate static let assertion = Data("ambush-dev-app-assertion-v1".utf8).base64EncodedString()
  ...
  func testEnrollmentPinsTranscriptsAndPersistsOpaqueGrant() async throws {
    ...
    URLProtocolStub.handler = { request in
      ...
      case ("POST", "http://push.example/v1/installations"):
        ...
        XCTAssertEqual(body["app_profile"] as? String, "ambush-ios-dogfood")
        return Self.response(
          request,
          status: 201,
          json: [
    ...
    try assertMatchesVector(
      ...
      expectedSHA256: "efbb56a569d2d24f0b77eff3250b7a988d6f2bcbd8beb2a082262800bf9cfdc7",
      fixture: makeFixtureTranscript(name: "enroll", replacements: [])
    )
    try assertMatchesVector(
      "delegate",
      actual: appAttest.clientData[1],
      expectedSHA256: "94337d86503ba85bddf232b9ee9d251ea057928547fd7f3cf69b952831226568",
  ...
  func testRealAppAttestFailsLoudlyWhenUnsupported() async throws {
    ...
    let provider = AmbushDCAppAttestProvider(
      service: service,
    ...
    do {
      ...
      XCTAssertEqual(error as? AmbushDevPushEnrollmentError, .appAttestUnsupported)
    }
  ...
  func testRealAppAttestGeneratesPersistsAndMapsAttestation() async throws {
    ...
    let provider = AmbushDCAppAttestProvider(service: service, keyIdStore: keyIdStore)
    ...
    XCTAssertEqual(prepared, AmbushDevAttestation(keyId: Self.keyId, attestation: ""))
  ...
  func testRealAppAttestRejectsInvalidGeneratedKeyBeforePersistence() async throws {
    for invalidKeyId in [
      ...
      do {
        ...
        XCTAssertEqual(error as? AmbushDevPushEnrollmentError, .invalidAppAttestKeyId)
      }
  ...
  func testRealAppAttestRejectsMismatchedPreparedKey() async throws {
    ...
    do {
      _ = try await provider.attestation(
        AmbushDevAttestation(keyId: otherKeyId, attestation: ""),
  ...
  func testKeychainStoreReadsKeyIdAndIncludesAccessGroup() throws {
    ...
    let store = AmbushAppAttestKeyIdKeychainStore(
      accessGroup: "group.ambush",
    ...
    XCTAssertEqual(capturedQuery[kSecAttrService as String] as? String, "ambush.push.app-attest")
    XCTAssertEqual(capturedQuery[kSecAttrAccount as String] as? String, "key-id-v1")
    XCTAssertEqual(capturedQuery[kSecAttrAccessGroup as String] as? String, "group.ambush")
  ...
  func testKeychainStoreReturnsNilOnMissAndRejectsInvalidData() throws {
    let missing = AmbushAppAttestKeyIdKeychainStore(
      accessGroup: nil,
    ...
    for invalidKeyId in [
      ...
      let invalid = AmbushAppAttestKeyIdKeychainStore(
        accessGroup: nil,
      ...
      XCTAssertThrowsError(try invalid.keyId(), "Accepted invalid key ID: \(invalidKeyId)") {
        XCTAssertEqual($0 as? AmbushDevPushEnrollmentError, .invalidAppAttestKeyId)
      }
    }
  }

  func testKeychainStoreMigratesKeyIdFromLegacyService() throws {
    var copyCallCount = 0
    ...
    let store = AmbushAppAttestKeyIdKeychainStore(
      ...
      copyMatching: { query, result in
        copyCallCount += 1
        guard (query as! [String: Any])[kSecAttrService as String] as? String
          == "buzz.push.app-attest"
        else {
    ...
    XCTAssertEqual(try store.keyId(), Self.keyId)
    XCTAssertEqual(copyCallCount, 2)
    XCTAssertEqual(updatedQuery[kSecAttrService as String] as? String, "ambush.push.app-attest")
  ...
  func testKeychainStoreAddsMissingKeyIdWithDeviceOnlyAccessibility() throws {
    ...
    XCTAssertEqual(addedItem[kSecAttrAccessGroup as String] as? String, "group.ambush")
  }

  func testKeychainStoreSurfacesReadUpdateAndAddErrors() throws {
    let readFailure = AmbushAppAttestKeyIdKeychainStore(
      accessGroup: nil,
    ...
    let updateFailure = AmbushAppAttestKeyIdKeychainStore(
      accessGroup: nil,
    ...
    let addFailure = AmbushAppAttestKeyIdKeychainStore(
      accessGroup: nil,
      update: { _, _ in errSecItemNotFound },
  ...
  func testReusesPersistedUnexpiredGrant() async throws {
    let existing = AmbushPushEndpointGrantRecord(
      relayOrigin: "wss://relay.example",
      relayPubkey: Self.relayPubkey,
      relayMetadataPubkey: Self.relayPubkey,
      installationId: Self.installationId,
  ...
  func testRejectsMultipleCurrentRelayKeysBeforeGatewayEnrollment() async throws {
    ...
    do {
      ...
      XCTAssertEqual(error as? AmbushDevPushEnrollmentError, .invalidRelayDescriptor)
    }
    XCTAssertEqual(URLProtocolStub.requests.count, 1)
  }
  ...
  func testFailsLoudlyOnUnexpectedGatewayStatus() async throws {
    ...
    } catch let error as AmbushDevPushEnrollmentError {
      XCTAssertEqual(
        error,
        .unexpectedStatus(
  ...
  ) throws -> AmbushDevPushEnrollmentDriver {
    let configuration = URLSessionConfiguration.ephemeral
    configuration.protocolClasses = [URLProtocolStub.self]
    return try AmbushDevPushEnrollmentDriver(
...
private final class MemoryGrantStore: AmbushPushEndpointGrantStore {
  var saved: [AmbushPushEndpointGrantRecord]
  var pending: [AmbushPushPendingEnrollmentRecord] = []
  var grantSaveFailuresRemaining: Int
  init(
    records: [AmbushPushEndpointGrantRecord] = [],
    grantSaveFailuresRemaining: Int = 0
  ) {
    saved = records
    self.grantSaveFailuresRemaining = grantSaveFailuresRemaining
  }
  func records() throws -> [AmbushPushEndpointGrantRecord] { saved }
...
private final class RecordingAppAttest: AmbushDevAppAttesting {
  var clientData: [Data] = []

  func prepareAttestation() async throws -> AmbushDevAttestation {
    AmbushDevAttestation(
      keyId: AmbushDevPushEnrollmentDriverTests.keyId,
      attestation: AmbushDevPushEnrollmentDriverTests.attestation
    )
  }
  ...
  func assertion(clientData: Data) async throws -> String {
    self.clientData.append(clientData)
    return AmbushDevPushEnrollmentDriverTests.assertion
  }
}

private final class MemoryAppAttestKeyIdStore: AmbushAppAttestKeyIdStoring {
...
private final class RecordingDCAppAttestService: AmbushDCAppAttestServicing {
  ...
  init(
    ...
    generatedKeyId: String = AmbushDevPushEnrollmentDriverTests.keyId,
