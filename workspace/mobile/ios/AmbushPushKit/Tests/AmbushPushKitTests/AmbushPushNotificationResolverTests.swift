...
final class AmbushPushNotificationResolverTests: XCTestCase {
  ...
  func testDecodeResolutionFiltersOwnPubkeyEvent() {
    let result = AmbushPushNotificationResolver.decodeResolution(
  ...
  func testPreviewBodySanitizesCodeLinksAndWhitespace() {
    ...
    XCTAssertEqual(
      AmbushPushNotificationResolver.previewBody(content),
      "Before [code] inline docs image [link] After"
    )
  }

  func testPreviewBodyTruncatesTo178CharactersIncludingEllipsis() {
    let preview = AmbushPushNotificationResolver.previewBody(String(repeating: "x", count: 200))
  ...
  func testResolveSucceedsAndMutatesGatewayContent() throws {
    ...
    XCTAssertEqual(
      ...
      AmbushPushPresentationIdentity.conversation(
  ...
  func testStaleVerifiedCacheIsUsedWhileOneBoundedRefreshFails() throws {
    ...
    let relayPubkey = try Self.pubkey(for: Self.relayPrivateKey)
    let staleAt = Self.now - Int(AmbushPushPresentationCacheStore.freshnessLifetime) - 1
    let snapshot = AmbushPushPresentationCacheSnapshot(
      profiles: [
        AmbushPushCachedProfile(
          communityID: "community-id",
  ...
  func testOversizedPresentationRefreshFallsBackWithoutBlockingMessage() throws {
    ...
    URLProtocolStub.handler = { request in
      ...
      var oversized = Data(
        repeating: 0x20,
        count: AmbushPushNotificationResolver.maximumPresentationResponseBytes
      )
  ...
  func testBoundedInlineAvatarProfileRefreshStillResolvesDisplayName() throws {
    ...
    XCTAssertLessThan(
      ...
      AmbushPushNotificationResolver.maximumPresentationResponseBytes
    )
    URLProtocolStub.handler = { request in
      Self.response(
        request,
  ...
  func makeResolver(
    ...
    now: Date = Date()
  ) -> AmbushPushNotificationResolver {
  ...
  func resolve(_ resolver: AmbushPushNotificationResolver) -> AmbushPushResolution? {
    let completed = expectation(description: "resolver completed")
    var result: AmbushPushResolution?
    resolver.resolve {
      result = $0
      completed.fulfill()
    }
