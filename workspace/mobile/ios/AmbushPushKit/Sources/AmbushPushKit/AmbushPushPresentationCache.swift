...
public struct AmbushPushCachedProfile: Codable, Equatable, Sendable {
...
public struct AmbushPushCachedChannel: Codable, Equatable, Sendable {
  public let communityID: String
  public let relayOrigin: String
  public let channelID: String
  public let relayMetadataPubkey: String
  public let displayName: String?
  /// Relay-verified Ambush channel type, when recognized.
...
public struct AmbushPushPresentationCacheSnapshot: Codable, Equatable, Sendable {
  public static let currentVersion = 1

  public let version: Int
  public var communities: [PushLeaseCommunity]
  public var profiles: [AmbushPushCachedProfile]
  public var channels: [AmbushPushCachedChannel]

  public init(
    version: Int = currentVersion,
    communities: [PushLeaseCommunity] = [],
    profiles: [AmbushPushCachedProfile] = [],
    channels: [AmbushPushCachedChannel] = []
  ) {
...
public struct AmbushPushProfileCacheUpdate: Sendable {
...
public final class AmbushPushPresentationCacheStore: @unchecked Sendable {
  ...
  public func updateProfiles(
    communityID: String,
    relayOrigin: String,
    updates: [AmbushPushProfileCacheUpdate]
  ) throws -> Set<String> {
    ...
    for update in updates {
      ...
      let entry = AmbushPushCachedProfile(
  ...
  public func updateChannels(
    ...
    for event in metadataEvents {
      ...
      let entry = AmbushPushCachedChannel(
        communityID: communityID,
        relayOrigin: canonicalRelayOrigin,
        channelID: channelID,
    ...
    for event in membershipEvents {
      ...
      snapshot.channels[index] = AmbushPushCachedChannel(
  ...
  public func updateAvatar(
      ...
      && snapshot.profiles[index].avatarPNG != normalizedPNG
    {
      ...
      snapshot.profiles[index] = AmbushPushCachedProfile(
  ...
  private func loadLocked() -> AmbushPushPresentationCacheSnapshot {
    guard let values = try? fileURL.resourceValues(forKeys: [.fileSizeKey]),
      let fileSize = values.fileSize,
      fileSize <= Self.maximumSnapshotBytes
    else { return AmbushPushPresentationCacheSnapshot() }
    return AmbushPushPresentationCacheSnapshot.decode(try? Data(contentsOf: fileURL))
  }

  private func writeLocked(_ snapshot: AmbushPushPresentationCacheSnapshot) throws {
  ...
  static func encodedBoundedSnapshot(
    _ snapshot: AmbushPushPresentationCacheSnapshot
  ) throws -> Data {
  ...
  static func enforceBounds(_ snapshot: inout AmbushPushPresentationCacheSnapshot) {
  ...
  private static func enforceChannelCountBound(
    _ snapshot: inout AmbushPushPresentationCacheSnapshot
  ) {
  ...
  private static func removingAvatar(
    from profile: AmbushPushCachedProfile
  ) -> AmbushPushCachedProfile {
    AmbushPushCachedProfile(
      communityID: profile.communityID,
      relayOrigin: profile.relayOrigin,
  ...
  private static func removingMemberDigests(
    from channel: AmbushPushCachedChannel
  ) -> AmbushPushCachedChannel {
    AmbushPushCachedChannel(
  ...
  private static func removeOldestEntries(
    _ count: Int,
    from snapshot: inout AmbushPushPresentationCacheSnapshot
  ) {
    for _ in 0..<count {
  ...
  private static func profileNewestFirst(
    _ lhs: AmbushPushCachedProfile,
    _ rhs: AmbushPushCachedProfile
  ) -> Bool {
    lhs.cachedAt == rhs.cachedAt ? lhs.eventID > rhs.eventID : lhs.cachedAt > rhs.cachedAt
  }

  private static func channelNewestFirst(
    _ lhs: AmbushPushCachedChannel,
    _ rhs: AmbushPushCachedChannel
  ) -> Bool {
  ...
  private static func channelLastCachedAt(_ channel: AmbushPushCachedChannel) -> Int {
...
public enum AmbushPushPresentationIdentity {
  ...
  private static func scoped(namespace: String, values: [String]) -> String {
    ...
    return "ambush.\(namespace).\(VerifiedNostrEvent.hex(SHA256.hash(data: encoded)))"
  }
}
