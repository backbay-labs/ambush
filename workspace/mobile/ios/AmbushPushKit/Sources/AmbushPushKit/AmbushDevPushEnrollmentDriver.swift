...
public struct AmbushPushEndpointGrantRecord: Codable, Equatable, Sendable {
...
public protocol AmbushPushEndpointGrantStore {
  func records() throws -> [AmbushPushEndpointGrantRecord]
  func save(_ record: AmbushPushEndpointGrantRecord) throws
  func pendingEnrollment(
    relayOrigin: String,
    appProfile: String
  ) throws -> AmbushPushPendingEnrollmentRecord?
  func savePendingEnrollment(_ record: AmbushPushPendingEnrollmentRecord) throws
  func removePendingEnrollment(relayOrigin: String, appProfile: String) throws
}

public enum AmbushDevPushEnrollmentError: Error, LocalizedError, Equatable {
...
protocol AmbushDevAppAttesting {
  func prepareAttestation() async throws -> AmbushDevAttestation
  func attestation(_ prepared: AmbushDevAttestation, clientData: Data) async throws
    -> AmbushDevAttestation
  func assertion(clientData: Data) async throws -> String
}

struct AmbushDevAttestation: Equatable {
  let keyId: String
  let attestation: String
}

private enum AmbushSecureRandom {
  static func bytes(count: Int) throws -> Data {
    var bytes = [UInt8](repeating: 0, count: count)
    let status = SecRandomCopyBytes(kSecRandomDefault, bytes.count, &bytes)
    guard status == errSecSuccess else {
      throw AmbushDevPushEnrollmentError.randomGenerationFailed(status)
    }
    return Data(bytes)
  }
}

private enum AmbushAppAttestKeyId {
...
protocol AmbushAppAttestKeyIdStoring {
  func keyId() throws -> String?
  func saveKeyId(_ keyId: String) throws
}

struct AmbushAppAttestKeyIdKeychainStore: AmbushAppAttestKeyIdStoring {
  private static let service = "ambush.push.app-attest"
  private static let legacyService = "buzz.push.app-attest"
  ...
  func keyId() throws -> String? {
    if let keyId = try readKeyId(service: Self.service) {
      return keyId
    }
    guard let legacyKeyId = try readKeyId(service: Self.legacyService) else {
      return nil
    }
    try saveKeyId(legacyKeyId)
    return legacyKeyId
  }

  private func readKeyId(service: String) throws -> String? {
    ...
    guard let data = result as? Data,
      ...
      AmbushAppAttestKeyId.isValid(keyId)
    else {
      throw AmbushDevPushEnrollmentError.invalidAppAttestKeyId
    }
    return keyId
  }

  func saveKeyId(_ keyId: String) throws {
    guard AmbushAppAttestKeyId.isValid(keyId) else {
      throw AmbushDevPushEnrollmentError.invalidAppAttestKeyId
    }
    let data = Data(keyId.utf8)
    let updateStatus = update(
      baseQuery(service: Self.service) as CFDictionary,
...
protocol AmbushDCAppAttestServicing {
...
extension DCAppAttestService: AmbushDCAppAttestServicing {}

struct AmbushDCAppAttestProvider: AmbushDevAppAttesting {
  private let service: AmbushDCAppAttestServicing
  private let keyIdStore: AmbushAppAttestKeyIdStoring

  init(
    service: AmbushDCAppAttestServicing = DCAppAttestService.shared,
    keyIdStore: AmbushAppAttestKeyIdStoring
  ) {
  ...
  func prepareAttestation() async throws -> AmbushDevAttestation {
    ...
    return AmbushDevAttestation(keyId: keyId, attestation: "")
  }

  func attestation(
    _ prepared: AmbushDevAttestation,
    clientData: Data
  ) async throws -> AmbushDevAttestation {
    precondition(!clientData.isEmpty, "Enrollment client data must not be empty")
    try requireSupportedService()
    guard AmbushAppAttestKeyId.isValid(prepared.keyId),
  ...
  private func requireSupportedService() throws {
    guard service.isSupported else {
      throw AmbushDevPushEnrollmentError.appAttestUnsupported
    }
  }
}

/// Enrollment and delegation driver for real App Attest and the gated debug bypass.
public final class AmbushDevPushEnrollmentDriver {
  public static let appProfile = "ambush-ios-dogfood"
  public static let endpointEpoch: Int64 = 1

  private let gatewayBaseURL: URL
  private let store: AmbushPushEndpointGrantStore
  private let session: URLSession
  private let appAttest: AmbushDevAppAttesting
  ...
  public convenience init(
    gatewayBaseURL: URL,
    store: AmbushPushEndpointGrantStore,
    ...
    try self.init(
      ...
      store: store,
      session: session,
      appAttest: AmbushDCAppAttestProvider(
        keyIdStore: AmbushAppAttestKeyIdKeychainStore(
          accessGroup: appAttestKeychainAccessGroup
        )
      ),
      now: Date.init,
      lifetimeSeconds: 2_592_000,
      installationIdBytes: { try AmbushSecureRandom.bytes(count: 16) }
    )
  }
  ...
  init(
    ...
    store: AmbushPushEndpointGrantStore,
    session: URLSession,
    appAttest: AmbushDevAppAttesting,
    now: @escaping () -> Date,
    lifetimeSeconds: Int64,
    installationIdBytes: @escaping () throws -> Data = {
      try AmbushSecureRandom.bytes(count: 16)
    }
  ) throws {
    guard Self.isHTTPOrigin(gatewayBaseURL), lifetimeSeconds > 0 else {
      throw AmbushDevPushEnrollmentError.invalidGatewayURL
    }
  ...
  public func endpointGrants() throws -> [AmbushPushEndpointGrantRecord] {
    try store.records()
  }
  ...
  public func enroll(
    deviceToken: Data,
    relayURL: URL
  ) async throws -> AmbushPushEndpointGrantRecord {
      ...
      current.expiresAt > nowSeconds + 300
    {
      ...
      let refreshed = AmbushPushEndpointGrantRecord(
    ...
    // to an Ambush community. A second origin served by the same relay therefore
    ...
    {
      let record = AmbushPushEndpointGrantRecord(
        relayOrigin: relayOrigin.text,
    ...
    var pending: AmbushPushPendingEnrollmentRecord
      ...
      let existing = UUID(uuidString: handle)
    {
      let expiresAt =
        ...
        : renewedExpiration
      pending = AmbushPushPendingEnrollmentRecord(
        relayOrigin: relayOrigin.text,
        relayPubkey: relayPubkey,
        endpointHash: endpointHash,
        appProfile: Self.appProfile,
      ...
      let enrollmentClientData = try AmbushPushTranscript.enroll(
      ...
      guard attestation.keyId == preparedAttestation.keyId else {
        throw AmbushDevPushEnrollmentError.invalidResponse(route: "development attestation")
      }
      pending = AmbushPushPendingEnrollmentRecord(
        relayOrigin: relayOrigin.text,
        relayPubkey: relayPubkey,
      ...
      handle == existing.uuidString.lowercased()
    {
      ...
      guard let challengeId = pending.challengeId,
        ...
        throw AmbushDevPushEnrollmentError.invalidResponse(
      ...
      do {
        installation = try await enrollInstallation(
          ...
          attestation: AmbushDevAttestation(keyId: keyId, attestation: attestation)
        )
      } catch AmbushDevPushEnrollmentError.unexpectedStatus(
    ...
    if generationBase > 0 {
      ...
      guard !overflow, next > 0 else {
        throw AmbushDevPushEnrollmentError.generationExhausted
      }
      generation = next
    } else {
      generation = 1
    ...
    let delegationClientData = try AmbushPushTranscript.delegate(
  ...
  private func challenge() async throws -> Challenge {
    ...
    guard let id = UUID(uuidString: response.challengeId),
      ...
      throw AmbushDevPushEnrollmentError.invalidResponse(route: "v1/installations/challenges")
    }
  ...
  private func enrollInstallation(
    ...
    endpoint: String,
    expiresAt: Int64,
    attestation: AmbushDevAttestation
  ) async throws -> UUID {
    ...
    guard let installation = UUID(uuidString: response.installationHandle),
      ...
      throw AmbushDevPushEnrollmentError.invalidResponse(route: "v1/installations")
    }
    return installation
  }
  ...
  private func delegate(
    ...
    guard !response.endpointGrant.isEmpty, response.endpointGrant.utf8.count <= 4_096 else {
      throw AmbushDevPushEnrollmentError.invalidResponse(route: "v1/delegations")
    }
  ...
  private func fetchCurrentRelayKeys(from relayOrigin: URL) async throws -> RelayKeys {
    ...
    do {
      ...
      throw AmbushDevPushEnrollmentError.invalidRelayDescriptor
    }
  ...
  private func post<Request: Encodable, Response: Decodable>(
    ...
    do {
      ...
      throw AmbushDevPushEnrollmentError.invalidResponse(route: route)
    }
  }
  ...
  private static func expectStatus(
    ...
    guard http.statusCode == expected else {
      ...
      throw AmbushDevPushEnrollmentError.unexpectedStatus(
  ...
  private static func relayOrigin(_ url: URL) throws -> (url: URL, text: String) {
    guard url.scheme == "ws" || url.scheme == "wss",
      ...
      throw AmbushDevPushEnrollmentError.invalidRelayURL
    }
