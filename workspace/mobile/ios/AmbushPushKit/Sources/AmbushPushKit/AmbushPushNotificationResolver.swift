...
/// Content resolved from unread Ambush events for a mutable push notification.
public struct AmbushPushResolution: Decodable, Equatable, Sendable {
  public let title: String
  public let body: String
  public let subtitle: String?
  public let threadIdentifier: String?
  public let navigationTarget: AmbushPushNavigationTarget?
  ...
  public init(
    ...
    navigationTarget: AmbushPushNavigationTarget? = nil,
...
/// Resolves the content used to mutate a generic Ambush push notification.
public protocol AmbushPushNotificationResolving {
  func resolve(completion: @escaping (AmbushPushResolution?) -> Void)
}

/// Reads configured Ambush communities and resolves their newest unread event.
public final class AmbushPushNotificationResolver: AmbushPushNotificationResolving {
  ...
  public init(
    ...
    presentationCacheLifetime: TimeInterval = AmbushPushPresentationCacheStore.freshnessLifetime
  ) {
  ...
  public func resolve(completion: @escaping (AmbushPushResolution?) -> Void) {
  ...
  private func resolvePresentation(
    ...
    completion: @escaping (AmbushPushResolution?) -> Void
  ) {
    let snapshot = AmbushPushPresentationCacheSnapshot.decode(loadPresentationCacheData())
    let relayOrigin = AmbushPushPresentationCacheStore.canonicalRelayOrigin(community.relayUrl)
    ...
    let membershipNeedsRefresh: Bool = {
      ...
      guard memberCount <= AmbushPushPresentationCacheStore.maximumMembersPerChannel
      else { return false }
    ...
    refreshPresentation(
      ...
      let profile =
        refreshedProfileEvent.flatMap {
          guard
            AmbushPushPresentationCacheStore.shouldReplace(
  ...
  ) -> (AmbushPushResolution, VerifiedNostrEvent)? {
  ...
  private static func ephemeralChannel(
    ...
    cached: AmbushPushCachedChannel?,
    communityID: String,
    relayOrigin: String,
    ...
    let membership = membershipEvent.flatMap {
      AmbushPushPresentationCacheStore.normalizedChannelMembership(
        $0,
        communityID: communityID,
        channelID: channelID
      )
    ...
    return AmbushPushCachedChannel(
      communityID: communityID,
      relayOrigin: relayOrigin,
      channelID: channelID,
      relayMetadataPubkey: relayMetadataPubkey,
      displayName: metadataEvent.map {
        AmbushPushPresentationCacheStore.normalizedDisplayName(tagValue("name", in: $0))
      } ?? cached?.displayName,
      channelType: metadataEvent.map {
        AmbushPushPresentationCacheStore.normalizedChannelType(tagValue("t", in: $0))
      } ?? cached?.channelType,
  ...
  private static func communicationConversation(
    ...
    let currentUserDigest = AmbushPushPresentationIdentity.channelMember(
      communityID: community.id,
      channelID: channel.channelID,
      pubkey: currentUser
    )
    guard memberDigests.contains(currentUserDigest) else { return nil }
    let senderDigest = AmbushPushPresentationIdentity.channelMember(
      communityID: community.id,
      channelID: channel.channelID,
      pubkey: event.pubkey
    )
