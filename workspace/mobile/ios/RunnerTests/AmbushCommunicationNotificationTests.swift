...
final class AmbushCommunicationNotificationTests: XCTestCase {
  func testIntentUsesVerifiedSenderAvatarAndGroupName() throws {
    ...
    let descriptor = try XCTUnwrap(
      AmbushCommunicationNotificationDescriptor(resolution: resolution)
    )

    let intent = AmbushCommunicationNotificationPresenter.makeIntent(descriptor)
    ...
    XCTAssertEqual(intent.content, "Hello Ambush")
  ...
  func testMissingVerifiedRecipientCountUsesOrdinaryPresentation() {
    XCTAssertNil(
      AmbushCommunicationNotificationDescriptor(
  ...
  func testPresentationFallsBackWhenDonationFails() {
    ...
    ordinary.body = "Hello Ambush"
    var updateCalled = false
    let presenter = AmbushCommunicationNotificationPresenter(
    ...
    presenter.present(
      ...
      XCTAssertEqual(content.body, "Hello Ambush")
      completed.fulfill()
    }
  ...
  private func communicationResolution(
    ...
    return AmbushPushResolution(
      ...
      body: "Hello Ambush",
      subtitle: "Community",
      threadIdentifier: AmbushPushPresentationIdentity.conversation(
        communityID: communityID,
        channelID: channelID
      ),
      navigationTarget: AmbushPushNavigationTarget(
        eventID: "message-id",
        communityID: communityID,
        channelID: channelID
      ),
      ...
      conversationIdentifier: AmbushPushPresentationIdentity.conversation(
        communityID: communityID,
        channelID: channelID
      ),
...
final class AmbushPushSnapshotEnrichmentTests: XCTestCase {
  func testMetadataAuthorityUsesCurrentAppProfileForMatchingRelay() {
    let correctProfile = grant(
      appProfile: AmbushDevPushEnrollmentDriver.appProfile,
      generation: 2,
    ...
    XCTAssertEqual(
      AmbushPushSnapshotBridge.relayMetadataPubkey(
...
final class AmbushPushNotificationResponseTests: XCTestCase {
  func testValidDefaultActionRoutesAndCompletesExactlyOnce() {
    ...
    var routedTargets: [AmbushPushNavigationTarget] = []
    var forwarded = 0
    var completions = 0

    AmbushPushNotificationResponseCoordinator.handle(
      actionIdentifier: UNNotificationDefaultActionIdentifier,
      userInfo: [AmbushPushNavigationTarget.userInfoKey: target.userInfoValue],
  ...
  func testMalformedTargetFallsBackToOneCompletion() {
    ...
    AmbushPushNotificationResponseCoordinator.handle(
      actionIdentifier: UNNotificationDefaultActionIdentifier,
      userInfo: [AmbushPushNavigationTarget.userInfoKey: ["event_id": ""]],
      onTarget: { routedTargets.append($0) },
