//! Tauri commands for exporting and importing `ambush-team-snapshot v1` files.
//!
...
const LEGACY_TEAM_ERROR: &str =
    "Legacy team files are no longer supported. Export an ambush-team-snapshot v1 .team.json or .team.png instead.";
...
pub async fn confirm_team_snapshot_import(
    ...
    for (member, definition) in snapshot.members.iter().zip(definitions) {
        ...
        let (agent_keys, private_key_nsec, pubkey, auth_tag) = {
            ...
            // NIP-OA auth tag: bridge nostr 0.37 → 0.36 (ambush-sdk) via hex round-trip.
            let compat_owner = nostr::Keys::parse(&owner_keys.secret_key().to_secret_hex())
                .map_err(|e| format!("failed to bridge owner keys: {e}"))?;
            let compat_agent = nostr::PublicKey::from_hex(&pubkey)
                .map_err(|e| format!("failed to bridge agent pubkey: {e}"))?;
            let auth_tag = Some(
                ambush_sdk_pkg::nip_oa::compute_auth_tag(&compat_owner, &compat_agent, "")
    ...
    for (m, snap_member) in minted.iter().zip(snapshot.members.iter()) {
        ...
        if memory_total > 0 {
            ...
            for (idx, entry) in snap_member.memory.entries.iter().enumerate() {
                let body = if entry.slug == ambush_core_pkg::engram::CORE_SLUG {
                    ambush_core_pkg::engram::Body::Core {
                        profile: entry.body.clone(),
                    }
                } else {
                    ambush_core_pkg::engram::Body::Memory {
                ...
                match ambush_core_pkg::engram::build_event(
                    &m.agent_keys,
                    &owner_pubkey,
                    &body,
                    created_at,
...
fn retain_agent_pending(app: &AppHandle, state: &AppState, record: &ManagedAgentRecord) {
    ...
    use ambush_core_pkg::kind::KIND_MANAGED_AGENT;
    use nostr::JsonUtil;
    ...
    if let Err(e) = result {
        eprintln!("ambush-desktop: team-snapshot-import retain-agent: {e}");
    }
}
