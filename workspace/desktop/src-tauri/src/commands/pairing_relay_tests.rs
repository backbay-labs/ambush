...
fn configured_pairing_relay_takes_precedence_over_legacy_path() {
    let document = serde_json::json!({
        "pairing_relay_url": "wss://pairing.ambush.example.com",
        "supported_nips": [43]
    });

    assert_eq!(
        pairing_relay_from_nip11(&document),
        PairingRelay::Configured("wss://pairing.ambush.example.com".to_string())
    );
}
...
fn invalid_pairing_relay_url_falls_back_to_legacy_path() {
    let document = serde_json::json!({
        "pairing_relay_url": "https://pairing.ambush.example.com",
        "supported_nips": [43]
    });
...
fn configured_pairing_relay_resolves_to_configured_url() {
    let resolved = resolve_pairing_relay_url(
        "wss://flint.communities.ambush.example.com",
        PairingRelay::Configured("wss://pairing.ambush.example.com".to_string()),
    )
    .expect("resolve configured pairing relay");

    assert_eq!(resolved, "wss://pairing.ambush.example.com");
}

#[test]
fn legacy_pairing_relay_appends_pair_path() {
    let resolved = resolve_pairing_relay_url(
        "wss://flint.communities.ambush.example.com/community",
        PairingRelay::LegacyPath,
    )
    ...
    assert_eq!(
        resolved,
        "wss://flint.communities.ambush.example.com/community/pair"
    );
}
