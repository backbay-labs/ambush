...
/// Read the workspace owner pubkey without holding the lock. Used to populate `AMBUSH_ACP_AGENT_OWNER`
...
fn normalize_relay_mesh(
    ...
    if model_ref.is_empty() {
        return Err("Ambush shared compute model is required".to_string());
    }
    if backend != &BackendKind::Local {
        return Err("Ambush shared compute agents must use the local backend".to_string());
    }
...
pub async fn create_managed_agent(
    ...
    // The harness has its own validator (ambush-acp/src/config.rs) but we want
    ...
    let auth_tag = {
        ...
        // Bridge nostr 0.37 → 0.36 (ambush-sdk) via hex round-trip.
        ...
        let tag = ambush_sdk_pkg::nip_oa::compute_auth_tag(&compat_owner, &compat_agent, "")
    ...
    let (agent, resolved_avatar_url) = {
        ...
        let record = ManagedAgentRecord {
            ...
            agent_args,
            mcp_command,
            // AMBUSH_ACP_TURN_TIMEOUT is deprecated and ignored by the harness;
...
pub async fn start_managed_agent(
            ...
            .load(std::sync::atomic::Ordering::Acquire)
    {
        ...
        tauri::async_runtime::spawn(async move {
            ...
            {
                eprintln!(
                    "ambush-desktop: profile reconciliation failed for agent {reconcile_pubkey}: {e}"
                );
            }
        });
    }
