...
fn with_no_goose_config<T>(body: impl FnOnce() -> T) -> T {
    ...
    std::env::set_var("GOOSE_PATH_ROOT", "/nonexistent-ambush-test-path");
    let output = body();
    match prior {
...
/// not the stale byte as `AmbushExplicit`.
#[test]
...
/// instances only.
#[test]
fn definition_less_explicit_record_model_keeps_ambush_explicit_origin() {
    let mut record = agent_record();
...
/// `AmbushExplicit` (its true source — NOT `PersonaDefault`). FAILS against the
/// shipped no-persona early-return, which left X as primary and Y struck.
#[test]
fn genuine_explicit_live_switch_renders_y_over_x_ambush_explicit_secondary() {
    let mut record = agent_record();
    record.persona_id = None;
    record.model = Some("model-x".to_string());
    let personas: Vec<AgentDefinition> = vec![];
    ...
    assert_eq!(model.overridden_origin, Some(ConfigOrigin::AmbushExplicit));
}
...
fn global_default_live_switch_renders_global_model_as_secondary_global_default() {
    ...
    assert_eq!(
        ...
        "override baseline origin must be GlobalDefault, not PersonaDefault or AmbushExplicit"
    );
}
...
fn reserved_key_in_inherited_persona_env_is_stripped() {
    ...
    // AMBUSH_PRIVATE_KEY is a reserved key — must be stripped.
    persona
        .env_vars
        .insert("AMBUSH_PRIVATE_KEY".to_string(), "nsec-secret".to_string());
    ...
    assert!(
        !tiers.persona_env.contains_key("AMBUSH_PRIVATE_KEY"),
...
fn reserved_key_in_definition_env_shaped_map_is_stripped_by_sanitize() {
    ...
    raw.insert("AMBUSH_PRIVATE_KEY".to_string(), "nsec-secret".to_string());
    raw.insert("GOOSE_MODEL".to_string(), "harness-model".to_string());

    let sanitized = sanitize_inherited_env(&raw);

    assert!(
        !sanitized.contains_key("AMBUSH_PRIVATE_KEY"),
...
fn baked_env_non_secret_key_shows_real_value() {
    let entries = baked_env_from_map(&[("AMBUSH_AGENT_PROVIDER", "databricks_v2")]);
    assert_eq!(entries.len(), 1);
    assert_eq!(entries[0].key, "AMBUSH_AGENT_PROVIDER");
...
fn baked_env_empty_value_filtered_out() {
    let entries = baked_env_from_map(&[("AMBUSH_AGENT_PROVIDER", "")]);
...
fn baked_env_mixed_keys_correct_masking() {
    let entries = baked_env_from_map(&[
        ...
        ("AMBUSH_AGENT_MODEL", "goose-claude-opus-4-8"),
    ...
    let provider = entries
        .iter()
        .find(|e| e.key == "AMBUSH_AGENT_PROVIDER")
        .unwrap();
    ...
    let model = entries
        .iter()
        .find(|e| e.key == "AMBUSH_AGENT_MODEL")
        .unwrap();
...
fn baked_env_thinking_effort_is_unmasked() {
    // AMBUSH_AGENT_THINKING_EFFORT is a non-secret enum — must not be masked.
    let entries = baked_env_from_map(&[("AMBUSH_AGENT_THINKING_EFFORT", "medium")]);
    assert_eq!(entries.len(), 1);
    let effort = entries
        .iter()
        .find(|e| e.key == "AMBUSH_AGENT_THINKING_EFFORT")
        .unwrap();
...
fn baked_env_thinking_summary_is_unmasked() {
    // AMBUSH_AGENT_THINKING_SUMMARY is a non-secret enum — must not be masked.
    let entries = baked_env_from_map(&[("AMBUSH_AGENT_THINKING_SUMMARY", "detailed")]);
    assert_eq!(entries.len(), 1);
    let summary = entries
        .iter()
        .find(|e| e.key == "AMBUSH_AGENT_THINKING_SUMMARY")
        .unwrap();
...
fn baked_env_allowlist_is_case_insensitive() {
    ...
    assert!(super::is_safe_to_reveal("ambush_agent_provider"));
    assert!(super::is_safe_to_reveal("AMBUSH_AGENT_PROVIDER"));
    assert!(super::is_safe_to_reveal("ambush_agent_model"));
    assert!(super::is_safe_to_reveal("AMBUSH_AGENT_MODEL"));
    assert!(super::is_safe_to_reveal("ambush_agent_thinking_effort"));
    assert!(super::is_safe_to_reveal("AMBUSH_AGENT_THINKING_EFFORT"));
    assert!(super::is_safe_to_reveal("ambush_agent_thinking_summary"));
    assert!(super::is_safe_to_reveal("AMBUSH_AGENT_THINKING_SUMMARY"));
