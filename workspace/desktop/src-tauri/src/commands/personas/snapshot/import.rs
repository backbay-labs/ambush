//! Import-side helpers for `ambush-agent-snapshot v1`.
//!
...
/// Decode a `ambush-agent-snapshot v1` manifest from raw bytes.
///
...
/// Import a `ambush-agent-snapshot v1` file as a brand-new agent.
///
/// Phase sequence:
...
fn retain_agent_pending(app: &AppHandle, state: &AppState, record: &ManagedAgentRecord) {
    ...
    if let Err(e) = result {
        eprintln!("ambush-desktop: snapshot-import retain-agent: {e}");
    }
}
