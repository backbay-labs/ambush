...
fn migrate_legacy_retention_into(
    ...
    match crate::managed_agents::retention::migrate_legacy_retention_db(
        ...
        Ok(copied) => {
            eprintln!(
                "ambush-desktop: adopted {copied} legacy retained event(s) into this community"
            )
        }
        Err(error) => eprintln!("ambush-desktop: legacy retention migration failed: {error}"),
    }
}
...
pub async fn apply_workspace(
    ...
    tokio::task::spawn_blocking(move || {
        ...
        if let Some(nest) = nest.as_deref() {
            if let Err(error) = write_persisted_repos_dir(nest, effective_repos_dir.as_deref()) {
                eprintln!("ambush-desktop: persist repos dir failed: {error}");
            }
            if let Err(error) = ensure_repos_symlink(nest, effective_repos_dir.as_deref()) {
                eprintln!("ambush-desktop: repos dir setup failed: {error}");
                let _ = app.emit("repos-dir-error", error);
            }
        }
    ...
    // The built-in research agent migration may have renamed stopped agents.
    // Reconcile their relay profiles independently of runtime restore;
    // successful writes record this relay while retaining the agent for other
    // communities, and failures retry on the next workspace apply.
    ...
    #[cfg(feature = "mesh-llm")]
    {
        ...
        tauri::async_runtime::spawn(async move {
            ...
            if restore_pending {
                ...
                {
                    eprintln!("ambush-desktop: failed to restore Share Compute: {error}");
                }
            }
            ...
            if restore_pending {
                ...
                {
                    eprintln!("ambush-desktop: failed to restore managed agents: {error}");
                }
            }
        });
        return Ok(());
