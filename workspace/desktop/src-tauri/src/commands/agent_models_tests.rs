...
fn saved_agent_model_discovery_uses_record_snapshot_for_definition_less_agent() {
    let record: crate::managed_agents::ManagedAgentRecord = serde_json::from_str(
        r#"{
            ...
            "env_vars": {
                ...
                "AMBUSH_PRIVATE_KEY": "must-not-leak"
            },
    ...
    assert!(!discovery.env.contains_key("AMBUSH_PRIVATE_KEY"));
...
fn effective_discovery_provider_prefers_the_explicit_provider() {
    let env = BTreeMap::from([(
        "AMBUSH_AGENT_PROVIDER".to_string(),
    ...
    assert_eq!(
        effective_discovery_provider(Some("anthropic"), Some("AMBUSH_AGENT_PROVIDER"), &env)
            .as_deref(),
        Some("anthropic")
    );
}
...
fn effective_discovery_provider_recovers_baked_provider_when_record_has_none() {
    ...
    for provider in [None, Some(""), Some("   ")] {
        assert_eq!(
            effective_discovery_provider(provider, Some("AMBUSH_AGENT_PROVIDER"), &env).as_deref(),
            Some("databricks_v2"),
...
/// what the developer happens to have exported (e.g. `AMBUSH_AGENT_PROVIDER`).
const UNSET_PROVIDER_VAR: &str = "AMBUSH_TEST_UNSET_DISCOVERY_PROVIDER";
...
const UNSET_CREDENTIAL: &str = "AMBUSH_TEST_UNSET_DISCOVERY_CREDENTIAL";
...
fn env_derived_provider_falls_through_when_its_credential_is_missing() {
    ...
    // Ambush's env — so the provider is visible here and the credential is not.
...
fn effective_discovery_provider_reads_the_runtimes_own_env_var() {
    // goose keys its provider off GOOSE_PROVIDER, so a AMBUSH_AGENT_PROVIDER in
...
fn absent_filter_value_uses_process_value_when_available() {
    const TEST_FILTER_ENV: &str = "AMBUSH_TEST_DATABRICKS_MODEL_FILTER";
...
fn databricks_filtered_empty_response_is_authoritative() {
    let filter = ambush_agent_pkg::config::DatabricksModelFilter::parse(Some("allowed-*")).unwrap();
...
// Parse/filter/pagination tests live in crates/ambush-agent/src/catalog.rs
...
fn update_request_turn_timeout_parses_for_wire_compat() {
    ...
    // (AMBUSH_ACP_TURN_TIMEOUT is deprecated and ignored by the harness). That
...
fn databricks_passive_auth_error_has_reachable_create_flow_guidance() {
    ...
    assert!(error.contains("ambush-agent auth databricks"));
}
...
fn openrouter_draft_agent_model_discovery_derives_provider_env() {
    ...
    assert_eq!(
        merged.get("AMBUSH_AGENT_PROVIDER").map(String::as_str),
        Some("openrouter"),
...
fn draft_agent_model_discovery_env_omits_provider_when_absent() {
    let merged =
        draft_agent_model_discovery_env("ambush-agent", None, &BTreeMap::new(), &BTreeMap::new());
    assert!(
        !merged.contains_key("AMBUSH_AGENT_PROVIDER"),
...
/// `AMBUSH_PRIVATE_KEY` proves a reserved key cannot ride in on a harness
...
fn draft_agent_model_discovery_env_layers_all_three_tiers_in_order() {
    ...
    let definition_env = BTreeMap::from([
        ...
        // Collides with tier 1: `ambush-agent`'s own provider env var, which the
        // `provider` argument derives below.
        ("AMBUSH_AGENT_PROVIDER".to_string(), "openai".to_string()),
        ...
        ("AMBUSH_PRIVATE_KEY".to_string(), "do-not-leak".to_string()),
    ]);
    ...
    // Tier 1 (floor): `Some("openrouter")` derives AMBUSH_AGENT_PROVIDER.
    let merged = draft_agent_model_discovery_env(
        "ambush-agent",
        Some("openrouter"),
        &definition_env,
    ...
    let expected: &[(&str, Option<&str>)] = &[
        ...
        ("AMBUSH_AGENT_PROVIDER", Some("openai")),
        // Tier 3 over tier 2.
