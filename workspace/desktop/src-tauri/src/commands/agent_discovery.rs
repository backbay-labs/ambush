...
/// built-in id (goose, claude, codex, ambush-agent) returns an error without
...
fn install_acp_runtime_blocking(
    ...
    // Ambush-private prefix before execution.
...
async fn restart_single_agent_after_install(
    ...
    let runtime_keys = match stop_result {
        ...
        Ok(Err(e)) => {
            eprintln!("ambush-desktop: install_acp_runtime: skipping restart of {pubkey}: {e}");
            return InstallRestartOutcome::Skipped;
        }
        Err(e) => {
            eprintln!(
                "ambush-desktop: install_acp_runtime: spawn_blocking failed for stop of {pubkey}: {e}"
            );
            return InstallRestartOutcome::Skipped;
        }
    };
        ...
        .await
    {
        Ok(_) => {
            eprintln!(
                "ambush-desktop: install_acp_runtime: restarted setup-mode agent {pubkey} after install"
            );
            InstallRestartOutcome::Restarted
        }
        Err(e) => {
            eprintln!(
                "ambush-desktop: install_acp_runtime: failed to start {pubkey} after install: {e}"
            );
            if let Err(save_err) = persist_last_error_on_install(app, pubkey, &e) {
                eprintln!(
                    "ambush-desktop: install_acp_runtime: failed to persist last_error for {pubkey}: {save_err}"
                );
            }
...
/// costs Ambush's managed Node/npm dirs their precedence. A positional rather than
...
/// Ambush-managed npm locations set, and the user's PATH set. This is the
...
///
/// On Windows, resolves Git Bash via `resolve_bash_path` (skips `AMBUSH_SHELL`
...
fn install_shell_command(command: &str) -> Result<std::process::Command, String> {
    ...
    // where the composed PATH would otherwise be Ambush's managed Node dirs
    ...
    let managed: Vec<std::path::PathBuf> = [
        crate::managed_agents::ambush_managed_node_bin_dir(),
        crate::managed_agents::ambush_managed_npm_bin_dir(),
    ]
    .into_iter()
    .flatten()
    .collect();
...
/// Windows: Git Bash via `resolve_bash_path` — skips `AMBUSH_SHELL` because install
/// commands use bash-only `-l -c` syntax. A `AMBUSH_SHELL=pwsh` user gets a green
...
/// Strips hermit-managed vars and establishes the Ambush-managed npm prefix so adapters
...
fn apply_npm_env(cmd: &mut std::process::Command) {
    ...
    if let Some(prefix) = crate::managed_agents::ambush_managed_npm_prefix() {
...
fn install_powershell_command(command: &str) -> std::process::Command {
    ...
    // Compose PATH: managed Ambush dirs first, then inherited process PATH.
...
mod tests {
    ...
    fn test_install_shell_args_shape_per_platform() {
        let composed = std::ffi::OsString::from("/ambush/node/bin:/usr/bin");
        let windows_composed = std::ffi::OsString::from(r"C:\ambush\node;C:\Windows\system32");
    ...
    fn test_composed_path_survives_a_profile_that_clears_it() {
        ...
        let composed = std::ffi::OsString::from("/ambush/sentinel/bin:/usr/bin:/bin");
        ...
        assert!(
            path.contains("/ambush/sentinel/bin"),
    ...
    fn test_install_shell_command_includes_process_path_on_windows() {
        ...
        // The sentinel must appear LAST — managed Ambush dirs must have precedence.
        assert!(
    ...
    /// ambush-agent has no install commands on any platform.
    #[test]
    fn test_ambush_agent_has_no_install_commands() {
        let ambush = crate::managed_agents::known_acp_runtime_exact("ambush-agent").unwrap();
        assert!(
            ambush.cli_install_commands_for_os().is_empty(),
            "ambush-agent ships with the app — must never have install commands"
        );
    }
    ...
    fn test_powershell_command_argv_exact() {
        ...
        let body = "$ErrorActionPreference='Stop'; $installer=Join-Path $env:TEMP 'ambush-install-codex.ps1'; Invoke-RestMethod https://chatgpt.com/codex/install.ps1 -OutFile $installer; & $installer; exit $LASTEXITCODE";
    ...
    fn test_powershell_command_claude_catalog_dequoted() {
        let cmd = super::install_powershell_command(
            r#"powershell.exe -NoProfile -ExecutionPolicy Bypass -Command "$ErrorActionPreference='Stop'; $installer=Join-Path $env:TEMP 'ambush-install-claude.ps1'; Invoke-RestMethod https://claude.ai/install.ps1 -OutFile $installer; & $installer; exit $LASTEXITCODE""#,
        );
        assert_eq!(
            cmd.get_args()
                .map(|a| a.to_string_lossy().into_owned())
            ...
            vec![
                ...
                "Bypass",
                "-Command",
                "$ErrorActionPreference='Stop'; $installer=Join-Path $env:TEMP 'ambush-install-claude.ps1'; Invoke-RestMethod https://claude.ai/install.ps1 -OutFile $installer; & $installer; exit $LASTEXITCODE",
            ],
    ...
    fn test_powershell_command_goose_catalog_dequoted() {
        let cmd = super::install_powershell_command(
            r#"powershell.exe -NoProfile -ExecutionPolicy Bypass -Command "$env:CONFIGURE='false'; $ErrorActionPreference='Stop'; $installer=Join-Path $env:TEMP 'ambush-install-goose.ps1'; Invoke-RestMethod https://raw.githubusercontent.com/aaif-goose/goose/main/download_cli.ps1 -OutFile $installer; & $installer; exit $LASTEXITCODE""#,
        );
        assert_eq!(
            cmd.get_args()
                .map(|a| a.to_string_lossy().into_owned())
            ...
            vec![
                ...
                "Bypass",
                "-Command",
                "$env:CONFIGURE='false'; $ErrorActionPreference='Stop'; $installer=Join-Path $env:TEMP 'ambush-install-goose.ps1'; Invoke-RestMethod https://raw.githubusercontent.com/aaif-goose/goose/main/download_cli.ps1 -OutFile $installer; & $installer; exit $LASTEXITCODE",
            ],
...
/// Returns the Windows-only Git Bash prerequisite used by ambush-agent's shell MCP.
