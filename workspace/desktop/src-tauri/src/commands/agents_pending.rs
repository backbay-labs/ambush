...
pub(crate) fn retain_managed_agent_pending(
    ...
    if let Err(e) = result {
        eprintln!("ambush-desktop: agent-retain: {e}");
    }
}
...
pub(crate) fn tombstone_managed_agent_pending(
    ...
    if let Err(e) = result {
        eprintln!("ambush-desktop: agent-tombstone: {e}");
    }
}
...
pub(crate) fn tombstone_managed_agent_at(
    ...
    use ambush_core_pkg::kind::{KIND_IA_ARCHIVE_REQUEST, KIND_MANAGED_AGENT};
    use nostr::JsonUtil;
...
pub(crate) fn build_agent_archive_request(
        ...
        .eq_ignore_ascii_case(agent_pubkey)
    {
        ...
        let tag_json = ambush_sdk_pkg::nip_oa::compute_auth_tag(keys, &agent, "")
...
mod tests {
    ...
    fn agent_tombstone_enqueues_archive_with_persona_id_from_head_atomically() {
        ...
        use ambush_core_pkg::kind::KIND_IA_ARCHIVE_REQUEST;
