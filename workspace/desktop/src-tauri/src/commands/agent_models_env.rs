...
/// with the key in goose's own keyring rather than Ambush's env) would otherwise
...
impl DiscoveryProvider {
    ...
    /// the caller must fall through to the `ambush-acp models` subprocess rather
...
/// — even on an internal build that bakes `AMBUSH_AGENT_PROVIDER=databricks_v2`
/// and a `DATABRICKS_HOST`. Discovery then degrades to the `ambush-acp models`
