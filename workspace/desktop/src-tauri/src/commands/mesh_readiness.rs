//! Startup readiness for Ambush shared compute.
//!
...
fn mesh_readiness_failure_message(
    ...
    match failure {
        MeshReadinessFailure::CatalogNeverSynced => format!(
            "Ambush shared compute connected to the serving member but could not sync \
        ...
        MeshReadinessFailure::RoutingNeverCompleted => format!(
            "Ambush shared compute found \"{model_id}\" on a serving member but inference \
