...
fn databricks_agent_provider(provider: &str) -> ambush_agent_pkg::config::Provider {
    if provider.trim().eq_ignore_ascii_case("databricks_v2")
        || provider.trim().eq_ignore_ascii_case("databricks-v2")
    {
        ambush_agent_pkg::config::Provider::DatabricksV2
    } else {
        ambush_agent_pkg::config::Provider::Databricks
    }
}
...
pub(super) fn databricks_sign_in_required_error() -> String {
    "Databricks sign-in is required; save this agent, then open its model picker to sign in, or run `ambush-agent auth databricks`"
        .to_string()
}

pub(super) fn databricks_sign_in_timed_out_error() -> String {
    "Databricks sign-in timed out; open the model picker to retry, or run `ambush-agent auth databricks`"
        .to_string()
}
...
pub(super) async fn discover_databricks_models(
    ...
    let parsed_filter = ambush_agent_pkg::config::DatabricksModelFilter::parse(filter.as_deref())
        .map_err(|error| format!("invalid DATABRICKS_MODEL_FILTER: {error}"))?;
    let config = ambush_agent_pkg::config::Config::for_discovery(
    ...
    let entries = match ambush_agent_pkg::discover_databricks_models(&config).await {
        Ok(entries) => entries,
        Err(ambush_agent_pkg::AgentError::LlmAuth(_))
            if should_start_interactive_auth(&api_key) =>
        {
            let _auth = AUTH_GATE.lock().await;
            match ambush_agent_pkg::discover_databricks_models(&config).await {
                ...
                Err(ambush_agent_pkg::AgentError::LlmAuth(_)) => {
                    ...
                    run_interactive_databricks_auth(
                        ambush_agent_pkg::authenticate_databricks(&host),
                        AUTH_FLOW_TIMEOUT,
                        &AUTH_COOLDOWNS,
                        &host,
                        &redaction_env,
                    ...
                    .await?;
                    ambush_agent_pkg::discover_databricks_models(&config)
                        .await
                        .map_err(|error| {
        ...
        Err(ambush_agent_pkg::AgentError::LlmAuth(error)) if !api_key.is_empty() => {
...
pub(super) fn databricks_models_response(
    provider_name: &str,
    entries: Vec<ambush_agent_pkg::ModelEntry>,
    selected_model: Option<String>,
    filter: Option<&ambush_agent_pkg::config::DatabricksModelFilter>,
...
where
    Fut: std::future::Future<Output = Result<(), ambush_agent_pkg::AgentError>>,
{
