...
pub(crate) struct HarnessDefinition {
    ...
    /// first and LOSES on conflict with Ambush-injected vars — `AMBUSH_MANAGED_AGENT`
...
fn validate_harness_definition(def: &HarnessDefinition) -> Result<(), String> {
    ...
    // `AMBUSH_ACP_AGENT_ARGS` env transport (clap `value_delimiter = ','` on the
    // ambush-acp side), so a literal comma inside one argument would silently
    ...
    // reserved-key bypass exploit (AMBUSH_AUTH_TAG=x forgery shape), rejects
...
fn builtin_ids() -> impl Iterator<Item = &'static str> {
    const TIER1: &[&str] = &["goose", "claude", "codex", "ambush-agent"];
...
// spawn time, causing silent fallback to ambush-agent.
//
...
// registry, the caller gets a typed error — never a silent ambush-agent fallback.
...
mod tests {
    ...
    fn builtin_ids_are_rejected() {
        ...
        for id in &["goose", "claude", "codex", "ambush-agent"] {
    ...
    fn validate_rejects_malformed_key_with_equals_sign() {
        // AMBUSH_AUTH_TAG=x is the documented reserved-key bypass shape:
        // the key contains '=' so Command::env would produce
        // `AMBUSH_AUTH_TAG=x=forged` in the child env.
        let mut env = BTreeMap::new();
        env.insert("AMBUSH_AUTH_TAG=x".to_string(), "forged".to_string());
        let def = HarnessDefinition {
        ...
        assert!(
            err.contains("AMBUSH_AUTH_TAG"),
            "error must name the offending key: {err}"
        );
    }

    #[test]
    fn validate_rejects_reserved_key_ambush_managed_agent() {
        // AMBUSH_MANAGED_AGENT and AMBUSH_MANAGED_AGENT_START_NONCE are the
        // ownership markers — supplying them in a definition must be rejected.
        let mut env = BTreeMap::new();
        env.insert(
            "AMBUSH_MANAGED_AGENT".to_string(),
        ...
        let err = validate_harness_definition_pub(&def).unwrap_err();
        assert!(
            err.contains("reserved by Ambush"),
    ...
    fn validate_rejects_reserved_key_case_insensitive() {
        // AMBUSH_PRIVATE_KEY in any casing must be blocked.
        let mut env = BTreeMap::new();
        env.insert("ambush_private_key".to_string(), "secret".to_string());
        let def = HarnessDefinition {
    ...
    /// validation boundary — the comma-delimited `AMBUSH_ACP_AGENT_ARGS`
