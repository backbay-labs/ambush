...
use super::agent_env::{build_ambush_agent_provider_defaults, idle_pool_sleep_env};
...
#[cfg(test)]
use process::{
    ambush_marker_entry, name_matches_interpreter, name_matches_known_binary,
    terminate_runtime_receipt_with, valid_agent_runtime_receipt_with,
};
pub(crate) use process::{
    current_instance_id, process_belongs_to_us, process_has_ambush_marker, process_is_running,
...
pub fn spawn_agent_child(
    ...
    let resolved_mcp_command: Option<std::path::PathBuf> = if effective_mcp_command.is_empty() {
        ...
        match resolve_command(effective_mcp_command) {
            ...
            None => {
                eprintln!(
                    "ambush-desktop: mcp_command {effective_mcp_command:?} not found, skipping"
                );
                None
            }
        }
    ...
    //   - bundled sidecars (ambush, ambush-acp, etc.) via exe parent (Contents/MacOS/)
    ...
    command.env("AMBUSH_PRIVATE_KEY", &record.private_key_nsec);
    command.env("AMBUSH_RELAY_URL", &effective_relay_url);
    command.env("AMBUSH_ACP_LAZY_POOL", if lazy { "true" } else { "false" });
    command.env("AMBUSH_ACP_IDLE_POOL_SLEEP", idle_pool_sleep_env(lazy));
    command.env("AMBUSH_ACP_AGENT_COMMAND", &resolved_agent_command);
    command.env("AMBUSH_ACP_AGENT_ARGS", agent_args.join(","));
    match &resolved_mcp_command {
        Some(mcp_cmd) => {
            command.env("AMBUSH_ACP_MCP_COMMAND", mcp_cmd);
        }
        None => {
            command.env("AMBUSH_ACP_MCP_COMMAND", "");
        }
    }
    // Enable MCP hook tools (_Stop, _PostCompact) for agents that need them.
    // Uses "*" because build_mcp_servers() hard-codes the server name to "ambush-mcp".
    ...
    // into AMBUSH_ACP_SETUP_PAYLOAD.  ambush-acp detects this env var on startup
    // and enters the minimal setup-listener mode instead of the agent pool.
    //
    // SECURITY: AMBUSH_ACP_SETUP_PAYLOAD is in RESERVED_ENV_KEYS so user env
    ...
    // source and ambush-acp only transports the payload.
    //
    // The JSON format mirrors `setup_mode::SetupPayload` in ambush-acp:
    ...
    let spawned_setup_mode;
    {
        ...
        let setup_payload_json =
            if let AgentReadiness::NotReady { requirements } = agent_readiness(&effective) {
                ...
                match serde_json::to_string(&payload) {
                    ...
                    Err(e) => {
                        eprintln!(
                            "ambush-desktop: failed to serialize setup payload for {}: {e}",
                            record.name
                        );
                        None
                    }
        ...
        //   1. AMBUSH_ACP_SETUP_PAYLOAD is in RESERVED_ENV_KEYS, so
        ...
        command.env_remove("AMBUSH_ACP_SETUP_PAYLOAD");

        // Set the payload only when desktop computed NotReady.
        if let Some(json) = setup_payload_json {
            command.env("AMBUSH_ACP_SETUP_PAYLOAD", json);
            eprintln!(
                "ambush-desktop: agent {} not ready — spawning in setup-listener mode",
                record.name
            );
        }
    }
    // Emit AMBUSH_ACP_IDLE_TIMEOUT only when explicitly set; the harness
    // DEFAULT_IDLE_TIMEOUT_SECS is the single source of truth. The deprecated
    // AMBUSH_ACP_TURN_TIMEOUT pinned agents to a stale default (320s).
    if let Some(idle) = record.idle_timeout_seconds {
        command.env("AMBUSH_ACP_IDLE_TIMEOUT", idle.to_string());
    }

    if let Some(max_dur) = record.max_turn_duration_seconds {
        command.env("AMBUSH_ACP_MAX_TURN_DURATION", max_dur.to_string());
    }
    let acp_n = super::acp_agents_value(effective_command, record.parallelism);
    command.env("AMBUSH_ACP_AGENTS", acp_n);
    command.env("AMBUSH_ACP_MULTIPLE_EVENT_HANDLING", "steer");
    command.env("AMBUSH_ACP_DEDUP", "queue");
    ...
    if let Some(instructions) = &team_instructions {
        command.env("AMBUSH_ACP_TEAM_INSTRUCTIONS", instructions);
    } else {
        command.env_remove("AMBUSH_ACP_TEAM_INSTRUCTIONS");
    }
    ...
    if let Some(prompt) = &effective_prompt {
        command.env("AMBUSH_ACP_SYSTEM_PROMPT", prompt);
    } else {
        command.env_remove("AMBUSH_ACP_SYSTEM_PROMPT");
    }
    ...
    // told the same thing: `AMBUSH_ACP_MODEL=auto` would name a model the mesh
    // never advertises, leaving ambush-acp to warn and fall back on every new
    // session while `AMBUSH_AGENT_MODEL` said `mesh`.
    #[cfg(feature = "mesh-llm")]
    ...
    if let Some(model) = acp_model.as_deref() {
        command.env("AMBUSH_ACP_MODEL", model);
    } else {
        command.env_remove("AMBUSH_ACP_MODEL");
    }
    ...
    build_ambush_agent_provider_defaults(&mut command);
    if let Some(meta) = runtime_meta {
    ...
    command.env_remove("AMBUSH_ACP_PRIVATE_KEY");
    command.env_remove("AMBUSH_ACP_API_TOKEN");
    command.env_remove("AMBUSH_API_TOKEN");

    if let Some(ref auth_tag) = record.auth_tag {
        command.env("AMBUSH_AUTH_TAG", auth_tag);
    } else {
        command.env_remove("AMBUSH_AUTH_TAG");
    }
    ...
    command.env("AMBUSH_ACP_RELAY_OBSERVER", "true");

    // Git credential helper: NIP-98 auth for Ambush relay git via git-credential-nostr.
    // Ephemeral GIT_CONFIG_COUNT env vars scoped to relay HTTP URL; NOSTR_PRIVATE_KEY mirrors AMBUSH_PRIVATE_KEY.
    if let Some(cred_helper) = resolve_command("git-credential-nostr") {
        ...
        eprintln!(
            "ambush-desktop: git-credential-nostr not found — agent {} will not have automatic Ambush git auth",
            record.name,
        );
    }

    // User env (descriptor.env): fully-layered floor→runtime→definition→global→persona→agent,
    // reserved-key filtered. Written last so user-explicit values win over Ambush-set env.
    ...
    // user-supplied AMBUSH_ACP_EFFORT_LEVEL entry, mirroring the A1 model-authority pattern
    ...
    // AMBUSH_ACP_MODEL is removed (live ACP switches only; two authorities in the same env
    // would be ambiguous).
    ...
    // Ambush shared compute is stored as a native provider; derive the OpenAI-compatible
    ...
    command
        .env("AMBUSH_MANAGED_AGENT", current_instance_id(app))
        .env("AMBUSH_MANAGED_AGENT_START_NONCE", &start_nonce);
    ...
    // terminal pops for ambush-acp.exe and lingers (the app itself sets
...
fn child_rust_log_filter() -> String {
    match std::env::var("RUST_LOG") {
        Ok(existing) if existing.contains("ambush_acp") => existing,
        Ok(existing) if !existing.trim().is_empty() => format!("{existing},ambush_acp=info"),
        _ => "ambush_acp=info".to_string(),
    }
}
