...
pub struct AgentDefinition {
    ...
    /// Ambush spawns. When deploying from this persona, this runtime is pre-selected in the UI.
    ...
    /// direct). Ambush stores and passes through without interpretation.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    ...
    /// Opaque to Ambush — keys and values are runtime-specific.
    ///
...
pub struct ManagedAgentRecord {
    ...
    /// Deprecated: `AMBUSH_ACP_TURN_TIMEOUT` is ignored by the harness and the
    ...
    /// Idle timeout in seconds (`AMBUSH_ACP_IDLE_TIMEOUT`): how long the agent
    ...
    /// Inbound author gate mode. Translates to `AMBUSH_ACP_RESPOND_TO`.
    #[serde(default)]
    ...
    /// inference through Ambush's relay-mesh local endpoint; the `model_ref` is
    ...
    /// Canonical Claude Code effort level. Injected as `AMBUSH_ACP_EFFORT_LEVEL` at spawn
...
pub struct ManagedAgentProcess {
    ...
    /// `AMBUSH_ACP_SETUP_PAYLOAD` was set at launch because the agent was
...
pub enum AuthStatus {
    ...
    /// This runtime does not have a login step (e.g. goose, ambush-agent).
    NotApplicable,
...
pub struct InstallStepResult {
    ...
    /// recognized condition (e.g. EACCES writing Ambush's private npm prefix).
...
pub const DEFAULT_ACP_COMMAND: &str = "ambush-acp";
/// ~5 min (320s) — matches the CLI harness default (AMBUSH_ACP_IDLE_TIMEOUT).
...
// ── Inbound author gate ──────────────────────────────────────────────────────
//
// Mirrors `ambush-acp`'s `--respond-to` CLI flag and the related
// `--respond-to-allowlist` option. Persisted per agent so the desktop can
// translate the user's choice into `AMBUSH_ACP_RESPOND_TO` /
// `AMBUSH_ACP_RESPOND_TO_ALLOWLIST` env vars at spawn time.
//
...
impl RespondTo {
    /// CLI/env wire string (matches `ambush-acp`'s `--respond-to`).
...
///
/// Rules mirror `ambush-acp/src/config.rs::validate_allowlist`:
