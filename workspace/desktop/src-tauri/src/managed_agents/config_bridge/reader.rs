...
pub(crate) fn read_config_surface(
    ...
    let (file_config, file_was_read) = runtime_meta
        ...
        .and_then(|id| match id {
            ...
            "ambush-agent" => super::ambush_agent::read_config_file().map(|c| (c, true)),
            _ => None,
        })
    ...
    for (k, v) in &record.env_vars {
        ...
        advanced.push(ConfigField {
            ...
            label: k.clone(),
            value: Some(v.clone()),
            origin: ConfigOrigin::AmbushExplicit,
            schema_type: ConfigFieldType::String,
...
fn build_thinking_field(
    ...
    // Tier ordering:
    //   record env > record.effort_level (canonical Ambush-persisted) > ACP >
    ...
    let tiers_list: &[(Option<&str>, ConfigOrigin)] = &[
        (rec_env, ConfigOrigin::AmbushExplicit),
        (canonical_effort, ConfigOrigin::AmbushExplicit),
...
/// is env-only. `AMBUSH_ACP_SYSTEM_PROMPT` is not reserved and is therefore
...
fn build_system_prompt_field(
    ...
    const PROMPT_ENV_KEY: &str = "AMBUSH_ACP_SYSTEM_PROMPT";
    ...
    let tiers_list: &[(Option<&str>, ConfigOrigin)] = &[
        (rec_env, ConfigOrigin::AmbushExplicit),       // record env
        ...
        (struct_record, ConfigOrigin::AmbushExplicit), // struct record
        (
