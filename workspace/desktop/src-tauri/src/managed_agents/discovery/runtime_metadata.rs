...
pub(crate) struct KnownAcpRuntime {
    ...
    /// `Some(dir)` → Ambush creates a symlink at `<nest>/<dir>/ambush-cli`
    /// pointing to the canonical `.agents/skills/ambush-cli`. `None` → this
    ...
    /// authenticated. `None` for runtimes that have no login step (goose, ambush-agent).
