...
fn login_shell_lookup_treats_command_as_data() {
    ...
    let marker =
        std::env::temp_dir().join(format!("ambush-discovery-marker-{}", uuid::Uuid::new_v4()));
...
#[cfg(unix)]
#[test]
fn resolve_command_prefers_ambush_managed_npm_shim_over_path() {
    ...
    assert_eq!(
        ...
        "Ambush-managed npm shim must win over PATH/global shims"
    );
}
...
fn cheap_discovery_never_spawns_login_shell_even_when_cold() {
    ...
    fs::write(
        ...
        r#"{
            ...
            "command": "ambush-absent-command-xyzzy",
            "args": []
        }"#,
    )
    .unwrap();
...
/// dirs + cache, so `ambush-agent` reported "not installed" at every cold launch.
...
fn cheap_path_resolves_workspace_sidecar_without_cache() {
    ...
    let dir = std::env::temp_dir().join(format!("ambush-sidecar-{}", uuid::Uuid::new_v4()));
    std::fs::create_dir_all(&dir).expect("create temp dir");
    let bin = dir.join("ambush-agent");
