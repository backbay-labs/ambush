...
use super::{
    ...
    AMBUSH_AGENT_AVATAR_URL, CLAUDE_CODE_AVATAR_URL, CODEX_AVATAR_URL, GOOSE_AVATAR_URL,
};
...
#[test]
fn default_agent_command_resolves_bundled_ambush_agent() {
    // The default must be bundled ambush-agent, never bare `goose` on a stock Windows install.
    assert_eq!(default_agent_command(), "ambush-agent");
    assert_eq!(
...
#[test]
fn resolves_ambush_agent_avatar() {
    assert_eq!(
        managed_agent_avatar_url("ambush-agent"),
        Some(AMBUSH_AGENT_AVATAR_URL.to_string())
    );
    assert_eq!(
        managed_agent_avatar_url("/usr/local/bin/ambush-agent"),
        Some(AMBUSH_AGENT_AVATAR_URL.to_string())
    );
}
...
fn explicit_path_resolution_ignores_non_executable_files() {
    ...
    let dir = std::env::temp_dir().join(format!("ambush-discovery-path-{}", uuid::Uuid::new_v4()));
    std::fs::create_dir_all(&dir).expect("create temp dir");
    let bin = dir.join("ambush-acp");
...
/// must return `Err` containing "DANGLING_HARNESS_ID" — NEVER the ambush-agent default.
...
#[test]
fn try_record_agent_command_no_runtime_id_defaults_to_ambush_agent() {
    let record = record_with(None, None, None);
    let result = try_record_agent_command(&record, &[]);
    assert_eq!(
        result,
...
fn create_time_override_none_when_persona_runtime_not_installed() {
    ...
    // falls back to the default (`ambush-agent`) and sends THAT command with
    ...
    // persona is re-edited. Baking `Some("ambush-agent")` here is the exact
    // bug this resolver chain exists to kill.
    let personas = vec![persona_with_runtime("p1", Some("claude"))];
    assert_eq!(
        create_time_agent_command_override(Some("p1"), &personas, Some("ambush-agent"), false),
        None
    );
}
...
fn probe_codex_acp_version_parses_full_semver_output() {
    ...
    let dir = std::env::temp_dir().join(format!("ambush-probe-1x-{}", uuid::Uuid::new_v4()));
    std::fs::create_dir_all(&dir).expect("create temp dir");
...
fn probe_codex_acp_version_returns_none_for_nonzero_exit() {
    ...
    let dir = std::env::temp_dir().join(format!("ambush-probe-0x-{}", uuid::Uuid::new_v4()));
    std::fs::create_dir_all(&dir).expect("create temp dir");
    let bin = dir.join("codex-acp");
...
fn codex_adapter_availability_available_for_minimum_supported_binary() {
    ...
    let dir = std::env::temp_dir().join(format!("ambush-avail-1x-{}", uuid::Uuid::new_v4()));
    std::fs::create_dir_all(&dir).expect("create temp dir");
    let bin = dir.join("codex-acp");
    std::fs::write(
        &bin,
...
fn codex_adapter_availability_outdated_for_0x_binary() {
    ...
    let dir = std::env::temp_dir().join(format!("ambush-avail-0x-{}", uuid::Uuid::new_v4()));
    std::fs::create_dir_all(&dir).expect("create temp dir");
    let bin = dir.join("codex-acp");
    std::fs::write(&bin, "#!/bin/sh\nexit 1\n").expect("write script");
    std::fs::set_permissions(&bin, std::fs::Permissions::from_mode(0o755)).expect("chmod script");
...
/// The strict three-component parse fails closed: a version Ambush cannot compare
...
fn probe_codex_acp_version_returns_none_for_hung_direct_child() {
    ...
    let dir = std::env::temp_dir().join(format!("ambush-probe-hung-{}", uuid::Uuid::new_v4()));
    std::fs::create_dir_all(&dir).expect("create temp dir");
    let bin = dir.join("codex-acp");
    std::fs::write(
        &bin,
...
fn probe_codex_acp_version_returns_version_when_descendant_holds_pipe_open() {
    ...
    let dir = std::env::temp_dir().join(format!("ambush-probe-pipe-{}", uuid::Uuid::new_v4()));
    std::fs::create_dir_all(&dir).expect("create temp dir");
    let bin = dir.join("codex-acp");
    std::fs::write(
        &bin,
...
/// id resolves to the custom command — NOT the ambush-agent default.
///
...
/// NOT silently fall back to ambush-agent.
///
...
/// summary path renders the *missing id* (not a silent ambush-agent fallback),
