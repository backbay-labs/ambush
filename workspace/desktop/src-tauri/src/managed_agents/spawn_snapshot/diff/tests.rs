...
fn base() -> SpawnConfigSnapshot {
    SpawnConfigSnapshot {
        acp_command: "ambush-acp".into(),
        command: "goose".into(),
        args: vec!["--mode".into(), "acp".into()],
        mcp_command: "goose-mcp".into(),
        env: BTreeMap::from([
            ("OPENAI_API_KEY".to_string(), SECRET.to_string()),
            ("AMBUSH_LOG".to_string(), "info".to_string()),
        ]),
...
fn env_key_removal_is_removed_without_a_payload() {
    let mut after = base();
    after.env.remove("AMBUSH_LOG");
    assert_eq!(
        change_at(&diff(&base(), &after), "env.AMBUSH_LOG"),
...
fn allowlisted_env_key_shows_plain_value() {
    // AMBUSH_AGENT_THINKING_EFFORT is on the safe-to-reveal allowlist — the user
    // must be able to see actual enum values like "medium → high".
    let mut before = base();
    before
        .env
        .insert("AMBUSH_AGENT_THINKING_EFFORT".into(), "medium".into());
    let mut after = before.clone();
    after
        .env
        .insert("AMBUSH_AGENT_THINKING_EFFORT".into(), "high".into());
    assert_eq!(
        change_at(&diff(&before, &after), "env.AMBUSH_AGENT_THINKING_EFFORT"),
        &RestartChange::Value {
...
fn allowlisted_env_key_is_case_insensitive() {
    ...
    before
        .env
        .insert("ambush_agent_provider".into(), "anthropic".into());
    let mut after = before.clone();
    after
        .env
        .insert("ambush_agent_provider".into(), "openai".into());
    assert_eq!(
        change_at(&diff(&before, &after), "env.ambush_agent_provider"),
        &RestartChange::Value {
