...
pub fn backfill_persona_snapshots(app: &tauri::AppHandle) -> Result<(), String> {
    ...
    for record in records.iter_mut() {
        ...
        let Some(persona) = personas.iter().find(|p| p.id == persona_id) else {
            eprintln!(
                "ambush-desktop: persona-snapshot backfill: agent {} links persona {persona_id} which no longer exists; leaving it orphaned — spawn will refuse it",
                record.pubkey
            );
            continue;
        };
...
pub async fn restore_managed_agents_on_launch(
    ...
    let mut agents_to_start: Vec<super::ManagedAgentRecord>;
    {
        ...
        // Exact-path sweep: kill any ambush-acp process whose executable path
        ...
        // AMBUSH_MANAGED_AGENT injection or lost their PID-file receipt.
        //
        ...
        tauri::async_runtime::spawn(async move {
            ...
            if let Err(e) =
                crate::commands::reconcile_agent_profile(&state, &reconcile_app, &pubkey, &data)
                    .await
            {
                eprintln!("ambush-desktop: profile reconciliation failed for agent {pubkey}: {e}");
            }
        });
    }
...
pub(crate) fn spawn_pending_profile_reconciliations(app: &tauri::AppHandle, workspace_relay: &str) {
    ...
    let items = match crate::commands::load_pending_profile_reconciliations(app, workspace_relay) {
        Ok(items) => items,
        Err(error) => {
            eprintln!("ambush-desktop: failed to load pending profile reconciliations: {error}");
            return;
        }
    };
                ...
                .await
            {
                Ok(outcome) if profile_reconcile_completed(outcome) => {
                    if let Err(error) = crate::commands::mark_profile_reconciled(
                        ...
                        eprintln!(
                            "ambush-desktop: failed to record profile reconciliation for agent {pubkey}: {error}"
                        );
                    }
                }
                Ok(_) => {}
                Err(error) => eprintln!(
                    "ambush-desktop: profile reconciliation failed for agent {pubkey}: {error}"
                ),
            }
        });
    }
