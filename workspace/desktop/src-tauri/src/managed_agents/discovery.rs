...
use crate::managed_agents::{
    ambush_managed_command_path, ambush_managed_node_bin_dir, ambush_managed_npm_bin_dir,
...
const AMBUSH_AGENT_AVATAR_URL: &str =
    "https://raw.githubusercontent.com/block/ambush/refs/heads/main/crates/ambush-agent/ambush-agent.png";
fn common_binary_paths() -> &'static [PathBuf] {
    ...
    PATHS.get_or_init(|| {
        ...
        if let Some(managed_node_bin) = ambush_managed_node_bin_dir() {
            paths.insert(0, managed_node_bin);
        }
        if let Some(managed_bin) = ambush_managed_npm_bin_dir() {
...
const KNOWN_ACP_RUNTIMES: &[KnownAcpRuntime] = &[
    KnownAcpRuntime {
        ...
        cli_install_hint: "Ambush talks to Goose through the Goose CLI.",
    ...
    KnownAcpRuntime {
        ...
        cli_install_hint: "Ambush talks to Claude Code through the Claude Code CLI.",
        adapter_install_hint: "Ambush talks to the Claude Code CLI through an ACP adapter. Install it with: npm install -g @agentclientprotocol/claude-agent-acp.",
    ...
    KnownAcpRuntime {
        ...
        mcp_command: Some("ambush-dev-mcp"),
        mcp_hooks: false,
        ...
        cli_install_hint: "Ambush talks to Codex through the Codex CLI.",
        adapter_install_hint: "Ambush talks to the Codex CLI through an ACP adapter. Install it with: npm install -g @agentclientprotocol/codex-acp.",
    ...
    KnownAcpRuntime {
        ...
        aliases: &[],
        avatar_url: AMBUSH_AGENT_AVATAR_URL,
        mcp_command: Some("ambush-dev-mcp"),
        mcp_hooks: true,
        underlying_cli: None,
        ...
        adapter_install_commands: &[],
        cli_install_instructions_url: "https://github.com/backbay-labs/ambush",
        adapter_install_instructions_url: "https://github.com/backbay-labs/ambush",
        cli_install_hint: "Ships with the Ambush desktop app.",
        adapter_install_hint: "",
        skill_dir: None,
...
/// request supplies none. Resolves the bundled `ambush-agent` from the catalog so
...
/// is not on PATH on a stock Windows install; ambush-agent ships with the app.)
pub fn default_agent_command() -> String {
    known_acp_runtime_exact("ambush-agent")
        .and_then(|p| p.commands.first().copied())
        .unwrap_or("ambush-agent")
        .to_string()
}
...
pub fn resolve_command(command: &str) -> Option<PathBuf> {
    if let Some(managed) = resolve_ambush_managed_command(command) {
        return Some(managed);
    }
...
///
/// Consults the Ambush-managed shim dir (a filesystem stat, never a spawn) and
...
pub fn resolve_command_cached(command: &str) -> Option<PathBuf> {
    ...
    // Bundled sidecars (e.g. `ambush-agent`) ship next to the app executable, so
    ...
    // until a forced discovery warmed the resolve cache, so `ambush-agent` (which
...
fn resolve_ambush_managed_command(command: &str) -> Option<PathBuf> {
    let basenames = command_basenames(command);
    basenames
        .iter()
        .find_map(|basename| ambush_managed_command_path(command, basename))
}
...
///
/// Ambush rewrites these catalog commands to an app-private npm prefix before
...
pub fn missing_command_message(command: &str, role: &str) -> String {
    ...
    format!(
        "{role} `{command}` was not found. Make sure it is installed and on your PATH. Antivirus software can quarantine bundled binaries — if that happened, restore the file or reinstall Ambush. (Source builds: see TESTING.md.)"
    )
}
...
/// The oldest `codex-acp` version supported by Ambush managed agents.
///
/// Older 1.x adapters are detected successfully, but can still bundle a Codex runtime
/// that does not reliably give `ambush` CLI subprocesses outbound relay access.
///
...
fn discover_acp_runtime_phase1(runtime: &'static KnownAcpRuntime, force: bool) -> PartialEntry {
    ...
    // node_required now means Ambush cannot provide npm for this platform.
    // On supported desktop platforms, Ambush downloads a private Node/npm
    ...
    let node_required = matches!(
        ...
        && ambush_managed_node_bin_dir().is_none()
        && resolve("npm").is_none()
