...
//!
//! A small set of *reserved* keys includes Ambush's identity, secrets, security
...
//! (GOOSE_MODE, AMBUSH_ACP_MODEL, AMBUSH_ACP_SYSTEM_PROMPT, …) remain freely
//! overridable. Power users can still bypass their dedicated UI fields.
//! `AMBUSH_ACP_AGENTS` is reserved because Desktop applies harness-specific caps
...
/// `=`. That means a key like `AMBUSH_AUTH_TAG=x` with value `forged`
/// lands as `AMBUSH_AUTH_TAG=x=forged` in the child env and
/// `getenv("AMBUSH_AUTH_TAG")` returns `"x=forged"` — a full reserved-
...
pub fn validate_user_env_keys(env_vars: &BTreeMap<String, String>) -> Result<(), String> {
    ...
    if !reserved.is_empty() {
        return Err(format!(
            "the following env vars are reserved by Ambush and cannot be overridden: {}",
            reserved.join(", ")
        ));
    }
...
/// - `AMBUSH_AGENT_PROVIDER`, `AMBUSH_AGENT_MODEL` — agent runtime selection
/// - `AMBUSH_AGENT_THINKING_EFFORT` — non-secret enum (none/minimal/low/medium/high/xhigh/max)
/// - `AMBUSH_AGENT_THINKING_SUMMARY` — non-secret enum (auto/concise/detailed)
...
pub(crate) fn merged_user_env(
    ...
    merged.retain(|k, v| {
        if is_reserved_env_key(k) {
            eprintln!(
                "ambush-desktop: ignoring reserved env var `{k}` from persona/agent overrides"
            );
            return false;
        }
        if !is_well_formed_env_key(k) {
            ...
            eprintln!(
                "ambush-desktop: ignoring malformed env var key `{}` from persona/agent overrides",
        ...
        if v.contains('\0') {
            ...
            eprintln!(
                "ambush-desktop: ignoring env var `{k}` with NUL byte in value"
            );
            return false;
        }
        if v.len() > MAX_ENV_VALUE_BYTES {
            eprintln!(
                "ambush-desktop: ignoring env var `{k}` with oversize value ({} bytes > {MAX_ENV_VALUE_BYTES})",
                v.len()
            );
            return false;
        }
