...
fn nest_dir_is_under_home() {
    if let Some(dir) = nest_dir() {
        // Accepts both .ambush (prod) and .ambush-dev (dev) depending on
        // whether init_nest_dir was called before this test ran.
        let name = dir.file_name().and_then(|n| n.to_str()).unwrap_or("");
        assert!(
            name == NEST_DIR_PROD || name == NEST_DIR_DEV,
            "nest_dir must end with .ambush or .ambush-dev, got {dir:?}"
        );
    }
}

#[test]
fn init_nest_dir_prod_sets_ambush() {
    ...
    if let Some(d) = dir {
        ...
        assert!(
            name == NEST_DIR_PROD || name == NEST_DIR_DEV,
            "nest_dir suffix must be .ambush or .ambush-dev, got {d:?}"
        );
    }
}
...
fn nest_skill_contains_safe_mention_workflow() {
    assert!(AMBUSH_CLI_SKILL_MD.contains("--mention <hex-or-npub>"));
    assert!(AMBUSH_CLI_SKILL_MD.contains("every presentation-only name that should notify"));
    assert!(AMBUSH_CLI_SKILL_MD
        .contains("permits unresolved or ambiguous `@Name` text as presentation-only"));
    assert!(AMBUSH_CLI_SKILL_MD.contains("signed event's `mention_pubkeys`"));
    assert!(AMBUSH_CLI_SKILL_MD.contains("no follow-up verification command is needed"));
    assert!(AMBUSH_CLI_SKILL_MD.contains("Add membership separately only when authorized"));
    assert!(AMBUSH_CLI_SKILL_MD.contains("never changes membership automatically"));
}
...
fn ensure_nest_skill_dir_has_700_permissions() {
    ...
    // Symlinks (e.g. .goose/skills/ambush-cli) are skipped by the chmod loop.
    for dir in [
        ".agents",
        ".agents/skills",
        ".agents/skills/ambush-cli",
        ".goose",
        ".goose/skills",
        ".claude",
        ".claude/skills",
...
fn ensure_nest_migrates_old_skill_dir() {
    ...
    let _ = fs::remove_file(root.join(".claude/skills/ambush-cli"));
    let _ = fs::remove_dir_all(root.join(".agents/skills/ambush-cli"));
    let old_skill_dir = root.join(".claude/skills/ambush-cli");
    ...
    let _ = fs::remove_file(root.join(".agents/skills/ambush-cli/.skill-version"));
    ...
    let new_skill = root.join(".agents/skills/ambush-cli/SKILL.md");
    ...
    let old_path = root.join(".claude/skills/ambush-cli");
    assert!(
        old_path
            .symlink_metadata()
            .unwrap()
...
fn ensure_skill_symlinks_are_idempotent() {
    ...
    for dir in [".goose/skills", ".claude/skills", ".codex/skills"] {
        ...
        assert_eq!(
            ...
            "symlink at {dir}/ambush-cli should use relative target"
        );
    }
}
...
fn ensure_skill_symlinks_skips_existing_path_during_initial_pass() {
    ...
    // migrates pre-existing real directories at .claude/skills/ambush-cli to
    ...
    let real_dir = root.join(".claude/skills/ambush-cli");
    ...
    assert!(
        real_dir
            ...
            .file_type()
            .is_symlink(),
        ".claude/skills/ambush-cli should be migrated to a symlink"
    );
    // The canonical path now holds the migrated content.
    let canonical = root.join(".agents/skills/ambush-cli/SKILL.md");
    assert_eq!(
...
fn ensure_skill_symlinks_skip_dangling_symlink() {
    ...
    let dangling = codex_skills.join("ambush-cli");
...
#[test]
fn cli_link_name_prod_is_ambush() {
    assert_eq!(cli_link_name(false), "ambush");
}

#[test]
fn cli_link_name_dev_is_ambush_dev() {
    assert_eq!(cli_link_name(true), "ambush-dev");
}
...
fn ensure_cli_symlink_creates_symlink_prod() {
    ...
    fs::write(exe_parent.join("ambush"), "binary").unwrap();

    let local_bin = tmp.path().join("local_bin");
    fs::create_dir_all(&local_bin).unwrap();

    // Prod link name is "ambush"; simulate the symlink creation path.
    let link = local_bin.join(cli_link_name(false));
    std::os::unix::fs::symlink(exe_parent.join("ambush"), &link).unwrap();
    assert!(link.symlink_metadata().unwrap().file_type().is_symlink());
    assert_eq!(fs::read_link(&link).unwrap(), exe_parent.join("ambush"));
}
...
fn ensure_cli_symlink_creates_symlink_dev() {
    ...
    // Dev link must be "ambush-dev", never "ambush".
    assert_eq!(cli_link_name(true), "ambush-dev");
    ...
    assert!(!local_bin.join("ambush").exists());
}
...
fn ensure_cli_symlink_does_not_clobber_regular_file_dev() {
    ...
    fs::create_dir_all(&local_bin).unwrap();
    let link = local_bin.join(cli_link_name(true));
    fs::write(&link, "user-installed ambush-dev binary").unwrap();

    // Regular files at the dev path are also preserved.
    assert!(link.symlink_metadata().unwrap().file_type().is_file());
    assert_eq!(
        fs::read_to_string(&link).unwrap(),
        "user-installed ambush-dev binary"
    );
}
...
fn refresh_agents_md_upgrades_attribution_and_preserves_owned_content() {
    ...
    fs::write(
        ...
        "# Ambush Nest\n\n## Git Commit Identity\n\n\
         ...
         <!-- BEGIN AMBUSH MANAGED — regenerated automatically, do not edit below -->\n\
...
fn refresh_skill_md_writes_version_file() {
    ...
    ensure_nest_at(&root).unwrap();
    let version =
        fs::read_to_string(root.join(".agents/skills/ambush-cli/.skill-version")).unwrap();
...
fn refresh_agents_md_preserves_managed_section() {
    ...
    assert!(
        content.starts_with("# Ambush Nest"),
...
fn refresh_skill_overwrites_on_version_bump() {
    ...
    let skill_md = root.join(".agents/skills/ambush-cli/SKILL.md");
