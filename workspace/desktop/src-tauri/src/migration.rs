//! Worktree data sync and on-launch reconciliation for the Ambush desktop app.
//!
...
//! directory (`com.backbay.ambush.app.dev`). Only runs when
//! `AMBUSH_SHARE_IDENTITY=1` and `AMBUSH_PRIVATE_KEY` is set. All dev
...
//! fix-up of persisted built-in command names from the Sprout→Ambush rename.
//!
...
/// receive their identity via the `AMBUSH_PRIVATE_KEY` env var.
...
/// `com.backbay.ambush.app.dev.my-branch`). Rejects prefix-collisions such as
/// `com.backbay.ambush.app.developer`. This is the authoritative dev/prod
...
fn run_boot_migrations_inner(app: &tauri::AppHandle, reset_completed: bool) -> Result<(), String> {
    ...
    // On dev builds, copy `.repos-dir` from ~/.ambush → ~/.ambush-dev BEFORE
...
/// agent's accumulated knowledge in `~/.sprout` while `~/.ambush` booted empty —
...
pub fn migrate_legacy_nest() -> bool {
    let Some(home) = dirs::home_dir() else {
        eprintln!("ambush-desktop: nest-migration: cannot resolve home directory");
        return false;
    };
    // Destination is the current build's nest dir (`.ambush` or `.ambush-dev`).
    let Some(current_nest) = crate::managed_agents::nest_dir() else {
        eprintln!("ambush-desktop: nest-migration: cannot resolve nest directory");
        return false;
    };
...
fn migrate_legacy_nest_at(legacy: &Path, current: &Path) -> bool {
    ...
    if current.file_name().is_some_and(|n| n == ".ambush-dev")
    ...
    for name in LEGACY_NEST_KNOWLEDGE {
        ...
        let result = if src.is_dir() {
            ...
            // `ensure_nest` writes a default `~/.ambush/AGENTS.md` before this
        ...
        match result {
            Ok(()) => eprintln!(
                "ambush-desktop: nest-migration: migrated {} to {}",
                src.display(),
                dst.display()
            ),
            Err(error) => eprintln!(
                "ambush-desktop: nest-migration: failed to migrate {} to {}: {error}",
                src.display(),
                dst.display()
            ),
        }
...
/// knowledge migration. Presence of this file means `~/.ambush` content has
/// already been copied into `~/.ambush-dev` and subsequent boots can skip the
...
/// also copies into `~/.ambush-dev` and could otherwise set the sentinel early.
...
/// Injectable core: copy `.repos-dir` from `<home>/.ambush/` into `dev_nest`,
...
pub(crate) fn migrate_dev_repos_dir_at(home: &Path, dev_nest: &Path) {
    let src = home.join(".ambush").join(".repos-dir");
    if !src.exists() {
        return;
    }
    ...
    if let Err(e) = std::fs::create_dir_all(dev_nest) {
        eprintln!(
            "ambush-desktop: dev-nest-migration: failed to create dev nest {}: {e}",
            dev_nest.display()
        );
        return;
    }
    match std::fs::copy(&src, &dst) {
        Ok(_) => eprintln!(
            "ambush-desktop: dev-nest-migration: migrated .repos-dir to {}",
            dst.display()
        ),
        Err(e) => eprintln!("ambush-desktop: dev-nest-migration: failed to copy .repos-dir: {e}"),
    }
}
...
/// One-time migration of dev-build nest contents from `~/.ambush` → `~/.ambush-dev`.
///
/// When a dev build first boots after this change ships, it switches from the
/// shared `~/.ambush` nest to a dedicated `~/.ambush-dev` nest. Without migration,
...
/// `~/.ambush` is never deleted — prod builds continue to use it normally.
///
/// Completion is tracked by a [`DEV_NEST_MIGRATED_SENTINEL`] file written into
/// `~/.ambush-dev`. Using an explicit sentinel (rather than RESEARCH/PLANS file
/// presence) decouples this migration from the `.sprout` → `~/.ambush-dev`
/// migration that runs earlier in the same boot, which might otherwise populate
/// RESEARCH/PLANS and incorrectly suppress the `~/.ambush` copy.
///
...
pub fn migrate_dev_nest() -> bool {
    let Some(home) = dirs::home_dir() else {
        eprintln!("ambush-desktop: dev-nest-migration: cannot resolve home directory");
        return false;
    };
    let legacy = home.join(".ambush");
    let current = home.join(".ambush-dev");
    ...
    if copied {
        ...
        if let Err(e) = std::fs::write(&sentinel, "") {
            eprintln!(
                "ambush-desktop: dev-nest-migration: failed to write sentinel {}: {e}",
                sentinel.display()
            );
        }
    }
...
fn patch_json_records(
    ...
    let Ok(mut records) = serde_json::from_str::<Vec<serde_json::Value>>(&content) else {
        eprintln!(
            "ambush-desktop: patch-json-records: failed to parse {}",
            path.display()
        );
        return;
    };
    ...
    if changed {
        if let Ok(bytes) = serde_json::to_vec_pretty(&records) {
            if let Err(e) = crate::managed_agents::atomic_write_json_restricted(path, &bytes) {
                eprintln!("ambush-desktop: patch-json-records: {e}");
            }
        }
    }
}
...
fn refresh_builtin_agent_avatars_in_file(
    ...
    let Ok(mut records) = serde_json::from_str::<Vec<serde_json::Value>>(&contents) else {
        eprintln!(
            "ambush-desktop: refresh-builtin-agent-avatars: invalid JSON in {}",
            path.display()
        );
        return;
    };
    ...
    if changed {
        if let Ok(bytes) = serde_json::to_vec_pretty(&records) {
            if let Err(e) = crate::managed_agents::atomic_write_json_restricted(path, &bytes) {
                eprintln!("ambush-desktop: refresh-builtin-agent-avatars: {e}");
            }
        }
    }
}
...
///
/// Guards:
/// - `AMBUSH_SHARE_IDENTITY` must be `"1"`
/// - `AMBUSH_PRIVATE_KEY` must parse as valid `nostr::Keys`
...
pub fn sync_shared_agent_data(app: &tauri::AppHandle) {
    ...
    let is_shared = std::env::var("AMBUSH_SHARE_IDENTITY")
        .map(|v| v == "1")
        .unwrap_or(false);
    if !is_shared {
        return;
    ...
    // Guard: AMBUSH_PRIVATE_KEY must be a valid nostr key.
    let has_valid_key = std::env::var("AMBUSH_PRIVATE_KEY")
        .ok()
        .and_then(|k| k.parse::<nostr::Keys>().ok())
        .is_some();
    if !has_valid_key {
        eprintln!("ambush-desktop: shared-agent-sync: no valid AMBUSH_PRIVATE_KEY, skipping");
        return;
    }
    ...
    let current_dir = match app.path().app_data_dir() {
        Ok(dir) => dir,
        Err(e) => {
            eprintln!("ambush-desktop: shared-agent-sync: cannot resolve app data dir: {e}");
            return;
        }
    };
    ...
    if !is_dev {
        eprintln!(
            "ambush-desktop: shared-agent-sync: skipping — data dir is not a dev dir ({})",
            current_dir.display()
        );
        return;
    }
    ...
    let canonical_dir = match canonical_dev_data_dir(&current_dir) {
        Some(dir) => dir,
        None => {
            eprintln!("ambush-desktop: shared-agent-sync: canonical dir has no parent");
            return;
        }
    };
    ...
    if !canonical_dir.exists() {
        eprintln!(
            "ambush-desktop: shared-agent-sync: canonical dir does not exist: {}",
            ...
            if sibling_file.is_file() && !sibling_file.is_symlink() {
                if let Some(file_parent) = canonical_file.parent() {
                    if let Err(e) = std::fs::create_dir_all(file_parent) {
                        eprintln!(
                            "ambush-desktop: shared-agent-sync: failed to create {}: {e}",
                            file_parent.display()
                        );
                        break;
                    }
                ...
                eprintln!(
                    "ambush-desktop: shared-agent-sync: seeded {rel} from {}",
                    sibling.display()
                );
                break;
            }
                ...
                if let Ok(entries) = std::fs::read_dir(parent) {
                    for entry in entries.flatten() {
                        ...
                        if sibling_dir.is_dir() && !sibling_dir.is_symlink() {
                            ...
                            eprintln!(
                                "ambush-desktop: shared-agent-sync: migrated {rel} from {}",
                                sibling.display()
                            );
                            break;
                        }
    ...
    if synced > 0 {
        eprintln!(
            "ambush-desktop: shared-agent-sync: {synced} item(s) linked to {}",
            canonical_dir.display()
        );
    }
}
...
fn reconcile_mcp_commands_in_file(path: &Path) {
    ...
    patch_json_records(path, |obj| {
        ...
        if !current.is_empty() && current != "ambush-mcp-server" {
            return false;
        }
        eprintln!(
            "ambush-desktop: runtime-reconcile: {:?} ({:?}): mcp_command {:?} → {:?}",
...
fn replace_command_field(
    ...
    eprintln!(
        "ambush-desktop: command-rename-reconcile: {:?}: {field} {:?} → {:?}",
        obj.get("name").and_then(|v| v.as_str()).unwrap_or("?"),
        current,
        replacement,
    );
    ...
    patch_json_records(path, |obj| {
            ...
            .map(str::to_string)
        {
            if acp_command == "sprout-acp" {
                changed |= replace_command_field(obj, "acp_command", "ambush-acp".to_string());
            }
        }
        ...
        if agent_command == "sprout-agent" {
            agent_command = "ambush-agent".to_string();
            ...
            .map(str::to_string)
        {
            match mcp_command.as_str() {
                "sprout-dev-mcp" => {
                    changed |=
                        replace_command_field(obj, "mcp_command", "ambush-dev-mcp".to_string());
                }
                "sprout-mcp" | "sprout-mcp-server" | "ambush-mcp-server" => {
                    let replacement = if agent_command == "ambush-agent" {
                        "ambush-dev-mcp"
                    } else {
                        ""
                    };
...
fn reconcile_legacy_persona_runtimes_in_file(path: &Path) {
    patch_json_records(path, |obj| {
        ...
        eprintln!(
            "ambush-desktop: command-rename-reconcile: persona {:?}: runtime {:?} → {:?}",
        ...
        obj.insert(
            "runtime".to_string(),
            serde_json::Value::String("ambush-agent".to_string()),
        );
        true
    });
}
...
fn rewrite_legacy_persona_md_runtime(content: &str) -> Option<String> {
    let (frontmatter, body) = ambush_persona_pkg::persona::split_frontmatter(content).ok()?;
    ...
    *runtime = serde_yaml::Value::String("ambush-agent".to_string());
...
fn reconcile_legacy_team_persona_runtime_files(dir: &Path) {
    ...
    for entry in entries.flatten() {
        ...
        match std::fs::write(&path, updated) {
            Ok(()) => {
                eprintln!(
                    "ambush-desktop: command-rename-reconcile: updated {}",
                    path.display()
                );
            }
            Err(error) => {
                eprintln!(
                    "ambush-desktop: command-rename-reconcile: failed to update {}: {error}",
                    path.display()
                );
            }
        }
...
/// Reconcile exact built-in command values persisted before the Sprout→Ambush
...
fn reconcile_databricks_v1_to_v2_in_file(path: &Path, rewrite_v1_provider: bool) {
    ...
    patch_json_records(path, |obj| {
        ...
        // marks this as a Block build (AMBUSH_AGENT_PROVIDER == "databricks_v2").
        ...
        if rewrite_v1_provider && obj.get("provider").and_then(|v| v.as_str()) == Some("databricks")
        {
            let name = obj
                ...
                .to_string();
            eprintln!(
                "ambush-desktop: databricks-v1-to-v2: {name:?}: provider \"databricks\" → \"databricks_v2\"",
            );
            obj.insert(
                "provider".to_string(),
            ...
            // (AMBUSH_AGENT_MODEL from runtime_metadata_env_vars takes priority in
            // ambush-agent config.rs). Clearing it lets the baked V2 default win.
            if obj.remove("model").is_some() {
                eprintln!(
                    "ambush-desktop: databricks-v1-to-v2: {name:?}: cleared stale V1 model field",
                );
            }
            changed = true;
        }
        ...
        if let Some(serde_json::Value::Object(env_vars)) = obj.get_mut("env_vars") {
            ...
            for key in stale_keys {
                ...
                eprintln!("ambush-desktop: databricks-v1-to-v2: removed stale env_vars[\"{key}\"]",);
                changed = true;
            }
        }
...
/// `AMBUSH_AGENT_PROVIDER=databricks_v2`): the structured `provider` field is
...
///
/// In both cases, stale `AMBUSH_AGENT_PROVIDER` / `AMBUSH_AGENT_MODEL` /
...
pub fn reconcile_databricks_v1_to_v2(app: &tauri::AppHandle) {
    ...
    // On Block builds, the baked env contains AMBUSH_AGENT_PROVIDER=databricks_v2.
