...
pub fn fold_personas_into_agent_store(app: &tauri::AppHandle) {
    ...
    match fold_personas_in_dir(&base_dir) {
        ...
        Ok(Some(folded)) => {
            eprintln!(
                "ambush-desktop: persona-store-fold: {folded} definitions folded into the unified store"
            );
        }
        Err(e) => eprintln!("ambush-desktop: persona-store-fold: {e}"),
    }
}
    ...
    fn fold_moves_custom_personas_and_retires_the_file() {
        ...
        write_personas_json(
            ...
            &serde_json::json!([
                ...
                { "id": "builtin:fizz", "display_name": "Anvil", "system_prompt": "P",
                  "is_builtin": true,
