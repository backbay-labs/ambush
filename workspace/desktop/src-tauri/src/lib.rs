...
pub fn run() {
        ...
        .build()
    {
        Ok(runtime) => {
            ...
            eprintln!(
                "ambush-mesh: installed tokio runtime with {} MiB worker stacks",
        ...
        Err(error) => {
            ...
            eprintln!(
                "ambush-mesh: failed to build big-stack tokio runtime, using default: {error}"
            );
        }
    }
                ...
                .on_webview_ready(|webview| {
                    ...
                    #[cfg(target_os = "macos")]
                    {
                        ...
                        tauri::async_runtime::spawn(async move {
                            ...
                            {
                                eprintln!(
                                    "ambush-desktop: initial render did not commit before reveal timeout"
                                );
                            }
    ...
    let app = app_menu::install(builder)
        .register_asynchronous_uri_scheme_protocol("ambush-media", |ctx, request, responder| {
            let app = ctx.app_handle().clone();
            tauri::async_runtime::spawn(async move {
                let response = media_proxy::handle_ambush_media(&app, &request).await;
        ...
        .setup(move |app| {
            ...
            if let Err(e) = resolve_persisted_identity(&app_handle, &state) {
                eprintln!("ambush-desktop: fatal: identity resolution failed: {e}");
                std::process::exit(1);
            }
            ...
            if let Err(e) = backfill_persona_snapshots(&app_handle) {
                eprintln!("ambush-desktop: persona-snapshot backfill failed: {e}");
            }
            ...
            // Create the Ambush nest (~/.ambush or ~/.ambush-dev for dev builds) before
            ...
            if let Err(error) = ensure_nest() {
                eprintln!("ambush-desktop: failed to create nest: {error}");
            }
            ...
            // from the shared ~/.ambush nest into the new dedicated ~/.ambush-dev
            // nest so no work is lost when the nest is first namespaced.
            // Runs only when nest_dir() resolved to ~/.ambush-dev (dev instance).
            // Suppressed after a reset so re-importing ~/.ambush into ~/.ambush-dev
            // doesn't re-populate what was just wiped.
            let is_dev_nest = managed_agents::nest_dir()
                .and_then(|p| p.file_name().map(|n| n.to_os_string()))
                .is_some_and(|n| n == ".ambush-dev");
            ...
            if let Ok(exe) = std::env::current_exe() {
                if let Some(parent) = exe.parent() {
                    if let Err(error) = managed_agents::ensure_cli_symlink(parent, is_dev_nest) {
                        eprintln!("ambush-desktop: failed to create CLI symlink: {error}");
                    }
                }
            }
            ...
            if !recovery_mode {
                ...
                tauri::async_runtime::spawn(async move {
                    ...
                    loop {
                        ...
                        .await
                        {
                            eprintln!("ambush-desktop: event-flush: {e}");
                        }
    ...
    app.run(move |app_handle, event| match event {
        ...
        RunEvent::WindowEvent {
            ...
            // Keep the webview alive so Ambush can be reopened from its tray menu.
            api.prevent_close();
            if let Some(window) = app_handle.get_webview_window("main") {
                if let Err(error) = window.hide() {
                    eprintln!("ambush-desktop: failed to hide main window: {error}");
                }
            }
        }
        RunEvent::WindowEvent {
            ...
            if is_active_huddle_window {
                if let Err(error) = app_handle.emit("huddle-companion-returned", ()) {
                    eprintln!("ambush-desktop: failed to restore huddle drawer: {error}");
                }
            }
        }
        ...
        RunEvent::Exit => {
            ...
            // End the process only after Ambush and Mesh shutdown above, while
