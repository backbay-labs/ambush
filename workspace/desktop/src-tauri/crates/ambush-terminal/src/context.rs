...
//! shells interpolate into prompts. A `PS1` containing `$AMBUSH_CHANNEL` turns a
...
/// Returns true if `name` is safe to expose as `AMBUSH_CHANNEL`.
///
...
/// The value to expose as `AMBUSH_CHANNEL`: the name when it is safe, otherwise
/// the channel UUID.
...
/// first `=`. A key `AMBUSH_CHANNEL=x` with value `y` lands as
/// `AMBUSH_CHANNEL=x=y`, so `getenv("AMBUSH_CHANNEL")` returns `"x=y"` — a way to
...
///
/// `AMBUSH_CHANNEL` carries the validated display value; `AMBUSH_CHANNEL_ID` is
...
pub fn context_vars(context: &GuiContext) -> Vec<(&'static str, String)> {
    let mut vars = vec![
        ("AMBUSH_CHANNEL_ID", context.channel_id.clone()),
        ("AMBUSH_CHANNEL", channel_display(context).to_owned()),
        ("AMBUSH_NPUB", context.npub.clone()),
        ("AMBUSH_RELAY_URL", context.relay_url.clone()),
        ("AMBUSH_TERM_SESSION", context.session_id.clone()),
        ("AMBUSH_TERM_VERSION", env!("CARGO_PKG_VERSION").to_owned()),
    ];
    // Absent rather than empty when the user is not in a thread: `-n
    // "$AMBUSH_THREAD_ID"` and `${AMBUSH_THREAD_ID+set}` should agree.
    if let Some(thread_id) = &context.thread_id {
        vars.push(("AMBUSH_THREAD_ID", thread_id.clone()));
    }
    vars
}
