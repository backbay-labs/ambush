...
//!
//! Ambush's own process runs under Hermit activation, so its `PATH` leads with
//! the repo's hermit `bin` and the hermit cache. Inheriting that verbatim
//! hands the user a shell whose `cargo`, `node`, and `python` are Ambush's
...
//! answer for the user's machine, not for Ambush's build.
//!
...
//! Ambush never been in the picture.

/// The default user `PATH` for a spawned shell.
///
/// This intentionally does not consult Ambush's own `PATH`. A login shell reads
...
/// normally instead of layering it on top of Ambush's build toolchain.
#[cfg(unix)]
