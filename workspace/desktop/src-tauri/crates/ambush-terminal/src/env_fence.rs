...
//!
//! Ambush's own process holds `AMBUSH_PRIVATE_KEY` (an nsec), `AMBUSH_AUTH_TAG`, and
...
/// Keys the child is allowed to inherit from Ambush's own environment.
///
...
/// Values Ambush sets on the child unconditionally, overriding any inherited
...
/// it is injected rather than inherited. Ambush's own `SHELL` and the shell we
...
pub fn fence_env(cmd: &mut CommandBuilder, path: &str, shell: &str) {
    ...
    // 3. Apply Ambush's own terminal identity.
