...
export const THEME_STORAGE_KEY = "ambush-theme";
const CACHE_KEY = "ambush-theme-cache";
export const ACCENT_STORAGE_KEY = "ambush-accent-color";
export const GLASS_BACKGROUND_STORAGE_KEY = "ambush-glass-background";
export const GLASS_OPACITY_STORAGE_KEY = "ambush-glass-opacity";
export const PROMINENT_ACTIVE_TAB_STORAGE_KEY = "ambush-prominent-active-tab";
...
function readStoredTheme(fallback: SyntaxThemeName): SyntaxThemeName {
  // block/ambush#5078 — WebKit throws SecurityError from getItem under a
...
function applyAccentColor(value: string) {
  ...
  if (value === NEUTRAL_ACCENT) {
    ...
    root.style.setProperty("--ambush-selected-accent", foreground);
    root.style.setProperty(
      "--ambush-video-review-accent",
      VIDEO_REVIEW_NEUTRAL_ACCENT,
    );
    root.style.setProperty(
      "--ambush-video-review-accent-foreground",
      VIDEO_REVIEW_NEUTRAL_ACCENT,
    );
  ...
  root.style.setProperty("--ambush-selected-accent", accentHsl);
  root.style.setProperty("--ambush-video-review-accent", accentHsl);
  root.style.setProperty(
    "--ambush-video-review-accent-foreground",
...
function resolveEffectiveAccent(
  ...
  return isAmbushTheme(themeName) ? NEUTRAL_ACCENT : accentColor;
}

/** Toggle the Ambush-specific gradient marker independently from glass. */
function applyAmbushSidebar(themeName: string) {
  const root = document.documentElement;
  if (isAmbushTheme(themeName)) {
    root.setAttribute("data-ambush-sidebar", "");
    // Keep the concrete Ambush variant on the root as well as the generic
    ...
    root.setAttribute("data-ambush-theme", themeName);
  } else {
    root.removeAttribute("data-ambush-sidebar");
    root.removeAttribute("data-ambush-theme");
  }
}
...
function applyCachedVars(): string | null {
  try {
    ...
    applyAmbushSidebar(themeName);
    glassThemeReady = true;

    const accent = getStorageItem(ACCENT_STORAGE_KEY) ?? DEFAULT_ACCENT;
    // Pin Ambush themes to the neutral accent here too, matching applyTheme.
    // Otherwise a cached Ambush theme + non-neutral stored accent flashes the
...
async function applyTheme(name: SyntaxThemeName): Promise<{
  ...
  // new theme for a frame — the flicker seen when switching to Ambush. Ambush
...
export function ThemeProvider({
  ...
  const [accentColor, setAccentColorState] = useState<string>(() => {
    // block/ambush#5078 — use the throw-safe accessor for init-time reads; a
  ...
  // The stronger selected-row treatment belongs exclusively to Ambush. Keep
  // the saved preference so it is restored when the user returns to Ambush,
  // but remove the live marker for every other theme.
  useEffect(() => {
    setProminentActiveTabActive(
      prominentActiveTab && isAmbushTheme(effectiveTheme),
    );
  ...
  // changes. applyTheme already applies the (Ambush-neutral-aware) accent in the
