...
function useAmbushLinkContextMenu({
  href,
  interactive,
  onOpenLink,
}: {
  ...
  const contextMenu =
    position && href ? (
      <MediaContextMenu
        dataAttributes={["data-ambush-link-context-menu"]}
        items={[
          {
            label: "Open link",
            onSelect: () => {
...
export function AmbushLinkChip({
  children,
  className,
  href,
  icon: Icon,
  ...
  const { contextMenu, onContextMenuCapture } = useAmbushLinkContextMenu({
    href,
    interactive,
    onOpenLink,
  });
...
export function AmbushInlineLink({
  children,
  href,
  interactive,
  onOpenLink,
