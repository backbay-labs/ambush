...
/**
 * The finished Ambush mark as a plain static SVG — no SMIL, no scripting, no
 * animation machinery. Geometry matches the final keyframe of the
 * AmbushLogoAnimation morph (v8 variant), rendered in `currentColor`, so it
...
export function AmbushMark({ className }: { className?: string }) {
  const maskId = `ambush-mark-cutouts-${useId().replace(/[^a-zA-Z0-9_-]/g, "")}`;
