...
export type AmbushLogoAnimationProps = {
  ariaLabel?: string;
  className?: string;
  fullScreen?: boolean;
  loop?: boolean;
...
export default function AmbushLogoAnimation({
  ariaLabel = "Ambush logo animation",
  className = "",
  fullScreen = true,
  loop = false,
  loopRestSeconds = 0,
  ...
  const maskId = `ambush-logo-cutouts-${idSuffix}`;
  const textureId = `ambush-logo-texture-${idSuffix}`;
  const texture = TEXTURES[config.texture ?? "soft"] ?? TEXTURES.soft;
  const classes = [
    "ambush-logo",
    fullScreen && "ambush-logo--screen",
    !fullScreen && "ambush-logo--compact",
    showBackground && "ambush-logo--background",
    className,
  ]
    .filter(Boolean)
    .join(" ");
