...
export const POOF_TRIGGER_CLASS = "ambush-poof-trigger";
export const POOF_ORIGIN_CLASS = "ambush-poof-origin";
export const POOF_POINTER_ORIGIN_CLASS = "ambush-poof-pointer-origin";
...
type PoofStyle = CSSProperties & {
  "--ambush-poof-size": string;
  "--ambush-poof-x": string;
  "--ambush-poof-y": string;
};
...
export function PoofBurstProvider({ children }: { children: React.ReactNode }) {
  ...
  return (
    <>
      {children}
      <div aria-hidden="true" className="ambush-poof-layer">
        {bursts.map((burst) => (
          <div
            className="ambush-poof-burst"
            key={burst.id}
            style={
              {
                "--ambush-poof-size": `${burst.size}px`,
                "--ambush-poof-x": `${burst.x}px`,
                "--ambush-poof-y": `${burst.y}px`,
              } as PoofStyle
            }
          >
            {POOF_FRAMES.map((frame, index) => (
              <img
                alt=""
                className={`ambush-poof-frame ambush-poof-frame-${index + 1}`}
                decoding="async"
                draggable={false}
