...
function AnimatedCountSlot({
  ...
  if (direction === 0 || previous === current) {
    return (
      <span
        className={cn(
          "ambush-animated-count__slot",
          isDigit && "ambush-animated-count__slot--digit",
        )}
      >
  ...
  if (!isDigit) {
    return (
      <span
        className="ambush-animated-count__symbol"
  ...
  return (
    <span
      className="ambush-animated-count__slot ambush-animated-count__slot--digit"
      data-direction={direction > 0 ? "up" : "down"}
    >
      <span
        className="ambush-animated-count__reel"
        key={`${version}-${previous}-${current}`}
      >
        {direction > 0 ? (
          <>
...
export function AnimatedCount({ className, value }: AnimatedCountProps) {
  ...
  return (
    <span className={cn("ambush-animated-count", className)}>
      <span className="sr-only">{transition.current}</span>
      <span aria-hidden className="ambush-animated-count__motion">
        {slots.map((slot) => (
          <AnimatedCountSlot
      ...
      <span aria-hidden className="ambush-animated-count__static">
        {transition.current}
      </span>
    </span>
  );
