...
/**
 * Payload emitted by the Rust deep-link handler for `ambush://message?…`.
...
/**
 * Payload emitted by the Rust deep-link handler for `ambush://join?…` —
 ...
 *
 * When a `ambush://connect?relay=<url>` link is opened, the handler
 ...
 *
 * When a `ambush://join?relay=<url>&code=<invite>` link is opened (relay
 ...
 *
 * `ambush://message?…` is handled separately by `listenForNavigationDeepLinks`,
...
/**
 * Register a listener for `deep-link-entity` events — the `ambush://` share
