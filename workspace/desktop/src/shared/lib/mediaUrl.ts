 ...
 *
 * Only URLs hosted on the Ambush relay are rewritten. External Blossom URLs
 ...
 * (the Ambush relay), resulting in 404s.
 */
...
/**
 * Cached relay origin (e.g. "https://ambush-staging.example.com"),
 ...
 * (`normalize_host` in ambush-core), but the saved community relay URL keeps
...
/**
 * The relay origin (e.g. `https://ambush-staging.example.com`) if it has
...
/**
 * If `url` is a Blossom media URL hosted on the Ambush relay, rewrite it
 * to go through the local streaming proxy. External Blossom URLs and
 * non-Blossom URLs are returned unchanged.
 *
 * Falls back to ambush-media:// if the proxy port isn't available yet.
 */
export function rewriteRelayUrl(url: string): string {
  ...
  // was typed with uppercase (e.g. wss://PENDING-SEED.communities.ambush.example.com).
  if (cachedRelayOrigin) {
  ...
  return `ambush-media://localhost/media/${m[1]}`;
}
