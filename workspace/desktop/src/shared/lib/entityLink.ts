/**
 * `ambush://` deep links for Ambush-hosted git entities, mirroring
 * `features/messages/lib/messageLink.ts` for `ambush://message`.
 *
 * Formats:
 *   ambush://repo?owner=<owner-pubkey>&d=<repo-dtag>[&tab=<tab>][&commit=<git-hash>]
 *   ambush://project?owner=<owner-pubkey>&d=<project-dtag>[&tab=<tab>]
 *   ambush://pr?id=<event-id>&owner=<owner-pubkey>&d=<repo-dtag>
 *   ambush://issue?id=<event-id>&owner=<owner-pubkey>&d=<repo-dtag>
 *
 ...
 * overview. The CLI builder in `crates/ambush-cli/src/links.rs` emits the
 * same format — the two must stay compatible (see the golden-format tests
 * on both sides).
 */

const ENTITY_LINK_SCHEME = "ambush:";
...
export function buildRepoLink(input: {
  ...
  return `ambush://repo?owner=${input.owner.toLowerCase()}&d=${input.dtag}${tabSuffix(input.tab)}`;
}
...
export function buildCommitLink(input: {
  ...
  return `ambush://repo?owner=${input.owner.toLowerCase()}&d=${input.dtag}&tab=commits&commit=${input.commitHash.toLowerCase()}`;
}
...
export function buildProjectLink(input: {
  ...
  checkCoordinate(input.owner, input.dtag);
  return `ambush://project?owner=${input.owner.toLowerCase()}&d=${input.dtag}${tabSuffix(input.tab)}`;
}
...
export function buildPullRequestLink(input: {
  ...
  checkCoordinate(input.owner, input.dtag);
  return `ambush://pr?id=${input.id.toLowerCase()}&owner=${input.owner.toLowerCase()}&d=${input.dtag}`;
}
...
export function buildIssueLink(input: {
  ...
  checkEventId(input.id);
  checkCoordinate(input.owner, input.dtag);
  return `ambush://issue?id=${input.id.toLowerCase()}&owner=${input.owner.toLowerCase()}&d=${input.dtag}`;
}

/**
 * Cheap pre-check used by the markdown renderer and preview extraction
 * before parsing. `ambush://message` is intentionally excluded — it has its
 * own pill rendering path.
 */
export function isEntityLink(href: string | undefined | null): boolean {
  if (!href) return false;
  return (
    href.startsWith("ambush://pr?") ||
    href.startsWith("ambush://issue?") ||
    href.startsWith("ambush://repo?") ||
    href.startsWith("ambush://project?")
  );
}

/**
 * Parse a `ambush://pr|issue|repo?…` URL. Returns a discriminated result so
