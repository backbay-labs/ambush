// Source-first React Query keys for the perch feature area.
//
// Replaces the hand-maintained registry pattern of
// `shared/api/relayQueryInvalidation.ts` — a Set of relay-dependent key roots
// consulted as the React Query `predicate` on a degraded->connected
// transition (`useReconnectRelay.ts`, `useRelayAutoHeal.ts`). A query whose
// key[0] is not in that Set is never invalidated on reconnect and goes
// permanently stale — silently, and only under network churn.
//
// Perch keeps no registry. The source is the FIRST SEGMENT OF THE KEY, so the
// predicate is a comparison rather than a membership test and there is
// nothing to forget to update. Design: 14-CLIENT-ARCHITECTURE.md §4.
//
// Gate-line budget: 1000 (src/shared/api is a governed root). This file
// cannot live in `shared/api/tauri.ts` or `shared/api/types.ts`, both frozen.
...
const key = <
  const S extends PerchQuerySource,
...
export const perchKeys = {
   ...
   * (INV-15). A DAEMON read, not a relay one (D-FC-2): the daemon serves
   * `GET /metrics/perch/identities` and the relay is not the record.
   */
  admittedIssuers: () => key("daemon", "admitted-issuers"),
...
export const PERCH_FRESHNESS = {
  ...
  incident: {
    ...
    poll: false,
    invalidatesOnWrite: ["caseTimeline"],
    why: "An incident's member decisions are fixed at correlation; its measurements change when a verdict lands, which invalidates the case timeline too. Fetched when the canvas opens, never polled.",
  },
  policy: {
    staleTime: 60_000,
    poll: false,
  ...
  needsAction: {
    ...
    why: "Runs beside `holds`, never instead of it. build_needs_action_query has no status join (crates/ambush-db/src/store/feed.rs), so a DECIDED hold stays in it forever; the reconciler removes those against the daemon list.",
  },
  caseTimeline: {
    staleTime: Number.POSITIVE_INFINITY,
    poll: false,
  ...
  admittedIssuers: {
    ...
    poll: false,
    invalidatesOnWrite: [],
    why: "The set every marker parse and every 26xxx frame is checked against (INV-15), read from the daemon's unauthenticated identities endpoint per D-FC-2 because the relay is not the record and a forged kind:0 profile is one MessagesWrite away. Long stale time on purpose: it must be reference-stable or it defeats the memo on every evidence card. Invalidated by an explicit admission change, never by a timer.",
  },
  spoolHealth: {
    staleTime: 5_000,
    poll: 10_000,
 ...
 * default is `retry: 1` (shared/api/queryClient.ts).
 */
