...
/**
 * Read `issuer` and `seq` out of a swarm card body: line 0 is the marker,
 * and the envelope is the JSON inside the fenced block whose info string is
 * the marker's own `swarm:<slug>:v<n>`. Never throws; anything that is not
 * a well-formed card reads as null.
 *
 * Mirrors the wire mirror's `parseCardContent` discipline
 * (`features/perch/wire/marker.ts`, First card Task 2) for the two fields
 * gap tracking needs; once that module lands, replace this reader with
 * `parseCardContent` plus one JSON parse.
 */
