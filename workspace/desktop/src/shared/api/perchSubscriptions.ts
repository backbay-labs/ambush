// One reconciling subscription manager for every perch REQ.
//
// Modelled on the Map-based `syncSubs` reconciler in
// `features/channels/useLiveChannelUpdates.ts` (a Map<channelId, dispose> in
// a ref, diffed against the target set: dispose what left, open what arrived,
// bounded retry) — but hoisted out of a hook, because perch's REQs are
// declared by several features and the frame budget below is global, not
// per-feature. Design: 14-CLIENT-ARCHITECTURE.md §5.
//
// Every REQ goes through `relayClient.subscribeLive`
// (`shared/api/relayClientSession.ts`): it generates a `live-${uuid}` subId,
// registers {mode:"live", filter, onEvent, resolveReady}, sends
// ["REQ", subId, filter] through sendRawWithReconnectRetry, resolves readiness
// on EOSE or a 250 ms timeout, and returns an async disposer that CLOSEs.
// Perch needs no new client method and no new Tauri command for relay work:
// the socket lives in Rust and all Nostr framing is TypeScript.
//
// Gate-line budget: 1000 (src/shared/api is governed). If this file grows
// the split line is between the manager (§5) and gap detection (§6): they
// share no state and gap detection has no relay dependency at all, so it is
// a pure file move.
 ...
 * `shouldPageReconnectReplay` (`shared/api/relayReconnectReplay.ts`,
 ...
 * (`shared/api/channelReconnectRepair.ts`) carries NO
...
export function perchCaseLiveKinds(): number[] {
  // 46010 is the hold notice; 40100 is the case canvas; 39005 is the
  // relay-signed thread summary. The spread of CHANNEL_EVENT_KINDS is what
  // keeps this filter eligible for the paged reconnect repair; the Set
  // dedupes whichever of the three the constant already carries.
  return Array.from(
    new Set([
      ...CHANNEL_EVENT_KINDS,
      KIND_CHANNEL_THREAD_SUMMARY,
...
export function assertPerchRepairKindsCovered(
  ...
  const missing = PERCH_CASE_REPAIR_KINDS.filter(
    (k) => !repairKinds.includes(k),
  );
...
export function perchSteadyStateReqFrames(
...
/**
 * Receives every event from every perch REQ, tagged with the id of the REQ
 * that carried it. One sink for the whole manager; a consumer that wants a
 * subset filters on the id.
 */
...
/**
 * Opens in flight, keyed by id, holding the serialization each was asked
 * for. A later sync that wants a different filter (or none) for an id drops
 * or replaces the entry, and the open's completion handler disposes a REQ
 * whose serialization no longer matches — so a stale open can neither
 * resurrect a closed subscription nor be duplicated by a concurrent sync.
 */
const inflight = new Map<PerchSubscriptionId, string>();
...
/** Install (or with null, remove) the sink every open REQ delivers into. */
 ...
 * unchanged input performs no network work, and a concurrent call for the
 * same filter does not open a second REQ. Failures are per-subscription and
 * never reject — a failed open leaves that id absent so the next sync retries
 * it, exactly as `useLiveChannelUpdates`'s reconciler does.
 */
export async function syncPerchSubscriptions(
  ...
  for (const [id, entry] of active) {
    const wanted = target.get(id)?.filter ?? null;
  ...
  // An open still in flight for a filter nobody wants any more is retargeted
  // here; its completion handler below disposes it.
  for (const [id, serialized] of inflight) {
    const wanted = target.get(id)?.filter ?? null;
    if (wanted === null || serialize(wanted) !== serialized) {
      inflight.delete(id);
    }
  }
  ...
  const additions = specs
    .filter(
      (s) => s.filter !== null && !active.has(s.id) && !inflight.has(s.id),
    )
    .sort((a, b) => Number(b.priority) - Number(a.priority))
    .map(async (spec) => {
      const filter = spec.filter;
      if (!filter) return;
      const serialized = serialize(filter);
      inflight.set(spec.id, serialized);
      let dispose: () => Promise<void>;
      try {
        dispose = await relayClient.subscribeLive(filter, (event) =>
          sink?.(spec.id, event),
        );
      } catch {
        if (inflight.get(spec.id) === serialized) inflight.delete(spec.id);
        failed.push(spec.id);
        return;
      }
      // A newer sync may have retargeted or dropped this id while the REQ
      // was in flight, or a reset may have cleared the table; do not
      // resurrect what it decided against.
      if (inflight.get(spec.id) !== serialized || active.has(spec.id)) {
        void dispose().catch(() => {});
        return;
      }
      inflight.delete(spec.id);
      active.set(spec.id, { spec, dispose, serialized });
      opened += 1;
    });
...
/** The ids currently open. A diagnostics and test seam, never a data path. */
export function perchActiveSubscriptionIds(): readonly PerchSubscriptionId[] {
  return [...active.keys()];
}

/**
 * Community-switch fence, registered in the typed reset registry
 * (`features/communities/communityScopedRegistry.ts`). Runs after the relay
 * has been disconnected, so the CLOSE frames it sends never race a live
 * socket that belongs to the next community.
 */
...
// ===========================================================================
//
// Every published envelope carries a per-issuer monotonic `seq`. The client's
// job is to notice a hole and REFUSE TO SMOOTH IT.
//
...
// either a false gap or — worse — a false continuity. In v1 the QueryClient is
// already colony-scoped, so `colonyId` is the store's identity rather than a
// key segment; the reset below is what fences it on a community switch.
//
...
/** One hole in one issuer's sequence, open until the daemon serves it. */
...
function notifyGapChange(): void {
  gapVersion += 1;
  for (const listener of gapListeners) listener();
}

/**
 * Subscribe to changes of the open-gap set: a gap opened, a gap closed, or
 * the tracker was reset. A seq that opens nothing is not a change. Returns
 * the unsubscribe function, in the `useSyncExternalStore` shape.
 */
export function subscribePerchGaps(listener: () => void): () => void {
...
/** Monotonic version of the open-gap set; bumps only when its content changes. */
export function perchGapsVersion(): number {
  return gapVersion;
}
 ...
 * forward jump opens one. The first seq seen from an issuer is its baseline.
 */
...
/** Every gap not yet served by the daemon, in the order they were noticed. */
...
export function closePerchGap(issuer: string, expectedSeq: number): void {
  if (openGaps.delete(`${issuer}:${expectedSeq}`)) notifyGapChange();
}

/** Community-switch fence. Registered in the typed reset registry. */
export function resetPerchSeqTracking(): void {
  lastSeqByIssuer.clear();
  const hadGaps = openGaps.size > 0;
  openGaps.clear();
  if (hadGaps) notifyGapChange();
}
