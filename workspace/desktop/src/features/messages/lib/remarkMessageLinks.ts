/**
 * Remark plugin that detects bare `ambush://message?…` URLs in text nodes and
 * replaces each with a custom `message-link` HAST element. Legacy
 * `ambush://message?…` URLs are accepted during the rename. The `markdown.tsx`
 ...
 * and `www.`. Custom schemes like `ambush://` only reach the `<a>` component
 * override when the user wrote an explicit `[label](ambush://…)` link.
 *
...
const MESSAGE_URL_PATTERN = /(?:ambush|ambush):\/\/message\?[^\s<>"')\]]+/g;
