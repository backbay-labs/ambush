/**
 * `ambush://message` link encoding for "Copy link" / deep-link-to-message.
 *
 * Format: `ambush://message?channel=<uuid>&id=<eventId>[&thread=<rootId>]`
 */

const MESSAGE_LINK_SCHEME = "ambush:";
...
/**
 * Build a `ambush://message` URL for a given channel + message.
 *
...
/**
 * Parse a `ambush://message?…` URL. Returns a discriminated result so callers can
...
export function parseMessageLink(url: string): MessageLinkParseResult {
  ...
  // `new URL("ambush://message?…")` puts "message" in `hostname`.
...
export function isMessageLink(href: string | undefined | null): boolean {
  ...
  return href.startsWith("ambush://message?") || href === "ambush://message";
}
 ...
 * CommonMark autolinks (`<ambush://message?...>`) and explicitly labeled links
