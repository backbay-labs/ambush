...
test("buildMessageLink → parseMessageLink round-trips without thread", () => {
  ...
  assert.equal(url, `ambush://message?channel=${CHANNEL}&id=${MESSAGE}`);
...
test("buildMessageLink treats null/empty thread as absent", () => {
  ...
  assert.equal(a, `ambush://message?channel=${CHANNEL}&id=${MESSAGE}`);
  assert.equal(b, `ambush://message?channel=${CHANNEL}&id=${MESSAGE}`);
});
...
test("parseMessageLink rejects ambush:// with wrong host", () => {
  const r = parseMessageLink(`ambush://connect?relay=wss://example.com`);
  assert.equal(r.ok, false);
  assert.equal(r.ok === false && r.reason, "wrong-host");
});

test("parseMessageLink rejects missing channel", () => {
  const r = parseMessageLink(`ambush://message?id=${MESSAGE}`);
  assert.equal(r.ok, false);
  assert.equal(r.ok === false && r.reason, "missing-channel");
});

test("parseMessageLink rejects missing id", () => {
  const r = parseMessageLink(`ambush://message?channel=${CHANNEL}`);
  assert.equal(r.ok, false);
...
test("parseMessageLink accepts legacy ambush://message links", () => {
  const r = parseMessageLink(
    `ambush://message?channel=${CHANNEL}&id=${MESSAGE}`,
  );
...
test("isMessageLink matches ambush://message and legacy ambush://message", () => {
  assert.equal(
    isMessageLink(`ambush://message?channel=${CHANNEL}&id=${MESSAGE}`),
    true,
  );
  assert.equal(
    isMessageLink(`ambush://message?channel=${CHANNEL}&id=${MESSAGE}`),
  ...
  assert.equal(isMessageLink("ambush://connect?relay=wss://x"), false);
  assert.equal(isMessageLink("ambush://connect?relay=wss://x"), false);
...
test("resolveMessageLinkRenderTarget distinguishes autolinks from labeled links", () => {
  const href = `ambush://message?channel=${CHANNEL}&id=${MESSAGE}`;
