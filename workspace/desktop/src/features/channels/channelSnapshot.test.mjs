...
test("cache write evicts disposable entries and retries at quota", () => {
  const original = window.localStorage;
  const storage = new Map([
    ["ambush-channel-messages.v1:relay:old", "big"],
    ["ambush-timeline-skeleton-shape.v1:old", "small"],
  ]);
  window.localStorage = {
    get length() {
      return storage.size;
  ...
  try {
    ...
    assert.equal(storage.has("ambush-channel-messages.v1:relay:old"), false);
    assert.equal(storage.has("ambush-timeline-skeleton-shape.v1:old"), false);
  } finally {
    window.localStorage = original;
  }
});
