...
function gridDraws(view) {
  const grid = view.container.querySelector(
    ".ambush-terminal-viewport > canvas:not(.ambush-terminal-welcome)",
  );
...
function fixture(overrides = {}) {
  ...
  const renderTree = (nextProps) =>
    createElement(
      ...
      null,
      createElement("div", {
        className: "ambush-huddle-app-surface",
        tabIndex: -1,
      }),
...
async function ready(view) {
  await waitFor(() =>
    assert.ok(view.container.querySelector(".ambush-terminal-substrate")),
  );
}
...
test("mounted IME paths neither toggle nor emit preedit text", async () => {
  ...
  await ready(view);
  const substrate = view.container.querySelector(".ambush-terminal-substrate");
  toggleChord(true);
  assert.equal(substrate.dataset.terminalOwner, "ambush");
...
test("tab actions restore terminal input focus", async () => {
  ...
  const actions = [
    ...
    ["new", view.getByLabelText("New Ambush Term tab")],
  ];
...
test("drag resize batches visual updates and commits state only on release", async () => {
  ...
  await ready(view);
  const substrate = view.container.querySelector(".ambush-terminal-substrate");
  const handle = view.getByLabelText("Resize Ambush Term");
...
test("unmount cancels a queued drag update", async () => {
  ...
  await ready(view);
  const handle = view.getByLabelText("Resize Ambush Term");
  const previousHeight = handle.closest(".ambush-terminal-substrate").style
    .height;
  ...
  assert.equal(
    handle.closest(".ambush-terminal-substrate").style.height,
    previousHeight,
  );
});
...
async function expectWelcome(view, present) {
  ...
  await waitFor(() =>
    assert.equal(
      view.container.querySelector(".ambush-terminal-welcome") !== null,
      present,
    ),
  );
}
...
test("canvas failure atomically restores Ambush ownership", async () => {
  ...
  await waitFor(() => assert.equal(substrate.dataset.terminalOwner, "ambush"));
  toggleChord();
  await waitFor(() =>
    assert.equal(substrate.dataset.terminalOwner, "terminal"),
  );
...
test("cursor blink runs only while the terminal owns input and resets on input", async () => {
  ...
  try {
    ...
    await ready(subject.view);
    assert.equal(callbacks.size, 0, "Ambush ownership must pause blinking");
...
test("reduced motion keeps the terminal cursor solid", async () => {
  ...
  try {
    ...
    await waitFor(() =>
      assert.equal(
        subject.view.container.querySelector(".ambush-terminal-substrate")
...
test("tab chords stay inert while Ambush owns input", async () => {
  const subject = tabFixture();
  await ready(subject.view);
  // Deliberately not revealed: owner is "ambush".
  const spawn = press({ code: "KeyT", metaKey: true });
  ...
  assert.equal(
    ...
    false,
    "Ambush-mode ⌘T belongs to the rest of the app",
  );
});
...
test("mirrors the active canvas grid into a selectable plain-text layer", async () => {
  ...
  const selectionLayer = subject.view.container.querySelector(
    ".ambush-terminal-selection-layer",
  );
  await waitFor(() =>
    assert.equal(
