...
export function HostedCommunitiesSettingsCard() {
  ...
  const loadAccount = React.useCallback(async () => {
    ...
    if (
      ...
      // Ambush identity yet — that's the connect-card empty state, not an error to
      ...
      throw new Error(
        errorMessage(
          ...
          "Could not load the connected Ambush identity.",
        ),
      );
    }
  ...
  const connectIdentity = () =>
    run("Connecting Ambush identity…", async () => {
      ...
      if (response.error) {
        throw new Error(
          errorMessage(
            ...
            "Could not connect the Ambush identity.",
          ),
        );
      }
      ...
      if (response.error) {
        throw new Error(
          errorMessage(
            response.error,
            response.correlation_id,
            "Could not unpair the Ambush identity.",
          ),
        );
      }
      setIdentity(null);
      ...
      if (released.error) {
        throw new Error(
          errorMessage(
            ...
            "Could not release the previously connected Ambush identity.",
          ),
        );
      }
      ...
      if (bound.error) {
        ...
        throw new Error(
          bound.error.code === "pubkey_already_bound"
            ? "This device's Ambush identity is already reserved by another Builderlab account, so it can't be connected here. Sign in with that account, or transfer the identity there first."
            : errorMessage(
                bound.error,
                bound.correlation_id,
                "Could not connect this device's Ambush identity.",
              ),
        );
      }
      ...
      if (availabilityResponse.error || !availabilityResponse.available) {
        ...
        throw new Error(
          errorMessage(
            ...
            "That Ambush address is already taken.",
          ),
        );
      }
      const response = await invoke<CommunityMutationResponse>(
  ...
  return (
    <section className="space-y-6" data-testid="hosted-communities-settings">
      <SettingsSectionHeader
        ...
        description="Ambush works with any relay. This page is only for relay hosting provided by Block — sign in with a Builderlab account to create and manage Block-hosted communities. Builderlab sign-in is used on this page alone."
      />
          ...
          {!identity ? (
            <div className="rounded-xl border border-amber-500/40 bg-amber-500/5 p-5">
              ...
              >
                ...
                {action ?? "Connect Ambush identity"}
              </Button>
            </div>
          ) : identityMismatch ? (
              ...
              <div className="flex items-start gap-2">
                ...
                <div>
                  ...
                  >
                    ...
                    Ambush key, so connecting them here would join a relay this
                    device isn&apos;t a member of. Creating and connecting are
                    paused until the identities match.
                  </p>
...
function UnpairIdentityButton({
  ...
  return (
    <AlertDialog open={open} onOpenChange={setOpen}>
      ...
      <AlertDialogContent>
        <AlertDialogHeader>
          <AlertDialogTitle>Unpair this Ambush identity?</AlertDialogTitle>
          <AlertDialogDescription>
            Your Builderlab account will no longer be connected to this Ambush
      ...
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            ...
            member. The recipient needs a connected Ambush identity first, and
