...
function vectors(prefix) {
  return readdirSync(GOLDEN)
    .filter(
      (f) =>
        f.startsWith(prefix) && f.endsWith(".json") && f !== "manifest.json",
    )
    .sort()
...
test("the Rust suite pins the same hash", () => {
  // Half the point of two suites is that they cannot silently disagree. One
  // repository, two Cargo workspaces (00-DECISIONS.md D2): the Rust suite is
  // six directories up, under crates/, and this reads its constant directly.
  // A missing file is a FAILURE here, not a skip -- the crate is in-tree.
...
test("ResponseAction is internally tagged on `type`", () => {
  ...
  assert.equal(card.fact.hold.action_request.action.host_id, "host-ops-1");

  const wrong = structuredClone(card);
  wrong.fact.hold.action_request.action = {
    isolate_host: { host_id: "web-04" },
  };
...
test("Severity is SCREAMING_SNAKE and nothing else is", () => {
  ...
  const card = JSON.parse(vector("card-swarm-hold-v1").raw);
  assert.equal(card.fact.hold.severity, "CRITICAL");
...
// ──────────────────────────────────────── the registry's remainder parser

import { parseCardParts } from "./marker.ts";
