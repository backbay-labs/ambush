...
export type FindingFact = {
  ...
  /**
   * Present only when the bridge replaced `finding.evidence`. At the FACT
   * level, not inside `finding`: the finding envelope is carried by type and
   * cannot grow a field.
   */
  readonly evidence_truncated?: {
    readonly bytes: number;
    readonly sha256: string;
  };
  /** Loss observed before this card. Absent on a normal card. */
  readonly gap?: GapBlock;
};

/** Exactly four causes. A coalesce is not a gap and never appears here. */
export type GapBlockCause =
  | "broadcast_lagged"
  | "spool_evicted"
  | "spool_torn_tail"
...
/**
 * Loss the bridge observed before a card, carried inside the same signed
 * envelope so it cannot be lost independently of the card (11-BRIDGE-CRATE §3.6).
 */
export type GapBlock = {
  readonly cause: GapBlockCause;
  /** `broadcast_lagged` only: the bridge never saw the events and has no seq for them. */
  readonly count?: number | null;
  /** The three spool causes: an exact `seq` range, inclusive. */
  readonly from_seq?: number | null;
  readonly to_seq?: number | null;
  /** When the bridge first recorded the loss, daemon clock. */
  readonly noticed_at_ms: UnixMillis;
};
...
export type ReceiptFact = {
  ...
  readonly audit_trail: {
    ...
    readonly response: {
      readonly kind: "success" | "failure" | "skipped" | "guard_rejected";
