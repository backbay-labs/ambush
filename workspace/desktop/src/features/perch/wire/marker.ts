 ...
 * are the same shape with `swarm` in place of `buzz`, and the parser differs in
...
export const CARD_FENCE: Readonly<Record<CardKind, string>> = Object.freeze(
  Object.fromEntries(CARD_KINDS.map((k) => [k, `swarm:${k}:v1`])) as Record<
    CardKind,
    string
  >,
);
...
export const CARD_FACT_SCHEMA: Readonly<Record<CardKind, string>> =
  Object.freeze(
    Object.fromEntries(
...
export function routeCard(content: string): CardKind | null {
  ...
  const firstLine = (
    newline === -1 ? content : content.slice(0, newline)
  ).replace(/\r$/, "");
...
export function parseCardParts(
  ...
  const afterHuman =
    secondBreak === -1 ? "" : afterMarker.slice(secondBreak + 1);
