...
function isAmbushRuntime(runtime: AcpRuntimeCatalogEntry): boolean {
  return runtime.id.trim().toLowerCase() === "ambush-agent";
}

export function getRuntimeDisplayLabel(
  runtime: AcpRuntimeCatalogEntry,
): string {
  return isAmbushRuntime(runtime) ? "Ambush" : runtime.label;
}
...
export function RuntimeIcon({
  ...
  if (isAmbushRuntime(runtime)) {
    ...
    return <AmbushMark className={cn(className, "text-foreground")} />;
  }
