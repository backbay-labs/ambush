...
export type AgentReadinessResult =
  ...
  | { ready: true; reason: "ambush-agent" }
  | { ready: false };
 ...
 * Provider path: the preferred Ambush Agent or Goose runtime has provider and
...
export function resolveAgentReadiness(
  ...
  if (scope === "any") {
    for (const runtime of runtimes) {
      if (runtime.id === "ambush-agent") continue;
      if (
  ...
  const preferredRuntime =
    scope === "preferred"
      ...
      : runtimes.find((runtime) => runtime.id === "ambush-agent");
  ...
  if (
    preferredRuntime.id !== "ambush-agent" &&
  ...
  if (provider.length > 0 && model.length > 0) {
    ...
    if (allKeysPresent) {
      return { ready: true, reason: "ambush-agent" };
    }
  }
