/**
 * Source-of-truth constants for ambush-agent model-tuning configuration knobs.
 *
 ...
 * `./modelCapabilities`; this module owns only the ambush-agent env-var keys and
 * the runtime-id guard. Mirrors the `config.rs` ⇄ `model_capabilities.rs` seam
 * in `crates/ambush-agent`, where effort resolution is delegated to the manifest.
...
export const AMBUSH_AGENT_THINKING_EFFORT = "AMBUSH_AGENT_THINKING_EFFORT";

/** Env var key for the maximum output token count per turn. */
export const AMBUSH_AGENT_MAX_OUTPUT_TOKENS = "AMBUSH_AGENT_MAX_OUTPUT_TOKENS";

/** Env var key for the context window token limit. */
export const AMBUSH_AGENT_MAX_CONTEXT_TOKENS =
  "AMBUSH_AGENT_MAX_CONTEXT_TOKENS";

/** Env var key for the maximum number of LLM/tool rounds per turn. */
export const AMBUSH_AGENT_MAX_ROUNDS = "AMBUSH_AGENT_MAX_ROUNDS";

/**
 * Ordered set of valid thinking-effort values accepted by ambush-agent.
 * Re-exported from the manifest interpreter, which owns the canonical list
 * (mirrors `parse_thinking_effort` in `crates/ambush-agent/src/config.rs`).
 */
export const AMBUSH_AGENT_THINKING_EFFORT_VALUES = THINKING_EFFORT_VALUES;
...
/**
 * Returns true when the given runtime id is ambush-agent, which is the only
 * runtime that supports the tier-1 model-tuning knobs above.
 */
export function isAmbushAgentRuntime(runtimeId: string): boolean {
  return runtimeId === "ambush-agent";
}
