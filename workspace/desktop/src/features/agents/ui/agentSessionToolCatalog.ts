...
import type { AmbushToolInfo, ToolStatus } from "./agentSessionTypes";
...
const AMBUSH_READ_TOOLS = new Set([
  "get_messages",
  "get_channel_history",
  "get_thread",
  "search",
...
const AMBUSH_WRITE_TOOLS = new Set([
  "send_message",
  "send_diff_message",
  "edit_message",
  "delete_message",
...
const AMBUSH_TOOL_NAMES = new Set([
  ...AMBUSH_READ_TOOLS,
  ...AMBUSH_WRITE_TOOLS,
]);

const AMBUSH_TOOL_NAMES_BY_LENGTH = [...AMBUSH_TOOL_NAMES].sort(
  (left, right) => right.length - left.length,
);

const AMBUSH_TOOL_TITLE_ALIASES: Array<[RegExp, string]> = [
...
export function getAmbushToolInfo(title: string): AmbushToolInfo | null {
  const name = normalizeToolName(title);
  const isRead = AMBUSH_READ_TOOLS.has(name);
  const isWrite = AMBUSH_WRITE_TOOLS.has(name);
  if (!isRead && !isWrite) {
    return null;
  }
  ...
  if (name.includes("workflow") || name === "approve_step") {
    return {
      icon: Workflow,
      label: isRead
        ? "Reads workflow state from Ambush."
        : "Updates workflow state in Ambush.",
  ...
  if (
    ...
    return {
      icon: Hash,
      label: isRead
        ? "Reads channel context from the Ambush relay."
        : "Changes channel state in the Ambush relay.",
      tone: isWrite ? "write" : "read",
    };
  }
  if (
    ...
    return {
      icon: Users,
      label: isRead
        ? "Reads Ambush identity or presence data."
        : "Updates Ambush identity or membership data.",
  ...
  if (name.includes("search") || name === "get_feed") {
    return {
      icon: Search,
      label: "Searches relay-visible Ambush history.",
      tone: "read",
    };
  }
  if (
    ...
    return {
      icon: Send,
      label: "Publishes relay-visible Ambush activity.",
      tone: "write",
    };
  }
  ...
  return {
    icon: MessageSquare,
    label: isRead ? "Reads from Ambush." : "Writes to Ambush.",
    tone: isWrite ? "write" : "read",
  };
}

export function normalizeToolName(title: string): string {
  const knownName = findAmbushToolName(title, true);
  if (knownName) return knownName;

  const normalized = normalizeToolNameText(title).replace(/^ambush_/, "");
...
export function findAmbushToolName(value: string, includeShortNames: boolean) {
  const alias = findAmbushToolAlias(value);
  if (alias) return alias;

  const normalized = normalizeToolNameText(value);
  return (
    AMBUSH_TOOL_NAMES_BY_LENGTH.find(
      (name) =>
...
function findAmbushToolAlias(value: string) {
  const normalizedPhrase = value
    ...
    .replace(/\s+/g, " ");
  return (
    AMBUSH_TOOL_TITLE_ALIASES.find(([pattern]) =>
...
export function formatToolTitle(
  ...
  if (AMBUSH_READ_TOOLS.has(name) || AMBUSH_WRITE_TOOLS.has(name)) {
    return name
      .split("_")
