...
test("Ambush shared compute names the empty state and next action", () => {
  const status = formatModelDiscoveryErrorStatus(
    new Error("no Ambush shared compute serving members are available"),
    "relay-mesh",
  );
...
test("Ambush shared compute distinguishes relay lookup failures", () => {
  const status = formatModelDiscoveryErrorStatus(
    new Error("Ambush shared compute model discovery failed: relay offline"),
    "relay-mesh",
  );
...
test("Ambush shared compute names a missing relay member roster", () => {
  const status = formatModelDiscoveryErrorStatus(
    new Error("Ambush shared compute is waiting for the current member roster"),
    "relay-mesh",
  );
...
test("Databricks sign-in-required is a muted note pointing at the picker and CLI", () => {
  ...
  assert.match(status?.message ?? "", /ambush-agent auth databricks/);
});
...
test("auth-required errors name the agent and ask for sign-in", () => {
  // Real shape from run_agent_models_command wrapping ambush-acp stderr when
  // cursor-agent is signed out (spec ErrorCode::AuthRequired text).
  const status = formatModelDiscoveryErrorStatus(
    new Error(
      "ambush-acp models failed (exit 1): agent communication failed: Agent reported error (code -32000): Authentication required",
    ),
    "",
    "Cursor",
  );
...
test("non-auth -32000 errors do NOT get the sign-in copy", () => {
  ...
  const status = formatModelDiscoveryErrorStatus(
    new Error(
      "ambush-acp models failed (exit 1): Agent reported error (code -32000): model catalog fetch timed out",
    ),
    "anthropic",
    "Cursor",
  );
