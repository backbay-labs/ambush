...
} from "@/features/agents/ui/ambushAgentConfig";
import {
  EffortSelectField,
  NumericTuningFields,
  useEffortAutoClear,
  type NumericDescriptor,
} from "@/features/agents/ui/ambushAgentModelTuningFields";
...
export function AgentConfigFields({
  ...
  const bakedProvider = React.useMemo(
    () =>
      bakedEnv.find((e) => e.key === "AMBUSH_AGENT_PROVIDER")?.value ?? null,
    [bakedEnv],
  );
  ...
  const bakedEffort = React.useMemo(
    () =>
      bakedEnv.find((e) => e.key === AMBUSH_AGENT_THINKING_EFFORT)?.value ??
      null,
    [bakedEnv],
  );
