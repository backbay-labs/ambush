/**
 * Tier-1 ambush-agent model-tuning UI fields.
 *
...
} from "./ambushAgentConfig";

/**
 * Shared effort-select dropdown for the `AMBUSH_AGENT_THINKING_EFFORT` env var.
 *
 * Used by both `AmbushAgentModelTuningFields` (per-agent/persona dialogs) and
...
export function EffortSelectField({
   ...
   * Defaults to `"Inherit"`.
   *
   * Per-agent callers (AmbushAgentModelTuningFields) pass `"Inherit (agent default)"`
  ...
  const effortOptions: AgentDropdownOption[] = [
    ...
    ...AMBUSH_AGENT_THINKING_EFFORT_VALUES.flatMap((v) => {
...
/**
 * Auto-clear hook: resets `AMBUSH_AGENT_THINKING_EFFORT` to "" (Inherit) when
 ...
 *
 * - `maxOutputTokens` / `contextLimit`: minimum 1 — the ambush-agent runtime
 *   rejects 0 for these fields (crates/ambush-agent/src/config.rs:921-928).
...
export function AmbushAgentModelTuningFields({
  envVars,
  inheritedEnvVars,
  model,
  onEnvVarChange,
  ...
  const currentEffort = envVars[AMBUSH_AGENT_THINKING_EFFORT] ?? "";
  useEffortAutoClear({
    currentEffort,
    effortValid,
    onClear: () => onEnvVarChange(AMBUSH_AGENT_THINKING_EFFORT, ""),
  });

  return (
    <div className="space-y-4">
      <p className="text-2xs font-semibold uppercase tracking-wide text-muted-foreground">
        ambush-agent model tuning
      </p>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        ...
        <div className="space-y-1.5">
          <EffortSelectField
            ...
            inheritedEffort={inheritedEnvVars[AMBUSH_AGENT_THINKING_EFFORT]}
            inheritFallbackLabel="Inherit (agent default)"
            label="Thinking / Effort"
            onChange={(value) =>
              onEnvVarChange(AMBUSH_AGENT_THINKING_EFFORT, value)
            }
