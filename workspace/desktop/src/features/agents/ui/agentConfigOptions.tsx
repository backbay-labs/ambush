...
import { AMBUSH_AGENT_THINKING_EFFORT } from "./ambushAgentConfig";
...
export const PERSONA_FIELD_CONTROL_CLASS =
  "border-0 bg-transparent text-muted-foreground shadow-none outline-none ring-0 transition-colors duration-150 ease-out placeholder:text-muted-foreground focus:bg-transparent focus:text-muted-foreground focus:outline-hidden focus-visible:ring-0";
 ...
 *
 * Mirrors the Rust `readiness::ambush_agent_requirements` /
...
export const PERSONA_LLM_PROVIDER_OPTIONS: readonly PersonaModelOption[] = [
  ...
  { id: "relay-mesh", label: "Ambush shared compute" },
...
> = {
  ...
  "ambush-agent": [DEFAULT_MODEL_OPTION],
 ...
 *
 * ambush-agent and goose use provider-specific credentials; claude and codex
...
export function requiredCredentialEnvKeys(
  ...
  if (normalizedRuntime !== "ambush-agent" && normalizedRuntime !== "goose") {
    return [];
  }
...
export function runtimeSupportsLlmProviderSelection(runtimeId: string) {
  return runtimeId === "ambush-agent" || runtimeId === "goose";
}
...
export function resetConfigForHarnessChange(
  ...
  delete nextEnvVars[AMBUSH_AGENT_THINKING_EFFORT];
 ...
 *
 * Two classes of required field for provider-selection runtimes (ambush-agent,
...
export function computeLocalModeGate({
  ...
  /** Env keys that are not set in Ambush but are satisfied in the runtime's
