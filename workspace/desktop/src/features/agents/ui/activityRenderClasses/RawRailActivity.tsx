      ...
      <ActivityRowContent className="flex flex-col gap-3 py-2">
        {isRawPayload ? (
          props.item.sections.map((section) => (
            ...
            >
              <summary className="inline-flex max-w-full cursor-pointer list-none items-center gap-1.5 text-xs font-medium text-muted-foreground group-open/section:text-foreground">
                <span className="truncate">{section.title}</span>
                <ChevronDown className="h-3.5 w-3.5 shrink-0 text-muted-foreground transition-transform group-open/section:rotate-180 group-open/section:text-foreground" />
              </summary>
