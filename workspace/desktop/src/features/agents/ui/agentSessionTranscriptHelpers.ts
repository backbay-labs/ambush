...
export function parsePromptText(text: string): {
  ...
  const eventSection = sections.find((section) => {
    ...
    return title.startsWith("ambush event");
  });
  ...
  return {
    ...
    userTitle: eventKind ? titleCase(eventKind) : "Ambush event",
 ...
 *    `with_team()` in `ambush-acp/src/pool.rs`. Same two cases (start-of-string
 ...
 *    (produced by the now-removed `compose_prompt()` in ambush-persona), the body
...
export function parseSystemPromptSections(
  ...
  // with_team() in ambush-acp/src/pool.rs appends "\n\n[Team Instructions]\n{instructions}"
  ...
  switch (tag) {
    ...
    case "ambush-event":
      return attributes.type
        ? `Ambush event: ${attributes.type}`
        : "Ambush event";
    case "ambush-events":
      return `Ambush events — ${attributes.count} events`;
...
export function extractToolIdentity(update: Record<string, unknown>): {
  ...
  const knownName = candidates
    .map((candidate) => findAmbushToolName(candidate, true))
