 ...
 *   ambush-agent — classifies LLM failures into `AgentError` variants with
 *                  JSON-RPC codes (`-32001` auth, `-32002` model-not-found,
 *                  `-32000` generic), defined in `crates/ambush-agent/src/types.rs`.
 *   ambush-acp   — preserves the code structurally in
...
export function friendlyAgentLastError(
  ...
  // Match either the unwrapped ambush-agent prefix or the ambush-acp v0 wrap.
  if (
