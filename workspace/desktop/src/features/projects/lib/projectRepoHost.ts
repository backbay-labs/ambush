...
export function projectRepoHost(
  ...
  try {
    ...
    const isAmbushPath = /^\/git\/[0-9a-f]{64}\/[^/]+\/?$/i.test(
      clone.pathname,
    );

    if (clone.origin === relay.origin && isAmbushPath) {
      return { kind: "ambush" };
    }
...
/**
 * Human-readable location of a repository's git data — "github.com/backbay-labs/ambush"
 * for external repos (host + path, `.git` stripped), or "owner/repo" for
 * Ambush-hosted ones (the relay host and full owner pubkey carry no signal;
...
export function repositoryDisplayPath(
  ...
  if (projectRepoHost(cloneUrl, relayOrigin).kind === "ambush") {
