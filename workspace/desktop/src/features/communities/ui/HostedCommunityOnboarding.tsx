...
const MODAL_PRIMARY_ACTION_CLASS = `${ONBOARDING_PRIMARY_CTA_CLASS} !text-[rgb(var(--ambush-hosted-community-modal-action-fg))]`;
...
export function HostedCommunityOnboarding({
      ...
      if (released.error) {
        throw new Error(
          hostedCommunityErrorMessage(
            ...
            "Could not disconnect the account's previous Ambush identity.",
          ),
        );
      }
  ...
  const signInDialog = (
    ...
    >
      <DialogContent
        className="ambush-onboarding-neutral-theme max-w-[560px] text-foreground [&_button]:shadow-none"
      ...
      >
        <div className="mx-auto flex w-full max-w-sm flex-col items-center py-2 text-center">
          ...
          {!auth ? (
            ...
            <>
              <DialogTitle className="text-xl font-medium text-foreground">
                Finish connecting Ambush
              </DialogTitle>
              <DialogDescription className="mt-2 text-sm leading-6 text-foreground">
                Your Builderlab account
                {auth.email ? ` (${auth.email})` : ""} is ready. Connect this
                device’s Ambush identity to finish setup. Your private key stays
                on this device.
              </DialogDescription>
              {errorBox ? <div className="mt-5 w-full">{errorBox}</div> : null}
              <Button
            ...
            <>
              <DialogTitle className="text-xl font-medium text-foreground">
                This account uses a different Ambush identity
              </DialogTitle>
              <DialogDescription className="mt-2 text-sm leading-6 text-foreground">
                This account is connected to another Ambush identity. Reconnect
                this device, or sign out to use a different email.
              </DialogDescription>
              <p className="mt-4 w-full break-all rounded-xl bg-[rgb(var(--ambush-hosted-community-identity-bg)/0.5)] px-4 py-3 text-left font-mono text-xs text-foreground">
    ...
    <div className="flex min-h-[calc(100dvh-15.625rem)] w-full max-w-[920px] flex-col items-center text-center">
      ...
      <p className="mx-auto mt-2 max-w-[560px] text-sm leading-6 text-foreground">
        {hasCommunities
          ...
          : "Claim an Ambush address to get started."}
      </p>
                ...
                <p
                  ...
                  className={`relative z-10 mt-3 min-h-5 text-center text-sm ${
                    ...
                    availability === false || (name && !validName)
                      ? "text-destructive"
                      : "text-[color:var(--ambush-onboarding-backup-ink)]"
                  }`}
