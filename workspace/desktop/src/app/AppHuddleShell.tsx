...
import * as AmbushTheme from "@/app/AmbushThemeSurfaces";
    ...
    >
      <HuddleShortcutHandler>
        <RemindMeLaterProvider pubkey={currentPubkey}>
          <div
            className="ambush-huddle-shell relative h-dvh overflow-hidden overscroll-none"
          ...
          >
            <div
              aria-hidden="true"
              className={cn(
                "ambush-huddle-drawer-backdrop",
                isDrawerOpen && "ambush-huddle-drawer-backdrop-open",
              )}
            />
            <div
              className={cn(
                "ambush-huddle-app-surface z-10 flex min-h-0 flex-row overflow-hidden bg-background",
                isDrawerOpen &&
                  (isRoom
                    ? "ambush-huddle-app-surface-room-open"
                    : "ambush-huddle-app-surface-open"),
              )}
            >
              <AmbushTheme.GradientLayer />
              {children}
            </div>
            {isRoom || !isCompanionOpen ? (
              <div className="ambush-huddle-drawer-slot absolute inset-x-0 bottom-0 z-[2] h-(--ambush-huddle-drawer-height)">
                <AppHuddleBar
