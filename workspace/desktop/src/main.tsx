...
function resetDevWebviewStateFromUrl() {
  ...
  // WebKit groups every Ambush binary under one disk directory, but storage is
...
function configureDevE2eBridgeFromUrl() {
  ...
  e2eWindow.__AMBUSH_E2E__ ??= { mode: "mock" };
  ...
  window.localStorage.setItem("ambush-active-community-id", E2E_COMMUNITY_ID);
  window.localStorage.setItem(
...
function renderApp() {
  ReactDOM.createRoot(document.getElementById("root") as HTMLElement).render(
    <React.StrictMode>
      {/* block/ambush#5078 — catch any uncaught render error so a WebKit
...
async function installE2eBridgeIfConfigured() {
  ...
  if (
    ...
    !(window as E2eWindow).__AMBUSH_E2E__
  ) {
    return;
  }
