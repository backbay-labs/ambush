...
APP_BIN="$WORKDIR/squashfs-root/usr/bin/ambush-desktop"
if [[ ! -f "$APP_BIN" ]]; then
  echo "Error: app binary usr/bin/ambush-desktop not found — bundler layout changed; update fix-appimage.sh" >&2
  exit 1
fi
if [[ -e "$APP_BIN.bin" ]]; then
  echo "Error: usr/bin/ambush-desktop.bin already exists — shim already installed?" >&2
  exit 1
fi

# The real binary moves aside; ambush-desktop becomes a shim AppRun.wrapped execs.
...
exec -a "ambush-desktop" "$here/ambush-desktop.bin" "$@"
SHIM
chmod +x "$APP_BIN"
