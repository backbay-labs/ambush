...
function scanText(text, label) {
  ...
  lines.forEach((line, i) => {
    for (const rule of RULES) {
      rule.re.lastIndex = 0;
      let m;
      // biome-ignore lint/suspicious/noAssignInExpressions: the idiomatic global-regex walk
...
function selfTest() {
  ...
  for (const line of FIXTURE_CLEAN) {
    const hits = scanText(line, "<fixture>");
    if (hits.length)
      problems.push(`clean control WRONGLY FLAGGED (${hits[0].rule}): ${line}`);
  }
  return problems;
}
...
const OVERRIDES = new Set([
  ...
  // biome-ignore lint/suspicious/noTemplateCurlyInString: the allowlisted literal IS a template placeholder
...
if (problems.length) {
  console.error(
    "check-svg-font-size: SELF-TEST FAILED. The scanner is broken; fix it before trusting it.",
  );
...
if (files.length === 0) {
  console.error(
    "check-svg-font-size: no files under " +
      roots.join(", ") +
...
for (const f of files) {
  for (const h of scanText(await fs.readFile(f, "utf8"), f)) {
    ...
    const matched = [...OVERRIDES].some((o) => {
      const i = o.indexOf(":");
      return (
        h.file.replace(/\\/g, "/").endsWith(o.slice(0, i)) &&
        h.literal === o.slice(i + 1)
      );
    });
    if (matched) {
      usedOverrides.add(h.literal);
      continue;
    }
    hits.push(h);
  }
...
if (hits.length) {
  console.error(
    `check-svg-font-size: ${hits.length} frozen text size(s) in ${files.length} file(s)`,
  );
  for (const h of hits)
    console.error(
      `  ${h.file}:${h.line}  [${h.rule}]  ${h.literal}\n      ${h.hint}`,
    );
  process.exit(1);
}
console.error(
