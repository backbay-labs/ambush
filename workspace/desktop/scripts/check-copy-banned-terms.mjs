// workspace/desktop/scripts/check-copy-banned-terms.mjs, run by `pnpm check:copy`
// and by the `check` script beside check-px-text.mjs and
...
//   It is read from tools/copy-ban-list.tsv at the repository root -- one file,
//   two runners (16-INVARIANT-TESTS.md D2). A vendored copy is a second registry
//   and would drift, which is the failure mode the whole wave-2 review named.
//
//   When this file lived in a separate repository the list could genuinely be
//   unavailable, so an unset PERCH_BAN_LIST meant SKIP. The two repositories are
//   one checkout now, so the list is resolved from this script's own location
//   (../../../tools/copy-ban-list.tsv) and a missing file is an ERROR, not a
//   skip. PERCH_BAN_LIST and PERCH_AMBUSH_ROOT still override, which is what the
//   negative controls use. The --require flag is retained and now only matters
//   if someone vendors this file back out of the tree.
//
...
/** The repository root, from this script at workspace/desktop/scripts/. */
...
function resolveBanList() {
  ...
  if (process.env.PERCH_AMBUSH_ROOT) {
    return path.join(
      process.env.PERCH_AMBUSH_ROOT,
      "tools",
      "copy-ban-list.tsv",
    );
  }
  const inTree = path.join(REPO_ROOT, "tools", "copy-ban-list.tsv");
...
export function scanExtracted(rows, extracted) {
  ...
  for (const item of extracted) {
    for (const row of rows) {
      ...
      if (row.exempt?.test(normalized)) continue;
      hits.push({
        ...item,
        id: row.id,
...
function modeFor(file) {
  return /(^|\/)copy\.ts$|(^|\/)copy\/[^/]+\.ts$|Copy\.ts$/.test(file)
    ? "copy"
    : "markup";
}
...
function selfTest(rows, corpusDir) {
  ...
  for (const name of readdirSync(corpusDir)) {
    ...
    for (const hit of scanExtracted(
      rows,
      extractStrings(mode, name, content),
    )) {
  ...
  if (missing.length === 0 && extra.length === 0) {
    console.log(
      `copy-ban parity corpus: ${expected.size} (file, row) pair(s), exact match`,
    );
    return 0;
  }
  console.error(
    "copy-ban parity corpus MISMATCH between this scanner and expected.tsv:",
  );
  for (const k of missing)
    console.error(`  MISSING  ${k.replace("\t", " -> ")}`);
  for (const k of extra) console.error(`  EXTRA    ${k.replace("\t", " -> ")}`);
  console.error("");
  console.error(
    "expected.tsv is the contract BOTH scanners meet. If a ban genuinely",
  );
  console.error(
    "changed, change tools/copy-ban-list.tsv and expected.tsv together, and",
  );
  console.error(
    "re-run the Ambush-side gate over the same corpus before landing.",
  );
  return 1;
}
...
function main() {
  ...
  if (!banListPath) {
    const msg =
      "check-copy-banned-terms: no ban list. Expected " +
      `${path.join(REPO_ROOT, "tools", "copy-ban-list.tsv")}, or set PERCH_BAN_LIST / ` +
      "PERCH_AMBUSH_ROOT. The list is NOT vendored here on purpose: one file, two " +
      "runners (16-INVARIANT-TESTS.md D2). Refusing to report a pass without it.";
    console.error(msg);
    if (REQUIRE_LIST) {
      console.error(
        "--require was passed, so an unavailable ban list is an error.",
      );
    }
    return 1;
  }
  ...
  if (SELF_TEST) {
    const corpus =
      ...
      path.join(
        path.dirname(path.dirname(banListPath)),
        "tools",
        "fixtures",
        "copy-corpus",
      );
  ...
  if (selfTest(rows, corpus) !== 0) {
    console.error(
      "The parity corpus failed, so this scanner's verdict over src/ means nothing.",
    );
    return 1;
  }
  ...
  for (const file of files) {
    ...
    hits.push(
      ...scanExtracted(
        rows,
        extractStrings(modeFor(rel), rel, readFileSync(file, "utf8")),
      ),
    );
  }
