...
await page.addInitScript(() => {
  ...
  window.localStorage.setItem("ambush-communities", JSON.stringify([ws]));
  window.localStorage.setItem("ambush-active-community-id", id);
});
await page.addInitScript(
  ({ prefix, pubkeys }) => {
...
await page.addInitScript(() => {
  ...
  window.__AMBUSH_E2E__ = { mode: "mock" };
