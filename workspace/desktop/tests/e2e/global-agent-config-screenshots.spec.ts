...
// A runtime catalog with both a provider-selection runtime (ambush-agent) and a
...
test.describe("global agent config screenshots", () => {
  ...
  test("settings renders and saves the persisted preferred harness", async ({
    ...
    const saved = await page.evaluate(async () =>
      ...
      ).__AMBUSH_E2E_INVOKE_MOCK_COMMAND__?.("get_global_agent_config", null),
    );
  ...
  test("07-explicit-global-defaults-override-baked-labels", async ({
    ...
    await installMockBridge(page, {
      globalAgentConfig: {
        provider: "anthropic",
        model: "claude-opus-4-5",
        env_vars: { AMBUSH_AGENT_THINKING_EFFORT: "low" },
      },
      bakedBuildEnv: [
        {
          key: "AMBUSH_AGENT_PROVIDER",
  ...
  test("04-create-blocked-no-provider-no-global", async ({ page }) => {
    ...
    await expect(harness).toHaveText("Ambush Agent");
  ...
  test("unset defaults persist the visible Ambush Agent fallback", async ({
    page,
  }) => {
    await installMockBridge(page);
    await openCreateDialog(page);
  ...
  test("create defaults follow a preferred harness saved while open", async ({
    ...
    const defaults = page.getByTestId("agent-ai-defaults-notice");
    await expect(defaults).toContainText("Ambush Agent");
    await defaults
  ...
  test("create exposes setup guidance when no harness is available", async ({
    ...
    await selectDropdownOption(page, harness, "Ambush Agent (not installed)");
    await expect(
      customSection
        .locator("p")
        .filter({ hasText: "Ambush Agent is not installed." }),
    ).toContainText(
      "Ambush Agent is not installed. Ships with the Ambush desktop app. Visit Settings > Agents to set it up.",
    );
    await expect(page.getByTestId("persona-dialog-submit")).toBeDisabled();
  });
  ...
  test("05-create-enabled-with-global-provider", async ({ page }) => {
    ...
    await expect(
      defaultsSection.getByText("Ambush Agent", { exact: true }),
    ).toBeVisible();
