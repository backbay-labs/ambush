...
/** ambush-agent is always available and has no auth step. */
const AMBUSH_AGENT_AVAILABLE = {
  id: "ambush-agent",
  label: "Ambush Agent",
  avatar_url: "",
  availability: "available",
...
test.describe("Doctor panel state screenshots", () => {
  ...
  test("00-runtime-card-layout", async ({ page }) => {
    ...
    for (const runtimeId of ["goose", "claude", "ambush-agent"]) {
      await expect(
    ...
    await expect(page.getByTestId("doctor-runtime-codex")).not.toContainText(
      "Ambush talks to Codex through the Codex CLI.",
    );
        ...
        },
        {
          ...
          steps: [
            ...
            },
            {
              ...
              stdout: "",
              stderr:
                "The installer finished, but Ambush still could not use codex (observed: NotInstalled).",
              exit_code: null,
              hint: "Ambush requires the vendor CLI executable, not only its desktop app. If the CLI was installed while Ambush was open, restart Ambush and check again.",
            },
          ],
        },
      ],
  ...
  test("11-outdated-adapter-warning", async ({ page }) => {
    ...
    await expect(dialog).toContainText(
      "Older Ambush releases using the legacy adapter may lose community access",
    );
  ...
  test("09-install-output-line-and-log-pointer", async ({ page }) => {
    await installMockBridge(page, {
      ...
      installAcpRuntimeResult: {
        ...
        log_path: "/tmp/ambush-install-codex.log",
      },
    });
    ...
    await expect(installError).toContainText("/tmp/ambush-install-codex.log");
