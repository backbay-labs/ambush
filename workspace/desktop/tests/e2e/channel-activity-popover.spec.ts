...
async function pushMockInboxFeedItems(page: Page, items: MockInboxFeedItem[]) {
  await page.waitForFunction(
    () =>
      typeof (
        window as Window & { __AMBUSH_E2E_PUSH_MOCK_FEED_ITEM__?: unknown }
      ).__AMBUSH_E2E_PUSH_MOCK_FEED_ITEM__ === "function",
  );
  await page.evaluate(
  ...
  return page.evaluate(
    ({ channelId, pubkey }) => {
      const raw = window.localStorage.getItem(
        `ambush-forced-unread.v1:${pubkey}`,
      );
      if (!raw) return [];
...
async function seedChannelActivity(
  ...
  if (includeAgent) {
    await page.waitForFunction(
      () =>
        typeof (
          window as Window & { __AMBUSH_E2E_SEED_ACTIVE_TURNS__?: unknown }
        ).__AMBUSH_E2E_SEED_ACTIVE_TURNS__ === "function",
    );
    await page.evaluate(
