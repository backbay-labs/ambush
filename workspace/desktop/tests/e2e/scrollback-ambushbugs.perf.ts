...
/**
 * SCROLL-BACK latency profile for one channel (#ambush-bugs) against a LIVE
 ...
 *   AMBUSH_E2E_RELAY_URL=http://127.0.0.1:13000 \
 *   AMBUSH_COMMUNITY_HOST=sprout-oss.stage.blox.sqprod.co \
 *   AMBUSH_PERF_NSEC=nsec1... \
 *   npx playwright test --config=playwright.perf.config.ts scrollback-ambushbugs.perf.ts
 */

const RELAY_HTTP =
  process.env.AMBUSH_E2E_RELAY_URL ?? "https://sprout-oss.stage.blox.sqprod.co";
const NSEC = process.env.AMBUSH_PERF_NSEC ?? "";
const COMMUNITY_HOST = process.env.AMBUSH_COMMUNITY_HOST ?? "";
const TARGET_CHANNEL = process.env.AMBUSH_PERF_CHANNEL ?? "ambush-bugs";
const PAGES = Number(process.env.AMBUSH_PERF_PAGES ?? 10);

const IDENTITY_OVERRIDE_KEY = "ambush:e2e-identity-override.v1";
const ONBOARDING_PREFIX = "ambush-onboarding-complete.v1:";
const WELCOME_PREFIX = "ambush-welcome-channel-ensured.v2:";
...
function deriveIdentity(nsec: string) {
  ...
  if (decoded.type !== "nsec")
    throw new Error("AMBUSH_PERF_NSEC is not an nsec");
...
test("MEASURE: scroll-back pagination latency in target channel", async ({
  ...
  if (!NSEC) throw new Error("Set AMBUSH_PERF_NSEC to a real member nsec");
  ...
  await page.addInitScript(
    ({ ident, onboardingPrefix, welcomePrefix, relayUrl, overrideKey }) => {
      ...
      w.__AMBUSH_E2E__ = { ...(w.__AMBUSH_E2E__ ?? {}), identity: ident };
    },
    {
      ident: identity,
