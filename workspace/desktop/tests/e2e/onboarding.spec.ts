...
async function setRelayConnectionState(
  ...
  await page.waitForFunction(
    () =>
      typeof (
        window as Window & {
          __AMBUSH_E2E_SET_RELAY_CONNECTION_STATE__?: unknown;
        }
      ).__AMBUSH_E2E_SET_RELAY_CONNECTION_STATE__ === "function",
  );
  await page.evaluate((nextState) => {
    ...
    const setConnectionState =
      testWindow.__AMBUSH_E2E_SET_RELAY_CONNECTION_STATE__;
...
const HOME_SEEN_STORAGE_KEY_PREFIX = "ambush-home-feed-seen.v1:";
const COMMUNITY_ONBOARDING_TRANSACTION_STORAGE_KEY =
  "ambush-community-onboarding-transaction.v1";
...
async function seedOnboardingCompletion(page: Page, pubkey: string) {
  await page.addInitScript(
    ...
    {
      storageKey: `ambush-onboarding-complete.v1:${pubkey}`,
    },
  );
}
...
async function getMockChannels(page: Page) {
  return page.evaluate(async () => {
    ...
    const invoke =
      bridgeWindow.__AMBUSH_E2E_INVOKE_MOCK_COMMAND__ ??
...
test("fresh existing-identity path leads with private-key recovery", async ({
  ...
  await expect(
    page.getByText("Paste your private key to sign in to Ambush."),
  ).toBeVisible();
  ...
  await expect(
    phoneDialog.getByRole("heading", { name: "Use your Ambush identity" }),
  ).toBeVisible();
...
test("non-local runtime override keeps community selection without release flag", async ({
  ...
  await expect
    .poll(() => page.evaluate(() => localStorage.getItem("ambush-communities")))
    .toBeNull();
});
...
test("non-local default auto-connects when the release flag is enabled", async ({
  ...
  await expect
    .poll(() =>
      page.evaluate(() => {
        const raw = window.localStorage.getItem("ambush-communities");
...
test("first-community owner can connect an existing hosted community", async ({
      ...
      builderlabCommunities: [
        {
          id: "owned-community",
          name: "North Star",
          normalized_host: "north-star.communities.ambush.example.com",
        },
      ],
    },
    {
  ...
  await expect
    ...
    .toContain("wss://north-star.communities.ambush.example.com");
...
test("first-community owner can create and connect a hosted community", async ({
  ...
  await expect(
    page.getByRole("heading", { name: "Finish connecting Ambush" }),
  ).toBeVisible();
  ...
  await expect
    ...
    .toContain("wss://bee-lab.communities.ambush.example.com");
});
...
test("first-community X cancels a pending sign-in", async ({ page }) => {
  ...
  await expect
    .poll(() => page.evaluate(() => window.__AMBUSH_E2E_COMMANDS__ ?? []))
...
test("first-community owner can replace a mismatched account identity", async ({
  ...
  await expect(
    page.getByRole("heading", {
      name: "This account uses a different Ambush identity",
    }),
  ).toBeVisible();
  await page
...
test("first-community explains when the local identity belongs to another account", async ({
  ...
  await expect(
    page.getByText(
      "This device's Ambush identity belongs to a different Builderlab account and can't be moved from here. Sign out, then sign in with the account that already owns this identity.",
    ),
  ).toBeVisible();
  await expect(
    page.getByRole("heading", { name: "Finish connecting Ambush" }),
...
test("first-community shows the scenario cards for localhost", async ({
  ...
  await expect(
    page
      ...
      .locator(".ambush-onboarding-transition-line"),
...
test("first-community direct join reaches profile", async ({ page }) => {
  ...
  await page
    ...
    .fill("wss://onboarding.communities.ambush.example.com");
...
test("community onboarding reuses an existing relay profile", async ({
    ...
    ({ pubkey, transactionStorageKey }) => {
      ...
      window.localStorage.setItem(
        ...
        JSON.stringify({
          ...
          stage: "profile",
          relayUrl: "wss://onboarding.communities.ambush.example.com",
  ...
  await installMockBridge(
    ...
    {
      relayWsUrl: "wss://onboarding.communities.ambush.example.com",
      skipOnboardingSeed: true,
    },
  );
  await page.goto("/");
...
test("first-community direct join cancel returns to request access", async ({
  ...
  await expect
    .poll(() =>
      page.evaluate(
        (storageKey) => ({
          communities: window.localStorage.getItem("ambush-communities"),
...
test("replacing a pending upload disposes its verifier and local preview", async ({
  ...
  await page.addInitScript(() => {
    const testWindow = window as Window & {
      __AMBUSH_E2E_REVOKED_OBJECT_URLS__?: string[];
    };
    const revokedUrls: string[] = [];
    const revokeObjectUrl = URL.revokeObjectURL.bind(URL);
    testWindow.__AMBUSH_E2E_REVOKED_OBJECT_URLS__ = revokedUrls;
  ...
  await expect
    .poll(() =>
      page.evaluate(() => {
        ...
        return testWindow.__AMBUSH_E2E_REVOKED_OBJECT_URLS__ ?? [];
      }),
    )
...
test("no-event profile cached then reloaded still sees onboarding", async ({
  ...
  const SELF_PROFILE_CACHE_KEY = `ambush-self-profile.v1:ws://localhost:3000:${TEST_IDENTITIES.tyler.pubkey}`;
  await page.addInitScript(
    ({ key, cache }) => {
...
test("failed avatar saves can continue without saving the avatar", async ({
  ...
  await page.evaluate(() => {
    const testWindow = window as Window & {
      __AMBUSH_E2E__?: { mock?: { profileUpdateError?: string } };
    };
    if (testWindow.__AMBUSH_E2E__?.mock) {
      testWindow.__AMBUSH_E2E__.mock.profileUpdateError =
...
async function commandCount(page: Page, command: string) {
  return page.evaluate(
    (target) =>
      window.__AMBUSH_E2E_COMMANDS__?.filter((entry) => entry === target)
        .length ?? 0,
    command,
  );
}
    ...
    .poll(() =>
      page.evaluate(
        () =>
          ...
          ).__AMBUSH_E2E_COMMANDS__?.includes("plugin:websocket|disconnect") ??
          false,
      ),
    )
    .toBe(true);
...
test("same-relay identity replacement rebuilds the community boundary (A→B→A cache and draft isolation)", async ({
  ...
  const DRAFT_STORE_KEY_PREFIX = "ambush-drafts.v2";
  ...
  await page.evaluate(() => {
    const testWindow = window as Window & {
      ...
      __AMBUSH_E2E_PREVIOUS_QUERY_CLIENT__?: unknown;
    };
    const client = testWindow.__AMBUSH_E2E_QUERY_CLIENT__;
    if (!client) {
    ...
    testWindow.__AMBUSH_E2E_PREVIOUS_QUERY_CLIENT__ = client;
  });
  ...
  await expect
    .poll(() =>
      page.evaluate(() => {
        ...
        if (client === testWindow.__AMBUSH_E2E_PREVIOUS_QUERY_CLIENT__) {
...
test("denied on relay A then paste relay B invite URL switches community to B", async ({
  ...
  await page
    .getByLabel("I agree to the Ambush Terms of Service and Privacy Policy.")
    .check();
  await page.getByTestId("invite-redeem-submit").click();
