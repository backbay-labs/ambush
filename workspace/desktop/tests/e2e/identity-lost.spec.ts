...
test("normal first launch uses the already-persisted identity", async ({
  ...
  // The onboarding shell paints the night room, flat.
  await expect(gate).toHaveCSS("background-color", "rgb(23, 23, 23)");
  await expect(gate).toHaveCSS("background-image", "none");
  await expect(gate).toHaveCSS("color", "rgb(201, 193, 183)");
  await expect(
    page.getByRole("button", { name: "Create a new identity key" }),
  ).toHaveCSS("background-color", "rgb(201, 193, 183)");
  ...
  const commands = await page.evaluate(
    () =>
      (
        window as Window & {
          __AMBUSH_E2E_COMMAND_PAYLOADS__?: Array<{ command: string }>;
        }
      ).__AMBUSH_E2E_COMMAND_PAYLOADS__ ?? [],
  );
...
test("lost boot offers phone recovery with a single-use QR", async ({
  ...
  await expect(
    page.getByText("Scan this code with a signed-in Ambush phone."),
  ).toBeVisible();
  await expect(
...
test("phone recovery uses the desktop pairing card semantics", async ({
  ...
  await expect(qrCode.locator(".ambush-qr-cell-reveal").first()).toHaveCSS(
    "animation-name",
    "ambush-qr-cell-reveal",
  );
  ...
  await expect(
    card.getByText(
      "This gives this desktop permanent access to your Ambush identity. Only continue if you trust it.",
    ),
  ).toBeVisible();
  await expect(
    ...
    .poll(() =>
      page.evaluate(
        () =>
          ...
          ).__AMBUSH_E2E_COMMAND_PAYLOADS__?.some(
