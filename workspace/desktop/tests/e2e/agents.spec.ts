...
async function waitForInvokeBridge(page: import("@playwright/test").Page) {
  await page.waitForFunction(
    () => {
      ...
      return (
        typeof tauriWindow.__AMBUSH_E2E_INVOKE_MOCK_COMMAND__ === "function" ||
...
async function invokeTauri<T>(
  ...
  return page.evaluate(
    async ({ command: targetCommand, payload: targetPayload }) => {
      ...
      const invoke =
        tauriWindow.__AMBUSH_E2E_INVOKE_MOCK_COMMAND__ ??
...
async function countCommandInvocations(
  ...
  return page.evaluate(
    (targetCommand) =>
      ...
      ).__AMBUSH_E2E_COMMANDS__?.filter((invoked) => invoked === targetCommand)
        .length ?? 0,
    command,
  );
}
...
test("catalog hides built-ins and shows the shared-agent empty state", async ({
  ...
  for (const personaName of ["Anvil", "Lantern", "Sextant"]) {
  ...
  await page
    ...
    .getByRole("button", { name: "Close" })
    .click();
  await page.getByLabel("Open actions for Anvil").click();
  await page.getByRole("menuitem", { name: "Share" }).click();
...
test("built-in persona edits persist", async ({ page }) => {
  ...
  await dialog.getByLabel("Agent name").fill("My Anvil");
...
async function readAgentShareCommands(
  ...
  return page.evaluate(
    () =>
      (
        window as Window & {
          __AMBUSH_E2E_COMMAND_LOG__?: AgentShareCommand[];
        }
...
test("custom personas share with people and keep export separate", async ({
  ...
  await expect(
    exportDialog.getByRole("button", { name: "Send in Ambush" }),
  ).toHaveCount(0);
  ...
  expect(copiedAgent?.html).toContain("data-ambush-agent-snapshot");
  ...
  await page.evaluate(() => {
    ...
    (
      window as Window & {
        __AMBUSH_RECIPIENT_POPOVER_OBSERVER__?: MutationObserver;
        __AMBUSH_RECIPIENT_POPOVER_STATE_CHANGES__?: string[];
      }
    ).__AMBUSH_RECIPIENT_POPOVER_OBSERVER__ = observer;
    (
      window as Window & {
        __AMBUSH_RECIPIENT_POPOVER_STATE_CHANGES__?: string[];
      }
    ).__AMBUSH_RECIPIENT_POPOVER_STATE_CHANGES__ = stateChanges;
  });
  await recipientSearch.click();
  await waitForAnimations(page);
  const recipientPopoverStateChanges = await page.evaluate(() => {
    ...
    trackedWindow.__AMBUSH_RECIPIENT_POPOVER_OBSERVER__?.disconnect();
    return trackedWindow.__AMBUSH_RECIPIENT_POPOVER_STATE_CHANGES__ ?? [];
  });
  ...
  await expect(bobChip).not.toHaveClass(/ambush-poof-trigger/);
  await waitForAnimations(page);
  ...
  await expect(page.locator(".ambush-poof-burst")).toHaveCount(0);
  await page.mouse.up();
...
test("people sharing blocks a timeout before encoding or upload", async ({
  ...
  await page.evaluate(() => {
    ...
    ).__AMBUSH_E2E_ACTIVATE_TIMEOUT__?.(Date.now() + 60_000);
  });
...
test("people sharing rechecks destination eligibility after encoding", async ({
  ...
  await page.evaluate(() => {
    ...
    testWindow.__AMBUSH_E2E_MUTATE_CHANNEL__?.({
      channelId: "d1ec7000-d000-4000-8000-000000000001",
      channelType: "forum",
    });
    return testWindow.__AMBUSH_E2E_INVALIDATE_CHANNELS__?.();
  });
...
test("inactive built-ins cannot be used to create teams", async ({ page }) => {
  ...
  expect(error).toBe("Lantern is not in My Agents.");
});
...
test("built-in removal failures show up from My Agents", async ({ page }) => {
  ...
  await page.getByLabel("Open actions for Lantern").click();
  await page.getByRole("menuitem", { name: "Delete" }).click();

  await expect(
    page
      .locator("[data-sonner-toast]")
      .filter({ hasText: "Lantern is still referenced by a team." }),
  ).toBeVisible();
});
