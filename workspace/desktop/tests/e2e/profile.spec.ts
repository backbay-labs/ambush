    ...
    .poll(async () => {
      return page.evaluate((channelName) => {
        return (
          ...
          ).__AMBUSH_E2E_HAS_MOCK_LIVE_SUBSCRIPTION__?.({ channelName }) ??
          false
        );
      }, channelName);
    })
...
test("keeps the saved profile description after a community round trip", async ({
  ...
  await page.addInitScript((seed) => {
    window.localStorage.setItem("ambush-communities", JSON.stringify(seed));
    window.localStorage.setItem("ambush-active-community-id", seed[0].id);
  }, communities);
  await page.goto("/");
...
test("nests the avatar edit button in a clipped notch", async ({ page }) => {
  // Under the Ambush default theme the settings nav overrides `--sidebar-active`
  ...
  // only holds outside the Ambush theme.
  await page.addInitScript(() => {
    window.localStorage.setItem("ambush-theme", "github-light");
  });
  await page.goto("/");
...
test("uploads local profile avatar files before saving", async ({ page }) => {
  ...
  const pastedAvatarUrl = await page.evaluate(
    () => new URL("/ambush.svg", window.location.href).href,
  );
...
test("renders emoji avatars with a static background layer", async ({
  ...
  await expect(avatarPreview).not.toHaveClass(/ambush-avatar-squish/);
      ...
      return {
        heroHeight: Number.parseFloat(
          getComputedStyle(layout).getPropertyValue(
            "--ambush-profile-sticky-hero-height",
          ),
        ),
...
test("notification settings drive the Inbox badge and desktop alerts", async ({
  ...
  async function getAppBadgeCount() {
    return page.evaluate(() => {
      const win = window as Window & {
        __AMBUSH_E2E_APP_BADGE_COUNT__?: number;
      };

      return win.__AMBUSH_E2E_APP_BADGE_COUNT__ ?? 0;
    });
  }
  ...
  await page.evaluate(() => {
    ...
    win.__AMBUSH_E2E_PUSH_MOCK_FEED_ITEM__?.({
      category: "mention",
  ...
  await expect
    .poll(() =>
      page.evaluate(() => {
        const win = window as Window & {
          __AMBUSH_E2E_NOTIFICATIONS__?: Array<{
            body: string | null;
            title: string;
          }>;
        };

        return win.__AMBUSH_E2E_NOTIFICATIONS__?.length ?? 0;
      }),
    )
    .toBe(1);
  ...
  const notifications = await page.evaluate(() => {
    ...
    return win.__AMBUSH_E2E_NOTIFICATIONS__ ?? [];
  });
  ...
  const clickedNotification = await page.evaluate(() => {
    const win = window as Window & {
      __AMBUSH_E2E_CLICK_NOTIFICATION__?: (index: number) => boolean;
    };

    return win.__AMBUSH_E2E_CLICK_NOTIFICATION__?.(0) ?? false;
  });
...
test("opens settings with the keyboard shortcut and updates theme", async ({
  ...
  // Default is Ambush in System mode; Playwright's default color scheme is
  // light, so the app boots with the light Ambush theme.
  await expect
    .poll(() =>
  ...
  await expect
    .poll(() => page.evaluate(() => localStorage.getItem("ambush-theme")))
    .toBe("github-light");
...
test("supports webview zoom keyboard shortcuts", async ({ page }) => {
  ...
  const getTextScaleState = () =>
    page.evaluate(() => ({
      ...
      storedScale: localStorage.getItem("ambush:text-scale"),
      webviewZoom: (window as Window & { __AMBUSH_E2E_WEBVIEW_ZOOM__?: number })
        .__AMBUSH_E2E_WEBVIEW_ZOOM__,
    }));
...
test("storage clear resets composed font size and keyboard zoom across windows", async ({
  ...
  const readTypographyState = () =>
    page.evaluate(() => {
      ...
      probe.style.fontSize = "var(--ambush-type-rem)";
      ...
      return {
        ...
        rootFontSize: getComputedStyle(document.documentElement).fontSize,
        typeRemPx,
        textScale: localStorage.getItem("ambush:text-scale"),
      };
    });
