...
// loading gate keeps the engraving index up as an overlay above the already
...
// actionability); this spec opts back in via __AMBUSH_E2E__.bootSplashHoldMs.

test("boot splash overlay holds with an engraving index, then dismisses", async ({
  page,
}) => {
  ...
  // The index is actually animating while the overlay holds — pure CSS, no SMIL.
  const segmentState = await overlay
    .locator(".ambush-index__seg--live")
    .evaluate((segment) => {
      const animation = segment.getAnimations()[0];
      return {
        name: getComputedStyle(segment).animationName,
        state: animation?.playState,
      };
    });
  expect(segmentState).toEqual({
    name: "ambush-index-engrave-live",
    state: "running",
  });
