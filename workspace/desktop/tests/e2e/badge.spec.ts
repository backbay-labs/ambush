...
async function getBadgeState(page: import("@playwright/test").Page) {
  return page.evaluate(() => {
    ...
    return {
      state: w.__AMBUSH_E2E_APP_BADGE_STATE__ ?? "none",
      count: w.__AMBUSH_E2E_APP_BADGE_COUNT__ ?? 0,
    };
  });
}
...
test("top-level @mention shows an accent-colored numeric badge on its channel", async ({
  ...
  await page.addInitScript(() => {
    window.localStorage.setItem("ambush-theme", "slack-ochin");
    window.localStorage.setItem("ambush-accent-color", "#22c55e");
  });
  await page.goto("/");
  await page.getByTestId("channel-general").click();
  await expect(page.getByTestId("chat-title")).toHaveText("general");
...
test("remote read-state rollback is ignored while local mark-unread still increments badge", async ({
  ...
  await page.evaluate(
    ({ clientId, slotId, channelId, ts }) => {
      ...
      ).__AMBUSH_E2E_EMIT_MOCK_READ_STATE__?.({
        clientId,
        slotId,
