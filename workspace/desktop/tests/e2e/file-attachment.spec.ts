...
test("photos upload before Send without a queued spoiler control", async ({
  ...
  await page.evaluate(() => {
    const e2e = (
      window as Window & {
        __AMBUSH_E2E__?: { mock?: { uploadDelayMs?: number } };
      }
    ).__AMBUSH_E2E__;
...
test("shows upload feedback before transferring a large file", async ({
  ...
  await page.evaluate(async (id) => {
    await window.__AMBUSH_E2E_EMIT_MEDIA_UPLOAD_PHASE__?.({
      id,
  ...
  await page.evaluate(async (id) => {
    ...
    await window.__AMBUSH_E2E_EMIT_MEDIA_UPLOAD_PROGRESS__?.({
      id,
      sent: 42,
      total: 100,
    });
...
for (const theme of ["ambush", "ambush-dark", "github-light", "github-dark"]) {
