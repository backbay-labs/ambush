...
export class TwoRelayHarness {
  ...
  static async create(relays: readonly RelaySpec[]) {
    return new TwoRelayHarness(
      await mkdtemp(join(tmpdir(), "ambush-ae-e2e-")),
      relays,
    );
  }

  async startRelays(binary = process.env.AMBUSH_E2E_RELAY_BIN) {
    if (!binary)
      throw new Error("AMBUSH_E2E_RELAY_BIN is required for the live gate");
    await Promise.all(
  ...
  async restartRelay(name: string, binary = process.env.AMBUSH_E2E_RELAY_BIN) {
    if (!binary)
      throw new Error("AMBUSH_E2E_RELAY_BIN is required for the live gate");
  ...
  async startAcp(
    ...
    const binary = process.env.AMBUSH_E2E_ACP_BIN;
    if (!binary)
      throw new Error("AMBUSH_E2E_ACP_BIN is required for the live gate");
    return this.spawnOwned(name, binary, [], {
      AMBUSH_RELAY_URL: relayWsUrl,
      AMBUSH_PRIVATE_KEY: privateKey,
      AMBUSH_AUTH_TAG: "",
      AMBUSH_ACP_LAZY_POOL: "true",
      AMBUSH_ACP_AGENT_COMMAND: process.execPath,
      AMBUSH_ACP_AGENT_ARGS: resolve("tests/e2e/fixtures/fake-acp-agent.mjs"),
      AMBUSH_E2E_CLI_BIN: process.env.AMBUSH_E2E_CLI_BIN,
      ...extraEnv,
    });
  }
  ...
  private async startRelay(binary: string, relay: RelaySpec) {
    const child = this.spawnOwned(relay.name, binary, [], {
      ...
      AMBUSH_BIND_ADDR: `127.0.0.1:${relay.ports.main}`,
      AMBUSH_HEALTH_PORT: String(relay.ports.health),
      AMBUSH_METRICS_PORT: String(relay.ports.metrics),
