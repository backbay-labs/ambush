...
test("duplicate owned agents preserve provenance and exact pubkey selection", async ({
  ...
  await page.evaluate(
    async ({ channelId, pubkey }) => {
      const invoke = window.__AMBUSH_E2E_INVOKE_MOCK_COMMAND__;
...
test("thread autocomplete keeps multiple long names readable in a narrow panel", async ({
  ...
  await page.addInitScript(() => {
    window.sessionStorage.setItem("ambush.desktop.thread-panel-width", "300");
  });
  await page.goto("/");
  await page.getByTestId("channel-general").click();
  await expect(page.getByTestId("chat-title")).toHaveText("general");
...
test("cached relay-agent suggestions are removed when channel authorization disappears", async ({
  ...
  await page.evaluate(async (channelId) => {
    ...
    bridge.__AMBUSH_E2E_MUTATE_CHANNEL__?.({ channelId, channelType: null });
...
test("forum sends revalidate relay-agent authorization before signing", async ({
  ...
  await page.evaluate(() => {
    window.__AMBUSH_E2E__.mock ??= {};
    window.__AMBUSH_E2E__.mock.agentListDelayMs = 1_000;
    window.__AMBUSH_E2E__.mock.relayAgentListErrors = Array(100).fill(
...
test("managed agents use the channel roster for membership labels", async ({
  ...
  await expect
    .poll(() =>
      page.evaluate(
        (channelId) =>
          window.__AMBUSH_E2E_QUERY_CLIENT__?.getQueryState([
            "channels",
            channelId,
            "members",
          ])?.status,
...
test("relay-agent directory errors fail closed and recover after a fresh fetch", async ({
  ...
  await page.evaluate(() => {
    window.__AMBUSH_E2E__.mock ??= {};
    window.__AMBUSH_E2E__.mock.agentListDelayMs = 1_000;
    void window.__AMBUSH_E2E_QUERY_CLIENT__?.invalidateQueries({
      queryKey: ["relay-agents"],
    });
  });
  await expect
    .poll(async () =>
      page.evaluate(
        () =>
          window.__AMBUSH_E2E_QUERY_CLIENT__?.getQueryState(["relay-agents"])
            ?.fetchStatus,
      ),
    )
    .toBe("fetching");
...
test("managed agents keep their p tag when relay discovery fails before send", async ({
  ...
  await page.evaluate(() => {
    window.__AMBUSH_E2E__.mock ??= {};
    window.__AMBUSH_E2E__.mock.relayAgentListErrors = Array(5).fill(
...
test("targeted revocation before send causes no agent side effects", async ({
  ...
  await page.evaluate((pubkey) => {
    window.__AMBUSH_E2E__.mock ??= {};
    window.__AMBUSH_E2E__.mock.relayAgentRevalidationRevokedPubkeys = [pubkey];
...
test("selected relay agents revoked during send emit no p tag", async ({
  ...
  await page.evaluate(() => {
    window.__AMBUSH_E2E__.mock ??= {};
    window.__AMBUSH_E2E__.mock.agentListDelayMs = 300;
  });
  await page.getByTestId("send-message").click();
  ...
  expect(commandLog).toEqual(
    expect.arrayContaining([
      expect.objectContaining({
        command: "send_channel_message",
        payload: expect.objectContaining({
          content: expect.stringContaining("<!-- ambush:wave:v1 -->"),
        }),
      }),
    ]),
  );
