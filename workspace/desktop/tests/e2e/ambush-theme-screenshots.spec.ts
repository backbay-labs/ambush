...
const SHOTS = "test-results/ambush-theme";
const THEME_STORAGE_KEY = "ambush-theme";
const GLASS_BACKGROUND_STORAGE_KEY = "ambush-glass-background";
const GLASS_OPACITY_STORAGE_KEY = "ambush-glass-opacity";
const PROMINENT_ACTIVE_TAB_STORAGE_KEY = "ambush-prominent-active-tab";
const CONVERSATION_DENSITY_STORAGE_KEY =
  "ambush.appearance.conversationDensity";
const FONT_SIZE_STORAGE_KEY = "ambush.appearance.fontSize";
...
async function expectAmbushSidebarPalette(page: Page, mode: "light" | "dark") {
  ...
  await expect(
    page.locator("[data-ambush-sidebar-secondary]").first(),
...
async function expectAmbushGradientPaint(
  ...
  const paint = await page.evaluate(() => {
    ...
    const appSurface = document.querySelector(".ambush-huddle-app-surface");
    const lightLayer = document.querySelector('[data-ambush-gradient="light"]');
    const darkLayer = document.querySelector('[data-ambush-gradient="dark"]');
    ...
    return {
      ...
      theme: root.getAttribute("data-ambush-theme"),
...
async function expectAmbushSettingsPalette(page: Page, mode: "light" | "dark") {
  ...
  await expectAmbushGradientPaint(page, mode);
    ...
    .poll(() =>
      page.evaluate((storageKey) => {
        ...
        return {
          ...
          isDark: root.classList.contains("dark"),
          ambushTheme: root.getAttribute("data-ambush-theme"),
          gradientTop: styles.getPropertyValue("--ambush-gradient-top").trim(),
          gradientBottom: styles
            .getPropertyValue("--ambush-gradient-bottom")
            .trim(),
        };
      }, THEME_STORAGE_KEY),
    )
...
test("ambush light sidebar gradient", async ({ page }) => {
  ...
  await page
    .getByTestId("app-sidebar")
    .screenshot({ path: `${SHOTS}/01-ambush-light-sidebar.png` });
});
...
test("ambush dark sidebar gradient", async ({ page }) => {
  ...
  await page
    .getByTestId("app-sidebar")
    .screenshot({ path: `${SHOTS}/02-ambush-dark-sidebar.png` });
});
...
test("app font size and conversation density apply independently", async ({
  ...
  // type tokens ride on `--ambush-type-rem`, which itself derives from the
  ...
  const readScale = () =>
    root.evaluate((element) => {
      const PROBE_ID = "ambush-e2e-conversation-scale-probe";
      const tokens = {
...
test("settings nav uses Ambush active pill + hover (light)", async ({
  ...
  // Appearance is the active section here; its nav row should carry the Ambush
  ...
  await expectAmbushSettingsPalette(page, "light");
...
for (const { mode, theme } of [
  ...
  test(`${theme} ignores the Ambush prominent preference`, async ({ page }) => {
    await seedTheme(page, theme);
    await page.addInitScript(
      ({ key }) => window.localStorage.setItem(key, "true"),
      { key: PROMINENT_ACTIVE_TAB_STORAGE_KEY },
...
test("glass background keeps the content panel solid", async ({ page }) => {
  ...
  await expect(page.locator(".ambush-theme-gradient-underlay")).not.toHaveCSS(
    "background-image",
    "none",
  );
  ...
  await expect(opacitySlider).toHaveClass(/ambush-avatar-framing-slider/);
  ...
  await expect(
    opacitySlider.locator(".ambush-avatar-framing-slider-handle"),
  ).toHaveCSS("opacity", "1");
  await expect(root).toHaveAttribute("data-glass-background", "");
  const ambushSettingOrder = await page
  ...
  expect(ambushSettingOrder).toEqual([
  ...
  await expect(page.locator(".ambush-theme-gradient-underlay")).toHaveCSS(
    "background-image",
    "none",
  );
  ...
  await expect
    .poll(() =>
      page.evaluate(() =>
        (window.__AMBUSH_E2E_COMMAND_LOG__ ?? []).some(
          (entry) =>
...
test("non-Ambush glass preserves the selected theme sidebar tint", async ({
  page,
}) => {
  ...
  const root = page.locator("html");
  await expect(root).not.toHaveAttribute("data-ambush-sidebar", "");
  await page.getByTestId("glass-background-toggle").click();
  await expect(root).toHaveAttribute("data-glass-background", "");

  const tint = await page
    .locator(".ambush-theme-gradient-layer")
    .evaluate((element) => {
...
test("accent picker reveals/hides when toggling Ambush", async ({ page }) => {
  // Start on a non-Ambush theme so the accent picker is present, then select the
  // Ambush tile — the picker should animate out and unmount. Reselecting a
  // non-Ambush tile brings it back. Asserts the presence toggle (the motion
  ...
  const nonAmbushSettingOrder = await page
    .getByTestId("appearance-theme-card")
    .locator(
    ...
    .evaluateAll((rows) => rows.map((row) => row.getAttribute("data-testid")));
  expect(nonAmbushSettingOrder).toEqual([
    "appearance-color-mode-row",
    "theme-style-row",
  ...
  // Back to a non-Ambush theme — picker returns.
