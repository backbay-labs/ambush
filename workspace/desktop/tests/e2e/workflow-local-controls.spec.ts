...
declare global {
  interface Window {
    __AMBUSH_WORKFLOW_BATCH_CALLS__?: {
      eventBatches: number[];
      userBatches: number[];
    };
  }
...
test("round-trips and reopens structured message-text conditions", async ({
  ...
  const text = 'deploy "ambush"\\path';
  const expression =
    'str_ends_with(trigger_text, "deploy \\"ambush\\"\\\\path")';
  const dialog = await openCreateWorkflow(page, name);
...
test("workflow grid batches card author and message presentation reads", async ({
  ...
  await page.evaluate(async () => {
    ...
    window.__AMBUSH_WORKFLOW_BATCH_CALLS__ = calls;
  ...
  await expect
    .poll(() =>
      page.evaluate(() => {
        const calls = window.__AMBUSH_WORKFLOW_BATCH_CALLS__;
        return {
