...
: "${AMBUSH_K8S_TEST_CONTEXT:?set the explicit disposable/local kubectl context}"
: "${AMBUSH_SPRIG_IMAGE:?set an immutable image reference (name@sha256:<64 hex>)}"

if [[ ! "$AMBUSH_SPRIG_IMAGE" =~ ^[^[:space:]@]+@sha256:[0-9a-fA-F]{64}$ ]]; then
    echo "error: AMBUSH_SPRIG_IMAGE must be name@sha256:<64 hex>" >&2
    exit 2
fi

CONTEXT="$AMBUSH_K8S_TEST_CONTEXT"
IMAGE="$AMBUSH_SPRIG_IMAGE"
PULL_POLICY="${AMBUSH_K8S_TEST_PULL_POLICY:-IfNotPresent}"
case "$PULL_POLICY" in
    Always|IfNotPresent|Never) ;;
    *) echo "error: invalid AMBUSH_K8S_TEST_PULL_POLICY: $PULL_POLICY" >&2; exit 2 ;;
esac

MANAGED_BY="ambush-backend-kubernetes"
BINDING_VERSION="v1"
NAMESPACE="ambush-k8s-sprig-$(date +%s)-$RANDOM"
CREATED=0
...
cleanup() {
    ...
    foreign="$(kubectl --context "$CONTEXT" --namespace "$NAMESPACE" get pods -o json \
        | jq '[.items[] | select(.metadata.labels["app.kubernetes.io/managed-by"] != "ambush-backend-kubernetes" or .metadata.labels["ambush.backbay.com/binding-version"] != "v1")] | length')"
    if [[ "$foreign" != 0 ]]; then
...
kubectl --context "$CONTEXT" label namespace "$NAMESPACE" \
    ...
    "ambush.backbay.com/binding-version=$BINDING_VERSION"
...
spec:
  ...
  containers:
    - name: probe
      ...
      args:
        - 'test "\$(readlink /usr/local/bin/ambush-acp)" = sprig; echo DIGEST_ABI_OK'
YAML
