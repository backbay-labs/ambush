#!/usr/bin/env bash
# =============================================================================
# start-relay-for-tests.sh — Start the Ambush relay and its backing services
# =============================================================================
...
wait_healthy "Postgres" "ambush-postgres"
wait_healthy "Redis" "ambush-redis"
wait_healthy "MinIO" "ambush-minio"
...
export PGSCHEMA_PLAN_DB=ambush
export PGSCHEMA_PLAN_USER=ambush
export PGSCHEMA_PLAN_PASSWORD=ambush_dev

./bin/pgschema apply --file schema/schema.sql --auto-approve
docker exec -i -e PGPASSWORD="${PGPASSWORD}" ambush-postgres \
...
# psql is not on PATH in the hermit env; postgres runs as the ambush-postgres
...
if command -v psql >/dev/null 2>&1; then
  ...
  seed_psql() { docker exec -e PGPASSWORD="${PGPASSWORD}" ambush-postgres psql -U "${PGUSER}" -d "${PGDATABASE}" -qtA "$@"; }
fi
seed_psql -c "
...
if [[ "${SKIP_BUILD}" == "true" ]]; then
  for bin in ambush-relay git-credential-nostr; do
  ...
  cargo build --profile "${CARGO_PROFILE}" -p ambush-relay -p git-credential-nostr
  ok "Relay built"
fi
...
TEST_RELAY_PRIVATE_KEY="${AMBUSH_RELAY_PRIVATE_KEY:-$(openssl rand -hex 32)}"
...
MEMBERSHIP_ENV=()
if [[ "${AMBUSH_REQUIRE_RELAY_MEMBERSHIP:-}" == "true" ]]; then
  MEMBERSHIP_ENV+=(
    AMBUSH_REQUIRE_RELAY_MEMBERSHIP=true
    RELAY_OWNER_PUBKEY="${RELAY_OWNER_PUBKEY:?RELAY_OWNER_PUBKEY required with AMBUSH_REQUIRE_RELAY_MEMBERSHIP=true}"
  )
  : "${AMBUSH_RELAY_PRIVATE_KEY:?AMBUSH_RELAY_PRIVATE_KEY required with AMBUSH_REQUIRE_RELAY_MEMBERSHIP=true}"
  log "Membership gating enabled (NIP-43)"
fi

nohup env \
  DATABASE_URL=postgres://ambush:ambush_dev@localhost:5432/ambush \
  REDIS_URL=redis://localhost:6379 \
  RELAY_URL=ws://localhost:3000 \
  AMBUSH_BIND_ADDR=0.0.0.0:3000 \
  AMBUSH_RELAY_PRIVATE_KEY="${TEST_RELAY_PRIVATE_KEY}" \
  AMBUSH_REQUIRE_AUTH_TOKEN=false \
  AMBUSH_RECONCILE_CHANNELS=true \
  AMBUSH_GIT_PROBE_WRITERS=8 \
  ${MEMBERSHIP_ENV[@]+"${MEMBERSHIP_ENV[@]}"} \
  "./target/${CARGO_PROFILE}/ambush-relay" > /tmp/ambush-relay.log 2>&1 &
echo $! > /tmp/ambush-relay.pid

# ── Poll readiness ───────────────────────────────────────────────────────────

log "Waiting for relay readiness..."
for attempt in $(seq 1 60); do
  if ! kill -0 "$(cat /tmp/ambush-relay.pid)" 2>/dev/null; then
    err "Relay process died"
    cat /tmp/ambush-relay.log
    exit 1
  fi
