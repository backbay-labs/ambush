...
# can finish, so agent workers (goose, ambush-agent, ...) it spawned in their
...
# `AMBUSH_MANAGED_AGENT` env var from the shell: on macOS `pkill -f` matches only
...
#   <instance-id> is the desktop bundle identifier, e.g. `com.backbay.ambush.app.dev`
#   (main checkout) or `com.backbay.ambush.app.dev.my-branch` (a worktree).
