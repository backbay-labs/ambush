#!/usr/bin/env bash
# Uninstalls stale worktree-suffixed Ambush debug builds from booted iOS
# simulators and connected Android devices/emulators. Unsuffixed app installs
# (`com.backbay.ambush.dogfood.mobile` and `com.backbay.ambush.mobile`) are never
...
ios_prefix="com.backbay.ambush.dogfood.mobile."
android_prefix="com.backbay.ambush.mobile."
...
if [[ "$removed" == "0" ]]; then
    echo "no worktree-suffixed Ambush installs found (production apps untouched)"
elif [[ "$dry_run" == "1" ]]; then
