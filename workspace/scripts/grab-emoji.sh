#!/usr/bin/env bash
# grab-emoji.sh — Register custom Slack emoji in Ambush
#
# Looks up each emoji name in your Slack workspace and registers it in Ambush
# via `ambush emoji set`, making it available as :name: in the Ambush emoji picker.
#
# Usage:
#   SLACK_TOKEN=xoxp-... ./scripts/grab-emoji.sh [--name <ambush-name>] <emoji-name> [emoji-name ...]
#
# Options:
#   --name <ambush-name>  Override the shortcode used in Ambush (only valid with a single emoji)
#
# Env:
#   SLACK_TOKEN  — Slack user token (xoxp-...) with emoji:read scope
#
# Output:
#   name → registered as :name: in Ambush   on success
...
if [[ $# -eq 0 ]]; then
  echo "Usage: SLACK_TOKEN=xoxp-... $0 [--name <ambush-name>] <emoji-name> [emoji-name ...]" >&2
  exit 1
fi

if [[ -n "$AMBUSH_NAME" && $# -ne 1 ]]; then
...
if ! command -v ambush &>/dev/null; then
  echo "ERROR: 'ambush' not found in PATH. Install the Ambush CLI and retry." >&2
  exit 1
fi
...
for emoji_name in "$@"; do
  ...
  ambush_shortcode="${AMBUSH_NAME:-$emoji_name}"
  ...
  # Register in Ambush
  set_output=$(ambush emoji set --shortcode "$ambush_shortcode" --url "$emoji_url" 2>&1) || {
    echo "${emoji_name} → ERROR: ambush emoji set failed — ${set_output}"
    continue
  }

  echo "${emoji_name} → registered as :${ambush_shortcode}: in Ambush"
done
