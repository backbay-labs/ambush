#!/usr/bin/env bash
#
# ambush-adopt-prod-agents.sh — copy your installed (production) Ambush agent
...
#   every ambush-desktop-dev keychain entry, and the `_dev_migration_v1` marker.
...
#   reset wipes that Ambush cannot re-derive on its own: the agent RECORDS and
...
#
# WHAT THIS DOES NOT DO (verified against backbay-labs/ambush origin/main)
#   - It does NOT touch the ambush-desktop-dev keychain. Agent private keys are
...
#   AMBUSH_KEYCHAIN_SVC   keychain service    (default ambush-desktop)
#   AMBUSH_KEYCHAIN_ACCT  keychain account    (default secrets)
#
set -euo pipefail
...
PROD_DIR="$SUPPORT/com.backbay.ambush.app"
DEV_DIR="$SUPPORT/com.backbay.ambush.app.dev"
KEYCHAIN_SVC="${AMBUSH_KEYCHAIN_SVC:-ambush-desktop}"
KEYCHAIN_ACCT="${AMBUSH_KEYCHAIN_ACCT:-secrets}"
...
#     `ambush-desktop` for both the installed DMG
#     (/Applications/Ambush.app/Contents/MacOS/ambush-desktop) and dev builds
#     (target/<profile>/ambush-desktop via `tauri dev`). Match that path component
#     exactly so sidecars/helpers (ambush, ambush-dev-mcp, ambush-agent) don't
#     false-positive.
#
...
#     state) blocks.
ALLOWED_PROD_EXE="/Applications/Ambush.app/Contents/MacOS/ambush-desktop"
...
running_pids="$(pgrep -f '/ambush-desktop( |$)' 2>/dev/null || true)"
dmg_running=0
...
if [[ ${#dev_blocking[@]} -gt 0 ]]; then
  warn "A non-installed ambush-desktop process is running (dev build or unresolvable):"
...
if [[ $dmg_running -eq 1 ]]; then
  ...
  info "no running Ambush process detected"
fi
...
cat <<EOF
  1. Start your dev build (\`just production\` or \`just dev\`).
     - First boot re-runs the agent-key migration → ONE prod-keychain prompt
       (copies agent:<pubkey> keys ambush-desktop → ambush-desktop-dev).
  ...
  2. Worktree launches: worktree-suffixed dev dirs are symlinked to the
     canonical dev dir by sync_shared_agent_data ONLY when AMBUSH_SHARE_IDENTITY=1.
     If you launch from a worktree, export AMBUSH_SHARE_IDENTITY=1 (and
     AMBUSH_PRIVATE_KEY) or the worktree will mint its own duplicate agents.
EOF
if [[ "${AMBUSH_SHARE_IDENTITY:-}" != "1" ]]; then
  say "  NOTE: AMBUSH_SHARE_IDENTITY is not set in this shell — set it for worktree launches."
fi
exit 0
