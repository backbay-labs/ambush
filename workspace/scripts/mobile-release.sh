...
candidate  Publish the next immutable mobile-vX.Y.Z-rc.N candidate tag at the
           exact current commit of backbay-labs/ambush's remote main branch.
USAGE
  exit 2
}
...
case "$command" in
  candidate)
    ...
    run_url="$(printf '%s\n' "$dispatch_output" | awk '/^https:\/\/github\.com\/backbay-labs\/ambush\/actions\/runs\/[0-9]+$/ { if (found) exit 2; found = $0 } END { if (found) print found }')" || \
    ...
    run_id="${run_url##*/}"
    gh run watch "$run_id" --repo backbay-labs/ambush --exit-status --compact || \
    ...
    printf 'Published %s at origin/main commit %s through ambush-release-bot. Use this exact tag in Release Mobile.\n' \
      "$tag" "$main_sha"
    ;;
