...
helm lint deploy/charts/ambush-push-gateway >/dev/null
helm template push deploy/charts/ambush-push-gateway >"$out"
# Production values must attach push.ambush.xyz to an explicit Gateway.
production_args=(
  -f deploy/charts/ambush-push-gateway/values-production.yaml
  --set 'image.digest=sha256:aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa'
  --set 'profiles.dogfood.appAttestAppId=REALTEAM.com.backbay.ambush.dogfood.mobile'
...
helm lint deploy/charts/ambush-push-gateway "${production_args[@]}" >/dev/null
helm template push deploy/charts/ambush-push-gateway "${production_args[@]}" >"$production_out"
...
assert!(jenv.dig("AMBUSH_PUSH_RUNTIME_DATABASE_ROLE", "value") == "ambush_push_gateway_runtime")
...
required = Set.new(%w[
  DATABASE_URL AMBUSH_PUSH_DOGFOOD_APNS_CERT_PATH
  AMBUSH_PUSH_DOGFOOD_APNS_TOPIC AMBUSH_PUSH_DOGFOOD_APP_ATTEST_APP_ID
  AMBUSH_PUSH_GRANT_KEYS AMBUSH_PUSH_TOKEN_KEYS AMBUSH_PUSH_MAX_GRANT_LIFETIME_SECONDS
])
...
np = nps.find { |x| x.dig("metadata", "name") == "push-ambush-push-gateway" }
migration_np = nps.find { |x| x.dig("metadata", "name") == "push-ambush-push-gateway-migration" }
...
assert!(route.dig("spec", "hostnames").include?("push.ambush.xyz"))
RUBY

# Legacy token-auth values must fail rather than silently selecting the default
# certificate Secret.
if helm template push deploy/charts/ambush-push-gateway \
...
if helm template push deploy/charts/ambush-push-gateway --set httpRoute.enabled=true >/dev/null 2>&1; then
...
if helm template push deploy/charts/ambush-push-gateway -f deploy/charts/ambush-push-gateway/values-production.yaml >/dev/null 2>&1; then
...
helm template push deploy/charts/ambush-push-gateway \
...
np = xs.find do |x|
  x["kind"] == "NetworkPolicy" && x.dig("metadata", "name") == "push-ambush-push-gateway"
end
