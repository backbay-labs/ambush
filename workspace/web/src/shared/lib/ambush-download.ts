export const AMBUSH_RELEASES_URL =
  "https://github.com/backbay-labs/ambush/releases";
const AMBUSH_RELEASES_API_URL =
  "https://api.github.com/repos/backbay-labs/ambush/releases?per_page=10";
const CACHE_KEY = "ambush.latestDownload.v1";
const CACHE_TTL_MS = 60 * 60 * 1000;

export type AmbushDownloadPlatform = {
...
): AmbushDownloadPlatform["operatingSystem"] {
...
function normalizeArchitecture(
  value: string,
): AmbushDownloadPlatform["architecture"] {
...
export async function detectAmbushDownloadPlatform(
  navigatorValue: Navigator,
): Promise<AmbushDownloadPlatform> {
  const userAgentData = (
...
function assetPattern(platform: AmbushDownloadPlatform): RegExp | undefined {
...
export function selectAmbushDownloadUrl(
  releases: GitHubRelease[],
  platform: AmbushDownloadPlatform,
): string | undefined {
...
export async function resolveAmbushDownloadUrlForPlatform(
  platform: AmbushDownloadPlatform,
): Promise<string> {
  try {
  ...
  try {
    const response = await fetch(AMBUSH_RELEASES_API_URL, {
      headers: { Accept: "application/vnd.github+json" },
    });
    if (!response.ok) return AMBUSH_RELEASES_URL;
    const url = selectAmbushDownloadUrl(
      (await response.json()) as GitHubRelease[],
      platform,
    );
    if (!url) return AMBUSH_RELEASES_URL;
    try {
    ...
    return AMBUSH_RELEASES_URL;
  }
}

export async function resolveAmbushDownloadUrl(): Promise<string> {
  return resolveAmbushDownloadUrlForPlatform(
    await detectAmbushDownloadPlatform(navigator),
  );
}
