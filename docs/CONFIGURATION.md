...
- `tools/generate-sbom.sh` generates one CycloneDX 1.5 JSON SBOM per workspace crate, rejects duplicate keys, NaN/Infinity, malformed JSON, empty documents, and missing core component/dependency identity, then stages the validated files into a chosen output directory. A fake cargo-cyclonedx fixture reports the exact pinned version and writes `{}`; the structural validator must reject that false SBOM before the supply gate can pass
