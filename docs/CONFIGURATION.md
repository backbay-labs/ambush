...
- `response.hold_store_path`: where held destructive actions are recorded while they wait for a human decision. **Set this for any `mode: live_response` deployment.** Omitting it keeps holds in memory only, so a restart forgets every open hold: the action is not taken, no expiry record is published, and the operator's queue silently loses the rows.
- `response.hold_ttl_ms`: how long a held action stays decidable before the sweep expires it. The shipped default is `3600000` (one hour). Must be greater than zero: this bound is what keeps a hold nobody answers from waiting forever.
- `response.hold_ttl_ms_by_threat_class`: per-threat-class TTL overrides, keyed by the threat-class slug (`lateral_movement`, `execution`, and so on). A class with no override uses `hold_ttl_ms`. Each override must be greater than zero.
- `response.sweep_interval_ms`: how often holds are swept for expiry, stalled decisions and lost filings. The shipped default is `5000`. A hold can therefore outlive its TTL by at most `hold_ttl_ms + sweep_interval_ms`.
- `response.decide_stall_ms`: how long a decision may hold the compare-and-set claim without finishing before the sweep resolves it to `failed` with an honest unknown-outcome refusal. The shipped default is `60000`, equal to `policy.lease_ttl_ms`.
- `response.refile_after_ms`: how long a hold may sit in `created` with no accepted relay notice before the sweep republishes its created event so the bridge re-plans the filing. The shipped default is `30000`. Must be greater than zero: a hold re-filed on every tick floods the bridge's alarm spool. This is the bound on how long a hold the relay never learned of stays invisible to every console while the daemon still holds it.
- `response.governance_receipt_max_age_ms`: how old a governance receipt may be at decision time before the decision is refused as `governance.receipt_stale`. The shipped default is `86400000` (one day).
...
- `auth.context_token_env` defaults to `SWARM_OPERATOR_CONTEXT_TOKEN`. It previously defaulted to the same env var as the operator bearer (`SWARM_OPERATOR_TOKEN`), so a deployment that set only the bearer signed read-only context tokens with the credential that also grants approval and maintenance. A deployment that relied on that sets `operator_surface.auth.context_token_env: SWARM_OPERATOR_TOKEN` explicitly; every other deployment exports `SWARM_OPERATOR_CONTEXT_TOKEN` beside its bearers.
- `GET /v1/events/stream`, `GET /v1/demo/dashboard` and `GET /v1/demo/widget` all require a `context_token` query parameter. An absent or empty token answers `401` with `context_token is required`; the stream no longer answers with `Access-Control-Allow-Origin: *`, so a page the operator's browser happens to load can no longer read it cross-origin. A context token always carries a scope, so those three surfaces are readable only under one: the stream therefore no longer carries `evolution_status`, `agent_health`, `tamper_alert` or `case_promoted` frames, which only ever reached an unscoped -- that is, unauthenticated -- reader.
