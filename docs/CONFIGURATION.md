...
- `.github/workflows/ci.yml` fans out repo CI into parallel `fmt`, `panic-contract`, `build`, `clippy`, `test`, `fixture-freshness`, `platform-contract`, `proof-surfaces`, `solver-z3`, `jetstream`, `benchmark`, and `supply-chain` jobs. The build lane retains the small cargo-home cache but intentionally compiles cold and never restores or uploads `target/ci`: the hosted 8 GB target archive cost more time than the cold build. The other compilation lanes remain independent instead of waiting for build artifacts that no longer exist
