...
- `tools/check-supply-chain.sh` runs a metadata gate over `deny.toml`, then ONE `cargo deny check -D advisory-not-detected -D unmatched-skip -D unnecessary-skip advisories licenses bans sources` — `bans` with duplicates ENFORCED, so a new duplicate fails the gate and an accepted one has to be an exact `<crate>@<SemVer 2.0>` `[[bans.skip]]` entry — then a hard `cargo audit --deny warnings`
- every waiver in `deny.toml` carries parsed metadata, and the gate fails without it: `last-checked <YYYY-MM-DD>` and a `clears-when:` clause on both kinds of entry, plus `expires <YYYY-MM-DD>` and a `blast-radius:` note on an `[advisories] ignore`. An `expires` date in the past FAILS the build rather than warning, and the last-checked..expires window is capped at 180 days so a deadline cannot be written far enough out to stop being one. Duplicate waivers carry no expiry because the exact version pin already is one: valid SemVer 2.0 prerelease and build metadata are accepted; wildcard, range, operator, partial, and invalid SemVer specs are rejected; moving the pinned version triggers the duplicate it stopped covering or `unmatched-skip`; leaving the version present after it ceases to be a duplicate triggers `unnecessary-skip`. cargo-deny 0.19.4 renders accepted selectors with a leading `=` and normalizes build metadata out of matching, following SemVer precedence. The three `-D` flags turn cargo-deny's stale-waiver warnings into errors, so an exception that is no longer needed fails the build instead of lingering
