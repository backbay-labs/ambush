...
| **W3-33** · INTEGRATOR RULING | D4 gives First card `detect_only` and The hold `live_response`, and 13-PLAN-THE-HOLD's entry checklist expects `rulesets-dev/perch-dev.yaml` to already carry `live_response` plus `runtime.containment.lease_store_path` | conflict | **Two profiles, not a flip.** First card's acceptance rests on a profile that *cannot* execute, so flipping the file it was accepted on would retroactively invalidate that evidence. `rulesets-dev/perch-hold-dev.yaml` is the live-response profile, debug-signed like its sibling and differing in exactly four mode-forced places: `runtime.mode`, a durable `runtime.containment.lease_store_path`, a durable pheromone backend (`require_durable_live_response: true` refuses `in_memory` under `live_response`), and its own store directory so a live-response run cannot read or write the detect-only profile's holds. Both validate; a release daemon refuses both signatures. |
...
| **W3-34** · INTEGRATOR RULING | `13-PLAN-THE-HOLD.md` Task 28 exit criterion 2 reads the capability lease's TTL as `expires_at_ms - issued_at_ms == 60000` | conflict with `swarm_policy::CapabilityLease` | **The lease has no `issued_at_ms`.** The type carries exactly four fields — `capability_id`, `expires_at_ms`, `action`, `scope` — so that subtraction cannot be computed from any response. Measured live on a granted `isolate_host`: `expires_at_ms` minus the daemon's own compare-and-set instant is exactly 60000 ms, and minus the console's signed `decided_at_ms` is 60022 ms, the 22 ms being the round trip. **The criterion is restated as `expires_at_ms - <the decision instant the daemon recorded> == 60000`**, which is what the implementation guarantees and what a console can actually check. Adding `issued_at_ms` to the lease is a wire change nobody has argued for. |
| **W3-35** · INTEGRATOR RULING | `13-PLAN-THE-HOLD.md` Task 28 exit criterion 8 produces `refused_late` naming `runtime.containment_refused` by removing `lease_store_path` from the dev profile | does not reproduce | **The recipe does not fire on the dev profile.** Run live: `runtime.containment` removed entirely, the ruleset re-signed, the daemon restarted, and a granted `isolate_host` still returned `state: executed`, `outcome: granted_executed` and a capability lease. `RuntimeError::ContainmentRefused` needs all three of a containment action, a non-`DryRun` execution mode, and no bound store; the dev profile's sandbox adapter reports `mode: enforced` while never reaching `prepare_containment`, so the third condition is never asked. The code path is real and unit-covered (`exactly_one_of_the_four_skipped_producers_becomes_a_hold` builds the `Skipped` record directly), but **no live recipe for it exists yet** and this milestone does not claim one. Writing one means a response adapter that actually attempts containment. |
| **W3-36** · INTEGRATOR RULING | `14-PLAN-OPERATOR-COMPLETE.md` Task 17 records the END WATCH block through `POST /v1/operator/review/sessions` with `artifactRefs` naming cases and containment leases | the write is not implemented; the handoff's durable record is the relay message in each touched case | **The daemon's review session is a different object than the plan assumes.** `ReviewWorkbenchHarness::create_session` refuses an empty ref list (`crates/swarm-runtime-workbench/src/workbench/harness.rs:99`) and calls `ensure_artifact_exists` on every ref it is given; `ReviewArtifactRefKind` admits exactly six kinds, all of them handles into the workbench's OWN evidence stores (evidence bundle, evidence verification, promotion evidence packet, promotion review, canary run, production promotion). A case is a relay channel and a containment lease is a daemon record; neither is a workbench artifact and neither can be made into one without giving the workbench a resolver for objects it does not hold. So there is no body the console can send that this route accepts. Two changes were built and reverted rather than shipped: content negotiation on the route (correct in itself, but its only consumer would be a call that cannot succeed) and relaxing the one-ref minimum (a rule that belongs to the promotion-review workflow, not to this one). The console therefore publishes the END WATCH block as a plain `kind:9` into each touched case, which is the record an incoming operator actually reads, and `POST /v1/operator/review/sessions` stays on the INV-01 table listed-but-unimplemented — the state the table's own doc comment already describes for three of its five rows. **A daemon-side shift record, if one is wanted, is a new route and someone must decide it.** |
| **W3-37** · INTEGRATOR RULING | W3-29 leaves the telemetry spool at depth 1 per issuer until the `26000`-`26005` reducers supply a per-frame key | the key is per FRAME KIND, and the `26000` gauge cannot come from the spool at all | **The per-frame key landed; the ingest gauge needs a different mechanism, and that is a finding rather than a gap.** Keying by issuer makes two agents' health reports evict each other, so the key is the frame kind: `26001`, `26002` (health and actions share it, because they feed one frame), `26003`, `26005`, and one shared `other` for a record that does not decode — dropped records would lose telemetry with nothing to show for it. The key is derived from the record's own payload rather than passed in, because the receive loop appends without knowing what a frame is; that is its whole import discipline. **`Ingest` never reaches this spool.** `stream::classify` returns `DroppedAtSource` for it, so a `26000` key would name a slot holding one ingest event, and a gauge computed from it would report `accepted: 1` for a window that carried three thousand. **RESOLVED:** the window is `coalesce::IngestWindow`, owned by `SpoolSet` beside the slots. The receive loop does one `+= 1` on the append it already makes and owns no timer, so its 281 ms head room keeps the module boundary that defends it; the telemetry publisher drains the window once per tick. A second finding fell out of the same work: `MemorySpool` is last-wins, so at publish time the `26001` slot holds ONE snapshot and a `coalesced_from` derived from it would report `1` for a window that collapsed thirty — on the one field whose job is to admit the coalescing. The spool now counts puts per key and the count is passed in. |
...
| **D-FC-5** — who creates the twelve lane channels | **Default the plan builds under:** the bridge, idempotently, at startup (`swarm-perch-bridge/src/alarm.rs`), from the UUIDs committed in `perch.lane_channels`; the relay answers `duplicate: channel already exists` on every later start and the bridge treats that as success. `scripts/provision-perch.sh` no longer mints lanes (Ground Task 10 step 2 is amended: it prints keys, writes `.perch-dev/`, and adds relay memberships when `AMBUSH_REQUIRE_RELAY_MEMBERSHIP=true`). **Options:** (a) the script mints lanes with random ids and a local, unsigned copy of the ruleset carries them — rejected: the signed ruleset is the only config the daemon loads, and re-signing per machine multiplies sidecars; (b) extend `ambush channels create` with `--id` — a workspace CLI change for one script; kept as the fallback if the bridge must not hold `ChannelsWrite` on lanes. **Consequence:** a fresh relay database (`docker compose down -v`) recovers its lanes on the next daemon start with no operator action. **Measured 2026-09-04** against a native Postgres/Redis/relay stack: first start created twelve channels at the committed UUIDs and twenty-four memberships (alarm owner + operator member per lane); the second and third starts left the `channels` rows, their ids and their `created_at` byte-identical and added no membership row; deleting every `channels`, `channel_members` and `events` row and restarting brought all twelve back. The `docker compose down -v` spelling of that reset drops the volume and re-runs the relay's migrations too, and was not exercised — the local Docker daemon was unusable. **Dependents:** Task 13. | project owner |
| The eight `RoleGlyph` marks and nine `icons.perch.ts` domain icons (`17` §5.6, §13 item 1) | **Not drawn.** Every role renders its slug as text (`ThreatClassLabel`-style mono word) and every domain icon falls back to the nearest lucide mark already bundled (`CircleDot`, `Radar`, `Network`, `Timer`, `FileCheck`, `Undo2`). Options: (a) commission the seventeen marks on the 24×24 `createLucideIcon` grid (`workspace/desktop/src/shared/ui/icons.ts` pattern) — ~3 design-days, no engineering; (b) ship text-only permanently and delete `RoleGlyph`'s eight-state contract; (c) derive glyphs from the Quiet index mark by role hue (rejected: `colony.svg` assigns hues, and a hue is not a glyph). **Default: the text fallback, until decided.** Operator-complete Task 19 step 9 and Task 18 step 7 ship that fallback and are blocked on this. | project owner, with a designer |
| Where the watch claim is recorded (`04` §2.11; the `#watch` channel R-1 retired was its home) | **Not decided; the panel renders "no watch is claimed — classes 1–3 page everyone" until it is.** Options: (a) a standing ops channel provisioned by `scripts/provision-perch.sh` (config key `perch.ops_channel`), the claim as its `topic`, one relay-signed `kind:40099` `topic_changed` row per change — R-1 retired the channel only as the 26006 fence, not as a channel; ≈1 d, zero new kinds; (b) a NIP-33 addressable `kind:30078` authored by the claimant with `d = "perch-watch-claim"`, last-writer-wins by `created_at`, relay-generic and needing no provisioning, but with no relay-signed audit row and no takeover record; (c) the daemon field `on_shift_operator_pubkeys` plus `POST /v1/operator/watch/claim` (appendix §4 layer 4), which also narrows the hold's `p` tag — a sixth INV-01 write and a v2 daemon item. The panel's read model is identical under (a) and (b): `{holder_pubkey, since_ms, ttl_ms}`. Operator-complete Task 17 steps 8–11 are blocked on this; the read and render path builds against `lib/watchClaim.ts` regardless. | project owner |
| The engine chart's name after D3 (`09` §4.1 renames `swarm-team-six` → `ambush`; `workspace/deploy/charts/ambush/` is ALREADY the relay chart named `ambush`) | **Not decided; the engine chart keeps `swarm-team-six` and gains the relay chart as dependency alias `relay`.** Options: (a) rename the engine chart to `ambush` and keep the relay subchart aliased `relay` — Helm rewrites an aliased dependency's `.Chart.Name` to the alias so templates do not collide, but two charts named `ambush` in one repository is a review hazard and the rename breaks `helm upgrade` on existing releases; (b) rename the engine chart to `ambush-daemon`, which leaves the relay chart alone and makes the split legible, at the cost of contradicting `09` §4.2 exit criterion 6 as written; (c) leave both names as they are and record §4.1 as superseded. Operator-complete Task 21 step 9 is blocked on this; the rest of Task 21 composes the relay chart under the alias `relay`, which is correct under every option. | project owner |
