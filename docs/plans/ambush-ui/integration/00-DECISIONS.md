...
| **W3-37** · INTEGRATOR RULING | W3-29 leaves the telemetry spool at depth 1 per issuer until the `26000`-`26005` reducers supply a per-frame key | the key is per FRAME KIND, and the `26000` gauge cannot come from the spool at all | **The per-frame key landed; the ingest gauge needs a different mechanism, and that is a finding rather than a gap.** Keying by issuer makes two agents' health reports evict each other, so the key is the frame kind: `26001`, `26002` (health and actions share it, because they feed one frame), `26003`, `26005`, and one shared `other` for a record that does not decode — dropped records would lose telemetry with nothing to show for it. The key is derived from the record's own payload rather than passed in, because the receive loop appends without knowing what a frame is; that is its whole import discipline. **`Ingest` never reaches this spool.** `stream::classify` returns `DroppedAtSource` for it, so a `26000` key would name a slot holding one ingest event, and a gauge computed from it would report `accepted: 1` for a window that carried three thousand. The gauge needs a windowed counter fed at classification time, and where that window lives is a real decision: the receive loop's 281 ms head room is defended by a module boundary rather than by a timing test, so putting a windowing owner inside it trades against the thing that protects evidence. `coalesce::ingest_rate` takes the window as an argument and is tested; **the owner of that window is undecided and someone must choose it.** |
