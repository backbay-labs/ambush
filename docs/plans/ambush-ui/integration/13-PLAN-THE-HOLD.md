...
| 6 | **observed live 2026-09-05.** A hold left `created` at 13:27 UTC (its alarm steps lost with a spool set aside, W3-39) was reported `expired` by the daemon's list after its 3 600 000 ms TTL, with no operator action — the sweep ran; it was not driven on purpose, and the console's rendering of an expired hold stays mock-only |
| 7 | **not run.** The daemon was not killed between the compare-and-set and the outcome write |
| 8 | **replay half live; console half 2026-09-05** (replay through the console's leg 2 → `replayed: true`). The same body twice answered 200 `replayed: true` with the state unchanged, and a different decision on a decided hold answered 409 `hold_already_decided`, "re-read the hold". The 422 malformed-signature and 403 missing-`Approve` cases were **not** run live A leg 2 whose rationale differed from the signed one drew `422 Invalid signature` live on 2026-09-05: the daemon verifies `rationale_sha256` inside the preimage |
| 9 | **not run.** An unreconciled `46010` and an unadmitted frame are console behaviours |
| 10 | **live 2026-09-05, through the console's commands** (`evidence/walking-skeleton.md`, the conflict): two open intents, the first leg 2 `dispatched`, the second `superseded` with the winning intent and word from a re-read, and the supersession update card published and read back from the relay. The two-screen rendering remains the mock's |
