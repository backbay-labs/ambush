...
- [x] **Step 2: Provisioning.** `scripts/provision-perch.sh` uses `workspace/target/release/ambush` (the CLI) with a generated dev operator key to: mint the twelve lane channels named by `standard_threat_classes()` (open visibility), print the operator pubkey (for Task 9's `nostr_pubkey`), and write `.perch-dev/lane-channels.json` (git-ignored) mapping threat-class slug → channel UUID. Membership for the bridge identities is First card's job (it derives them). (Amended by 12-PLAN-FIRST-CARD.md D-FC-5: the lanes are created by the bridge; this script only prints the dev operator key and writes .perch-dev/.)
