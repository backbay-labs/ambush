...
| **`E` on a finding** | in `#lane-execution`, PROMOTE on the card for `suspicious_process_tree:hunt-evt-1-r28362` (event `93e1de8b`, a hunt with no case yet) → the case opened at `/cases/1e4e1c41-8383-40a9-9300-6d5a53d1a5e0` with its Timeline and Canvas tabs, the channel `case-1e4e1c41` in the sidebar within seconds; back in the lane the card's PROMOTE is disabled and CONFIRM / DISMISS / INVESTIGATE are enabled |
| **`D`, both legs** | DISMISS → "WHY DISMISS? PRESS D AGAIN TO RECORD." → a typed rationale → RECORD DISMISS. The card rendered the two legs as two facts: "AMBUSH recorded on Ambush · DAEMON acknowledged by the daemon". The daemon's `GET /v1/operator/findings/reviewed` then carried `{finding_id: suspicious_process_tree:hunt-evt-1-r28362, action: dismiss, false_positive: true, analyst_id: console, reviewed_at_ms: 1788743002267, incident_id: incident:perch-case:1e4e1c41-…, strategy_id: suspicious_process_tree, host_id: host-ops-1-r28362}`. That is First card's exit narrative (`E`, `D`, the review moving only on the acknowledged leg), driven by a person. Screenshot `69fdea49` |
...
15. **The evidence spool floods and the pacer cannot drain it.** Forty-five minutes after the restart the bridge's counters read `ingested_total{stream="evidence"} 20436`, `skipped_unpublished_total{stream="evidence"} 2127`, and no evidence card published at all; the newest cards on `#lane-execution` were still the ones from 2026-09-05, and the walk's own two findings never reached the lane. The escalation evaluator publishes a `RuntimeEvent::Escalation` every 100 ms while a class sits above its threshold (`stream.rs` classifies it to the disk-spooled evidence stream; no producer turns it into a card), and `Pacer::tick` spends a whole tick committing each record it cannot publish. The hold path was unaffected because the alarm stream is separate, which is why the grant above worked while the lane stood still. → Task 6 (the event) and Task 7 (the pacer).
16. *(note)* The dismissal's rationale is not in `GET /v1/operator/findings/reviewed` (the read carries `finding_id, reviewed_at_ms, action, analyst_id, false_positive, incident_id, strategy_id, host_id`); it lives on the relay in the verdict card. Not a defect against the plan; recorded so nobody looks for it on the daemon.
