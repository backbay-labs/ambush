...
4. **Assurance as the floor**, interleaved — phase 285 (assumption registry and invariant
   map) **LANDED** (`79840eb4d`, 2026-09-07) and 286 (deterministic simulation testing +
   durable dispatch-intent journal) **LANDED** (`ad4e1f4fb`, 2026-09-08, codex's repair,
   my acceptance); 287 (fuzz, loom, supply chain) **LANDED** (`9572f5214`, 2026-09-08) — **v1.79 Assurance
   Foundation (284-287) COMPLETE**; then 292 (pure decision core), 293 (Kani),
   294 (named safety properties and the partition-lease model).
...
| 2026-09-07 | Phase 285 (assumption registry and invariant mapping) merged to `main` (`79840eb4d`): the v1.79 **assurance floor** — `docs/assurance/{assumptions.toml,MAPPING.md,negative-registry.toml}` map 15 fail-closed invariants (swarm-policy 3 / runtime 5 / spine 4 / response 3) each to its enforcing `crate::module::function` + the assumption beneath it; 15 `// INVARIANT:` markers; two gates — `tools/check-mapping.sh` (marker↔row↔path sync) and `tools/check-negative-registry.sh` (a broken-variant test per invariant proving no row is vacuous). No production behaviour change (src edits = comment-only markers). Sequential SDD pipeline (T1→T4); T1/T3 task-reviewed clean, T2 self-verified under an external load spike. Whole-branch review (opus) = **Merge-with-nits**: one false SUMMARY sentence + a Source-line-drift note fixed (`244aa7860`). One honest deviation (`SpineEnvelopeCanonicalizationRequired`: a non-finite `f64` is unreachable through public serde_json 1.0.149, so the test falsifies the guard logic on a bare `f64` and asserts that unreachability). Push `--no-verify` (engine-only; the tsc-hang ruling). | `79840eb4d`; check-mapping 15/15 + check-negative-registry OK + check-gates-wired 0, 15 `negative_` tests green, fmt+clippy `-D warnings`, 0 `crates/*/src/*` logic edits |
| 2026-09-08 | Phase 286 (deterministic simulation testing) ACCEPTED + merged to `main` (`ad4e1f4fb`, merge `954c5cd02`): the receipt-before-action REPAIR. Codex — an external agent the user handed execution to — REJECTED my test-only DST-harness candidate (`1a5c9003b`) and repaired the ENGINE: a durable, fsynced, single-writer **dispatch-intent journal** (`crates/swarm-runtime/src/dispatch_journal.rs`, 874L); both `authorize_and_execute` paths reserve intent before any effect (fail-closed without a journal), a consumed identity never re-executes across crash/reopen/reload, enforced retries disabled (SIEM telemetry preserved). ADR `docs/decisions/0011`. Codex's sandbox could NOT run network regressions (TCP denied) or publish; I ran the full regression here (all pass), reviewed it (6-dimension adversarial workflow, 0 blockers/0 majors), fixed 2 findings (transient journal-poison `a70d5f191`; SIEM retry scope `f289573ea`), accepted + merged. Rejected `feat/dst-286` retired. | `ad4e1f4fb`; swarm-runtime lib 585, dispatch 21, DST 4 + 5000 nightly, ingest 177, response+http 0 fail; 6 gates + fmt + clippy `-D warnings`; 76 files +6936/-299 |
| 2026-09-08 | Phase 287 (fuzz, loom, supply-chain) merged to `main` (`9572f5214`) — **v1.79 Assurance Foundation (284-287) COMPLETE**. Three parallel task-reviewed tracks: FUZZ (fd9b46946) nightly-isolated cargo-fuzz workspace, 4 real-decoder targets + corpus conversion + nightly/smoke; LOOM (5a0791ecc) bounded_abstract_model pheromone+policy models + a REAL fix to a preexisting pheromone deposit/GC split-persistence race + falsifiable regression; SUPPLY (ea852a9b3) dated/justified deny.toml + drift-proof check-supply-chain.sh. Whole-branch review (opus)=Merge-with-nits (combined clippy -D warnings clean, race fix deadlock-free, 7 gates green, fuzz+loom out of shipped graph; one STATE nit fixed). Push clean (engine-only). | `9572f5214`; combined clippy -D warnings clean, pheromone 57+regression, 7 gates 0, 35 prod-src lines/3 crates |
