#!/usr/bin/env bash
#
# PROPOSED. Lands as AMBUSH `tools/check-perch-wire-parity.sh`.
#
# Perch wire field-set parity gate.
#
# WHY THIS EXISTS
#   The Perch wire contract lives in three places and neither repository has a
#   codegen step to keep them together:
#     1. crates/swarm-perch-wire/schemas/*.schema.json  (NORMATIVE)
#     2. crates/swarm-perch-wire/src/*.rs               (the producer)
#     3. BUZZ desktop/src/features/perch/wire/zod.ts    (the consumer)
...
#   in the SAME COMMIT, or CI fails in a way that looks like the gate is broken.
#
# WHY A FIXTURE
...
# ── LAYOUT RESOLUTION ────────────────────────────────────────────────────────
#
# This file lives in TWO trees over its life and must run green, with no env
# vars, from both:
#
#   (A) IN-REPO, its destination:  AMBUSH tools/check-perch-wire-parity.sh
#       -> crates/swarm-perch-wire/{schemas,src}
#   (B) IN THE BUILD TREE, where it is reviewed:
#       docs/plans/ambush-ui/build/skeleton/perch-wire/parity-gate.sh
#       -> ../../schemas and ./rust/src, ./ts/zod.ts
#
# The first revision hard-coded (A)'s paths against a REPO_ROOT of the script's
# PARENT directory, which in the build tree resolves to `build/skeleton/`. Run as
# committed it printed
#   VACUOUS: no schema directory at .../build/skeleton/crates/swarm-perch-wire/schemas
# and exited 2, so the "308 declared fields" the artifact quoted was reachable
# only with three env overrides nobody reading the file would guess. Exiting 2
# was the right refusal; pointing at a tree that exists in neither layout was
# not. Both layouts are probed now, in order, and the resolved one is PRINTED --
# a gate should say what it looked at.
#
# Explicit overrides still win, and they are how CI checks out BUZZ separately.
...
SCHEMA_DIR="$(resolve "${PERCH_WIRE_SCHEMAS:-}" \
  "$HERE/../../crates/swarm-perch-wire/schemas" \
  "$HERE/../../schemas")"
RUST_DIR="$(resolve "${PERCH_WIRE_RUST:-}" \
  "$HERE/../../crates/swarm-perch-wire/src" \
  "$HERE/rust/src")"
# The Buzz checkout is a sibling repository, not a submodule. When it is absent
# the gate reports which side it could not inspect and exits 2 -- VACUOUS, never
# 0. Reporting success over a region it never looked at is the exact failure
# mode this file's header names.
TS_FILE="$(resolve "${PERCH_WIRE_TS:-}" \
  "$HERE/../../../buzz/desktop/src/features/perch/wire/zod.ts" \
  "$HERE/ts/zod.ts")"
...
if not ts_file.is_file():
    print(
        ...
        "Set PERCH_WIRE_TS to the Buzz checkout, or run this gate only in a job "
        "that checks both repositories out. Exiting 2 rather than 0: a parity "
        "gate that inspected one side of two is not a parity gate.",
        file=sys.stderr,
    )
    sys.exit(2)
