...
//!
//! The marker format is Buzz's, not an invention: the one HTML-comment content
//! marker Buzz ships is
//! `WAVE_MESSAGE_MARKER = "<!-- buzz:wave:v1 -->"`
//! (`BUZZ desktop/src/features/messages/lib/waveMessage.ts:1`), sniffed by
//! `parseWaveMessageContent` (`:12-26`) from `MessageRow.renderBody`'s `default:`
//! arm (`BUZZ desktop/src/features/messages/ui/MessageRow.tsx:414-426`, call at
//! `:415`) in the renderer process. Perch's markers are the same shape with
//! `ambush` in place of `buzz`, which is why they cost zero of the four client
//! registration points: `kind:9` is already registered at all four.
//!
//! Perch's sniff is HARDENED in two ways Buzz's is not, both required by
//! `INV-15`:
//!
//! 1. Buzz's predicate is `content.trimStart().startsWith(MARKER)`, which fires
//!    on a marker anywhere at the start of the body including mid-line. Perch's
//!    requires the marker to be the ENTIRE first line.
//! 2. Buzz's has no issuer check at all. Perch's fires only when the event's raw
//!    signer resolves to an admitted bridge identity. The shipped precedent for
//!    that predicate is `getConfigNudgeAuthorPubkey`
...
/// in the registry is `DetectionFinding.evidence`
/// (`AMB crates/swarm-whisker/src/detector.rs:56`, a `serde_json::Value` built
/// from telemetry), which `Card::truncate_evidence` replaces with a byte count
/// and a hash rather than a smaller blob.
...
pub struct ContentParts<'a> {
    ...
    /// Flutter app, an FTS snippet and `buzz --format compact messages thread`
...
pub enum MarkerError {
    ...
    #[error("first line is not an ambush marker")]
    NoMarker,
...
mod tests {
    ...
    #[test]
    fn round_trip() {
        let body = build_content(CardKind::Hold, "hold h_1 · isolate_host · HIGH", "{\"a\":1}")
            .expect("builds");
