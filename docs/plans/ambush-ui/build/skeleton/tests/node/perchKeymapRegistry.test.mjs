// Target path in BUZZ: desktop/src/features/perch/lib/perchKeymapRegistry.test.mjs
//
// Run by `pnpm test` in desktop/, which is
// `node --import ./test-loader.mjs --experimental-strip-types --test "src/**/*.test.mjs"`
// (BUZZ desktop/package.json:17) `[V]` — so a `.test.mjs` beside a `.ts` file
// that it imports directly is the house pattern
// (BUZZ desktop/src/features/messages/lib/formatTimelineMessages.test.mjs
// imports "./formatTimelineMessages.ts") `[V]`.
//
...
test("the registry is not empty and every verdict verb is bound", () => {
  ...
  const bound = new Set(
    PERCH_BINDINGS.filter((binding) => binding.verb).map((binding) => binding.verb),
  );
...
test("the appendix section 2 keymap is bound exactly as ratified", () => {
  ...
  for (const [key, verb] of Object.entries(expected)) {
    const binding = PERCH_BINDINGS.find(
      (candidate) => candidate.key.toLowerCase() === key && candidate.verb === verb,
    );
...
test("D is Dismiss on a finding and is bound to no verb on a hold", () => {
  ...
  const dBindings = PERCH_BINDINGS.filter((binding) => binding.key.toLowerCase() === "d");
...
test("snooze is declared disabled on holds rather than omitted", () => {
  ...
  const snooze = PERCH_BINDINGS.find((binding) => binding.key.toLowerCase() === "s");
...
test("E means promote-to-a-case on every row type that offers it", () => {
  ...
  const promote = PERCH_BINDINGS.filter((binding) => binding.key.toLowerCase() === "e");
...
test("every binding names only known row types and a single-character or named key", () => {
  for (const binding of PERCH_BINDINGS) {
    ...
    assert.ok(binding.rowTypes.length > 0, `"${binding.key}" names no row type`);
    for (const rowType of [...binding.rowTypes, ...(binding.disabledOn ?? [])]) {
      assert.ok(ROW_TYPES.includes(rowType), `unknown row type "${rowType}"`);
    }
    assert.ok(binding.meaning.trim().length > 0, `"${binding.key}" has no meaning string`);
  }
});
