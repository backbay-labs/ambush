// PROPOSED — lands at BUZZ desktop/src/features/messages/ui/MessageThreadGuides.tsx
//
// Commit MR-1 of 15-FILE-SPLIT-PLAN.md. Pure extraction: every line below the
// props type is moved verbatim from MessageRow.tsx at eed74bde2 —
// :319-377 (the derivations) and :734-884 (the JSX) — with `message.depth`,
// `depthGuideDepths` and the four collapse callbacks now arriving as props
// instead of closing over MessageRow's parameter list. No expression is
// rewritten. `guideBleedRem` (MessageRow.tsx:466) moves with them because its
// only five readers are inside this JSX.
//
// Renders a fragment, so the DOM under MessageRow's outer
// `<div className="relative">` is byte-identical to today's.
 ...
 *
 * Moved from `MessageRow.tsx:67-72`; `MessageRow` re-exports it so
 * `MessageThreadPanel.tsx:42` and `MessageThreadSummaryRow.tsx:8` are untouched.
 */
...
export function MessageThreadGuides({
  ...
  // ── moved verbatim from MessageRow.tsx:319-377 ──────────────────────────
  ...
  // ── moved verbatim from MessageRow.tsx:466 ──────────────────────────────
  const guideBleedRem = isThreadReplyLayout ? 0.25 : 0;

  // ── moved verbatim from MessageRow.tsx:734-884 ──────────────────────────
  return (
    <>
